...
/* ===== CSI (C-Sky) Compatibility Functions for IPRO6LE drivers ===== */
/**
 * \brief   Get SysTimer value (low 32-bit)
 * \details Uses MCYCLE CSR for cycle count (avoids SysTimer include dependency)
 * \return  SysTimer value (low 32-bit)
 */
__STATIC_FORCEINLINE uint32_t csi_coret_get_value(void)
{
    return __RV_CSR_READ(CSR_MCYCLE);
}

/**
 * \brief   Get SysTimer value (high 32-bit)
 * \details Uses MCYCLEH CSR for upper 32 bits (RV32 only)
 * \return  SysTimer value (high 32-bit)
 */
__STATIC_FORCEINLINE uint32_t csi_coret_get_valueh(void)
{
#if __RISCV_XLEN == 32
    return __RV_CSR_READ(CSR_MCYCLEH);
#else
    return 0;  /* RV64 doesn't have MCYCLEH */
#endif
}

/**
 * \brief   Read MCYCLE CSR (low 32-bit)
 * \return  MCYCLE value (low 32-bit)
 */
__STATIC_FORCEINLINE uint32_t __get_MCYCLE(void)
{
    return __RV_CSR_READ(CSR_MCYCLE);
}

/**
 * \brief   Read MCYCLEH CSR (high 32-bit for RV32)
 * \return  MCYCLEH value (high 32-bit)
 */
__STATIC_FORCEINLINE uint32_t __get_MCYCLEH(void)
{
#if __RISCV_XLEN == 32
    return __RV_CSR_READ(CSR_MCYCLEH);
#else
...
/**
 * \brief   Read MSTATUS CSR (CSI compatibility)
 * \return  MSTATUS value
 */
__STATIC_FORCEINLINE unsigned long __get_MSTATUS(void)
{
    return __RV_CSR_READ(CSR_MSTATUS);
}

/**
 * \brief   Set MSTATUS CSR (CSI compatibility)
 * \param   mstatus Value to write
 */
__STATIC_FORCEINLINE void __set_MSTATUS(unsigned long mstatus)
{
    __RV_CSR_WRITE(CSR_MSTATUS, mstatus);
}

/* ===== CSI Cache API Compatibility Macros ===== */
/* Maps C-Sky CSI cache APIs to NMSIS equivalents (defined in core_feature_cache.h) */

/* Cache enable/disable functions */
#define csi_icache_enable()             EnableICache()
#define csi_icache_disable()            DisableICache()
#define csi_dcache_enable()             EnableDCache()
#define csi_dcache_disable()            DisableDCache()

/* Cache invalidate functions */
#define csi_icache_invalid()            MInvalICache()
#define csi_dcache_invalid()            MInvalDCache()

/* DCache clean (flush) functions */
#define csi_dcache_clean()              MFlushDCache()
#define csi_dcache_clean_invalid()      MFlushInvalDCache()

/* Range-based cache operations */
#define csi_dcache_clean_range(addr, size)          MFlushDCacheRange((unsigned long)(addr), (unsigned long)(size))
#define csi_dcache_invalid_range(addr, size)        MInvalDCacheRange((unsigned long)(addr), (unsigned long)(size))
#define csi_dcache_clean_invalid_range(addr, size)  MFlushInvalDCacheRange((unsigned long)(addr), (unsigned long)(size))
