...
static const ATTR_TCM_CONST_SECTION SPI_Flash_Cfg_Type flashCfg_Winb_128JW_128JV = {
    .resetCreadCmd = 0xff,
    .resetCreadCmdSize = 3,
    .mid = 0xef,
...
static const ATTR_TCM_CONST_SECTION Flash_Info_t flashInfos[] = {
    ...
    {
        .jedecID = 0x1465C8,
        //.name="GD_WQ80E_80_1833",
        .cfg = &flashCfg_Gd_WQ80E_WQ16E,
    },
    {
        .jedecID = 0x1565C8,
        //.name="GD_WQ16E_16_1833",
        .cfg = &flashCfg_Gd_WQ80E_WQ16E,
    },
    {
        .jedecID = 0x1665C8,
        //.name="GD_WQ32E_32_1833",
        .cfg = &flashCfg_Gd_LQ08C_LE16C_LQ32D_WQ32E,
    },
    {
        .jedecID = 0x1865C8,
...
#ifndef IPRO_USE_ROM_DRIVER
__WEAK
int ATTR_TCM_SECTION SF_Cfg_Init_Ext_Flash_Gpio(uint8_t extFlashPin)
{
    ...
    if (extFlashPin == 0) {
        gpiopins[0] = IPRO_EXTFLASH_CLK0_GPIO;
        gpiopins[1] = IPRO_EXTFLASH_CS0_GPIO;
        gpiopins[2] = IPRO_EXTFLASH_DATA00_GPIO;
        gpiopins[3] = IPRO_EXTFLASH_DATA10_GPIO;
        gpiopins[4] = IPRO_EXTFLASH_DATA20_GPIO;
        gpiopins[5] = IPRO_EXTFLASH_DATA30_GPIO;
    } else if (extFlashPin == 1) {
        gpiopins[0] = IPRO_EXTFLASH_CLK1_GPIO;
        gpiopins[1] = IPRO_EXTFLASH_CS1_GPIO;
        gpiopins[2] = IPRO_EXTFLASH_DATA01_GPIO;
        gpiopins[3] = IPRO_EXTFLASH_DATA11_GPIO;
        gpiopins[4] = IPRO_EXTFLASH_DATA21_GPIO;
        gpiopins[5] = IPRO_EXTFLASH_DATA31_GPIO;
    } else {
        return ERROR;
    }
...
#ifndef IPRO_USE_ROM_DRIVER
__WEAK
int ATTR_TCM_SECTION SF_Cfg_Deinit_Ext_Flash_Gpio(uint8_t extFlashPin)
{
    GLB_GPIO_Cfg_Type cfg;
    uint8_t gpiopins[6];
    uint8_t i = 0;
...
#ifndef IPRO_USE_ROM_DRIVER
__WEAK
int ATTR_TCM_SECTION SF_Cfg_Get_Flash_Cfg_Need_Lock(uint32_t flashID, SPI_Flash_Cfg_Type *pFlashCfg)
{
    uint32_t i;
...
#ifndef IPRO_USE_ROM_DRIVER
__WEAK
int ATTR_TCM_SECTION SF_Cfg_Init_Flash_Gpio(uint8_t flashPinCfg, uint8_t restoreDefault)
{
    uint8_t selEmbedded = 0;
    uint8_t swapIo2Cs = 0;
    uint8_t swapIo3Io0 = 0;

    if (restoreDefault) {
        /* Set Default first */
        GLB_Set_SFlash_IO_PARM(1, 1, 0);
        SF_Ctrl_Select_Pad(SF_IO_EMB_SWAP_IO3IO0);

        /* Default is set, so return */
        if (flashPinCfg == SF_IO_EMB_SWAP_IO3IO0) {
            return SUCCESS;
        }
    }
    ...
    if (flashPinCfg & (1 << 2)) {
        /* Init sf2 gpio */
        SF_Cfg_Init_Flash2_Gpio(!((flashPinCfg >> 5) & 1));
        selEmbedded = 0;
    } else if (flashPinCfg & (1 << 3)) {
        /* Init sf3 gpio */
    ...
    swapIo3Io0 = ((flashPinCfg >> 1) & 1);
    swapIo3Io0 = (!swapIo3Io0);
    swapIo2Cs = ((flashPinCfg >> 0) & 1);
    GLB_Set_SFlash_IO_PARM(selEmbedded, swapIo3Io0, swapIo2Cs);
...
#ifndef IPRO_USE_ROM_DRIVER
__WEAK
int ATTR_TCM_SECTION SF_Cfg_Init_Flash2_Gpio(uint8_t swap)
{
                                                ...
                                                uint8_t restoreDefault, SPI_Flash_Cfg_Type *pFlashCfg, uint8_t group, uint8_t bank)
{
    ...
    if (autoScan) {
        flashPin = 0;
        do {
            if (!IS_SF_CTRL_PIN_SELECT(flashPin)) {
                flashPin++;
                continue;
            }
            if (bank == SF_CTRL_FLASH_BANK1) {
                if (flashPin > SF_IO_EMB_SWAP_IO2CS_AND_SF2) {
                    jdecId = 0;
                    break;
                }
                if (flashPin < SF_IO_EXT_SF3) {
                    flashPin = SF_IO_EMB_SWAP_IO3IO0_AND_SF2_SWAP_IO3IO0;
                }
                if (flashPin == SF_IO_EXT_SF2) {
                    flashPin++;
                }
                SF_Cfg_Init_Flash2_Gpio(!((flashPin >> 5) & 1));
            } else {
                if (flashPin > SF_IO_EXT_SF2) {
                    jdecId = 0;
                    break;
...
#ifndef IPRO_USE_ROM_DRIVER
__WEAK
int ATTR_TCM_SECTION SF_Cfg_Flash_Init(uint8_t sel, const SF_Ctrl_Cfg_Type *pSfCtrlCfg, const SF_Ctrl_Bank2_Cfg *pBank2Cfg)
{
    uint8_t selEmbedded = 0;
    uint8_t swapIo2Cs = 0;
    uint8_t swapIo3Io0 = 0;
    ...
    if (sel & (1 << 2)) {
        SF_Cfg_Init_Flash2_Gpio(!((sel >> 5) & 1));
        selEmbedded = 0;
    } else if (sel & (1 << 3)) {
        SF_Cfg_Init_Ext_Flash_Gpio(1);
        selEmbedded = 0;
    ...
    if (sel <= SF_IO_EMB_SWAP_IO2CS) {
        ...
        if (pSfCtrlCfg != NULL) {
            SF_Ctrl_Set_IO_Delay(((sel&0xf) >> 2), pSfCtrlCfg->doDelay, pSfCtrlCfg->diDelay, pSfCtrlCfg->oeDelay);
        }
...
#ifndef IPRO_USE_ROM_DRIVER
__WEAK
int ATTR_TCM_SECTION SF_Cfg_Sbus2_Flash_Init(uint8_t sel, const SF_Ctrl_Bank2_Cfg *pBank2Cfg)
{
