...
int AON_ACOMP_Init(AON_ACOMP_ID_Type acompNo, AON_ACOMP_CFG_Type *cfg)
{
    ...
    if (acompNo == AON_ACOMP0_ID) {
        ...
        tmpVal = writel(tmpVal, AON_BASE + AON_ACOMP0_CTRL_OFFSET);
        ...
        tmpVal = reg_set_bits(tmpVal, AON_ACOMP0_LEVEL_SEL, cfg->scalingFactor);
        ...
        tmpVal = writel(tmpVal, AON_BASE + AON_ACOMP1_CTRL_OFFSET);
        ...
        tmpVal = reg_set_bits(tmpVal, AON_ACOMP1_LEVEL_SEL, cfg->scalingFactor);
    ...
    tmpVal = reg_set_bits(tmpVal, AON_ACOMP_VREF_SEL, cfg->vioSel);
    tmpVal = writel(tmpVal, AON_BASE + AON_ACOMP_CTRL_OFFSET);
...
int AON_ACOMP_Get_Result(AON_ACOMP_ID_Type acompNo)
{
    ...
    /* Disable ACOMP first */
    if (acompNo == AON_ACOMP0_ID) {
        if (reg_is_bit_set(tmpVal, AON_ACOMP0_OUT_RAW)) {
            return SET;
        } else {
            return RESET;
        }
    } else {
        if (reg_is_bit_set(tmpVal, AON_ACOMP1_OUT_RAW)) {
            return SET;
        } else {
            return RESET;
        }
...
uint32_t AON_ACOMP_Get_Postive_Input(AON_ACOMP_ID_Type acompNo)
{
    ...
    if (acompNo == AON_ACOMP0_ID) {
        tmpVal = readl(AON_BASE + AON_ACOMP0_CTRL_OFFSET);
        tmpVal = reg_get_bits(tmpVal, AON_ACOMP0_POS_SEL);
    } else {
        tmpVal = readl(AON_BASE + AON_ACOMP1_CTRL_OFFSET);
        tmpVal = reg_get_bits(tmpVal, AON_ACOMP1_POS_SEL);
    }
    return tmpVal;
}
