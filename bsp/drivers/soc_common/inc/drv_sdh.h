...
/** @addtogroup  IPRO7_Peripheral_Driver
 *  @{
 */
...
/**
 *  @brief SD port type definition
 */
typedef enum {
    SDH_ID,    /*!< SDH port define */
    SD_ID_MAX, /*!< SD MAX ID define */
} SD_ID_Type;
...
typedef enum {
    ...
    SDH_CAP_SUPPORT_VLT12 = 0x00000800,       /*!< SDH support voltage 1.2V */
    ...
    SDH_CAP_SUPPORT_HS200 = 0x00080000,       /*!< SDH support HS 200MHz */
...
#define SDH_INT_BOOT_ACK           ((uint32_t)(0x00002000)) /*!< SDH re-tuning event interrupt */
#define SDH_INT_BOOT_DONE          ((uint32_t)(0x00004000)) /*!< SDH re-tuning event interrupt */
...
typedef enum {
    ...
    SDH_SPEED_MODE_HS200, /*!< SDH speed :200MHZ (HS) */
} SDH_Speed_Mode_Type;
...
typedef struct
{
    ...
    int vlot18Enable;          /*!< SDH enable 1.8V */
    int highSpeed;             /*!< SDH enable high speed */
...
typedef struct
{
    int enableAutoCommand12; /*!< SDH enable auto CMD12 */
    int enableAutoCommand23; /*!< SDH enable auto CMD23 */
    int enableRelibaleWR;    /*!< SDH enable auto CMD23 with reliable write request */
    int enableIgnoreError;   /*!< SDH enable to ignore error event to read/write all the data */
    ...
    uint8_t block_size_adjust;
    uint8_t cmd23_emmc_mode;
} SDH_Data_Cfg_Type;
 ...
 *  @{
 */
#define IS_SD_ID_TYPE(type) (((type) == SDC_ID) || \
                              ((type) == SDH_ID) || \
                              ((type) == SD_ID_MAX))
...
#define SDH_ADMA2_ADDRESS_ALIGN (4U)
/* The alignment size for LENGTH filed in ADMA2's descriptor */
#define SDH_ADMA2_LENGTH_ALIGN (4U)
...
#define SDH_EnableIntSource(mask)                                                 \
    do {                                                                          \
        writel(readl(SDH_BASE + SDH_SD_NORMAL_INT_STATUS_INT_EN_OFFSET) | (mask), \
            SDH_BASE + SDH_SD_NORMAL_INT_STATUS_INT_EN_OFFSET);                   \
    } while (0)
...
#define SDH_AutoCmd12ErrStatus() readw(SDH_BASE + SDH_SD_AUTO_CMD12_ERROR_STATUS_OFFSET)
#define SDH_GetDMAErrStatus()    readw(SDH_BASE + SDH_SD_ADMA_ERROR_STATUS_OFFSET)
#define SDH_GetPresentStatus()   readl(SDH_BASE + SDH_SD_PRESENT_STATE_1_OFFSET)
...
/* Weak; a HAL may override it to wait for the DMA data phase at a lower
 * priority. 0 = data-phase event seen (re-read status), -1 = poll instead. */
...
/*@} end of group IPRO_Peripheral_Driver */


void sdh_set_single_no_blkcnt(int enable);
#endif /* __DRV_SDH_H__ */
