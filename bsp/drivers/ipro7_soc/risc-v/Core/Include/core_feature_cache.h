 ...
 *   * 0: Not present
 *   * 1: Present
 * 3. __CCM_PRESENT:  Define whether Cache Control and Maintainence(CCM) Unit is present or not.
 *   * 0: Not present
 *   * 1: Present
 */
#ifdef __cplusplus
 ...
 * @{
 *
 * Cache Control and Maintainence(CCM) for software to control and maintain
...
typedef enum CCM_OP_FINFO {
    ...
    CCM_OP_ECC_ERR = 0x4                /*!< Deprecated, ECC Error, this error code is removed in later CCM RTL design, please don't use it */
} CCM_OP_FINFO_Type;
...
#define CCM_CACHELINE_SIZE              (32)
#define CCM_CACHELINE_SHIFT             (5)
#define CCM_CACHELINE_ALIGN_MASK        (0xFFFFFFE0UL)   // cache line align
 ...
 * \remarks
 * - This function might not work for some old processors
 * - Please make sure the version of your processor contain ICACHE bit in mcfg_info
 ...
 * \remarks
 * - This function might not work for some old processors
 * - Please make sure the version of your processor contain DCACHE bit in mcfg_info
 * \return 1 if present otherwise 0
*/
...
__STATIC_FORCEINLINE void MFlushInvalDCacheRange(unsigned long addr, unsigned long size)
{
    unsigned long start, end;

    start = addr & CCM_CACHELINE_ALIGN_MASK;
    end = (addr + size + CCM_CACHELINE_SIZE - 1) & CCM_CACHELINE_ALIGN_MASK;
    MFlushInvalDCacheLines(start, (end - start) >> CCM_CACHELINE_SHIFT);
}

__STATIC_FORCEINLINE void MFlushDCacheRange(unsigned long addr, unsigned long size)
{
    unsigned long start, end;

    start = addr & CCM_CACHELINE_ALIGN_MASK;
    end = (addr + size + CCM_CACHELINE_SIZE - 1) & CCM_CACHELINE_ALIGN_MASK;
    MFlushDCacheLines(start, (end - start) >> CCM_CACHELINE_SHIFT);
}

__STATIC_FORCEINLINE void MInvalDCacheRange(unsigned long addr, unsigned long size)
{
    unsigned long start, end;

    start = addr & CCM_CACHELINE_ALIGN_MASK;
    end = (addr + size + CCM_CACHELINE_SIZE - 1) & CCM_CACHELINE_ALIGN_MASK;
    MInvalDCacheLines(start, (end - start) >> CCM_CACHELINE_SHIFT);
}

#endif /* defined(__CCM_PRESENT) && (__CCM_PRESENT == 1) */
#else /* defined(__DCACHE_PRESENT) && (__DCACHE_PRESENT == 1) */
__STATIC_FORCEINLINE void MFlushDCacheRange(unsigned long addr, unsigned long size) {}
__STATIC_FORCEINLINE void MInvalDCacheRange(unsigned long addr, unsigned long size) {}
__STATIC_FORCEINLINE void MFlushInvalDCacheRange(unsigned long addr, unsigned long size) {}
