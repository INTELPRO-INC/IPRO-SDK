 ...
 * \remarks
 * - MSFTRST register is introduced in Core version 1.3(\ref __IPRO_N_REV >= 0x0103)
 * - MSTOP register is renamed to MTIMECTL register in Core version 1.4(\ref __IPRO_N_REV >= 0x0104)
 * - CMPCLREN and CLKSRC bit in MTIMECTL register is introduced in Core version 1.4(\ref __IPRO_N_REV >= 0x0104)
 */
typedef struct {
 ...
 * \remarks
 * - For \ref __IPRO_N_REV >= 0x0104, the CMPCLREN bit in MTIMECTL is introduced,
