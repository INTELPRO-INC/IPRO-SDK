...
#include "risc-v/Core/Include/core_feature_eclic.h"
#include "hal_sys.h"
#include "hal_hbnram_mgnt.h"
#include "compiler/common.h"
#ifdef CONFIG_EXCEPTION_DUMP
#include "exc_dump.h"
#endif
...
typedef struct EXC_Frame {
    ...
    unsigned long s0;                /* s0: x8, saved register / frame pointer */
    unsigned long s1;                /* s1: x9, saved register */
...
/* Vector table must be in OCRAM for early boot (before PSRAM init) */
static ATTR_OCRAM_SECTION unsigned long vector_table[__ECLIC_INTNUM] __attribute__((aligned(512))) =
{
...
/* Exception handlers must be in OCRAM for early boot (before PSRAM init) */
static ATTR_OCRAM_BSS_SECTION unsigned long SystemExceptionHandlers[MAX_SYSTEM_EXCEPTION_NUM + 1];
...
/* Core interrupt handlers must be in OCRAM for early boot (before PSRAM init) */
static ATTR_OCRAM_BSS_SECTION unsigned long SystemCoreInterruptHandlers[SYSTEM_CORE_INTNUM];
...
static void system_default_exception_handler(unsigned long mcause, unsigned long sp)
{
    ...
    /* LP wakeup diagnostic — read HBN trace registers before anything else */
    {
        volatile uint32_t *rsv2 = (volatile uint32_t *)0x3008F108;
        volatile uint32_t *rsv3 = (volatile uint32_t *)0x3008F10C;
        printf("=== LP WAKEUP TRACE (HBN regs) ===\r\n");
        printf("HBN_RSV2=0x%08lx HBN_RSV3=0x%08lx\r\n",
               (unsigned long)*rsv2, (unsigned long)*rsv3);
        if ((*rsv2 & 0xFFFF0000) == 0x4C500000) {
            printf("LP step=0x%02lx, saved_RA=0x%08lx\r\n",
                   (unsigned long)(*rsv2 & 0xFF), (unsigned long)*rsv3);
        }
    }
    ...
    /* Which FreeRTOS task was running when the trap hit, and how close to
     * overflow was its stack? A fast stack overflow (deep CHIP C++ command
     * dispatch, crypto, _Unwind) faults on a wild write BEFORE the next
     * context switch, so the configCHECK_FOR_STACK_OVERFLOW hook never
     * fires — you land here instead. Without the task name + high-water
     * mark the faulting task is invisible, which is exactly the ambiguity
     * that turned the PDS-wake command-path crash into guess-and-patch.
     * Defensive: skip entirely if the scheduler hasn't produced a current
     * TCB yet (early-boot trap -> pxCurrentTCB == NULL). */
#if defined(CONFIG_FREERTOS_SUPPORT)
    {
        extern void *xTaskGetCurrentTaskHandle(void);
        extern char *pcTaskGetName(void *xTaskToQuery);
        extern unsigned long uxTaskGetStackHighWaterMark(void *xTask);
        void *cur = xTaskGetCurrentTaskHandle();
        printf("\r\n=== FAULTING TASK ===\r\n");
        if (cur != NULL) {
            printf("Task: %s\r\n", pcTaskGetName(cur));
            /* Words remaining at the closest-ever approach to the stack
             * limit. 0 (or a wild value) == this task overran its stack. */
            printf("Stack high-water-mark: %lu words (min free)\r\n",
                   uxTaskGetStackHighWaterMark(cur));
        } else {
            printf("Task: <none / pre-scheduler>\r\n");
        }
    }
#endif
...
#ifdef CONFIG_EXCEPTION_DUMP
    /* Offer crash dump to PC tool before halting or rebooting */
    {
        extern int board_get_debug_uart_index(void);
        exc_dump_wait_and_send(sp, board_get_debug_uart_index());
    }
#endif
...
void Exception_DumpFrame(unsigned long sp, uint8_t mode)
{
    ...
    printf("ra: 0x%lx, tp: 0x%lx, t0: 0x%lx, t1: 0x%lx, t2: 0x%lx\r\n" \
           ...
           "cause: 0x%lx, epc: 0x%lx\r\n", exc_frame->ra, exc_frame->tp, exc_frame->t0, \
           exc_frame->t1, exc_frame->t2, exc_frame->s0, exc_frame->s1, \
...
void Exception_PrintBacktrace(unsigned long sp, int limit)
{
    ...
    printf("Saved s0(fp): 0x%08lx, ra: 0x%08lx\r\n", exc_frame->s0, exc_frame->ra);
    printf("\r\n");
    ...
    /* Start walking from saved frame pointer (s0) */
    fp = exc_frame->s0;

    /* Check if frame pointer is valid before walking */
    if (!is_valid_stack_addr(fp, stack_low, stack_high)) {
        printf("  --  | s0=0x%08lx invalid, FP chain unavailable\r\n", fp);
        fp_walk_failed = 1;
    }
    ...
    while (!fp_walk_failed && depth < limit) {
        ...
        printf("  %02d  |  0x%08lx  |  0x%08lx  | Caller\r\n",
               depth++, fp, ra);
