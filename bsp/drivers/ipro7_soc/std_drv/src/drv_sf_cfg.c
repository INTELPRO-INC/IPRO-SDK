...
static const ATTR_TCM_CONST_SECTION SPI_Flash_Cfg_Type flashCfg_Span_S25FL = {
    .resetCreadCmd = 0xff,
    .resetCreadCmdSize = 3,
    .mid = 0x01,
    ...
    .qpiFastReadQioCmd = 0xec,
    .qpiFrQioDmyClk = 16 / 8,
    .qpiPageProgramCmd = 0x34,
    .writeVregEnableCmd = 0x50,
...
static const ATTR_TCM_CONST_SECTION Flash_Info_t flashInfos[] = {
    ...
    {
        .jedecID = 0x190201,
        .name = "Spansion_S25FS256",
        .cfg = &flashCfg_Span_S25FL,
    }

};

static void ATTR_TCM_SECTION SF_Cfg_Init_Internal_Flash_Gpio(void)
{
    uint32_t tmpVal;
    uint32_t gpioPin;
    uint32_t pinOffset;
    for (gpioPin = 44; gpioPin < 50; gpioPin++) {
...
void ATTR_TCM_SECTION SF_Cfg_Init_Ext_Flash_Gpio(uint8_t extFlashPin)
{
    ...
    if (extFlashPin == 0) {
        gpiopins[0] = EXTFLASH_CLK0_GPIO;
        gpiopins[1] = EXTFLASH_CS0_GPIO;
        gpiopins[2] = EXTFLASH_DATA00_GPIO;
        gpiopins[3] = EXTFLASH_DATA10_GPIO;
        gpiopins[4] = EXTFLASH_DATA20_GPIO;
        gpiopins[5] = EXTFLASH_DATA30_GPIO;
    } else if (extFlashPin == 1) {
        gpiopins[0] = EXTFLASH_CLK1_GPIO;
        gpiopins[1] = EXTFLASH_CS1_GPIO;
        gpiopins[2] = EXTFLASH_DATA01_GPIO;
        gpiopins[3] = EXTFLASH_DATA11_GPIO;
        gpiopins[4] = EXTFLASH_DATA21_GPIO;
        gpiopins[5] = EXTFLASH_DATA31_GPIO;
    } else {
        return;
    }
...
int ATTR_TCM_SECTION SF_Cfg_Get_Flash_Cfg_Need_Lock(uint32_t flashID, SPI_Flash_Cfg_Type *pFlashCfg)
{
    ...
    if (flashID == 0) {
        xipOffset = SF_Ctrl_Get_Flash_Image_Offset(0, 0);
        ...
        SF_Ctrl_Set_Flash_Image_Offset(xipOffset, 0, 0);

        if (arch_memcmp(buf, IMG_FLASH_CFG_MAGIC, 4) == 0) {
...
void ATTR_TCM_SECTION SF_Cfg_Init_Flash_Gpio(uint8_t flashPinCfg, uint8_t restoreDefault)
{
    ...
    if (flashPinCfg == SF_CTRL_SEL_EXTERNAL_FLASH_SF2) {
        SF_Cfg_Init_Ext_Flash_Gpio(0);
        SF_Ctrl_Select_Pad(SF_CTRL_PAD_SEL_SF2);
    } else if (flashPinCfg == SF_CTRL_SEL_EXTERNAL_FLASH_SF3) {
        SF_Cfg_Init_Ext_Flash_Gpio(1);
        SF_Ctrl_Select_Pad(SF_CTRL_PAD_SEL_SF3);
    } else {
        if (flashPinCfg == SF_CTRL_SEL_INTERNAL_FLASH_SWAP_NONE) {
            GLB_Set_Embedded_FLash_IO_PARM(0, 0, 0);
...
uint32_t ATTR_TCM_SECTION SF_Cfg_Flash_Identify(uint8_t callFromFlash,
                                                uint32_t autoScan, uint32_t flashPinCfg, uint8_t restoreDefault, SPI_Flash_Cfg_Type *pFlashCfg, uint8_t group, uint8_t bank)
{
    uint32_t jdecId = 0;
    uint32_t i = 0;
    uint32_t offset;
    ...
    arch_memcpy_fast(pFlashCfg, &flashCfg_Gd_Q80E_Q16E, sizeof(SPI_Flash_Cfg_Type));
