...
void GLB_DAC_Init(GLB_DAC_Cfg_Type *cfg)
{
    ...
    /* Set DAC config */
    tmpVal = readl(GLB_BASE + GLB_GPDAC_CTRL_OFFSET);
    ...
    if (ENABLE == cfg->resetChanA) {
        ...
        tmpVal = writel(tmpVal, GLB_BASE + GLB_GPDAC_CTRL_OFFSET);
        __NOP();
        __NOP();
        __NOP();
        __NOP();
...
void GLB_DAC_Set_ChanA_Config(GLB_DAC_Chan_Cfg_Type *cfg)
{
    ...
    tmpVal = readl(GLB_BASE + GLB_GPDAC_ACTRL_OFFSET);
    ...
    tmpVal = reg_set_bits(tmpVal, GLB_GPDAC_A_RNG, cfg->outRange);

    tmpVal = writel(tmpVal, GLB_BASE + GLB_GPDAC_ACTRL_OFFSET);
}
...
void GLB_DAC_Set_ChanB_Config(GLB_DAC_Chan_Cfg_Type *cfg)
{
    ...
    /* Set channel B config */
    tmpVal = readl(GLB_BASE + GLB_GPDAC_BCTRL_OFFSET);
    ...
    tmpVal = reg_set_bits(tmpVal, GLB_GPDAC_B_RNG, cfg->outRange);

    tmpVal = writel(tmpVal, GLB_BASE + GLB_GPDAC_BCTRL_OFFSET);
}
...
void GPIP_DAC_ChanB_Enable(void)
{
    ...
    tmpVal = readl(GPIP_BASE + GPIP_GPDAC_CONFIG_OFFSET);
    tmpVal = reg_set_bit(tmpVal, GPIP_GPDAC_EN2);
    writel(tmpVal, GPIP_BASE + GPIP_GPDAC_CONFIG_OFFSET);
}
...
void GPIP_DAC_ChanB_Disable(void)
{
    ...
    tmpVal = readl(GPIP_BASE + GPIP_GPDAC_CONFIG_OFFSET);
    tmpVal = reg_clr_bit(tmpVal, GPIP_GPDAC_EN2);
    writel(tmpVal, GPIP_BASE + GPIP_GPDAC_CONFIG_OFFSET);
}
...
void GLB_GPIP_DAC_Set_ChanB_Config(GLB_GPIP_DAC_ChanB_Cfg_Type *cfg)
{
    ...
    /* GPIP enable or disable channel */
    tmpVal = readl(GPIP_BASE + GPIP_GPDAC_CONFIG_OFFSET);
    tmpVal = reg_set_bits(tmpVal, GPIP_GPDAC_EN2, cfg->chanEn);
    writel(tmpVal, GPIP_BASE + GPIP_GPDAC_CONFIG_OFFSET);
    ...
    writel(tmpVal, GLB_BASE + GLB_GPDAC_BCTRL_OFFSET);
}
...
void GLB_DAC_Set_ChanA_Value(uint16_t val)
{
    ...
    tmpVal = readl(GLB_BASE + GLB_GPDAC_DATA_OFFSET);
    tmpVal = reg_set_bits(tmpVal, GLB_GPDAC_A_DATA, val);
    writel(tmpVal, GLB_BASE + GLB_GPDAC_DATA_OFFSET);
}
...
void GLB_DAC_Set_ChanB_Value(uint16_t val)
{
    ...
    tmpVal = writel(tmpVal, GLB_BASE + GLB_GPDAC_DATA_OFFSET);
}
