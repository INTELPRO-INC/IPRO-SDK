...
#ifndef __DRV_AUADC_H__
#define __DRV_AUADC_H__
...
typedef enum {
    ...
    AUADC_SOURCE_PDM_CH_LEFT,                /*!< AUADC Souce select PDM left (EXT INFO) */
    AUADC_SOURCE_PDM_CH_RIGHT,               /*!< AUADC Souce select PDM right (EXT INFO) */
}AUADC_Source_Type;
...
typedef struct {
    ...
    int dmaEn;                       /*!< enable DMA FIFO interface */
}AUADC_FifoCfg_Type;
...
typedef struct {
    ...
    int adc_clock_phase_invert_en;              /* adc clock phase invert */
    ...
    int pga_chopper_en;                         /* PGA chopper control, ENABLE or DISABLE */
    ...
    int sdm_lowpower_en;                        /* SDM lowpower funciton en, ENABLE: 0.6 of disable */
    ...
    int dem_en;                                 /* dem function control, ENABLE or DISABLE */
    AUADC_ADC_SDM_QuantizerGain_Type sdm_qg;            /* quantizer gain control for SDM */
    int sdm_dither_en;                          /* dither control, ENABLE or DISABLE */
    ...
    int pga_circuit_en;                         /* PGA related circuit enable */
    int sdm_circuit_en;                         /* SDM related circuit enable */
    int pga_positive_en;                        /* DISABLE, look into each channel will see high impedance, ENABLE:one of eight channel will be choose */
    int pga_negative_en;                        /* DISABLE, look into each channel will see high impedance, ENABLE:one of eight channel will be choose */
...
/* ========== Precision ADC (Measure Mode) API ========== */

#define AUADC_PADC_TIMEOUT  ((int32_t)0x80000000)

typedef struct {
    AUADC_ADC_AnalogCh_Type posi_ch;            /* Positive input channel AIN0~7 */
    AUADC_ADC_AnalogCh_Type nega_ch;            /* Negative input channel AIN0~7 (ignored if single-ended) */
    AUADC_ADC_PGA_Gain_Type pga_gain;           /* PGA gain 6~42dB */
    AUADC_ADC_PGA_Mode_Type pga_mode;           /* DC_DIFF or DC_SINGLE for measurement */
    AUADC_ADC_MeasureMode_Filter_Type filter;   /* Sinc3 or Low-latency */
    AUADC_ADC_MeasureMode_Rate rate;            /* Output data rate 2.5~4000 SPS */
} AUADC_PADC_Cfg_Type;
