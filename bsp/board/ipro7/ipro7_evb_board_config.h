/*
 * Copyright (c) 2026 IntelPro Technology Co., Ltd.
 * All rights reserved.
 *
 * This software and associated documentation files (the "Software") are
 * proprietary and confidential to IntelPro Technology Co., Ltd..
 *
 * Access to and use of this Software is limited exclusively to authorized 
 * customers and partners of IntelPro Technology Co., Ltd. who have entered into a 
 * valid license or non-disclosure agreement.
 *
 * Unauthorized copying, modification, distribution, or reverse engineering 
 * of this Software, via any medium, is strictly prohibited without 
 * prior written permission from IntelPro Technology Co., Ltd..
 *
 * SPDX-License-Identifier: Proprietary
 */

/**
 * @file ipro7_evb_board_config.h
 * @brief Auto-generated board configuration for ipro7_evb
 *
 * Generated by ipro_drv_gen tool
 * Chip: ipro7
 * Board: ipro7_evb
 *
 * DO NOT EDIT THIS FILE MANUALLY
 */

#ifndef __IPRO7_EVB_BOARD_CONFIG_H__
#define __IPRO7_EVB_BOARD_CONFIG_H__

#include "drv_clock.h"
#include "drv_gpio.h"
#include "drv_glb.h"
#include "drv_hbn.h"
#include "drv_timer.h"
#include "drv_pwm.h"
#include "drv_adc.h"

/*==============================================================================
 * Pin Configuration Table
 *============================================================================*/

/**
 * Pinmux configuration table for ipro7_evb
 * Format: { pin, function, mode, pull, drive, smt }
 */
#ifdef CONFIG_DISABLE_SDC_PINMUX
/* SF3 external flash mode: GPIO 28-33 are flash pins, NOT SD card */
#define BOARD_PINMUX_TABLE \
    { GLB_GPIO_PIN_0, 14, GPIO_MODE_AF, GPIO_PULL_UP, 0, 1 }, /* GPIO0: JTAG */ \
    { GLB_GPIO_PIN_1, 14, GPIO_MODE_AF, GPIO_PULL_UP, 0, 1 }, /* GPIO1: JTAG */ \
    { GLB_GPIO_PIN_2, 14, GPIO_MODE_AF, GPIO_PULL_UP, 0, 1 }, /* GPIO2: JTAG */ \
    { GLB_GPIO_PIN_11, 14, GPIO_MODE_AF, GPIO_PULL_UP, 0, 1 }, /* GPIO11: JTAG */ \
    { GLB_GPIO_PIN_15, 11, GPIO_MODE_OUTPUT, GPIO_PULL_DOWN, 2, 1 }, /* GPIO15: GPIO */ \
    { GLB_GPIO_PIN_20, 7, GPIO_MODE_AF, GPIO_PULL_UP, 0, 1 }, /* GPIO20: UART */ \
    { GLB_GPIO_PIN_21, 7, GPIO_MODE_AF, GPIO_PULL_UP, 0, 1 } /* GPIO21: UART */
#define BOARD_PINMUX_COUNT 7
#else
/* Standard EVB: GPIO 28-33 configured as SD card */
#define BOARD_PINMUX_TABLE \
    { GLB_GPIO_PIN_0, 14, GPIO_MODE_AF, GPIO_PULL_UP, 0, 1 }, /* GPIO0: JTAG */ \
    { GLB_GPIO_PIN_1, 14, GPIO_MODE_AF, GPIO_PULL_UP, 0, 1 }, /* GPIO1: JTAG */ \
    { GLB_GPIO_PIN_2, 14, GPIO_MODE_AF, GPIO_PULL_UP, 0, 1 }, /* GPIO2: JTAG */ \
    { GLB_GPIO_PIN_11, 14, GPIO_MODE_AF, GPIO_PULL_UP, 0, 1 }, /* GPIO11: JTAG */ \
    { GLB_GPIO_PIN_15, 11, GPIO_MODE_OUTPUT, GPIO_PULL_DOWN, 2, 1 }, /* GPIO15: GPIO */ \
    { GLB_GPIO_PIN_20, 7, GPIO_MODE_AF, GPIO_PULL_UP, 0, 1 }, /* GPIO20: UART */ \
    { GLB_GPIO_PIN_21, 7, GPIO_MODE_AF, GPIO_PULL_UP, 0, 1 }, /* GPIO21: UART */ \
    { GLB_GPIO_PIN_28, 27, GPIO_MODE_AF, GPIO_PULL_UP, 2, 1 }, /* GPIO28: SDC */ \
    { GLB_GPIO_PIN_29, 27, GPIO_MODE_AF, GPIO_PULL_UP, 2, 1 }, /* GPIO29: SDC */ \
    { GLB_GPIO_PIN_30, 27, GPIO_MODE_AF, GPIO_PULL_NONE, 2, 1 }, /* GPIO30: SDC */ \
    { GLB_GPIO_PIN_31, 27, GPIO_MODE_AF, GPIO_PULL_UP, 2, 1 }, /* GPIO31: SDC */ \
    { GLB_GPIO_PIN_32, 27, GPIO_MODE_AF, GPIO_PULL_UP, 2, 1 }, /* GPIO32: SDC */ \
    { GLB_GPIO_PIN_33, 27, GPIO_MODE_AF, GPIO_PULL_UP, 2, 1 } /* GPIO33: SDC */
#define BOARD_PINMUX_COUNT 13
#endif

/**
 * Initialize all board pinmux settings
 */
#define BOARD_PINMUX_INIT() \
    do { \
        static GLB_GPIO_Cfg_Type pinmux_table[] = { BOARD_PINMUX_TABLE }; \
        for (int i = 0; i < BOARD_PINMUX_COUNT; i++) { \
            GLB_GPIO_Init(&pinmux_table[i]); \
        } \
        GLB_GPIO_OUTPUT_Enable(GLB_GPIO_PIN_15); /* GPIO15: SW GPIO Output */ \
    } while(0)

/*==============================================================================
 * UART Routing Configuration
 *============================================================================*/

#define BOARD_GLB_UART_SIG_8_FUNC    GLB_UART_SIG_FUN_UART0_TXD
#define BOARD_GLB_UART_SIG_9_FUNC    GLB_UART_SIG_FUN_UART0_RXD

/**
 * Initialize UART signal routing
 */
#define BOARD_UART_ROUTING_INIT() \
    do { \
        GLB_UART_Fun_Sel(GLB_UART_SIG_8, GLB_UART_SIG_FUN_UART0_TXD); \
        GLB_UART_Fun_Sel(GLB_UART_SIG_9, GLB_UART_SIG_FUN_UART0_RXD); \
    } while(0)

/*==============================================================================
 * Peripheral Clock Configuration
 *============================================================================*/

/**
 * Initialize peripheral clocks
 * Generated API calls based on chip definition and board config
 */
#define BOARD_PERIPHERAL_CLOCK_INIT() \
    do { \
        GLB_Set_UART_CLK(1, HBN_UART_CLK_XCLK, 0); \
        GLB_Set_I2S_CLK(1, GLB_I2S_OUT_REF_CLK_NONE); \
        TIMER_Set_Clock(TIMER0_ID, TIMER_CH0, TIMER_CLKSRC_XTAL, 31); \
        TIMER_Set_Clock(TIMER0_ID, TIMER_CH1, TIMER_CLKSRC_XTAL, 31); \
        WDT_Set_Clock(WDT0_ID, TIMER_CLKSRC_32K, 31); \
        GLB_Set_SDH_CLK(1, GLB_SDH_CLK_PLL_96M, 1); \
        GLB_Set_USB_CLK(1); \
        GLB_Set_SPI_CLK(ENABLE, GLB_SPI_CLK_BCLK, 0); \
    } while(0)

/*==============================================================================
 * Clock Gating Configuration
 *============================================================================*/

/**
 * Ungate (enable) peripheral clocks
 */
#define BOARD_CLOCK_GATING_INIT() \
    do { \
        GLB_PER_Clock_UnGate(GLB_AHB_CLOCK_DMA_0); \
        GLB_PER_Clock_UnGate(GLB_AHB_CLOCK_UART0); \
        GLB_PER_Clock_UnGate(GLB_AHB_CLOCK_UART1); \
        GLB_PER_Clock_UnGate(GLB_AHB_CLOCK_SPI0); \
        GLB_PER_Clock_UnGate(GLB_AHB_CLOCK_SPI1); \
        GLB_PER_Clock_UnGate(GLB_AHB_CLOCK_TIMER); \
        GLB_PER_Clock_UnGate(GLB_AHB_CLOCK_USB); \
        GLB_PER_Clock_UnGate(GLB_AHB_CLOCK_SDH); \
        GLB_PER_Clock_UnGate(GLB_AHB_CLOCK_BLE); \
        GLB_PER_Clock_UnGate(GLB_AHB_CLOCK_M154); \
        GLB_PER_Clock_UnGate(GLB_AHB_CLOCK_SEC_ENG); \
        GLB_PER_Clock_UnGate(GLB_AHB_CLOCK_PKA); \
    } while(0)

/*==============================================================================
 * System Clock Configuration
 *
 * Values below are the authoritative per-board clock setup.  Override by
 * editing this header — no Kconfig fallback.
 *============================================================================*/

#define XTAL_TYPE                      EXTERNAL_XTAL_32M
#define BOARD_CLK_ROOT_SOURCE_MACRO    GLB_DLL_XTAL_32M
#define BOARD_CLK_SYS_CLK_MACRO        GLB_SYS_CLK_PLL192M
#define BOARD_CLK_HCLK_DIV             0      /* HCLK = SYS_CLK / (div + 1) */
#define BOARD_CLK_BCLK_DIV             1      /* BCLK = HCLK   / (div + 1) */

/**
 * Initialize system clock (root source and PLL).
 *
 * GLB_Set_System_CLK switches root to XTAL/RC32M, retunes PLL, then switches
 * back. The intermediate step pauses BCLK long enough to stall any code/data
 * sitting in OCRAM (BCLK-clocked). When boot2 has already programmed the
 * chip to the exact target (root + PLL_SEL match BOARD_CLK_SYS_CLK_MACRO),
 * skip the destructive retune. GLB_Set_System_CLK_Div has its own internal
 * skip-if-equal check, so calling it unconditionally is safe.
 *
 * Behaviour matrix:
 *   boot2=PLL192, board target=PLL192 → skip GLB_Set_System_CLK
 *   boot2=PLL192, board target=DLL128 → run full retune (different target)
 *   boot2=PLL192, board target=DLL128, divs match → divs no-op
 */
static inline int board_system_clock_already_at_target(void)
{
    uint32_t cfg0 = *(volatile uint32_t *)(GLB_BASE + GLB_CLK_CFG0_OFFSET);
    uint32_t root = (cfg0 >> GLB_HBN_ROOT_CLK_SEL_POS) &
                    ((1U << GLB_HBN_ROOT_CLK_SEL_LEN) - 1);
    uint32_t pll  = (cfg0 >> GLB_REG_PLL_SEL_POS) &
                    ((1U << GLB_REG_PLL_SEL_LEN) - 1);

    if ((BOARD_CLK_SYS_CLK_MACRO) == GLB_SYS_CLK_RC32M)
        return root == 0;
    if ((BOARD_CLK_SYS_CLK_MACRO) == GLB_SYS_CLK_XTAL)
        return root == 1;
    /* DLL/PLL output: root must be DLL (encoding 2 or 3) and PLL_SEL must
     * match (BOARD_CLK_SYS_CLK_MACRO - GLB_SYS_CLK_DLL25P6M). */
    return (root == 2 || root == 3) &&
           pll == ((BOARD_CLK_SYS_CLK_MACRO) - GLB_SYS_CLK_DLL25P6M);
}

#define BOARD_SYSTEM_CLOCK_INIT() \
    do { \
        if (!board_system_clock_already_at_target()) { \
            GLB_Set_System_CLK(BOARD_CLK_ROOT_SOURCE_MACRO, BOARD_CLK_SYS_CLK_MACRO); \
        } \
        GLB_Set_System_CLK_Div(BOARD_CLK_HCLK_DIV, BOARD_CLK_BCLK_DIV); \
    } while(0)

/**
 * Initialize MTimer to run at the current XCLK frequency.
 */
#define BOARD_MTIMER_CLK_INIT() \
    GLB_Set_MTimer_CLK(1, GLB_MTIMER_CLK_XCLK, \
        Clock_System_Clock_Get(DRV_SYSTEM_CLOCK_XCLK) / 1000 / 1000 / 2 - 1)

/**
 * XCLK source selection.  EVB uses external 32 MHz XTAL.
 */
#define BOARD_XCLK_INIT()    HBN_Set_XCLK_CLK_Sel(HBN_XCLK_CLK_XTAL)

/*==============================================================================
 * 32K Clock Configuration
 *============================================================================*/

#define BOARD_CLK_32K_SOURCE    RC32K
#define BOARD_CLK_32K_SOURCE_MACRO    HBN_32K_RC

/**
 * Initialize 32K clock source
 */
#define BOARD_32K_CLOCK_INIT() \
    do { \
        HBN_Power_Off_Xtal_32K(); \
        HBN_32K_Sel(HBN_32K_RC); \
    } while(0)

/*==============================================================================
 * Board Initialization
 *============================================================================*/

/**
 * Complete board peripheral initialization
 * Note: BOARD_SYSTEM_CLOCK_INIT and BOARD_32K_CLOCK_INIT should be called
 *       separately in system_clock_init() as they may need conditional handling
 */
#define BOARD_INIT() \
    do { \
        BOARD_PINMUX_INIT(); \
        BOARD_UART_ROUTING_INIT(); \
        BOARD_CLOCK_GATING_INIT(); \
        BOARD_PERIPHERAL_CLOCK_INIT(); \
    } while(0)

#endif /* __IPRO7_EVB_BOARD_CONFIG_H__ */
