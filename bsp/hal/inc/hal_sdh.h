...
#define SDIO_CMDTIMEOUT_MS   (1000)                 /* time out */
#define SDIO_DATATIMEOUT_MS  (5000)                 /* time out */
#define SDH_GET_TIME()  (mtimer_get_time_ms())        /* get time */
#define SDIO_SDCARD_INT_MODE (0)                    /* Interrupt mode, which can be paired with the OS */
#define SDH_DEBUG 1                                 /* debug printf */
#define SDH_DEBUG_TRANSFER 0                        /* debug printf */
#define EMMC_HIGHSPEED_MODE (0)


/*! @brief Reverse byte sequence in uint32_t */
#define SWAP_WORD_BYTE_SEQUENCE(x) (__builtin_bswap32(x))
...
typedef struct _mmc_ext_csd {
    uint8_t reserved_0_165[166];   // 0-165
    uint8_t rel_param;             // 166
    uint8_t rel_set;               // 167
    uint8_t reserved_168_176[9];   // 168-176
    uint8_t bootbusContions;       // 177
    uint8_t reserved_178;          // 178
    uint8_t partConfig;            // 179
    uint8_t reserved_180_182[3];   // 180-182
    uint8_t busWidth;              // 183
    uint8_t reserved_184;          // 184
    uint8_t hsTiming;              // 185
    uint8_t reserved_186;          // 186
    uint8_t powerClass;            // 187
    uint8_t reserved_188_191[4];   // 188-191
    uint8_t csdRev;                // 192
    uint8_t reserved_193;          // 193
    uint8_t csdStructure;          // 194
    uint8_t reserved_195;          // 195
    uint8_t cardType;              // 196
    uint8_t reserved_197_199[3];   // 197-199
    uint8_t powerClass_52_195;     // 200
    uint8_t powerClass_26_195;     // 201
    uint8_t powerClass_52_360;     // 202
    uint8_t powerClass_26_360;     // 203
    uint8_t reserved_204_221[18];  // 204-221
    uint8_t rel_wr_sec_count;      // 222
    uint8_t reserved_223_225[3];   // 223-225
    uint8_t bootMultSize;          // 226
    uint8_t reserved_227_511[285]; // 227-511
} mmc_ext_csd_t;
...
typedef struct _sd_card {
    union {
        sd_csd_t csd;           /*!< CSD for SDCard */
        mmc_ext_csd_t ext_csd;  /*!< EXT_CSD for EMMC */
    ...
    uint8_t type;               /*!< 1:sd, 2:emmc */
    uint8_t access_mode;        /*!< 0: byte mode, 1: sector mode */
    uint8_t rel_wr;             /*!< 0: not support rel_w,r 1: support rel_wr */
} sd_card_t;
/**
...
#define EXT_CSD_BOOT_BUS_CONTIONS           177  /* R/W */
...
/**
  * @brief  Mask for OCR Status R3 (OCR Register) for emmc
  */
#define EMMC_OCR_BUSY             (1<<31)
#define EMMC_SECTOR_MODE     ((uint32_t)0x40000000)

typedef enum _mmc_switch_mode {
    MMC_SWITCH_MODE_CMD_SET = 0x0,
    MMC_SWITCH_MODE_SET_BITS= 0x1,
    MMC_SWITCH_MODE_CLEAR_BITS = 0x2,
    MMC_SWITCH_MODE_WRITE_BYTE  =0x3,
} mmc_switch_mode_t;


#define SDH_ERR_MSG(a, ...) printf(a, ##__VA_ARGS__)
...
#if SDH_DEBUG_TRANSFER
#define SDH_MSG_TRANS(a, ...) printf(a, ##__VA_ARGS__)
#else
#define SDH_MSG_TRANS(a, ...)
#endif
