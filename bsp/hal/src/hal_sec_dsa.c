    ...
    for (; i >= k; i--) {
        ...
        for (j = 0; j < 8; j++) {
            ...
            if (isOne) {
                ...
                Sec_Eng_PKA_Read_Data(nregType, 5, (uint32_t *)pka_tmp, dataSize);
                printf("number:\r\n");
                platform_dump(pka_tmp, dataSize /*dataSize*4*/);
#endif
            }
...
#ifdef DSA_DBG
    printf("r:\r\n");
    platform_dump(r, dataSize /*dataSize*4*/);
#endif
    return 0;
}
...
int sec_dsa_decrypt_crt(uint32_t size, uint32_t *c, sec_dsa_crt_cfg_t *crtCfg, uint32_t *d, uint32_t *r)
{
    ...
    printf("qInv * (m1 - m2):\r\n");
    platform_dump(pka_tmp, dataSize /*dataSize*4*2*/);
#endif
