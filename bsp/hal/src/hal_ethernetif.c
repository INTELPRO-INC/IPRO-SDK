...
#define IP_ADDR1      (uint8_t)88
#define IP_ADDR2      (uint8_t)100
#define IP_ADDR3      (uint8_t)108
...
ATTR_NOCACHE_NOINIT_RAM_SECTION ATTR_EALIGN(4) uint8_t ethRxBuff[ETH_RXBUFNB][ETH_RX_BUFFER_SIZE] ATTR_EALIGN(4) = { 0 }; /* Ethernet Receive Buffers */
ATTR_NOCACHE_NOINIT_RAM_SECTION ATTR_EALIGN(4) uint8_t ethTxBuff[ETH_TXBUFNB][ETH_TX_BUFFER_SIZE] ATTR_EALIGN(4);         /* Ethernet Transmit Buffers */
...
void low_level_init(struct netif *netif)
{
    ...
    /* MAC address can be customized here if needed */
    #if 0
    {
        unsigned int mac[6];
        char* macaddr = getenv("MAC");
        if (macaddr != NULL) {
            sscanf(macaddr, "%02x:%02x:%02x:%02x:%02x:%02x", &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]);
            for (int i = 0; i < 6; i++)
                emac_cfg.mac_addr[i] = mac[i];
        }
    }
    #endif
    printf("MAC Addr: %02x:%02x:%02x:%02x:%02x:%02x\r\n", emac_cfg.mac_addr[0], emac_cfg.mac_addr[1], emac_cfg.mac_addr[2], emac_cfg.mac_addr[3], emac_cfg.mac_addr[4], emac_cfg.mac_addr[5]);
    /* set phy cfg */
#ifdef FPGA
    ...
    emac_phy_cfg_t phy_cfg = {
        ...
        .phy_id = 0x1cc810,     /*!< PHY OUI, masked */
        .phy_state = PHY_STATE_DOWN,
        .flags = PHY_BASIC_FEATURES | PHY_RTL8201F_S700_RMII_TIMINGS,
    };
#endif
...
static struct pbuf *low_level_input(struct netif *netif)
{
    ...
    EMAC_BD_Desc_Type *bd;
	  ...
	  if(bd->C_S_L & EMAC_BD_FIELD_MSK(RX_E)){
        ...
        if (bd->C_S_L & 0xFF) {
            //MSG("RX bd[%d] %x len: %x\r\n", thiz->rxIndexCPU, (int)bd->C_S_L & 0xFF, (int)bd->C_S_L >> 16);
            /* handle error */
            if (bd->C_S_L & EMAC_BD_FIELD_MSK(RX_OR)) {
                MSG("EMAC RX OR Error at %s:%d\r\n", __func__, __LINE__);
            }
            
            if (bd->C_S_L & EMAC_BD_FIELD_MSK(RX_RE)) {
                MSG("MAC RX RE Error at %s:%d\r\n", __func__, __LINE__);
            }
            
            if (bd->C_S_L & EMAC_BD_FIELD_MSK(RX_DN)) {
                MSG("MAC RX DN Error at %s:%d\r\n", __func__, __LINE__);
            }
            
            if (bd->C_S_L & EMAC_BD_FIELD_MSK(RX_TL)) {
                MSG("MAC RX TL Error at %s:%d\r\n", __func__, __LINE__);
            }
            
            if (bd->C_S_L & EMAC_BD_FIELD_MSK(RX_CRC)) {
                MSG("MAC RX CRC Error at %s:%d\r\n", __func__, __LINE__);
            }
            
            if (bd->C_S_L & EMAC_BD_FIELD_MSK(RX_LC)) {
...
//            MSG("RX bd after %x\r\n", (int)bd->C_S_L & 0xFF);
        ...
        if (h) {
            pbuf_take(h, (void *)(uintptr_t)bd->Buffer, pkt_len);
        }
        if ((++thiz->rxIndexCPU) > thiz->rxBuffLimit) {
            /* wrap back */
            thiz->rxIndexCPU = thiz->txBuffLimit + 1;
...
/**
  ...
  *
  * @param pvParameters the lwip network interface structure for this ethernetif
  */
void ethernetif_input(void *pvParameters)
{
...
#if CTX_TYPE
    static unsigned int rx_counter __attribute__((unused));
    err_t err = ERR_OK;
    EMAC_BD_Desc_Type *bd;
...
void dhcp_thread(void *argument)
{
    ...
    ip_addr_t ipaddr __attribute__((unused));
    ip_addr_t netmask __attribute__((unused));
    ip_addr_t gw __attribute__((unused));
    struct dhcp *dhcp;
    uint8_t iptxt[20];
...
//                    sprintf((char *)iptxt, "%s", ip4addr_ntoa(netif_ip4_addr(netif)));
                    MSG("IP address assigned by a DHCP server\r\n");
                } else {
