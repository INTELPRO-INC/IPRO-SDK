...
void platform_dump(uint8_t *data, uint32_t len);
...
static int sec_ecc_basic_parameter_cfg(uint32_t *n, uint32_t *prime_n, uint32_t *invr_n)
{
...
static int sec_ecc_point_mul_cfg(uint32_t *p, uint32_t *primeN_p, uint32_t *ori_1, uint32_t *bar2,
...
static int sec_ecdsa_point_double(sec_ecp_type id)
{
    /* index 2:BAR_Zero_x
     * index 3:BAR_Zero_y
     * index 4:BAR_Zero_z
...
static void sec_ecdsa_dump_temp_result()
{
    ...
    printf("2=\r\n");
    platform_dump(pka_tmp, ECP_SECP256R1_SIZE);
        ...
        for (j = 0; j < 8; j++) {
            ...
            if (isOne) {
                ...
                if (pka_p1_eq_inf == 1 && pka_p2_eq_inf == 0) {
                    ...
                    //printf("Error! infinite point + infinite point\r\n");
                    return -1;
                }
            }
...
/**
 * @brief Shamir's trick: compute u1*G + u2*Q in a single MSB-first loop
 *
 * @param id        curve ID (ECP_SECP256R1)
 * @param u1        8-word big-endian scalar
 * @param u2        8-word big-endian scalar
 * @param pub_x_bar public key Q.x in Montgomery form
 * @param pub_y_bar public key Q.y in Montgomery form
 * @return 0 on success, result point in P1(2,3,4)
 *
 * @note Precomputes table {G, Q, G+Q} and uses a software flag to track
 *       identity accumulator (PKA point_double turns identity into (0,0,0)
 *       which is not recognized by point_add_inf_check).
 */
static int sec_ecdsa_verify_shamir(uint8_t id,
                                   const uint32_t *u1, const uint32_t *u2,
                                   const uint32_t *pub_x_bar, const uint32_t *pub_y_bar)
{
    uint32_t tbl_x[3][8];
    uint32_t tbl_y[3][8];
    uint32_t tbl_z[3][8];
    const uint8_t *u1p = (const uint8_t *)u1;
    const uint8_t *u2p = (const uint8_t *)u2;
    uint32_t i, j, start;
    uint32_t acc_is_inf = 1;
    uint8_t b1, b2, idx;

    /* table[0] = G (Montgomery Jacobian) */
    memcpy(tbl_x[0], secp256r1_Gx, 32);
    memcpy(tbl_y[0], secp256r1_Gy, 32);
    memcpy(tbl_z[0], secp256r1_Zeroy, 32); /* BAR(1) = R mod P */

    /* table[1] = Q (already in Montgomery) */
    memcpy(tbl_x[1], pub_x_bar, 32);
    memcpy(tbl_y[1], pub_y_bar, 32);
    memcpy(tbl_z[1], secp256r1_Zeroy, 32); /* BAR(1) */

    /* table[2] = G + Q: load G into P1, Q into P2, add */
    Sec_Eng_PKA_Write_Data(ECP_SECP256R1_REG_TYPE, 2, tbl_x[0], ECP_SECP256R1_SIZE / 4, 0);
    Sec_Eng_PKA_Write_Data(ECP_SECP256R1_REG_TYPE, 3, tbl_y[0], ECP_SECP256R1_SIZE / 4, 0);
    Sec_Eng_PKA_Write_Data(ECP_SECP256R1_REG_TYPE, 4, tbl_z[0], ECP_SECP256R1_SIZE / 4, 0);
    Sec_Eng_PKA_Write_Data(ECP_SECP256R1_REG_TYPE, 5, tbl_x[1], ECP_SECP256R1_SIZE / 4, 0);
    Sec_Eng_PKA_Write_Data(ECP_SECP256R1_REG_TYPE, 6, tbl_y[1], ECP_SECP256R1_SIZE / 4, 0);
    Sec_Eng_PKA_Write_Data(ECP_SECP256R1_REG_TYPE, 7, tbl_z[1], ECP_SECP256R1_SIZE / 4, 0);
    sec_ecdsa_point_add(id);
    Sec_Eng_PKA_Read_Data(ECP_SECP256R1_REG_TYPE, 2, tbl_x[2], ECP_SECP256R1_SIZE / 4);
    Sec_Eng_PKA_Read_Data(ECP_SECP256R1_REG_TYPE, 3, tbl_y[2], ECP_SECP256R1_SIZE / 4);
    Sec_Eng_PKA_Read_Data(ECP_SECP256R1_REG_TYPE, 4, tbl_z[2], ECP_SECP256R1_SIZE / 4);

    /* Skip leading zero bytes where both u1 and u2 are zero */
    start = 0;
    while (start < 31 && u1p[start] == 0 && u2p[start] == 0) {
        start++;
    }

    /* MSB-first double-and-add loop */
    for (i = start; i < 32; i++) {
        for (j = 8; j > 0; j--) {
            if (!acc_is_inf) {
                sec_ecdsa_point_double(id);
            ...
            if (idx != 0) {
                idx--; /* 0-indexed: 0=G, 1=Q, 2=G+Q */
                if (acc_is_inf) {
                    /* First non-zero: load table point as ACC into P2 */
                    Sec_Eng_PKA_Write_Data(ECP_SECP256R1_REG_TYPE, 5, tbl_x[idx], ECP_SECP256R1_SIZE / 4, 0);
                    Sec_Eng_PKA_Write_Data(ECP_SECP256R1_REG_TYPE, 6, tbl_y[idx], ECP_SECP256R1_SIZE / 4, 0);
                    Sec_Eng_PKA_Write_Data(ECP_SECP256R1_REG_TYPE, 7, tbl_z[idx], ECP_SECP256R1_SIZE / 4, 0);
                    acc_is_inf = 0;
                } else {
                    /* ACC (P2) -> P1, load table into P2, add, result P1 -> P2 */
                    sec_ecdsa_copy_x2_to_x1(id);
                    Sec_Eng_PKA_Write_Data(ECP_SECP256R1_REG_TYPE, 5, tbl_x[idx], ECP_SECP256R1_SIZE / 4, 0);
                    Sec_Eng_PKA_Write_Data(ECP_SECP256R1_REG_TYPE, 6, tbl_y[idx], ECP_SECP256R1_SIZE / 4, 0);
                    Sec_Eng_PKA_Write_Data(ECP_SECP256R1_REG_TYPE, 7, tbl_z[idx], ECP_SECP256R1_SIZE / 4, 0);
                    sec_ecdsa_point_add(id);
                    /* Move result P1(2,3,4) -> P2(5,6,7) */
    ...
    /* Move final ACC from P2(5,6,7) to P1(2,3,4) */
    sec_ecdsa_copy_x2_to_x1(id);
...
{
    uint32_t pub_x_bar[8];
    uint32_t pub_y_bar[8];
    uint32_t pka_u1[8] = { 0 };
    uint32_t pka_u2[8] = { 0 };
    uint32_t i = 0;
...
#ifdef ECDSA_DBG
    printf("u1:\r\n");
    platform_dump(pka_u1, ECP_SECP256R1_SIZE);
#endif
...
#ifdef ECDSA_DBG
    printf("u2:\r\n");
    platform_dump(pka_u2, ECP_SECP256R1_SIZE);
#endif

    /* Step4: Shamir's trick - compute R = u1*G + u2*Q in one pass */
    Sec_Eng_PKA_CREG(ECP_SECP256R1_LT_REG_TYPE, 7, ECP_SECP256R1_SIZE / 4, 1);
    sec_ecc_point_mul_init(handle->ecpId);

    /* Convert Q public key to Montgomery form */
    Sec_Eng_PKA_Write_Data(ECP_SECP256R1_REG_TYPE, 5, (uint32_t *)handle->publicKeyx, ECP_SECP256R1_SIZE / 4, 0);
    Sec_Eng_PKA_CREG(ECP_SECP256R1_REG_TYPE, 2 * ECP_SECP256R1_LT_REG_INDEX - 1, ECP_SECP256R1_SIZE / 4, 1);
    Sec_Eng_PKA_CREG(ECP_SECP256R1_REG_TYPE, 2 * ECP_SECP256R1_LT_REG_INDEX, ECP_SECP256R1_SIZE / 4, 1);
    Sec_Eng_PKA_GF2Mont(ECP_SECP256R1_REG_TYPE, 5, ECP_SECP256R1_REG_TYPE, 5, 256,
                        ECP_SECP256R1_LT_REG_TYPE, ECP_SECP256R1_LT_REG_INDEX, ECP_SECP256R1_REG_TYPE, 0);
    Sec_Eng_PKA_Read_Data(ECP_SECP256R1_REG_TYPE, 5, pub_x_bar, ECP_SECP256R1_SIZE / 4);
    ...
    Sec_Eng_PKA_GF2Mont(ECP_SECP256R1_REG_TYPE, 6, ECP_SECP256R1_REG_TYPE, 6, 256,
                        ECP_SECP256R1_LT_REG_TYPE, ECP_SECP256R1_LT_REG_INDEX, ECP_SECP256R1_REG_TYPE, 0);
    Sec_Eng_PKA_Read_Data(ECP_SECP256R1_REG_TYPE, 6, pub_y_bar, ECP_SECP256R1_SIZE / 4);

    /* Clear LT register upper half (dirtied by GF2Mont) before point operations */
    Sec_Eng_PKA_CREG(ECP_SECP256R1_LT_REG_TYPE, 7, ECP_SECP256R1_SIZE / 4, 1);

    sec_ecdsa_verify_shamir(handle->ecpId, pka_u1, pka_u2, pub_x_bar, pub_y_bar);
    ...
    Sec_Eng_PKA_Mont2GF(ECP_SECP256R1_REG_TYPE, 2, ECP_SECP256R1_REG_TYPE, 5, ECP_SECP256R1_REG_TYPE, 6,
                        ECP_SECP256R1_LT_REG_TYPE, ECP_SECP256R1_LT_REG_INDEX, ECP_SECP256R1_REG_TYPE, 0);
    Sec_Eng_PKA_Read_Data(ECP_SECP256R1_REG_TYPE, 2, (uint32_t *)pub_x_bar, ECP_SECP256R1_SIZE / 4);
#ifdef ECDSA_DBG
    printf("R.x=\r\n");
    platform_dump(pub_x_bar, ECP_SECP256R1_SIZE);
#endif
...
int sec_ecdsa_sign(sec_ecdsa_handle_t *handle, const uint32_t *random_k, const uint32_t *hash, uint32_t hashLenInWord, uint32_t *r, uint32_t *s)
{
    ...
    while (maxTry1--) {
...
#ifdef ECDSA_DBG
        printf("Random k:\r\n");
        platform_dump(k, ECP_SECP256R1_SIZE);
#endif
...
#ifdef ECDSA_DBG
        printf("r:\r\n");
        platform_dump(r, ECP_SECP256R1_SIZE);
#endif
        sec_ecc_basic_parameter_init(handle->ecpId);
        /* step 3,get k^-1*/
        ...
        printf("k^-1 in Mont:\r\n");
        platform_dump(KInvert, ECP_SECP256R1_SIZE);
#endif
        ...
        printf("r*d:\r\n");
        platform_dump(pka_tmp, ECP_SECP256R1_SIZE * 2);
#endif
...
#ifdef ECDSA_DBG
        printf("s:\r\n");
        platform_dump(s, ECP_SECP256R1_SIZE);
#endif
