...
{
#ifdef CONFIG_COMPONENTS_YLOOP_ENABLE
  aos_post_event(EV_WIFI, evt->type, (unsigned long)evt->value);
#elif defined(CONFIG_IPRO_EVENT)
  /* Use ipro_event for WiFi events */
  ipro_event_post(IPRO_WIFI_EVENT, evt->type, &evt->value, sizeof(evt->value), 0);
#else
