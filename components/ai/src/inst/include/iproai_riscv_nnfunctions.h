...
int32_t iproai_convolve_s8_get_buffer_size(const nmsis_nn_dims *input_dims, const nmsis_nn_dims *filter_dims);
...
/* riscv_max_pool_u8() declaration removed: never defined in this tree, and
 * its only caller (forward_MAXPOOL_tflite_nmsis) was dead code. Declaring a
 * kernel that does not exist invites a link failure that reads as a
 * regression. NMSIS-NN provides riscv_max_pool_s8() if one is needed. */
