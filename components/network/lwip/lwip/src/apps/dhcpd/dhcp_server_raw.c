...
/* Announce ourselves as Router (opt 3) + DNS (opt 6) in OFFER/ACK. Default on
 * (SoftAP behaviour); see dhcpd_announce_router_dns() in dhcp_server.h. */
static u8_t lw_dhcpd_announce_router_dns = 1;

void dhcpd_announce_router_dns(int enable)
{
    lw_dhcpd_announce_router_dns = enable ? 1 : 0;
}
                  ...
                  u8_t *opt_buf, u16_t len)
{
    ...
    if (opt != NULL)
    {
        ...
        if (node != NULL)
        {
            //Fix http://jira.iprolab.com/browse/WIFI6-15
            if (0 == memcmp(node->chaddr, msg->chaddr, msg->hlen)) {
                return node;
            }
        }
    ...
    if (opt)
    {
        ...
        if (msg_type == DHCP_DISCOVER)
        {
            ...
            *opt_buf++ = 4;
            ...
            if (lw_dhcpd_announce_router_dns)
            {
