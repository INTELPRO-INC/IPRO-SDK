...
#ifdef RNMS_OTA_ENABLE
#include <rnm_ota.h>
#else
/* Stub when OTA is disabled (IPRO6LE first bring-up). */
static inline void rnms_handle_ota(rnms_t *rnm, rnm_msg_t *cmd)
{
    (void)rnm;
    (void)cmd;
}
...
void rnms_notify_sta_ip_addr(rnms_t *rnm)
{
    ...
    msg.hdr.cmd = IPRO_CMD_STA_IP_UPDATE_IND;
...
#endif
{
    ...
    struct ipro_wifi_scan_record *di;
    ...
    memcpy(di->bssid, item->bssid, sizeof(di->bssid));
    memcpy(di->ssid, item->ssid, sizeof(di->ssid) - 1);
...
static chr_t handle_scan_results(rnms_t *rnm, rnm_msg_t *cmd)
{
    ...
    cnt_limit = (msg_len_limit - sizeof(rnm_scan_ind_msg_t)) / sizeof(struct ipro_wifi_scan_record);
    cnt_total = wifi_mgmr_sta_scanlist_nums_get();
    if (cnt_total > cnt_limit) {
        cnt_total = cnt_limit;
    }
    ind_msg_len = sizeof(rnm_scan_ind_msg_t) + cnt_total * sizeof(struct ipro_wifi_scan_record);
...
#else
// XXX not complete
static void fill_sta_status(struct ipro_wifi_ap_record *record)
{
    int value_int;
    ...
    if (0 == wifi_mgmr_state_get(&value_int)) {
        if (WIFI_STATE_CONNECTED_IP_GOT == value_int ||
            ...
            record->link_status = IPRO_WIFI_LINK_STATUS_UP;
        } else {
            record->link_status = IPRO_WIFI_LINK_STATUS_DOWN;
        }
    } else {
        record->link_status = IPRO_WIFI_LINK_STATUS_UNKNOWN;
    }
...
static chr_t handle_sta_get_link_status(rnms_t *rnm, rnm_msg_t *cmd)
{
    ...
    if (wifi_mgmr_sta_state_get()) {
        ...
        rsp.record.link_status = IPRO_WIFI_LINK_STATUS_UP;
        rsp.record.rssi = rssi;
        rsp.record.channel = channel;
    } else {
        rsp.record.link_status = IPRO_WIFI_LINK_STATUS_DOWN;
    }
#else
...
// clang-format off
static const struct cmd_handler cmd_handler_table[IPRO_CMD_MAX] = {
    [IPRO_CMD_REBOOT]       = {handle_reboot},
    [IPRO_CMD_RESET]        = {handle_reset},
    [IPRO_CMD_HELLO]        = {handle_hello},
    [IPRO_CMD_PING]         = {handle_ping},
    [IPRO_CMD_GET_MAC_ADDR] = {handle_get_mac_addr},
    [IPRO_CMD_SCAN]         = {handle_scan},
    [IPRO_CMD_SCAN_RESULTS] = {handle_scan_results},
    // STA
    [IPRO_CMD_STA_CONNECT]            = {handle_sta_connect},
    [IPRO_CMD_STA_DISCONNECT]         = {handle_sta_disconnect},
    [IPRO_CMD_STA_SET_AUTO_RECONNECT] = {handle_sta_set_auto_reconnect},
    [IPRO_CMD_STA_GET_LINK_STATUS]    = {handle_sta_get_link_status},
    // AP
    [IPRO_CMD_AP_START]        = {handle_ap_start},
    [IPRO_CMD_AP_STOP]         = {handle_ap_stop},
    [IPRO_CMD_AP_GET_STA_LIST] = {handle_ap_get_sta_list},
    // Monitor
    [IPRO_CMD_MONITOR_START]       = {handle_monitor_start},
    [IPRO_CMD_MONITOR_STOP]        = {handle_monitor_stop},
    [IPRO_CMD_MONITOR_SET_CHANNEL] = {handle_montior_set_channel},
    [IPRO_CMD_MONITOR_GET_CHANNEL] = {handle_montior_get_channel},

    [IPRO_CMD_SET_LPM_MODE] = {handle_set_lpm_mode},

    [IPRO_CMD_GET_DEV_VERSION] = {handle_get_dev_version},
    [IPRO_CMD_OTA]             = {handle_ota},

    [IPRO_CMD_USER_EXT] = {handle_user_ext},
};
// clang-format on

static void check_transparent_txed(rnms_t *rnm, rnm_msg_t *msg, int output_ret)
{
    if (!(output_ret == RNM_OUTPUT_FN_TIMEOUT && msg->cmd.cmd == IPRO_CMD_PING &&
                rnm->heartbeat_timer)) {
        return;
    }
...
void rnms_msg_process(rnms_t *rnm, rnm_msg_t *msg)
{
    ...
    if (cmd_id < IPRO_CMD_MAX && cmd_handler_table[cmd_id].handler) {
