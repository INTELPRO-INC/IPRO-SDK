...
m=c^d
*/
static SEC_ENG_PKA_REG_SIZE_Type ipro_dsa_get_reg_size(uint32_t size)
{
    switch(size){
    case 64:
...
int32_t ipro_dsa_mexp_binary(uint32_t size,uint32_t *a,uint32_t *b,uint32_t *c,uint32_t *r)
{
    ...
    uint32_t isOne=0;
    uint8_t *p=(uint8_t *)b;
    SEC_ENG_PKA_REG_SIZE_Type nregType=ipro_dsa_get_reg_size(size);
    SEC_ENG_PKA_REG_SIZE_Type lregType=ipro_dsa_get_reg_size(size*2);
    ...
    MSG("number:\r\n");
    ipro_platform_dump(pka_tmp,dataSize*4);
#endif
    ...
    for(;i>=k;i--){
       ...
       for(j=0;j<8;j++){
            ...
            if(isOne){
                ...
                Sec_Eng_PKA_Read_Data(nregType,5,(uint32_t *)pka_tmp,dataSize);
                MSG("number:\r\n");
                ipro_platform_dump(pka_tmp,dataSize/*dataSize*4*/);
#endif
            }
            /* base = base * base % c */
...
#ifdef DSA_DBG
    MSG("r:\r\n");
    ipro_platform_dump(r,dataSize*4);
#endif
    return 0;
}
...
#ifdef DSA_DBG
    MSG("r:\r\n");
    ipro_platform_dump(r,dataSize/*dataSize*4*/);
#endif
    return 0;
}
...
int32_t ipro_dsa_decrypt_crt(uint32_t size,uint32_t *c,ipro_dsa_crt_cfg* crtCfg,uint32_t *d,uint32_t *r)
{
     ...
     * 10&11:qInv*(m1-m2)
     */
    ipro_dsa_mexp_mont(size,c,crtCfg->dP,crtCfg->p,crtCfg->invR_p,crtCfg->primeN_p,m1);
    ipro_dsa_mexp_mont(size,c,crtCfg->dQ,crtCfg->q,crtCfg->invR_q,crtCfg->primeN_q,m2);
   ...
   Sec_Eng_PKA_MSUB(nregType,4,nregType,4,nregType,5,nregType,7,1);
    ...
    MSG("qInv * (m1 - m2):\r\n");
    ipro_platform_dump(pka_tmp,dataSize/*dataSize*4*2*/);
#endif
...
#ifdef DSA_DBG
    MSG("r:\r\n");
    ipro_platform_dump(r,dataSize*4*2);
#endif
    return 0;
}
