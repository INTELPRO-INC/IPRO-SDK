...
int ipro_ecc_basic_parameter_init(uint8_t id)
{
...
int ipro_ecc_point_mul_init(uint8_t id)
{
...
void ipro_ecdsa_point_add_inf_check(uint8_t *pka_p1_eq_inf,uint8_t *pka_p2_eq_inf)
{
    uint8_t res[4];
...
void ipro_ecdsa_copy_x2_to_x1(uint8_t id)
{
    //X2->X1
...
void ipro_ecdsa_point_add(uint8_t id)
{
    /* index 2:BAR_Zero_x
     * index 3:BAR_Zero_y
     * index 4:BAR_Zero_z
...
void ipro_ecdsa_point_double(uint8_t id)
{
    /* index 2:BAR_Zero_x
     * index 3:BAR_Zero_y
     * index 4:BAR_Zero_z
...
#endif
int ipro_ecdsa_verify_point_mul(uint8_t id,const uint32_t *m )
{
    uint32_t i,j,k;
    uint32_t tmp;
    uint32_t isOne=0;
    ...
    for(;i>=k;i--){
        ...
        for(j=0;j<8;j++){
            isOne=tmp&(1<<j);
            if(isOne){
                ipro_ecdsa_point_add_inf_check(&pka_p1_eq_inf,&pka_p2_eq_inf);
                if(pka_p1_eq_inf==1 && pka_p2_eq_inf == 0){
                    //sum = X2
                    ipro_ecdsa_copy_x2_to_x1(id);
#ifdef ECDSA_DBG_DETAIL
                    MSG("sum = X2\r\n");
                    ipro_ecdsa_dump_temp_result();
#endif
...
int32_t ipro_ecdsa_verify( uint8_t id,const uint32_t *hash, uint32_t hashLen,const uint32_t *pkX,const uint32_t *pkY, const uint32_t *r, const uint32_t *s)
{
    ...
    /* Pointer check */
    if(hash==NULL || pkX==NULL || pkY==NULL || r==NULL ||s==NULL){
        return -1;
    }
    ...
    ipro_ecc_basic_parameter_init(id);
...
#ifdef ECDSA_DBG
    MSG("u1:\r\n");
    ipro_platform_dump(pka_u1,ECP_SECP256R1_SIZE);
#endif
...
#ifdef ECDSA_DBG
    MSG("u2:\r\n");
    ipro_platform_dump(pka_u2,ECP_SECP256R1_SIZE);
#endif
...
int32_t ipro_ecc_is_zero(uint8_t *a,uint32_t len)
{
    uint32_t i=0;
    for(i=0;i<len;i++){
        if(a[i]!=0){
...
int32_t ipro_ecc_cmp(uint8_t *a,uint8_t *b,uint32_t len)
{
    uint32_t i=0,j=0;
...
int32_t ipro_ecc_get_random_value(uint32_t *randomData,uint32_t *maxRef,uint32_t size)
{
    ...
    while(maxTry--){
        ret=platform_get_random((uint8_t *)randomData,size);
        if(ret<0){
            return -1;
        }
        if(maxRef!=NULL){
            if(ipro_ecc_cmp((uint8_t *)maxRef,(uint8_t *)randomData,size)>0){
                return 0;
            }
        }else{
            return 0;
...
{
	if(ipro_ecc_get_random_value(private_key,(uint32_t *)secp256r1N,32)<0){
		return -1;
	}
	return 0;
}
...
int32_t ipro_ecdsa_sign( uint8_t id,const uint32_t *private_key,const uint32_t *random_k,const uint32_t *hash,uint32_t *r,uint32_t *s)
{
    ...
    /* Pointer check */
    if(private_key==NULL||hash==NULL||r==NULL||s==NULL){
        return -1;
    }
    ...
    while(maxTry1--){
        /* step 1 ,get random k*/
        if(random_k==NULL){
            if(ipro_ecc_get_random_value(k,(uint32_t *)secp256r1N,32)<0){
                return -1;
            }
        }else{
...
#ifdef ECDSA_DBG
        MSG("Random k:\r\n");
        ipro_platform_dump(k,ECP_SECP256R1_SIZE);
#endif
        /*step 2, calc R=kG*/
        if(ipro_ecdh_get_public_key(id,NULL,NULL,k,Rx,Ry)<0){
            return -1;
        }
        if(ipro_ecc_is_zero((uint8_t *)Rx,32)){
            continue;
        }
        memcpy(r,Rx,32);
#ifdef ECDSA_DBG
        MSG("r:\r\n");
        ipro_platform_dump(r,ECP_SECP256R1_SIZE);
#endif
        ipro_ecc_basic_parameter_init(id);
        /* step 3,get k^-1*/
        ...
        MSG("k^-1 in Mont:\r\n");
        ipro_platform_dump(KInvert,ECP_SECP256R1_SIZE);
#endif
        ...
        /* Step 4,r*d     ((e + r * d) / k) */
        Sec_Eng_PKA_Write_Data(ECP_SECP256R1_REG_TYPE,4,(uint32_t *)private_key,ECP_SECP256R1_SIZE/4,0);
        ...
        MSG("r*d:\r\n");
        ipro_platform_dump(pka_tmp,ECP_SECP256R1_SIZE*2);
#endif
...
#ifdef ECDSA_DBG
        MSG("s:\r\n");
        ipro_platform_dump(s,ECP_SECP256R1_SIZE);
#endif
        /* Check s zero*/
        if(ipro_ecc_is_zero((uint8_t *)s,32)){
            continue;
        }
        return 0;
    }
