...
#include "ipro_sec_pka.h"
#include "bignum_ext.h"

struct ipro_device_s *bignum_pka;

#define Sec_Eng_PKA_MADD(dt, di, s0t, s0i, s1t, s1i, s2t, s2i, lastop) ipro_pka_madd(bignum_pka, s0i, s0t, di, dt, s1i, s1t, s2i, s2t, lastop)
#define Sec_Eng_PKA_MSUB(dt, di, s0t, s0i, s1t, s1i, s2t, s2i, lastop) ipro_pka_msub(bignum_pka, s0i, s0t, di, dt, s1i, s1t, s2i, s2t, lastop)
#define Sec_Eng_PKA_MMUL(dt, di, s0t, s0i, s1t, s1i, s2t, s2i, lastop) ipro_pka_mmul(bignum_pka, s0i, s0t, di, dt, s1i, s1t, s2i, s2t, lastop)
// #define Sec_Eng_PKA_LCMP()
#define Sec_Eng_PKA_Write_Data(dt, di, data, len, lastop) ipro_pka_write(bignum_pka, di, dt, data, len, lastop)
#define Sec_Eng_PKA_Read_Data(dt, di, data, len) ipro_pka_read(bignum_pka, di, dt, data, len)
#define Sec_Eng_PKA_Move_Data(dt, di, s0t, s0i, lastop) ipro_pka_movdat(bignum_pka, s0i, s0t, di, dt, lastop)
#define Sec_Eng_PKA_MREM(dt, di, s0t, s0i, s2t, s2i, lastop) ipro_pka_mrem(bignum_pka, s0i, s0t, di, dt, s2i, s2t, lastop)
#define Sec_Eng_PKA_MINV(dt, di, s0t, s0i, s2t, s2i, lastop) ipro_pka_minv(bignum_pka, s0i, s0t, di, dt, s2i, s2t, lastop)
#define Sec_Eng_PKA_MSQR(dt, di, s0t, s0i, s2t, s2i, lastop) ipro_pka_msqr(bignum_pka, s0i, s0t, di, dt, s2i, s2t, lastop)
#define Sec_Eng_PKA_CREG(dt, di, size, lastop) ipro_pka_clir(bignum_pka, di, dt, size, lastop)
#define Sec_Eng_PKA_Write_Immediate(dt, di, data, lastop) ipro_pka_slir(bignum_pka, di, dt, data, lastop)
#define Sec_Eng_PKA_LMUL2N(dt, di, s0t, s0i, bitshift, lastop) ipro_pka_lmul2n(bignum_pka, s0i, s0t, di, dt, bitshift, lastop)
#define Sec_Eng_PKA_Reset() bignum_pka = ipro_device_get_by_name("pka"); ipro_pka_init(bignum_pka)
#define Sec_Eng_PKA_BigEndian_Enable()
#define Sec_Eng_PKA_GF2Mont(dt, di, st, si, size, tt, ti, pt, pi) ipro_pka_gf2mont(bignum_pka, si, st, di, dt, ti, tt, pi, pt ,size)
#define Sec_Eng_PKA_MEXP(dt, di, s0t, s0i, s1t, s1i, s2t, s2i, lastop) ipro_pka_mexp(bignum_pka, s0i, s0t, di, dt, s1i, s1t, s2i, s2t, lastop)

#define HW_MAX(x, y) ((x) > (y) ? (x) : (y))

// Enable large number exp_mod to support RSA-4096, etc operations
#define IPRO_MPI_LARGE_NUM_SOFTWARE_MPI
...
int ipro_mpi_exp_mod_wo_lock(mbedtls_mpi *X, const mbedtls_mpi *A, const mbedtls_mpi *E, const mbedtls_mpi *N, mbedtls_mpi *_RN)
{
    int ret;
    uint8_t *tmp;
    int wsize;
...
#ifdef IPRO_MPI_LARGE_NUM_SOFTWARE_MPI
...
int mbedtls_mpi_exp_mod(mbedtls_mpi *X, const mbedtls_mpi *A, const mbedtls_mpi *E, const mbedtls_mpi *N, mbedtls_mpi *_RN)
{
    int ret;

    ipro_sec_pka_mutex_take();
    ret = ipro_mpi_exp_mod_wo_lock(X, A, E, N, _RN);
