...
struct ipro_device_s *ecp_pka;

#define Sec_Eng_PKA_MADD(dt, di, s0t, s0i, s1t, s1i, s2t, s2i, lastop) ipro_pka_madd(ecp_pka, s0i, s0t, di, dt, s1i, s1t, s2i, s2t, lastop)
#define Sec_Eng_PKA_MSUB(dt, di, s0t, s0i, s1t, s1i, s2t, s2i, lastop) ipro_pka_msub(ecp_pka, s0i, s0t, di, dt, s1i, s1t, s2i, s2t, lastop)
#define Sec_Eng_PKA_MMUL(dt, di, s0t, s0i, s1t, s1i, s2t, s2i, lastop) ipro_pka_mmul(ecp_pka, s0i, s0t, di, dt, s1i, s1t, s2i, s2t, lastop)
// #define Sec_Eng_PKA_LCMP()
#define Sec_Eng_PKA_Write_Data(dt, di, data, len, lastop) ipro_pka_write(ecp_pka, di, dt, data, len, lastop)
#define Sec_Eng_PKA_Read_Data(dt, di, data, len) ipro_pka_read(ecp_pka, di, dt, data, len)
#define Sec_Eng_PKA_Move_Data(dt, di, s0t, s0i, lastop) ipro_pka_movdat(ecp_pka, s0i, s0t, di, dt, lastop)
#define Sec_Eng_PKA_MREM(dt, di, s0t, s0i, s2t, s2i, lastop) ipro_pka_mrem(ecp_pka, s0i, s0t, di, dt, s2i, s2t, lastop)
#define Sec_Eng_PKA_MINV(dt, di, s0t, s0i, s2t, s2i, lastop) ipro_pka_minv(ecp_pka, s0i, s0t, di, dt, s2i, s2t, lastop)
#define Sec_Eng_PKA_MSQR(dt, di, s0t, s0i, s2t, s2i, lastop) ipro_pka_msqr(ecp_pka, s0i, s0t, di, dt, s2i, s2t, lastop)
#define Sec_Eng_PKA_CREG(dt, di, size, lastop) ipro_pka_clir(ecp_pka, di, dt, size, lastop)
#define Sec_Eng_PKA_Write_Immediate(dt, di, data, lastop) ipro_pka_slir(ecp_pka, di, dt, data, lastop)
#define Sec_Eng_PKA_LMUL2N(dt, di, s0t, s0i, bitshift, lastop) ipro_pka_lmul2n(ecp_pka, s0i, s0t, di, dt, bitshift, lastop)
#define Sec_Eng_PKA_Reset() ecp_pka = ipro_device_get_by_name("pka"); ipro_pka_init(ecp_pka)
#define Sec_Eng_PKA_BigEndian_Enable()

void Sec_Eng_PKA_LCMP(uint8_t *out, uint8_t s0t, uint8_t s0i, uint8_t s1t, uint8_t s1i)
{
    *out = ipro_pka_lcmp(ecp_pka, s0i, s0t, s1i, s1t);
}
