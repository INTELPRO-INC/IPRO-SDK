...
static ATTR_NOCACHE_NOINIT_RAM_SECTION struct ipro_aes_link_s link_ctx_temp;

void mbedtls_aes_init( mbedtls_aes_context *ctx )
{
    AES_VALIDATE( ctx != NULL );

    struct ipro_device_s *aes;

    aes = ipro_device_get_by_name("aes");

    memset( ctx, 0, sizeof( mbedtls_aes_context ) );
    ctx->aes = aes;

    ipro_group0_request_aes_access(aes);
    ipro_aes_link_init(aes);
}
                                  ...
                                  unsigned char output[16] )
{
    ...
    ipro_sec_aes_mutex_take();
    memcpy(ecb_enc_buf, input, 16);
    memcpy(&link_ctx_temp, &ctx->link_ctx, sizeof(struct ipro_aes_link_s));
    ipro_aes_link_update(ctx->aes, (uint32_t)&link_ctx_temp, ecb_enc_buf, ecb_dec_buf, 16);
    memcpy(&ctx->link_ctx, &link_ctx_temp, sizeof(struct ipro_aes_link_s));
    memcpy(output, ecb_dec_buf, 16);
    ipro_sec_aes_mutex_give();
                    ...
                    unsigned char *output )
{
    ...
    ipro_l1c_dcache_clean_invalidate_range((void *)input, length);
    ipro_l1c_dcache_clean_invalidate_range((void *)output, length);

    ipro_sec_aes_mutex_take();
    if( mode == MBEDTLS_AES_DECRYPT )
    {
        memcpy(&link_ctx_temp, &ctx->link_ctx, sizeof(struct ipro_aes_link_s));
        ipro_aes_link_update(ctx->aes, (uint32_t)&link_ctx_temp, input, output, length);
        memcpy(&ctx->link_ctx, &link_ctx_temp, sizeof(struct ipro_aes_link_s));
    }
    else
    {
