...
int usbh_submit_urb(struct usbh_urb *urb)
{
    ...
    if (timeout > 0) {
        ...
        ret = usb_osal_sem_take(qh->waitsem, timeout);
        if (ret < 0) {
            urb->timeout = 0;
            usbh_kill_urb(urb);
            return ret;
...
int usbh_test_mode(int mode)
{
    ...
    EHCI_HCOR->portsc[0] &= ~EHCI_PORTSC_PTC_MASK;

    switch (mode) {
        case 1: // Test_J
            EHCI_HCOR->portsc[0] |= EHCI_PORTSC_PTC_JSTATE;
            break;
        case 2: // Test_K
            EHCI_HCOR->portsc[0] |= EHCI_PORTSC_PTC_KSTATE;
            break;
        case 3: // TEST_SE0_NAK
            EHCI_HCOR->portsc[0] |= EHCI_PORTSC_PTC_SE0NAK;
            break;
        case 4: // Test_Packet
        {
            EHCI_HCOR->portsc[0] |= EHCI_PORTSC_PTC_PACKET;
            uint8_t *pp;
            uint8_t i;
            pp = temp;
            ...
            *pp++ = 0x7E;
            ...
            uint32_t regval = getreg32(BFLB_USB_BASE + USB_DMA_TFN_OFFSET);
            regval |= USB_ACC_CXF_HOV;
            putreg32(regval, BFLB_USB_BASE + USB_DMA_TFN_OFFSET);

            putreg32(temp, BFLB_USB_BASE + USB_DMA_CPS2_OFFSET);

            regval = getreg32(BFLB_USB_BASE + USB_DMA_CPS1_OFFSET);
            regval |= USB_DMA_TYPE_HOV;
            regval &= ~USB_DMA_LEN_HOV_MASK;
            regval |= (sizeof(temp) << USB_DMA_LEN_HOV_SHIFT);
            putreg32(regval, BFLB_USB_BASE + USB_DMA_CPS1_OFFSET);

            regval = getreg32(BFLB_USB_BASE + USB_DMA_CPS1_OFFSET);
            regval |= USB_DMA_START_HOV;
            putreg32(regval, BFLB_USB_BASE + USB_DMA_CPS1_OFFSET);

            timeout = 0;
            regval = getreg32(BFLB_USB_BASE + USB_DEV_ISG2_OFFSET);
            while (!(regval & USB_DMA_CMPLT_HOV)) {
                usb_osal_msleep(1);
                timeout++;
                if (timeout > 1000) {
            ...
            regval = getreg32(BFLB_USB_BASE + USB_DEV_ISG2_OFFSET);
            regval |= USB_DMA_CMPLT_HOV;
            putreg32(regval, BFLB_USB_BASE + USB_DEV_ISG2_OFFSET);
            EHCI_HCOR->portsc[0] |= EHCI_PORTSC_HC_TST_PKDONE;
            bl_irq_enable(USB_IRQn);
            break;
        }
