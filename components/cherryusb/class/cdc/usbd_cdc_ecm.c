...
struct pbuf *usbd_cdc_ecm_eth_rx(void)
{
    ...
    if (p == NULL) {
        /* pbuf pool momentarily exhausted (common under a sustained inbound
         * burst, e.g. an OTA upload). Drop THIS packet but MUST re-arm the OUT
         * read — otherwise the endpoint never restarts and RX wedges forever
         * (transfer dead-stops after a few tens of KB). TCP retransmits the
         * dropped segment, so no data is lost. */
        g_cdc_ecm_rx_data_length = 0;
        g_cdc_ecm_rx_data_buffer = NULL;
        usbd_ep_start_read(cdc_ecm_ep_data[CDC_ECM_OUT_EP_IDX].ep_addr,
                           g_cdc_ecm_rx_buffer, CDC_ECM_MAX_PACKET_SIZE);
        return NULL;
    }
