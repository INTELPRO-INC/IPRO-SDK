...
static int rndis_query_cmd_handler(uint8_t *data, uint32_t len)
{
    ...
    switch (cmd->Oid) {
        ...
        case OID_GEN_LINK_SPEED:
            /* ipro7 is full-speed only: 12 Mbps, unit = 100 bps */
        ...
        default:
            ...
            infomation_len = 0;
            USB_LOG_WRN("Unhandled query for Object ID 0x%lx\r\n", (unsigned long)cmd->Oid);
            break;
    }
...
{
    /* ipro7 is full-speed only, MPS = 64 */
...
int usbd_rndis_start_write(uint8_t *buf, uint32_t len)
{
    ...
    return usbd_ep_start_write(rndis_ep_data[RNDIS_IN_EP_IDX].ep_addr, buf, len);
}
...
int usbd_rndis_start_read(uint8_t *buf, uint32_t len)
{
    ...
    g_rndis_rx_data_length = 0;
    return usbd_ep_start_read(rndis_ep_data[RNDIS_OUT_EP_IDX].ep_addr, buf, len);
}
