...
static bool usbd_set_endpoint(const struct usb_endpoint_descriptor *ep_desc)
{
    ...
    ep_cfg.ep_mult = USB_GET_MULT(ep_desc->wMaxPacketSize);

    USB_LOG_INFO("Open ep:0x%02x type:%u mps:%u mult:%u\r\n",
                 ep_cfg.ep_addr, ep_cfg.ep_type, ep_cfg.ep_mps, ep_cfg.ep_mult);
...
#ifdef CONFIG_USBDEV_TEST_MODE
                extern void usbd_execute_test_mode(struct usb_setup_packet *setup);
...
void usbd_event_ep0_in_complete_handler(uint8_t ep, uint32_t nbytes)
{
    ...
    USB_LOG_DBG("EP0 send %ld bytes, %ld remained\r\n", nbytes, g_usbd_core.ep0_data_buf_residue);
...
void usbd_event_ep0_out_complete_handler(uint8_t ep, uint32_t nbytes)
{
    ...
    if (nbytes > 0) {
        ...
        USB_LOG_DBG("EP0 recv %ld bytes, %ld remained\r\n", nbytes, g_usbd_core.ep0_data_buf_residue);
