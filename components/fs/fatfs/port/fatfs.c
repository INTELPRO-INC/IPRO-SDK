/*
 * Copyright (C) 2015-2017 Alibaba Group Holding Limited
 * Modified 2025: Migrated from aos_compat to ipro_osal
 */
#include <stdio.h>
#include <string.h>
#include <sys/fcntl.h>
#include <stdlib.h>
#include <FreeRTOS.h>
#include <errno.h>
#include "fatfs.h"
#include "fatfs_diskio.h"
#include "ff.h"

#if defined(CONFIG_EVFS) && defined(CONFIG_EVFS_USE_FATFS)
/* Use EVFS - no legacy VFS needed */
#include "ipro_osal.h"
#else
/* Legacy VFS mode */
#include "vfs_inode.h"
#include "vfs_register.h"
#include <aos/kernel.h>
#endif

#if FF_USE_LFN == 0
#define MAX_NAME_LEN 12
#else
#define MAX_NAME_LEN FF_MAX_LFN
#endif

typedef struct _fsid_map_t
{
    int         index; /* Device index */
    const char *root;  /* The mount point of the physical device */
    const char *id;    /* The partition id of the device in fatfs */
} fsid_map_t;

#if defined(CONFIG_EVFS) && defined(CONFIG_EVFS_USE_FATFS)
/* EVFS mode - minimal dir struct, EVFS handles directory operations */
typedef struct _fat_dir_t
{
    DIR         ffdir;
    FILINFO     filinfo;
} fat_dir_t;
#else
/* Legacy VFS mode */
typedef struct _fat_dir_t
{
    aos_dir_t   dir;
    DIR         ffdir;
    FILINFO     filinfo;
    aos_dirent_t cur_dirent;
} fat_dir_t;
#endif

static fsid_map_t g_fsid[] = { { DEV_SD,  SD_MOUNTPOINT, SD_PARTITION_ID },
                               { DEV_MMC,  MMC_MOUNTPOINT, MMC_PARTITION_ID },
                               { DEV_USB, USB_MOUNTPOINT, USB_PARTITION_ID },
                               { DEV_RAM, RAM_MOUNTPOINT, RAM_PARTITION_ID } };

static FATFS *g_fatfs[FF_VOLUMES] = { 0 };

#if FF_USE_LFN == 3 /* Dynamic memory allocation */

/*------------------------------------------------------------------------*/
/* Allocate a memory block                                                */
/*------------------------------------------------------------------------*/

void *ff_memalloc(/* Returns pointer to the allocated memory block (null on not
                     enough core) */
                  UINT msize /* Number of bytes to allocate */
)
{
    return (void *)pvPortMalloc(
      msize); /* Allocate a new memory block with POSIX API */
}


/*------------------------------------------------------------------------*/
/* Free a memory block                                                    */
/*------------------------------------------------------------------------*/

void ff_memfree(void *mblock /* Pointer to the memory block to free */
)
{
    vPortFree(mblock); /* Free the memory block with POSIX API */
}

#endif


#if FF_FS_REENTRANT /* Mutal exclusion */

/*------------------------------------------------------------------------*/
/* Create a Synchronization Object                                        */
/*------------------------------------------------------------------------*/
/* This function is called in f_mount() function to create a new
/  synchronization object for the volume, such as semaphore and mutex.
/  When a 0 is returned, the f_mount() function fails with FR_INT_ERR.
*/

int ff_cre_syncobj(/* 1:Function succeeded, 0:Could not create the sync object
                    */
                   BYTE vol, /* Corresponding volume (logical drive number) */
                   FF_SYNC_t
                     *sobj /* Pointer to return the created sync object */
)
{
#if defined(CONFIG_EVFS) && defined(CONFIG_EVFS_USE_FATFS)
    int ret = ipro_osal_mutex_create((ipro_osal_mutex_t *)sobj);
    return (ret == IPRO_OSAL_OK) ? 1 : 0;
#else
    int ret = aos_mutex_new(sobj);
    return (ret == FR_OK) ? 1 : 0;
#endif
}


/*------------------------------------------------------------------------*/
/* Delete a Synchronization Object                                        */
/*------------------------------------------------------------------------*/
/* This function is called in f_mount() function to delete a synchronization
/  object that created with ff_cre_syncobj() function. When a 0 is returned,
/  the f_mount() function fails with FR_INT_ERR.
*/

int ff_del_syncobj(/* 1:Function succeeded, 0:Could not delete due to an error
                    */
                   FF_SYNC_t sobj /* Sync object tied to the logical drive to be
                                     deleted */
)
{
#if defined(CONFIG_EVFS) && defined(CONFIG_EVFS_USE_FATFS)
    ipro_osal_mutex_delete((ipro_osal_mutex_t *)&sobj);
#else
    aos_mutex_free(&sobj);
#endif
    return 1;
}


/*------------------------------------------------------------------------*/
/* Request Grant to Access the Volume                                     */
/*------------------------------------------------------------------------*/
/* This function is called on entering file functions to lock the volume.
/  When a 0 is returned, the file function fails with FR_TIMEOUT.
*/

int ff_req_grant(/* 1:Got a grant to access the volume, 0:Could not get a grant
                  */
                 FF_SYNC_t sobj /* Sync object to wait */
)
{
#if defined(CONFIG_EVFS) && defined(CONFIG_EVFS_USE_FATFS)
    int ret = ipro_osal_mutex_lock((ipro_osal_mutex_t *)&sobj, FF_FS_TIMEOUT);
    return (ret == IPRO_OSAL_OK) ? 1 : 0;
#else
    int ret = aos_mutex_lock(&sobj, FF_FS_TIMEOUT);
    return (ret == FR_OK) ? 1 : 0;
#endif
}


/*------------------------------------------------------------------------*/
/* Release Grant to Access the Volume                                     */
/*------------------------------------------------------------------------*/
/* This function is called on leaving file functions to unlock the volume.
 */

void ff_rel_grant(FF_SYNC_t sobj /* Sync object to be signaled */
)
{
#if defined(CONFIG_EVFS) && defined(CONFIG_EVFS_USE_FATFS)
    ipro_osal_mutex_unlock((ipro_osal_mutex_t *)&sobj);
#else
    aos_mutex_unlock(&sobj);
#endif
}

#endif

static int get_disk_index(int pdrv)
{
    int index;
    for (index = 0; index < FF_VOLUMES; index++) {
        if (g_fsid[index].index == pdrv) {
            return index;
        }
    }

    return -1;
}

/* VFS operations - only needed for legacy VFS mode */
#if !defined(CONFIG_EVFS) || !defined(CONFIG_EVFS_USE_FATFS)

static char *translate_relative_path(const char *path)
{
    int   len, prefix_len;
    char *relpath, *p;
    BYTE  pdrv;

    if (!path) {
        return NULL;
    }

    len = strlen(path);
    for (pdrv = 0; pdrv < FF_VOLUMES; pdrv++) {
        prefix_len = strlen(g_fsid[pdrv].root);
        if (strncmp(g_fsid[pdrv].root, path, prefix_len) == 0) {
            break;
        }
    }

    if (pdrv == FF_VOLUMES) {
        return NULL;
    }

    relpath = (char *)pvPortMalloc(len + 1);
    if (!relpath) {
        return NULL;
    }

    memset(relpath, 0, len + 1);
    memcpy(relpath, g_fsid[pdrv].id, strlen(g_fsid[pdrv].id));

    if (len > prefix_len) {
        p = (char *)(path + strlen(g_fsid[pdrv].root) + 1);
        memcpy(relpath + strlen(g_fsid[pdrv].id), p, len - prefix_len - 1);
    }
    relpath[len] = '\0';

    return relpath;
}

static int fatfs_mode_conv(int m)
{
    int res      = 0;
    int acc_mode = m & O_ACCMODE;
    if (acc_mode == O_RDONLY) {
        res |= FA_READ;
    } else if (acc_mode == O_WRONLY) {
        res |= FA_WRITE;
    } else if (acc_mode == O_RDWR) {
        res |= FA_READ | FA_WRITE;
    }

    if (m & O_CREAT) {
        res |= FA_OPEN_ALWAYS;
    }

    if (m & O_TRUNC) {
        res |= FA_CREATE_ALWAYS;
    }

    if (m & O_EXCL) {
        res |= FA_CREATE_NEW;
    }

    if (m & O_APPEND) {
        res |= FA_OPEN_APPEND;
    }

    if (res == 0) {
        res |= FA_OPEN_EXISTING;
    }
    return res;
}


static int fatfs_open(file_t *fp, const char *path, int flags)
{
    int   ret     = -EPERM;
    FIL  *f       = NULL;
    char *relpath = NULL;

    f = (FIL *)pvPortMalloc(sizeof(FIL));
    if (!f) {
        return -ENOMEM;
    }

    relpath = translate_relative_path(path);
    if (!relpath) {
        vPortFree(f);
        return -EINVAL;
    }

    ret = f_open(f, relpath, fatfs_mode_conv(flags));
    if (ret == FR_OK) {
        fp->f_arg = (void *)f;
        vPortFree(relpath);
        return ret;
    }

    vPortFree(relpath);
    vPortFree(f);
    return ret;
}

static int fatfs_close(file_t *fp)
{
    int  ret = -EPERM;
    FIL *f   = (FIL *)(fp->f_arg);

    if (f) {
        ret = f_close(f);
        if (ret == FR_OK) {
            vPortFree(f);
            fp->f_arg = NULL;
        }
    }

    return ret;
}

static ssize_t fatfs_read(file_t *fp, char *buf, size_t len)
{
    ssize_t nbytes;
    int     ret = -EPERM;
    FIL    *f   = (FIL *)(fp->f_arg);

    if (f) {
        if ((ret = f_read(f, (void *)buf, (UINT)len, (UINT *)&nbytes)) ==
            FR_OK) {
            return nbytes;
        }
    }

    return ret;
}

static ssize_t fatfs_write(file_t *fp, const char *buf, size_t len)
{
    ssize_t nbytes;
    int     ret = -EPERM;
    FIL    *f   = (FIL *)(fp->f_arg);

    if (f) {
        if ((ret = f_write(f, (void *)buf, (UINT)len, (UINT *)&nbytes)) ==
            FR_OK) {
            return nbytes;
        }
    }

    return ret;
}

static int fatfs_access(file_t *fp, const char *path, int amode)
{
    char   *relpath = NULL;
    FILINFO info;
    int     len, ret;

#if FF_FS_READONLY
    if (amode == W_OK) {
        return -EACCES;
    }
#endif

    relpath = translate_relative_path(path);
    if (!relpath) {
        return -EINVAL;
    }

    len = strlen(relpath);
    if (relpath[len - 1] == ':' && relpath[len] == '\0') {
        ret = FR_OK;
    } else {
        ret = f_stat(relpath, &info);
    }

    vPortFree(relpath);
    return ret;
}

static off_t fatfs_lseek(file_t *fp, off_t off, int whence)
{
    off_t new_pos = 0;
    int   ret     = -EPERM;
    FIL  *f       = (FIL *)(fp->f_arg);

    if (f) {
        if (whence == SEEK_SET) {
            new_pos = off;
        } else if (whence == SEEK_CUR) {
            off_t cur_pos = f_tell(f);
            new_pos       = cur_pos + off;
        } else if (whence == SEEK_END) {
            off_t size = f_size(f);
            new_pos    = size + off;
        } else {
            return -EINVAL;
        }

        if ((ret = f_lseek(f, new_pos)) != FR_OK) {
            return ret;
        }
    }

    return new_pos;
}

static int fatfs_sync(file_t *fp)
{
    int  ret = -EPERM;
    FIL *f   = (FIL *)(fp->f_arg);

    if (f) {
        ret = f_sync(f);
    }

    return ret;
}

static int fatfs_stat(file_t *fp, const char *path, struct stat *st)
{
    char   *relpath = NULL;
    FILINFO info;
    int     len, ret;

    relpath = translate_relative_path(path);
    if (!relpath) {
        return -EINVAL;
    }

    len = strlen(relpath);
    if (relpath[len - 1] == ':' && relpath[len] == '\0') {
        st->st_size = 0;
        st->st_mode = S_IRWXU | S_IRWXG | S_IRWXO | S_IFDIR;
        ret         = 0;
    } else {

        if ((ret = f_stat(relpath, &info)) == FR_OK) {
            st->st_size = info.fsize;
            st->st_mode = S_IRWXU | S_IRWXG | S_IRWXO |
                          ((info.fattrib & AM_DIR) ? S_IFDIR : S_IFREG);
        }
    }

    vPortFree(relpath);
    return ret;
}

static int fatfs_unlink(file_t *fp, const char *path)
{
    char *relpath = NULL;
    int   ret;

    relpath = translate_relative_path(path);
    if (!relpath) {
        return -EINVAL;
    }

    ret = f_unlink(relpath);

    vPortFree(relpath);
    return ret;
}

static int fatfs_rename(file_t *fp, const char *oldpath, const char *newpath)
{
    int   ret;
    char *oldname = NULL;
    char *newname = NULL;

    oldname = translate_relative_path(oldpath);
    if (!oldname) {
        return -EINVAL;
    }

    newname = translate_relative_path(newpath);
    if (!newname) {
        vPortFree(oldname);
        return -EINVAL;
    }

    ret = f_rename(oldname, newname);

    vPortFree(oldname);
    vPortFree(newname);
    return ret;
}

static aos_dir_t *fatfs_opendir(file_t *fp, const char *path)
{
    fat_dir_t *dp      = NULL;
    char      *relpath = NULL;

    relpath = translate_relative_path(path);
    if (!relpath) {
        return NULL;
    }

    dp = (fat_dir_t *)pvPortMalloc(sizeof(fat_dir_t) + MAX_NAME_LEN + 1);
    if (!dp) {
        vPortFree(relpath);
        return NULL;
    }

    memset(dp, 0, sizeof(fat_dir_t) + MAX_NAME_LEN + 1);
    if (f_opendir(&dp->ffdir, relpath) == FR_OK) {
        vPortFree(relpath);
        return (aos_dir_t *)dp;
    }

    vPortFree(relpath);
    vPortFree(dp);
    return NULL;
}

static aos_dirent_t *fatfs_readdir(file_t *fp, aos_dir_t *dir)
{
    fat_dir_t    *dp = (fat_dir_t *)dir;
    aos_dirent_t *out_dirent;

    if (!dp) {
        return NULL;
    }

    if (f_readdir(&dp->ffdir, &dp->filinfo) != FR_OK) {
        return NULL;
    }

    if (dp->filinfo.fname[0] == 0) {
        return NULL;
    }

    dp->cur_dirent.d_ino = 0;
    if (dp->filinfo.fattrib & AM_DIR) {
        dp->cur_dirent.d_type = AM_DIR;
    }

    strncpy(dp->cur_dirent.d_name, dp->filinfo.fname, MAX_NAME_LEN);
    dp->cur_dirent.d_name[MAX_NAME_LEN] = '\0';

    out_dirent = &dp->cur_dirent;
    return out_dirent;
}

static int fatfs_closedir(file_t *fp, aos_dir_t *dir)
{
    int        ret = -EPERM;
    fat_dir_t *dp  = (fat_dir_t *)dir;

    if (!dp) {
        return -EINVAL;
    }

    ret = f_closedir(&dp->ffdir);
    if (ret == FR_OK) {
        vPortFree(dp);
    }

    return ret;
}

static int fatfs_mkdir(file_t *fp, const char *path)
{
    int   ret     = -EPERM;
    char *relpath = NULL;

    relpath = translate_relative_path(path);
    if (!relpath) {
        return -EINVAL;
    }

    ret = f_mkdir(relpath);

    vPortFree(relpath);
    return ret;
}

static int fatfs_rmdir(file_t *fp, const char *path)
{
    int   ret     = -EPERM;
    char *relpath = NULL;

    relpath = translate_relative_path(path);
    if (!relpath) {
        return -EINVAL;
    }

    ret = f_rmdir(relpath);

    vPortFree(relpath);

    return ret;
}

static void fatfs_rewinddir(file_t *fp, aos_dir_t *dir)
{
    fat_dir_t *dp = (fat_dir_t *)dir;

    if (!dp) {
        return;
    }

    f_rewinddir(&dp->ffdir);

    return;
}

static long fatfs_telldir(file_t *fp, aos_dir_t *dir)
{
    fat_dir_t *dp = (fat_dir_t *)dir;

    if (!dp) {
        return -1;
    }

    return (long)(dp->ffdir.dptr);
}

static void fatfs_seekdir(file_t *fp, aos_dir_t *dir, long loc)
{
    fat_dir_t *dp = (fat_dir_t *)dir;

    if (!dp) {
        return;
    }

    dp->ffdir.dptr = loc;

    return;
}

static const fs_ops_t fatfs_ops = { .open      = &fatfs_open,
                                    .close     = &fatfs_close,
                                    .read      = &fatfs_read,
                                    .write     = &fatfs_write,
                                    .access    = &fatfs_access,
                                    .lseek     = &fatfs_lseek,
                                    .sync      = &fatfs_sync,
                                    .stat      = &fatfs_stat,
                                    .unlink    = &fatfs_unlink,
                                    .rename    = &fatfs_rename,
                                    .opendir   = &fatfs_opendir,
                                    .readdir   = &fatfs_readdir,
                                    .closedir  = &fatfs_closedir,
                                    .mkdir     = &fatfs_mkdir,
                                    .rmdir     = &fatfs_rmdir,
                                    .rewinddir = &fatfs_rewinddir,
                                    .telldir   = &fatfs_telldir,
                                    .seekdir   = &fatfs_seekdir,
                                    .ioctl     = NULL };

#endif /* !CONFIG_EVFS || !CONFIG_EVFS_USE_FATFS */

static int fatfs_dev_register(int pdrv)
{
    int    err, index;
    FATFS *fatfs = NULL;

    index = get_disk_index(pdrv);
    if (index < 0) {
        return -EINVAL;
    }

    if (g_fatfs[index] != NULL) {
        return FR_OK;
    }
    fatfs = (FATFS *)malloc(sizeof(FATFS));
    if (!fatfs) {
        return -ENOMEM;
    }

    err = f_mount(fatfs, g_fsid[index].id, 1);

    if (err == FR_OK) {
        g_fatfs[index] = fatfs;
#if defined(CONFIG_EVFS) && defined(CONFIG_EVFS_USE_FATFS)
        /* Skip legacy VFS registration when using EVFS */
        return FR_OK;
#else
        return aos_register_fs(g_fsid[index].root, &fatfs_ops, NULL);
#endif
    }

#if FF_USE_MKFS && !FF_FS_READONLY
    if (err == FR_NO_FILESYSTEM) {
        char *work = (char *)pvPortMalloc(FF_MAX_SS);
        if (!work) {
            err = -ENOMEM;
            goto error;
        }

        MKFS_PARM fs_para = {
            .fmt = FM_EXFAT,     /* Format option (FM_FAT, FM_FAT32, FM_EXFAT and FM_SFD) */
            .n_fat = 1,          /* Number of FATs */
            .align = 0,          /* Data area alignment (sector) */
            .n_root = 1,         /* Number of root directory entries */
            .au_size = 512 * 8,  /* Cluster size (byte) */
        };

        err = f_mkfs(g_fsid[index].id, &fs_para, work, FF_MAX_SS);
        vPortFree(work);

        if (err != FR_OK) {
            goto error;
        }

        f_mount(NULL, g_fsid[index].id, 1);
        err = f_mount(fatfs, g_fsid[index].id, 1);

        if (err == FR_OK) {
            g_fatfs[index] = fatfs;
#if defined(CONFIG_EVFS) && defined(CONFIG_EVFS_USE_FATFS)
            /* Skip legacy VFS registration when using EVFS */
            return FR_OK;
#else
            return aos_register_fs(g_fsid[index].root, &fatfs_ops, NULL);
#endif
        }
    }
error:
#endif
    free(fatfs);

    return err;
}


static int fatfs_dev_unregister(int pdrv)
{
    int err = FR_OK;
    int index;

    index = get_disk_index(pdrv);
    if (index < 0) {
        return -EINVAL;
    }

#if defined(CONFIG_EVFS) && defined(CONFIG_EVFS_USE_FATFS)
    /* EVFS mode - just unmount FatFS */
    f_mount(NULL, g_fsid[index].id, 1);
    vPortFree(g_fatfs[index]);
    g_fatfs[index] = NULL;
#else
    err = aos_unregister_fs(g_fsid[index].root);
    if (err == FR_OK) {
        f_mount(NULL, g_fsid[index].id, 1);
        vPortFree(g_fatfs[index]);
        g_fatfs[index] = NULL;
    }
#endif

    return err;
}

int fatfs_register(void)
{
    int err = -EINVAL;


#ifdef CONFIG_AOS_FATFS_SUPPORT_MMC
	if ((err = fatfs_dev_register(DEV_MMC)) != FR_OK) {
	}
#endif
	
#ifdef CONFIG_AOS_FATFS_SUPPORT_SD
	if ((err = fatfs_dev_register(DEV_SD)) != FR_OK) {
	}
#endif

#ifdef CONFIG_AOS_FATFS_SUPPORT_USB
    if ((err = fatfs_dev_register(DEV_USB)) != FR_OK) {
    }
#endif

#ifdef CONFIG_AOS_FATFS_SUPPORT_RAM
    if ((err = fatfs_dev_register(DEV_RAM)) != FR_OK) {
    }
#endif

    return err;
}

int fatfs_unregister(void)
{
    int err = -EINVAL;


#ifdef CONFIG_AOS_FATFS_SUPPORT_SD
    if ((err = fatfs_dev_unregister(DEV_SD)) != FR_OK) {
    }
#endif
				
#ifdef CONFIG_AOS_FATFS_SUPPORT_MMC
	if ((err = fatfs_dev_unregister(DEV_MMC)) != FR_OK) {
	}
#endif

#ifdef CONFIG_AOS_FATFS_SUPPORT_USB
    if ((err = fatfs_dev_unregister(DEV_USB)) != FR_OK) {
    }
#endif

#ifdef CONFIG_AOS_FATFS_SUPPORT_RAM
    if ((err = fatfs_dev_unregister(DEV_RAM)) != FR_OK) {
    }
#endif

    return err;
}

int fatfs_format(int pdrv, void (*progress_cb)(int))
{
    int    err, index;
    FATFS *fatfs = NULL;

    index = get_disk_index(pdrv);
    if (index < 0) {
        return -EINVAL;
    }

    if (g_fatfs[index] == NULL) {
        fatfs = (FATFS *)pvPortMalloc(sizeof(FATFS));
    } else {
        fatfs = g_fatfs[index];
        f_mount(NULL, g_fsid[index].id, 1);
    }

    char *work = (char *)pvPortMalloc(FF_MAX_SS);
    if (!work) {
        err = -ENOMEM;
        goto error;
    }
    
    MKFS_PARM fs_para = {
        .fmt = FM_EXFAT,     /* Format option (FM_FAT, FM_FAT32, FM_EXFAT and FM_SFD) */
        .n_fat = 1,          /* Number of FATs */
        .align = 0,          /* Data area alignment (sector) */
        .n_root = 1,         /* Number of root directory entries */
        .au_size = 512 * 128,  /* Cluster size (byte) */
    };
#if FF_FS_MKFS_PROGRESS_REPORT
    f_set_mkfs_cb(progress_cb);
#endif
    err = f_mkfs(g_fsid[index].id, &fs_para, work, FF_MAX_SS);
    vPortFree(work);
#if FF_FS_MKFS_PROGRESS_REPORT
    f_set_mkfs_cb(NULL);
#endif
    if (err != FR_OK) {
        goto error;
    }
    
    err = f_mount(fatfs, g_fsid[index].id, 1);

    if (err == FR_OK) {
        g_fatfs[index] = fatfs;
#if defined(CONFIG_EVFS) && defined(CONFIG_EVFS_USE_FATFS)
        /* EVFS mode - skip legacy VFS registration */
        return FR_OK;
#else
        return aos_register_fs(g_fsid[index].root, &fatfs_ops, NULL);
#endif
    }

error:
    vPortFree(fatfs);
    return err;
}

#if !FF_FS_READONLY && !FF_FS_NORTC
DWORD get_fattime (void)
{
    time_t t;
    struct tm *stm;

    t = time(0);
    stm = localtime(&t);

    if (stm->tm_year < 80)
        stm->tm_year = 80;
    return (DWORD)(stm->tm_year - 80) << 25 |
           (DWORD)(stm->tm_mon + 1) << 21 |
           (DWORD)stm->tm_mday << 16 |
           (DWORD)stm->tm_hour << 11 |
           (DWORD)stm->tm_min << 5 |
           (DWORD)stm->tm_sec >> 1;
}
#endif

