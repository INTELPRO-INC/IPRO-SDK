...
/* Filesystem */
#if defined(CONFIG_EVFS) && defined(CONFIG_EVFS_USE_ROMFS)
#include "evfs.h"
#include "evfs/romfs_fs.h"
#include "evfs_posix.h"
#define ROMFS_USE_EVFS 1
#else
#include <vfs.h>
#include <vfs_inode.h>
...
#ifdef ROMFS_USE_EVFS
    /* Get romfs total size from header (offset 8) */
    uint32_t romfs_size = U32HTONL(*((uint32_t *)romfs_root + 2));

    /* Register with EVFS using resource-based romfs */
    int ret = evfs_register_rsrc_romfs("romfs", (const uint8_t *)romfs_root, romfs_size, false);
    if (ret != EVFS_OK) {
        log_error("EVFS romfs registration failed: %d\r\n", ret);
        return ret;
    }

    /* Mount at /romfs mount point */
    ret = evfs_posix_mount(ROMFS_MOUNTPOINT, "romfs");
    if (ret != 0) {
        log_error("EVFS mount failed: %d\r\n", ret);
        return ret;
    }

    log_info("ROMFS mounted at %s via EVFS\r\n", ROMFS_MOUNTPOINT);
    return 0;
#else
