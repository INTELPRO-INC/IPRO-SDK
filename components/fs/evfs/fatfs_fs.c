...
typedef struct FatfsData_s {
  //FATFS *fs;
  uint8_t pdrv;
  char drive_prefix[8];  // Drive prefix like "sd:" or "0:"
  FILINFO info;
...
// Build full path with drive prefix
static void build_fatfs_path(char *dest, size_t dest_size, const char *prefix, const char *path) {
  if (path[0] == '/') {
    snprintf(dest, dest_size, "%s%s", prefix, path);
  } else {
    snprintf(dest, dest_size, "%s/%s", prefix, path);
  }
}
...
static int fatfs__open(Evfs *vfs, const char *path, EvfsFile *fh, int flags) {
  ...
  // Build full path with drive prefix
  char full_path[256];
  build_fatfs_path(full_path, sizeof(full_path), fs_data->drive_prefix, path);

  FRESULT status;
  status = f_open(&fil->fil, full_path, fs_flags);
...
static int fatfs__stat(Evfs *vfs, const char *path, EvfsInfo *info) {
  ...
  status = f_stat(full_path, &fs_data->info);
...
static int fatfs__rename(Evfs *vfs, const char *old_path, const char *new_path) {
  ...
  build_fatfs_path(old_full, sizeof(old_full), fs_data->drive_prefix, old_path);
  build_fatfs_path(new_full, sizeof(new_full), fs_data->drive_prefix, new_path);

  status = f_rename(old_full, new_full);
  return translate_error(status);
#else
  THROW(EVFS_ERR_NO_SUPPORT);
#endif
...
static int fatfs__open_dir(Evfs *vfs, const char *path, EvfsDir *dh) {
  ...
  status = f_opendir(&dir->dir, full_path);
  return translate_error(status);
...
// FatFS volume strings from FF_VOLUME_STRS: "ram", "flash", "sd", "mmc", "usb"
static const char * const fatfs_volume_strs[] = {"ram:", "flash:", "sd:", "mmc:", "usb:"};
#define FATFS_VOLUME_COUNT (sizeof(fatfs_volume_strs) / sizeof(fatfs_volume_strs[0]))
...
int evfs_register_fatfs(const char *vfs_name, uint8_t pdrv, bool default_vfs) {
  ...
  if(pdrv >= FATFS_VOLUME_COUNT) return EVFS_ERR_BAD_ARG;
  ...
  // Set drive prefix based on pdrv (e.g., "sd:" for DEV_SD)
  strncpy(fs_data->drive_prefix, fatfs_volume_strs[pdrv], sizeof(fs_data->drive_prefix) - 1);
  fs_data->drive_prefix[sizeof(fs_data->drive_prefix) - 1] = '\0';
