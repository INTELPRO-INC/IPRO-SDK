...
struct shell ATTR_PSRAM_SECTION _shell;
...
static char *shell_get_prompt(void)
{
    ...
    if (shell_prompt_custom) {
        snprintf(shell_prompt, sizeof(shell_prompt), "%s", shell_prompt_custom);
    } else {
        snprintf(shell_prompt, sizeof(shell_prompt), "%s", SHELL_DEFAULT_NAME);
    }
#if defined(SHELL_USING_FS)
    ...
    strncat(shell_prompt, "/>", sizeof(shell_prompt) - strlen(shell_prompt) - 1);
