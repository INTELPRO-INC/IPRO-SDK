 ...
 static atbm_void wpabuf_overflow(const struct wpabuf *buf, atbm_size_t len)
{
 ...
 int atbm_wpabuf_resize(struct wpabuf **_buf, atbm_size_t add_len)
{
 ...
 struct wpabuf * wpabuf_alloc_1(atbm_size_t len,atbm_uint32 call)
{
 ...
 struct wpabuf * atbm_wpabuf_alloc_ext_data(atbm_uint8 *data, atbm_size_t len)
{
 ...
 struct wpabuf * atbm_wpabuf_alloc_copy(const atbm_void *data, atbm_size_t len)
{
 ...
 struct wpabuf * atbm_wpabuf_dup(const struct wpabuf *src)
{
...
/**
 * atbm_wpabuf_free - Free a wpabuf
 * @buf: wpabuf buffer
 */
 atbm_void atbm_wpabuf_free(struct wpabuf *buf)
{
	if (buf == NULL)
		return;
	if (buf->flags & WPABUF_FLAG_EXT_DATA)
 ...
 atbm_void atbm_wpabuf_clear_free(struct wpabuf *buf)
{
	if (buf) {
...
{
	atbm_uint8 *pos = atbm_wpabuf_put(buf, 3);
...
{
	if (data)
		atbm_memcpy(atbm_wpabuf_put(buf, len), data, len);
}
				  ...
				  const struct wpabuf *src)
{
 ...
 atbm_void * atbm_wpabuf_put(struct wpabuf *buf, atbm_size_t len)
{
...
/**
 * atbm_wpabuf_concat - Concatenate two buffers into a newly allocated one
 * @a: First buffer
 * @b: Second buffer
 ...
 */
 struct wpabuf * atbm_wpabuf_concat(struct wpabuf *a, struct wpabuf *b)
{
...
/**
 * atbm_wpabuf_zeropad - Pad buffer with 0x00 octets (prefix) to specified length
 ...
 */
 struct wpabuf * atbm_wpabuf_zeropad(struct wpabuf *buf, atbm_size_t len)
{
	struct wpabuf *ret;
	atbm_size_t blen;
	...
	if (ret) {
		atbm_memset(atbm_wpabuf_put(ret, len - blen), 0, len - blen);
