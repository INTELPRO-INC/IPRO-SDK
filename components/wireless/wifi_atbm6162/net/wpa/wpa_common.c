				      ...
				      atbm_uint32 pairwise_cipher,atbm_uint16 key_index)
{
	...
	case ATBM_WPA_CIPHER_TKIP:		
		...
		atbm_memcpy(connect->key,ptk->tk1,16);
		atbm_memcpy(&connect->key[16],ptk->u.tk2,16);
...
atbm_uint32 atbm_wpa_cipher_to_suite(int proto, int cipher)
{
...
int atbm_wpa_parse_wpa_ie_wpa(const atbm_uint8 *wpa_ie, atbm_size_t wpa_ie_len,
				struct atbmwifi_wpa_ie_data *data)
{
...
int atbm_rsn_cipher_put_suites(atbm_uint8 *pos, int ciphers)
{
	int num_suites = 0;
...
int atbm_wpa_cipher_put_suites(atbm_uint8 *pos, int ciphers)
{
	int num_suites = 0;
...
int atbm_wpa_write_rsn_ie(struct atbmwifi_cfg *conf, atbm_uint8 *buf, atbm_size_t len,
		     const atbm_uint8 *pmkid)
{
	...
	suite = atbm_wpa_cipher_to_suite(ATBM_WPA_PROTO_RSN, conf->group_cipher);
	if (suite == 0) {
	...
	res = atbm_rsn_cipher_put_suites(pos, conf->pairwise_cipher);
	num_suites += res;
...
int wpa_write_wpa_ie(struct atbmwifi_cfg *conf, atbm_uint8 *buf, atbm_size_t len)
{
	...
	suite = atbm_wpa_cipher_to_suite(ATBM_WPA_PROTO_WPA, conf->group_cipher);
	if (suite == 0) {
	...
	num_suites = atbm_wpa_cipher_put_suites(pos, conf->pairwise_cipher);
	if (num_suites == 0) {
...
atbm_void wpa_comm_init_extra_ie(struct atbmwifi_vif *priv)
{
	...
	{
		wpa_ie_len = atbm_wpa_write_rsn_ie(config,
...
int atbm_wpa_eapol_key_mic(const atbm_uint8 *key, int ver, const atbm_uint8 *buf, atbm_size_t len,
		      atbm_uint8 *mic)
{
			     ...
			     struct wpa_eapol_ie_parse *ie)
{
	...
	if (pos[1] > ATBM_RSN_SELECTOR_LEN + 2 &&
		...
		atbm_wpa_hexdump_key(MSG_DEBUG, "WPA: SMK in EAPOL-Key",
	...
	if (pos[1] > ATBM_RSN_SELECTOR_LEN + 2 &&
		...
		atbm_wpa_hexdump(MSG_DEBUG, "WPA: Nonce in EAPOL-Key",
			    (const atbm_uint8*)pos, pos[1] + 2);
		return 0;
	}
	...
	if (pos[1] > ATBM_RSN_SELECTOR_LEN + 2 &&
		...
		atbm_wpa_hexdump(MSG_DEBUG, "WPA: IGTK in EAPOL-Key",
				(const atbm_uint8*)pos, pos[1] + 2);
		return 0;
	}
#endif /* CONFIG_IEEE80211W */
...
/**
 * atbm_wpa_pmk_to_ptk - Calculate PTK from PMK, addresses, and nonces
 ...
 */
atbm_void atbm_wpa_pmk_to_ptk(const atbm_uint8 *pmk, atbm_size_t pmk_len, const char *label,
...
int atbm_wpa_compare_rsn_ie(int ft_initial_assoc,
...
int eapol_input(struct atbmwifi_vif *priv,struct atbm_buff *skb)
{
	...
	atbmwifi_wpa_event_queue((atbm_wpa_event_data)priv,(atbm_wpa_event_data)skb,(atbm_wpa_event_data)ATBM_NULL,WPA_EVENT__EAP_RX,ATBM_WPA_EVENT_NOACK);
	return 1;
drop:
...
int wpa_drv_send_eapol(struct atbmwifi_vif *priv,const atbm_uint8 *dest,
				    atbm_uint16 proto, const atbm_uint8 *buf, atbm_size_t len, ATBM_BOOL noenc)
