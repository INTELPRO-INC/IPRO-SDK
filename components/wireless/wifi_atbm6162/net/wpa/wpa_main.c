 ...
 *
 * 文件说明:
 *   本文件实现 WPA Supplicant 核心功能，包括 WPA/WPA2/WPA3 认证、密钥管理、
 *   4 次握手协议、组密钥握手等。是 WiFi 安全连接的核心模块。
 *
 * 主要功能:
 *   - WPA Supplicant 初始化和销毁
 *   - 4 次握手协议实现（EAPOL-Key 处理）
 *   - PTK/GTK 密钥派生和安装
 *   - RSN/WPA IE 解析和生成
 *   - SAE 认证支持（WPA3）
 *   - 802.11w 管理帧保护
 ...
 *
 * 主要函数:
 *   - init_wpa_supplicant(): 初始化 WPA Supplicant
 *   - free_wpa_supplicant(): 释放 WPA Supplicant
 *   - atbmwifi_wpa_sm_rx_eapol(): 处理 EAPOL 帧
 *   - wpa_supplicant_process_1_of_4(): 处理 Message 1
 *   - wpa_supplicant_process_3_of_4(): 处理 Message 3
 *   - atbmwifi_wpa_supplicant_rx_eapol(): EAPOL 接收入口
 *
*****************************************************************************************************************/
...
struct wpa_supplicant *atbm_g_wpa_s;
			  ...
			  void *ctx, enum pmksa_free_reason reason)
{
 ...
 if (sm->cur_pmksa == entry) {
	 ...
	 atbm_pmksa_cache_clear_current(sm);
 ...
 /**************************************************************************************************************
 * 函数名: init_wpa_supplicant
 *
 * 功能描述:
 *   初始化 WPA Supplicant 实例。该函数为 STA 模式分配并初始化 WPA Supplicant 结构体，
 *   包括 WPA 状态机、SAE 配置、扩展能力等。
 *
 * 在连接流程中的位置:
 *   STA 模式初始化阶段
 *   在 atbmwifi_start_station() 中调用
 *
 * 参数说明:
 *   @priv: 虚拟接口指针，包含接口的配置和状态信息
 *
 * 返回值:
 *   成功: 返回 wpa_supplicant 结构体指针
 *   失败: 返回 NULL
 *
 * 初始化内容:
 *   - wpa_supplicant 结构体
 ...
 *   - SAE 组配置（如果启用 WPA3）
 *   - PMKSA 缓存（如果启用 SAE）
 *   - 扩展能力（Multi-BSSID 支持等）
 *
 * 注意事项:
 *   - 返回的指针保存在 priv->appdata 中
 *   - 需要调用 free_wpa_supplicant() 释放资源
 *   - SAE 支持需要 CONFIG_SAE 宏定义
 *
 * 相关函数:
 *   - free_wpa_supplicant(): 释放 WPA Supplicant
 *   - atbmwifi_start_station(): 调用此函数
 *
 **************************************************************************************************************/
...
//	p_info("init_wpa_supplicant");
	...
	atbm_g_wpa_s = wpa_s;
#if CONFIG_SAE
	wpa_s->wpa->pmksa = atbm_pmksa_cache_init(wpa_sm_pmksa_free_cb, wpa_s->wpa, wpa_s->wpa);	
 	atbm_memcpy(wpa_s->sae_groups, groups, sizeof(groups));
#endif
#if CONFIG_HE
	if(priv->hw_priv->chip_version != OCEANUS_NO_WIFI6){
		wpa_s->extended_capa_len = 11;
		wpa_s->extended_capa[2] |= ATBM_WLAN_EXT_CAPA3_MULTI_BSSID_SUPPORT;
	}
#endif
	return wpa_s;
}
...
/**************************************************************************************************************
 * 函数名: free_wpa_supplicant
 *
 * 功能描述:
 *   释放 WPA Supplicant 实例及其相关资源。该函数在 STA 模式关闭时调用，
 *   负责清理所有 WPA 相关的内存和状态。
 *
 * 在连接流程中的位置:
 *   STA 模式关闭阶段
 *   在 atbmwifi_stop_sta() 中调用
 *
 * 参数说明:
 *   @priv: 虚拟接口指针，包含接口的配置和状态信息
 *
 ...
 *
 * 释放的资源:
 *   - ap_wpa_ie: AP 的 WPA IE
 *   - ap_rsn_ie: AP 的 RSN IE
 *   - assoc_wpa_ie: 关联时使用的 WPA IE
 *   - sae_token: SAE 令牌（如果启用 SAE）
 *   - sae_data: SAE 数据（如果启用 SAE）
 *   - PMKSA 缓存（如果启用 SAE）
 *   - wpa_supplicant 结构体本身
 *
 * 注意事项:
 *   - 调用前应确保已断开连接
 *   - 函数会将 priv->appdata 设置为 NULL
 *   - 重复调用是安全的（会检查 appdata 是否为 NULL）
 *
 * 相关函数:
 *   - init_wpa_supplicant(): 初始化 WPA Supplicant
 *   - atbmwifi_stop_sta(): 调用此函数
 *
 **************************************************************************************************************/
 atbm_void free_wpa_supplicant(struct atbmwifi_vif *priv)
{
	if(priv->appdata){
		...
		atbm_wpabuf_free(wpa_s->sae_data);
		wpa_s->sae_data = ATBM_NULL;
		sae_clear_data(&wpa_s->sae);
		atbm_pmksa_cache_deinit(wpa_s->wpa->pmksa);
#endif /* CONFIG_SAE */
...
#if CONFIG_SAE
atbm_void atbm_wpa_sm_set_pmk(struct atbmwifi_wpa_sm *sm, const atbm_uint8 *pmk, atbm_size_t pmk_len,
	const atbm_uint8 *pmkid, const atbm_uint8 *bssid)
{
	if (sm == NULL)
		return;

	atbm_wpa_hexdump_key(MSG_DEBUG, "WPA: Set PMK based on external data",
		pmk, pmk_len);
	sm->pmk_len = pmk_len;
	...
	if (bssid) {
		atbm_pmksa_cache_add(sm->pmksa, pmk, pmk_len, pmkid, ATBM_NULL, 0,
...
/**
* atbm_wpa_sm_set_pmk_from_pmksa - Set PMK based on the current PMKSA
* @sm: Pointer to WPA state machine data from wpa_sm_init()
*
* Take the PMK from the current PMKSA into use. If no PMKSA is active, the PMK
* will be cleared.
*/
atbm_void atbm_wpa_sm_set_pmk_from_pmksa(struct atbmwifi_wpa_sm *sm)
{
	if (sm == NULL)
	return;

	if (sm->cur_pmksa) {
		atbm_wpa_hexdump_key(MSG_DEBUG,
...
static atbm_void __wpa_supplicant_event_assoc(struct atbmwifi_vif *priv,atbm_uint16 linkid)
{
	...
	if(priv->hw_priv->chip_version != OCEANUS_NO_WIFI6) {
		if(priv->bss.sta_priv.he_used &&
				atbmwifi_disbale_ap_tx_trigger(priv->hw_priv->chip_priv) &&
				priv->bss.sta_priv.rate.he_cap.he_cap_elem.mac_cap_info[3] & ATBM_IEEE80211_HE_MAC_CAP3_OMI_CONTROL){
			atbm_queue_work(priv->hw_priv, priv->set_omi_work);
		}
	}
#endif
...
/*�������ȵ��õ�WPA_EVENT__RX_PKG ����һ�Σ���ʱ���������),�ٵ���
//WPA_EVENT__SUPPLICANT_ASSOCIATED������һ�Σ���ʱ���������ˣ�
*/
...
atbm_void atbm_wpa_supplicant_mark_disassoc(struct wpa_supplicant *wpa_s)
{
	int bssid_changed;
					    ...
					    const atbm_uint8 *addr)
{
	...
	atbm_wpa_supplicant_mark_disassoc(wpa_s);
					       ...
					       const atbm_uint8 *buf, atbm_size_t len)
{
	...
	if (sm->tptk_set) {
		...
		atbm_wpa_eapol_key_mic(sm->tptk.kck, ver, buf, len,
				  key->key_mic);
	...
	if (!ok && sm->ptk_set) {
		atbm_memset(key->key_mic, 0, 16);
		atbm_wpa_eapol_key_mic(sm->ptk.kck, ver, buf, len,
				  key->key_mic);
		if (atbm_memcmp(mic, key->key_mic, 16) != 0) {
			wpa_printf(MSG_WARNING, "WPA: Invalid EAPOL-Key MIC "
				   "- dropping packet");
...
int atbm_wpa_supplicant_parse_ies(const atbm_uint8 *buf, atbm_size_t len,
			     struct wpa_eapol_ie_parse *ie)
{
	...
	for (pos = buf, end = pos + len; pos + 1 < end; pos += 2 + pos[1]) {
		...
		if (pos + 2 + pos[1] > end) {
			...
			atbm_wpa_hexdump(MSG_DEBUG, "WPA: Key Data",
		...
		if (*pos == ATBM_WLAN_EID_RSN) {
			...
			atbm_wpa_hexdump(MSG_DEBUG, "WPA: RSN IE in EAPOL-Key",
				    (const atbm_uint8*)ie->rsn_ie, ie->rsn_ie_len);
#if CONFIG_IEEE80211R
		} else if (*pos == WLAN_EID_MOBILITY_DOMAIN) {
			ie->mdie = pos;
			ie->mdie_len = pos[1] + 2;
			atbm_wpa_hexdump(MSG_DEBUG, "WPA: MDIE in EAPOL-Key",
				    (const atbm_uint8*)ie->mdie, ie->mdie_len);
		} else if (*pos == ATBM_WLAN_EID_FAST_BSS_TRANSITION) {
			ie->ftie = pos;
			ie->ftie_len = pos[1] + 2;
			atbm_wpa_hexdump(MSG_DEBUG, "WPA: FTIE in EAPOL-Key",
				    (const atbm_uint8*)ie->ftie, ie->ftie_len);
		} else if (*pos == WLAN_EID_TIMEOUT_INTERVAL && pos[1] >= 5) {
			if (pos[2] == WLAN_TIMEOUT_REASSOC_DEADLINE) {
				ie->reassoc_deadline = pos;
				atbm_wpa_hexdump(MSG_DEBUG, "WPA: Reassoc Deadline "
					    "in EAPOL-Key",
					    (const atbm_uint8*)ie->reassoc_deadline, pos[1] + 2);
			} else if (pos[2] == WLAN_TIMEOUT_KEY_LIFETIME) {
				ie->key_lifetime = pos;
				atbm_wpa_hexdump(MSG_DEBUG, "WPA: KeyLifetime "
					    "in EAPOL-Key",
					    (const atbm_uint8*)ie->key_lifetime, pos[1] + 2);
			} else {
				atbm_wpa_hexdump(MSG_DEBUG, "WPA: Unrecognized "
			...
			atbm_wpa_hexdump(MSG_DEBUG, "WPA: Unrecognized EAPOL-Key "
...
atbm_void atbm_wpa_eapol_key_send(struct atbmwifi_wpa_sm *sm, const atbm_uint8 *kck,
			int ver, const atbm_uint8 *dest, atbm_uint16 proto,
			atbm_uint8 *msg, atbm_size_t msg_len, atbm_uint8 *key_mic, ATBM_BOOL noenc)
{
...
//			wifi_printk(WIFI_DBG_MSG, "atbm_wpa_eapol_key_send: Failed to read BSSID "
//				   "\n\r");

		} else {
			dest = sm->bssid;

//			wifi_printk(WIFI_DBG_MSG, "atbm_wpa_eapol_key_send: Use BSSID (" MACSTR
...
//				   MAC2STR(dest));
	...
	if (key_mic &&
	    atbm_wpa_eapol_key_mic(kck, ver, msg, msg_len, key_mic)) {


		wifi_printk(WIFI_WPA, "atbm_wpa_eapol_key_send: "
			   "version %d MIC\n\r", ver);
...
//	wpa_sm_ether_send(sm, dest, proto, msg, msg_len);
	...
	wpa_drv_send_eapol(sm->wpa_s->priv,dest,proto,msg,msg_len, noenc);
...
out:
 ...
 int atbm_wpa_supplicant_send_2_of_4(struct atbmwifi_wpa_sm *sm, const unsigned char *dst,
...
//	atbm_wpa_hexdump(MSG_DEBUG, "WPA: WPA IE for msg 2/4", (const atbm_uint8*)wpa_ie, wpa_ie_len);
...
//��2��?������?
	...
	atbm_wpa_eapol_key_send(sm, ptk->kck, ver, dst, ATBM_ETH_P_EAPOL,
			   rbuf, rlen, reply->key_mic, ATBM_TRUE);
			  ...
			  struct atbmwifi_wpa_ptk *ptk)
{
	...
	atbm_wpa_pmk_to_ptk(sm->pmk, sm->pmk_len, "Pairwise key expansion",
...
//	wpa_sm_set_state(sm, ATBM_WPA_4WAY_HANDSHAKE);
	...
	if (sm->proto == ATBM_WPA_PROTO_RSN) {
		...
		atbm_wpa_supplicant_parse_ies(_buf, len, &ie);
		if (ie.pmkid) {
...
//			   ,ptk->kck[1],ptk->kck[5],ptk->kck[10],ptk->kck[15]);
		...
		atbm_set_rekey_data(sm->wpa_s->priv,ptk,1);
		wifi_printk(WIFI_ALWAYS,"set rekey to firmware \n");
#endif
#endif
	...
	if (atbm_wpa_supplicant_send_2_of_4(sm, sm->bssid, key, ver, sm->snonce,
...
/*		wpa_report_ie_mismatch(sm, "IE in 3/4 msg does not match "
	...
	if ((ie->wpa_ie && sm->ap_wpa_ie &&
	    ...
	    (ie->rsn_ie && sm->ap_rsn_ie &&
	     atbm_wpa_compare_rsn_ie(atbmwifi_wpa_key_mgmt_ft(sm->key_mgmt),
 ...
 int atbm_wpa_supplicant_send_4_of_4(struct atbmwifi_wpa_sm *sm, const unsigned char *dst,
			       const struct atbmwifi_wpa_eapol_key *key,
...
atbm_void atbmwifi_wpa_event_connect_timeout(struct atbmwifi_vif *priv)
{
...
static atbm_void wpa_supplicant_timeout(atbm_void *eloop_ctx, atbm_void *timeout_ctx)
{
	...
	atbmwifi_wpa_event_queue((atbm_wpa_event_data)wpas->priv,(atbm_wpa_event_data)timeout_ctx,(atbm_wpa_event_data)ATBM_NULL,
...
{
	atbmwifi_wpa_event_queue((atbm_wpa_event_data)priv,(atbm_wpa_event_data)ATBM_NULL,(atbm_wpa_event_data)ATBM_NULL,WPA_EVENT__EAP_TX_STATUS,ATBM_WPA_EVENT_NOACK);	
}
...
#if CONFIG_HE
			if(priv->hw_priv->chip_version != OCEANUS_NO_WIFI6){
				/*Disallow AP send trigger*/
				wifi_printk(WIFI_DBG_MSG, "he:%d omi suporrt:%x\n", priv->bss.sta_priv.he_used,
						priv->bss.sta_priv.rate.he_cap.he_cap_elem.mac_cap_info[3]);

				if(priv->bss.sta_priv.he_used && 
						atbmwifi_disbale_ap_tx_trigger(priv->hw_priv->chip_priv) &&
						priv->bss.sta_priv.rate.he_cap.he_cap_elem.mac_cap_info[3] & ATBM_IEEE80211_HE_MAC_CAP3_OMI_CONTROL){
					wifi_printk(WIFI_ALWAYS, "start set_omi_work\n");
					atbm_queue_work(priv->hw_priv, priv->set_omi_work);
				}
			}
#endif
			       ...
			       struct wpa_eapol_ie_parse *ie)
{
	...
	if (ie->igtk_len != ATBM_WPA_IGTK_KDE_PREFIX_LEN +
		...
		atbm_wpa_hexdump_key(MSG_DEBUG, "WPA: IGTK",
...
//	wpa_sm_set_state(sm, ATBM_WPA_4WAY_HANDSHAKE);
	...
	atbm_wpa_supplicant_parse_ies(pos, len, &ie);
//	atbm_wpa_supplicant_parse_ies(sm->ap_rsn_ie, sm->ap_rsn_ie_len, &ie);
...
//				   keylen, MAC2STR(sm->bssid));
	...
	*/
	if (atbm_wpa_supplicant_send_4_of_4(sm, sm->bssid, key, ver, key_info,
				       ATBM_NULL, 0, &sm->ptk)) {
		goto failed;
	}
					     ...
					     struct atbmwifi_wpa_gtk_data *gd)
{
	...
	atbm_wpa_supplicant_parse_ies(keydata, keydatalen, &ie);
	if (ie.gtk && !(key_info & ATBM_WPA_KEY_INFO_ENCR_KEY_DATA)) {
...
//��ȫ�ͷ�
	...
	atbm_wpa_eapol_key_send(sm, sm->ptk.kck, ver, sm->bssid, ATBM_ETH_P_EAPOL,
			   rbuf, rlen, reply->key_mic, ATBM_FALSE);
...
/**************************************************************************************************************
 * 函数名: atbmwifi_wpa_sm_rx_eapol
 *
 * 功能描述:
 *   处理接收到的 EAPOL 帧，实现 WPA/WPA2/WPA3 4 次握手协议。该函数是 WiFi 安全认证的核心，
 *   负责解析 EAPOL-Key 帧，验证 MIC，派生密钥，并完成密钥安装。
 *
 * 在连接流程中的位置:
 *   阶段 7: 4 次握手
 *   在关联成功后，收到 AP 发送的 EAPOL 帧时调用
 *
 * 参数说明:
 *   @priv:     虚拟接口指针，包含接口的配置和状态信息
 *   @sm:       WPA 状态机指针
 *   @src_addr: 源 MAC 地址（AP 的 MAC 地址）
 *   @buf:      EAPOL 帧数据
 *   @len:      EAPOL 帧长度
 *
 * 返回值:
 ...
 * 处理流程:
 *   1. 验证帧长度和格式
 *   2. 解析 802.1X 头部和 EAPOL-Key 头部
 *   3. 验证密钥类型（WPA/RSN）
 *   4. 根据 key_info 判断是哪条消息
 *   5. 验证 MIC（Message 3/4）
 *   6. 处理密钥数据（解密 GTK 等）
 *   7. 发送响应消息
 *   8. 安装密钥
 *
 ...
 *   - wpa_supplicant_process_1_of_4(): 处理 Message 1
 *   - wpa_supplicant_process_3_of_4(): 处理 Message 3
 *   - wpa_supplicant_send_2_of_4(): 发送 Message 2
 *   - wpa_supplicant_send_4_of_4(): 发送 Message 4
 *
 **************************************************************************************************************/
...
const atbm_uint8 * atbm_wpa_bss_get_ie(const atbm_uint8 *ie_buff, int ie_len, atbm_uint8 ie)
{
...
const atbm_uint8 * atbm_wpa_bss_get_vendor_ie(const atbm_uint8 *ie_buff, int ie_len, atbm_uint32 vendor_type)
{
	const atbm_uint8 *end, *pos;
		     ...
		     struct atbmwifi_wpa_ie_data *data)
{
	if (wpa_ie_len >= 1 && wpa_ie[0] == ATBM_WLAN_EID_RSN)
		...
		return atbm_wpa_parse_wpa_ie_wpa(wpa_ie, wpa_ie_len, data);
}
...
#if 0
		atbm_wpa_hexdump(MSG_DEBUG, "WPA: set AP WPA IE", ie, len);
#endif
...
#if 0
		atbm_wpa_hexdump(MSG_DEBUG, "WPA: set AP RSN IE", ie, len);
#endif
...
int atbm_wpa_gen_wpa_ie(struct atbmwifi_wpa_sm *sm, atbm_uint8 *wpa_ie, atbm_size_t wpa_ie_len)
{
	if (sm->proto == ATBM_WPA_PROTO_RSN)
				    ...
				    atbm_size_t *wpa_ie_len)
{
	...
	if (wpa_ie == ATBM_NULL)
		return -1;
	res = atbm_wpa_gen_wpa_ie(sm, wpa_ie, *wpa_ie_len);
	if (res < 0)
		return -1;
	*wpa_ie_len = res;

	atbm_wpa_hexdump(MSG_DEBUG, "WPA: Set own WPA IE default",
		    wpa_ie, *wpa_ie_len);
...
void wpa_supplicant_set_suites_to_config(struct atbmwifi_vif *priv)
{
	...
	if((config->privacy == 0)&&(bss->privacy == 0)){
		...
		if((!bss->sta_priv.wpa)/*&&(!bss->sta_priv.wps)*/){
			...
			if((config->password_len != 5) && (config->password_len != 13) && (config->password_len != 10) && (config->password_len != 26)){
				wifi_printk(WIFI_DBG_ERROR, "wpa_supplicant_set_suites_to_config:failed ,ATBM_FALSE keylen for wep\n\r");
				return;
			}
			...
			if((config->password_len== 5)||(config->password_len == 13))
			{
				...
				wpa_common_install_wepkey(priv, (char *)password_ascii,config->group_cipher,config->key_id,0/*Sta linkid == 0*/);
			}
			      ...
			      atbm_uint8 *wpa_ie, atbm_size_t *wpa_ie_len)
{
	...
	if (bss) {
		bss_wpa = atbm_wpa_bss_get_vendor_ie(bss->information_elements, 
			bss->len_information_elements, 
			WPA_IE_VENDOR_TYPE);
		bss_rsn = atbm_wpa_bss_get_ie(bss->information_elements, 
			bss->len_information_elements, 
			ATBM_WLAN_EID_RSN);
	} 
	else {
...
#endif /* CONFIG_IEEE80211W */
#if CONFIG_SAE
	} else if (0/*sel & ATBM_WPA_KEY_MGMT_FT_SAE*/) {
...
/*
	if (ssid->key_mgmt &
		...
		atbm_wpa_sm_set_pmk(wpa_s->wpa, ssid->psk, ATBM_PMK_LEN);
	else
		atbm_wpa_sm_set_pmk_from_pmksa(wpa_s->wpa);
*/
/*	wpa_printf(MSG_DEBUG, "WPA: atbm_wpa_sm_set_pmk:%02x %02x %02x %02x %02x %02x \n",
	ssid->psk[0],
	ssid->psk[1],
	ssid->psk[2],
	ssid->psk[3],
...
/**
 * atbm_eapol_sm_rx_eapol - Process received EAPOL frames
 ...
 * -1 failure
 */
int atbm_eapol_sm_rx_eapol(struct eapol_sm *sm, const atbm_uint8 *src, const atbm_uint8 *buf,
		      atbm_size_t len)
{
	int res = 1;
	return res;
...
/*		atbm_wpabuf_free(wpa_s->pending_eapol_rx);
		wpa_s->pending_eapol_rx = atbm_wpabuf_alloc_copy(buf, len);
...
//	wifi_printk(WIFI_DBG_MSG,"rx_eapol:k=%d,flag=%d\n",config->key_mgmt,wpa_s->drv_flags);
	...
	if (!atbmwifi_wpa_key_mgmt_wpa_psk(config->key_mgmt) &&
	    ...
	    atbm_eapol_sm_rx_eapol(wpa_s->eapol, src_addr, buf, len) > 0*/)
	{
...
atbm_void wpa_authen_assc_timeout(atbm_void *data1,atbm_void *data)
{
	atbmwifi_wpa_event_queue((atbm_wpa_event_data)data1,(atbm_wpa_event_data)data,(atbm_wpa_event_data)ATBM_NULL,
				WPA_EVENT__CONNECT_TIMEOUT,ATBM_WPA_EVENT_NOACK);
}
...
static atbm_void __wpa_prepare_auth(struct atbmwifi_vif *priv)
{
	...
	if(ap->sta_priv.wpa)
	{
		if(atbmwifi_wpa_supplicant_set_suites(wpa_s,ap, ssid,
	...
	if (atbmwifi_wpa_key_mgmt_sae(wpa_s->key_mgmt))
	{
		...
		rsn = atbm_wpa_bss_get_ie(priv->bss.information_elements, priv->bss.len_information_elements, ATBM_WLAN_EID_RSN);
		if (!rsn) {
			wpa_printf(MSG_DEBUG,
	...
	if (priv->config.auth_alg == ATBM_WLAN_AUTH_SAE &&
	    atbm_pmksa_cache_set_current(wpa_s->wpa, NULL, priv->bssid, ssid, 0,
				    NULL,
				    wpa_s->key_mgmt == ATBM_WPA_KEY_MGMT_FT_SAE ?
...
atbm_void wpa_prepare_auth_direct(struct atbmwifi_vif *priv)
{
...
static atbm_void __wpa_prepare_assciating(struct atbmwifi_vif *priv)
{
	...
	if(ap->sta_priv.wpa)
	{
		...
		if(atbmwifi_wpa_sm_set_assoc_wpa_ie_default(wpa_s->wpa, wpa_ie, &wpa_ie_len)){
			atbm_kfree(wpa_ie);
			return;
		}
	}
...
{
	atbmwifi_wpa_event_queue((atbm_wpa_event_data)priv,(atbm_wpa_event_data)ATBM_NULL,(atbm_wpa_event_data)ATBM_NULL,WPA_EVENT__SUPPLICANT_ASSOCIAT,ATBM_WPA_EVENT_NOACK);	 /*Already in HIF task*/
}
 ...
 int wpa_wsc_rx_process(struct atbmwifi_vif *priv, atbm_uint8 *buf, atbm_size_t len)
{
	...
	switch(eap_type){
		...
		case ATBM_EAP_TYPE_EXPANDED:/*WPS*/
			...
			wpa_buf = atbm_wpabuf_alloc_copy(pos, length);
 ...
 int wpa_wsc_tx_process(atbm_void *ctx, int type, const atbm_uint8 *buf, atbm_size_t len)
{
	...
	res = wpa_drv_send_eapol(priv, dst, ATBM_ETH_P_EAPOL, msg, msglen, ATBM_FALSE);
