 ...
 struct wpabuf * dh_init(const struct dh_group *dh, struct wpabuf **priv)
{
	...
	atbm_wpabuf_clear_free(*priv);
	*priv = wpabuf_alloc(dh->prime_len);
	if (*priv == NULL)
		return NULL;
	
	if (atbmwifi_os_get_random(atbm_wpabuf_put(*priv, dh->prime_len), dh->prime_len))
	{
		atbm_wpabuf_clear_free(*priv);
		*priv = NULL;
		return NULL;
	...
	if (crypto_mod_exp(dh->generator, dh->generator_len,
			   ...
			   &pv_len) < 0) {
		atbm_wpabuf_clear_free(pv);
		wpa_printf(MSG_INFO, "DH: crypto_mod_exp failed");
		return NULL;
	}

	atbm_wpabuf_put(pv, pv_len);
				 ...
				 const struct dh_group *dh)
{
	...
	if (crypto_mod_exp(wpabuf_head(peer_public), wpabuf_len(peer_public),
		...
		atbm_wpabuf_clear_free(shared);
		wpa_printf(MSG_INFO, "DH: crypto_mod_exp failed");
		return NULL;
	}
	atbm_wpabuf_put(shared, shared_len);
