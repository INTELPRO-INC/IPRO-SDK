...
{
	if(atbm_mbedtls_mpi_fill_random((atbm_mbedtls_mpi *)r, (atbm_mbedtls_mpi_bitlen((atbm_mbedtls_mpi *) m) + 7) / 8, os_get_random_f_rng, (void *)atbmwifi_os_get_random))
		return -1;
...
#define MOD_MUL( N )    do { MBEDTLS_MPI_CHK( atbm_ecp_modp( &N, e->group ) ); INC_MUL_COUNT } \
                        while( 0 )
...
struct crypto_ec * crypto_ec_init(int group)
{
	int built = 0;
	struct crypto_ec *e;
	atbm_mbedtls_ecp_group_id grp_id;
	...
	atbm_mbedtls_ecp_group_init(e->group);
	if (atbm_mbedtls_ecp_group_load(e->group, grp_id) != 0)
		goto done;
	built = 1;
...
struct crypto_ec_point * crypto_ec_point_init(struct crypto_ec *e)
{
	...
	if (!pt)
		return NULL;
	atbm_mbedtls_ecp_point_init(pt);
...
void crypto_ec_point_deinit(struct crypto_ec_point *p, int clear)
{
	...
	if (!p)
		return;
	atbm_mbedtls_ecp_point_free(point);
	atbm_kfree(point);
}
			...
			struct crypto_ec_point *c)
{
	...
	ret = atbm_mbedtls_ecp_muladd(e->group, (mbedtls_ecp_point *)c, &mul, (mbedtls_ecp_point *)a, &mul, (mbedtls_ecp_point *)b);
...
{
	int ret = atbm_mbedtls_ecp_mul( e->group, (mbedtls_ecp_point *)res, (atbm_mbedtls_mpi *)b, (mbedtls_ecp_point *)p, NULL, NULL );
	return ret == 0 ? 0 : -1;
}
...
{
	int ret = atbm_ecp_safe_invert_jac(e->group, (mbedtls_ecp_point *)p, 1);
	return ret == 0 ? 0 : -1;
}
...
{
	return atbm_mbedtls_ecp_is_zero((mbedtls_ecp_point *) p);
}

int crypto_ec_point_is_on_curve(struct crypto_ec *e,
				const struct crypto_ec_point *p)
{
	return !atbm_mbedtls_ecp_check_pubkey(e->group, (mbedtls_ecp_point *)p);
}
...
int crypto_ec_point_cmp(const struct crypto_ec *e,
			...
			const struct crypto_ec_point *b)
{
	return atbm_mbedtls_ecp_point_cmp((mbedtls_ecp_point *) a, (mbedtls_ecp_point *) b);
}
...
struct wpabuf * crypto_ecdh_get_pubkey(struct crypto_ecdh *ecdh, int inc_y)
{
	...
	ret = crypto_bignum_to_bin((struct crypto_bignum *)
				   ecdh->ec->key.pubkey.x, atbm_wpabuf_put(buf, len),
				   len, len);
	if (ret < 0)
		goto fail;
	if (inc_y) {
		ret = crypto_bignum_to_bin((struct crypto_bignum *)
					   ...
					   atbm_wpabuf_put(buf, len), len, len);
		if (ret < 0)
			goto fail;
	}
...
struct wpabuf * crypto_ecdh_set_peerkey(struct crypto_ecdh *ecdh, int inc_y,
					const atbm_uint8 *key, atbm_size_t len)
{
	int ret;
	...
	ret = wc_ecc_shared_secret_ex(&ecdh->ec->key, point,
				      atbm_wpabuf_put(secret, key_len), &key_len);
	if (ret != MP_OKAY)
		goto fail;
