...
int sae_set_group(struct sae_data *sae, int group)
{
	...
	sae_clear_data(sae);
	tmp = sae->tmp = (struct sae_temporary_data *)atbm_kzalloc(sizeof(*tmp), GFP_KERNEL);
	if (tmp == NULL)
		return -1;
...
void sae_clear_temp_data(struct sae_data *sae)
{
	...
	atbm_wpabuf_free(tmp->anti_clogging_token);
	atbm_kfree(tmp->pw_id);
	atbm_bin_clear_free(tmp, sizeof(*tmp));
	sae->tmp = NULL;
}
				 ...
				 atbm_uint8 *pwd_value)
{
	...
	atbm_wpa_hexdump_key(MSG_DEBUG, "SAE: pwd-seed", pwd_seed, SHA256_MAC_LEN);
	...
	if (atbmwifi_sha256_prf_bits(pwd_seed, SHA256_MAC_LEN, "SAE Hunting and Pecking",
		...
		return -1;
	if (bits % 8)
		atbm_buf_shift_right(pwd_value, sae->tmp->prime_len, 8 - bits % 8);
	atbm_wpa_hexdump_key(MSG_DEBUG, "SAE: pwd-value",
				 ...
				 struct crypto_bignum *pwe)
{
	...
	if (atbmwifi_sha256_prf_bits(pwd_seed, SHA256_MAC_LEN, "SAE Hunting and Pecking",
			    ...
			    bits) < 0)
		return -1;
	atbm_wpa_hexdump_key(MSG_DEBUG, "SAE: pwd-value", pwd_value,
			sae->tmp->prime_len);
			      ...
			      atbm_size_t password_len, const char *identifier)
{
	...
	int res = -1;
	atbm_uint8 found = 0; /* 0 (ATBM_FALSE) or 0xff (ATBM_TRUE) to be used as const_time_*
		       * mask */
	...
	atbm_wpa_hexdump_ascii_key(MSG_DEBUG, "SAE: password",
...
fail:
	...
	atbm_bin_clear_free(tmp_password, password_len);
...
fail:
	...
	atbm_bin_clear_free(pwe_buf, prime_len * 2);
...
static int sae_derive_k_ecc(struct sae_data *sae, atbm_uint8 *k)
{
	...
	atbm_wpa_hexdump_key(MSG_DEBUG, "SAE: k", k, sae->tmp->prime_len);
...
static int sae_derive_keys(struct sae_data *sae, const atbm_uint8 *k)
{
	...
	atbmwifi_hmac_sha256(null_key, sizeof(null_key), k, sae->tmp->prime_len,
		    keyseed);
	atbm_wpa_hexdump_key(MSG_DEBUG, "SAE: keyseed", keyseed, sizeof(keyseed));
	...
	atbm_wpa_hexdump(MSG_DEBUG, "SAE: PMKID", val, SAE_PMKID_LEN);
	...
	atbm_wpa_hexdump_key(MSG_DEBUG, "SAE: KCK", sae->tmp->kck, SAE_KCK_LEN);
	atbm_wpa_hexdump_key(MSG_DEBUG, "SAE: PMK", sae->pmk, SAE_PMK_LEN);
		      ...
		      const struct wpabuf *token, const char *identifier)
{
	...
	if (token) {
		...
		atbm_wpa_hexdump(MSG_DEBUG, "SAE: Anti-clogging token",
			    wpabuf_head(token), wpabuf_len(token));
	}
	pos = atbm_wpabuf_put(buf, sae->tmp->prime_len);
	crypto_bignum_to_bin(sae->tmp->own_commit_scalar, pos,
			     sae->tmp->prime_len, sae->tmp->prime_len);
	atbm_wpa_hexdump(MSG_DEBUG, "SAE: own commit-scalar",
		    pos, sae->tmp->prime_len);
	if (sae->tmp->ec) {
		pos = atbm_wpabuf_put(buf, 2 * sae->tmp->prime_len);
		crypto_ec_point_to_bin(sae->tmp->ec,
				       sae->tmp->own_commit_element_ecc,
				       pos, pos + sae->tmp->prime_len);
		atbm_wpa_hexdump(MSG_DEBUG, "SAE: own commit-element(x)",
			    pos, sae->tmp->prime_len);
		atbm_wpa_hexdump(MSG_DEBUG, "SAE: own commit-element(y)",
			    pos + sae->tmp->prime_len, sae->tmp->prime_len);
	} else {
		pos = atbm_wpabuf_put(buf, sae->tmp->prime_len);
		crypto_bignum_to_bin(sae->tmp->own_commit_element_ffc, pos,
				     sae->tmp->prime_len, sae->tmp->prime_len);
		atbm_wpa_hexdump(MSG_DEBUG, "SAE: own commit-element",
			    pos, sae->tmp->prime_len);
	}
				   ...
				   atbm_size_t *token_len)
{
	...
	atbm_wpa_hexdump(MSG_DEBUG, "SAE: Anti-Clogging Token", *pos, tlen);
	if (token)
		*token = *pos;
	if (token_len)
		*token_len = tlen;
				   ...
				   const atbm_uint8 *end)
{
	...
	atbm_wpa_hexdump(MSG_DEBUG, "SAE: Peer commit-scalar",
		    *pos, sae->tmp->prime_len);
	*pos += sae->tmp->prime_len;
					...
					const atbm_uint8 *end)
{
	...
	atbm_wpa_hexdump(MSG_DEBUG, "SAE: Peer commit-element(x)",
		    *pos, sae->tmp->prime_len);
	atbm_wpa_hexdump(MSG_DEBUG, "SAE: Peer commit-element(y)",
		    *pos + sae->tmp->prime_len, sae->tmp->prime_len);
					...
					const atbm_uint8 *end)
{
	...
	atbm_wpa_hexdump(MSG_DEBUG, "SAE: Peer commit-element", *pos,
		    sae->tmp->prime_len);
...
{
	atbm_wpa_hexdump(MSG_DEBUG, "SAE: Possible elements at the end of the frame",
		    pos, end - pos);
	...
	atbm_wpa_hexdump_ascii(MSG_DEBUG, "SAE: Received Password Identifier",
			  (const atbm_uint8 *)sae->tmp->pw_id, pos[1] -  1);
	return ATBM_WLAN_STATUS_SUCCESS;
}
...
void sae_write_confirm(struct sae_data *sae, struct wpabuf *buf)
{
	...
	if (sae->tmp->ec)
		sae_cn_confirm_ecc(sae, sc, sae->tmp->own_commit_scalar,
				   ...
				   sae->tmp->peer_commit_element_ecc,
				   atbm_wpabuf_put(buf, SHA256_MAC_LEN));
	else
...
int sae_check_confirm(struct sae_data *sae, const atbm_uint8 *data, atbm_size_t len)
{
	...
	if (atbm_memcmp(verifier, data + 2, SHA256_MAC_LEN) != 0) {
		...
		atbm_wpa_hexdump(MSG_DEBUG, "SAE: Received confirm",
			    data + 2, SHA256_MAC_LEN);
		atbm_wpa_hexdump(MSG_DEBUG, "SAE: Calculated verifier",
