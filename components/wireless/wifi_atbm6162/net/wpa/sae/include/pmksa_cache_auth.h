...
pmksa_cache_auth_add(struct rsn_pmksa_cache *pmksa,
		     const atbm_uint8 *pmk, atbm_size_t pmk_len, const atbm_uint8 *pmkid,
		     const atbm_uint8 *kck, atbm_size_t kck_len,
		     const atbm_uint8 *aa, const atbm_uint8 *spa, int session_timeout,
		     struct eapol_state_machine *eapol, int akmp);
struct rsn_pmksa_cache_entry *
pmksa_cache_auth_create_entry(const atbm_uint8 *pmk, atbm_size_t pmk_len, const atbm_uint8 *pmkid,
			      const atbm_uint8 *kck, atbm_size_t kck_len, const atbm_uint8 *aa,
...
int pmksa_cache_auth_list(struct rsn_pmksa_cache *pmksa, char *buf, atbm_size_t len);
void pmksa_cache_auth_flush(struct rsn_pmksa_cache *pmksa);
int pmksa_cache_auth_list_mesh(struct rsn_pmksa_cache *pmksa, const atbm_uint8 *addr,
			       char *buf, atbm_size_t len);
