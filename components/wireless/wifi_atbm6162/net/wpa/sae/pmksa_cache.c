...
{
	atbm_bin_clear_free(entry, sizeof(*entry));
}
...
static void pmksa_cache_set_expiration(struct rsn_pmksa_cache *pmksa)
{
	...
	//atbm_pmksa_cache_get(pmksa, pmksa->sm->bssid, ATBM_NULL, ATBM_NULL, 0);
}


/**
 * atbm_pmksa_cache_add - Add a PMKSA cache entry
 * @pmksa: Pointer to PMKSA cache data from atbm_pmksa_cache_init()
...
atbm_pmksa_cache_add(struct rsn_pmksa_cache *pmksa, const atbm_uint8 *pmk, atbm_size_t pmk_len,
		...
		const atbm_uint8 *cache_id)
{
	...
	entry = (struct rsn_pmksa_cache_entry *)atbm_kzalloc(sizeof(*entry), GFP_KERNEL);
	if (entry == ATBM_NULL)
		return ATBM_NULL;
	...
	return atbm_pmksa_cache_add_entry(pmksa, entry);
}

struct rsn_pmksa_cache_entry *
atbm_pmksa_cache_add_entry(struct rsn_pmksa_cache *pmksa,
		      struct rsn_pmksa_cache_entry *entry)
{
	...
	while (pos) {
		if (atbm_memcmp(entry->aa, pos->aa, ATBM_ETH_ALEN) == 0) {
			...
			if (!pos->opportunistic)
				atbm_pmksa_cache_flush(pmksa, entry->network_ctx,
						  pos->pmk, pos->pmk_len);
...
/**
 * atbm_pmksa_cache_flush - Flush PMKSA cache entries for a specific network
 * @pmksa: Pointer to PMKSA cache data from atbm_pmksa_cache_init()
 ...
 * @pmk_len: PMK length
 */
void atbm_pmksa_cache_flush(struct rsn_pmksa_cache *pmksa, void *network_ctx,
...
/**
 * atbm_pmksa_cache_deinit - Free all entries in PMKSA cache
 * @pmksa: Pointer to PMKSA cache data from atbm_pmksa_cache_init()
 */
void atbm_pmksa_cache_deinit(struct rsn_pmksa_cache *pmksa)
{
...
/**
 * atbm_pmksa_cache_get - Fetch a PMKSA cache entry
 * @pmksa: Pointer to PMKSA cache data from atbm_pmksa_cache_init()
 ...
 */
struct rsn_pmksa_cache_entry * atbm_pmksa_cache_get(struct rsn_pmksa_cache *pmksa,
			...
			const atbm_uint8 *aa)
{
	...
	new_entry = atbm_pmksa_cache_add(pmksa, old_entry->pmk, old_entry->pmk_len,
...
/**
 * atbm_pmksa_cache_get_opportunistic - Try to get an opportunistic PMKSA entry
 * @pmksa: Pointer to PMKSA cache data from atbm_pmksa_cache_init()
 ...
 */
struct rsn_pmksa_cache_entry *
atbm_pmksa_cache_get_opportunistic(struct rsn_pmksa_cache *pmksa, void *network_ctx,
...
/**
 * atbm_pmksa_cache_get_current - Get the current used PMKSA entry
 * @sm: Pointer to WPA state machine data from wpa_sm_init()
 * Returns: Pointer to the current PMKSA cache entry or %ATBM_NULL if not available
 */
struct rsn_pmksa_cache_entry * atbm_pmksa_cache_get_current(struct atbmwifi_wpa_sm *sm)
{
	if (sm == ATBM_NULL)
		return ATBM_NULL;
	return sm->cur_pmksa;
...
/**
 * atbm_pmksa_cache_clear_current - Clear the current PMKSA entry selection
 * @sm: Pointer to WPA state machine data from wpa_sm_init()
 */
void atbm_pmksa_cache_clear_current(struct atbmwifi_wpa_sm *sm)
{
	if (sm == ATBM_NULL)
		return;
...
/**
 * atbm_pmksa_cache_set_current - Set the current PMKSA entry selection
 * @sm: Pointer to WPA state machine data from wpa_sm_init()
 ...
 */
int atbm_pmksa_cache_set_current(struct atbmwifi_wpa_sm *sm, const atbm_uint8 *pmkid,
			    ...
			    int akmp)
{
	...
	if (pmkid)
		atbm_wpa_hexdump(MSG_DEBUG, "RSN: Search for PMKID",
	...
	sm->cur_pmksa = ATBM_NULL;
	if (pmkid)
		sm->cur_pmksa = atbm_pmksa_cache_get(pmksa, ATBM_NULL, pmkid,
						network_ctx, akmp);
	if (sm->cur_pmksa == ATBM_NULL && bssid)
		sm->cur_pmksa = atbm_pmksa_cache_get(pmksa, bssid, ATBM_NULL,
						network_ctx, akmp);
	if (sm->cur_pmksa == ATBM_NULL && try_opportunistic && bssid)
		sm->cur_pmksa = atbm_pmksa_cache_get_opportunistic(pmksa,
							      network_ctx,
							      bssid, akmp);
	if (sm->cur_pmksa) {
		atbm_wpa_hexdump(MSG_DEBUG, "RSN: PMKSA cache entry found - PMKID",
...
struct rsn_pmksa_cache_entry * atbm_pmksa_cache_head(struct rsn_pmksa_cache *pmksa)
{
	return pmksa->pmksa;
}
...
/**
 * atbm_pmksa_cache_init - Initialize PMKSA cache
 ...
 */
struct rsn_pmksa_cache *
atbm_pmksa_cache_init(void (*free_cb)(struct rsn_pmksa_cache_entry *entry,
				 void *ctx, enum pmksa_free_reason reason),
		 void *ctx, struct atbmwifi_wpa_sm *sm)
{
	struct rsn_pmksa_cache *pmksa;

	pmksa = (struct rsn_pmksa_cache *)atbm_kzalloc(sizeof(*pmksa), GFP_KERNEL);
	if (pmksa) {
