...
int atbm___wpa_send_eapol(struct atbmwifi_cfg *config,
		      ...
		      int keyidx, int encr, int force_version, ATBM_BOOL noenc);
...
extern atbm_void atbm_hostapd_deinit_wps(struct hostapd_data *hapd);
extern int atbm_hostapd_init_wps(struct atbmwifi_vif *priv);
...
static atbm_void wpa_send_eapol_timeout(atbm_void *data1,atbm_void *data2)
{
	...
	hostapd = (struct hostapd_data *)(priv->appdata);
	if(!hostapd)
		return;
...
atbm_size_t atbm_merge_byte_arrays(atbm_uint8 *res, atbm_size_t res_len,
...
static atbm_void wpa_send_eapol(struct atbmwifi_cfg *config,
			   ...
			   const atbm_uint8 *key_rsc, const atbm_uint8 *nonce,
			   const atbm_uint8 *kde, atbm_size_t kde_len,
			   int keyidx, int encr, ATBM_BOOL noenc)
{
	...
	atbm___wpa_send_eapol(config, sm, key_info, key_rsc, nonce, kde, kde_len,
			 keyidx, encr, 0, noenc);
	...
	atbmwifi_eloop_register_timeout(WPA_SEND_EAPOL_TIMEOUT, WPA_SEND_EAPOL_TIMEOUT_MS,
			  ...
			  struct atbmwifi_wpa_group *group)
{
	...
	if (wpa_gmk_to_gtk(group->GMK, "Group key expansion",
		...
		ret = -1;
/*
	atbm_wpa_hexdump_key(MSG_DEBUG, "GTK",
...
*/
	...
	if (config->ieee80211w != ATBM_NO_MGMT_FRAME_PROTECTION) {
		...
		atbm_wpa_hexdump_key(MSG_DEBUG, "IGTK",
			   ...
			   struct atbmwifi_ieee802_11_elems *elems)
{
	...
	sta->supported_rates_len = atbm_merge_byte_arrays(
...
atbm_wpa_auth_sta_init(struct atbmwifi_cfg *config,struct hostapd_sta_info *sta)
{
	...
	sm = &sta->atbmwifi_wpa_sm_bufs;
	memset(sm,0,sizeof(struct atbmwifi_wpa_state_machine));
	...
	//if (sm == ATBM_NULL)
	//	return ATBM_NULL;
	atbm_memcpy(sm->addr, sta->addr, ATBM_ETH_ALEN);
	sm->group = &hostapd->group;
	//wpa_group_init(config,sm->group,0);
	return sm;
}
int atbm_wpa_validate_wpa_ie(struct atbmwifi_vif *priv,
			struct atbmwifi_wpa_state_machine *sm,
			const atbm_uint8 *wpa_ie, atbm_size_t wpa_ie_len)
{
	...
	struct hostapd_sta_info *sta = ATBM_NULL;
...
//		wifi_printk(WIFI_DBG_INIT,"parame ATBM_FALSE(1)\n");
	...
	sta = atbm_ap_get_sta(hapd, sm->addr);
...
//		wifi_printk(WIFI_DBG_INIT,"version ATBM_FALSE(%d)\n",version);
	...
	} else 
	{
		...
		if((res = atbm_wpa_parse_wpa_ie_wpa(wpa_ie, wpa_ie_len, &data))<0)
		{
//			wifi_printk(WIFI_DBG_INIT,"data ie ATBM_FALSE\n");
	...
	if (res<0) 
	{
		...
		atbm_wpa_hexdump(MSG_DEBUG, "WPA/RSN IE", wpa_ie, wpa_ie_len);
#endif
...
//		wifi_printk(WIFI_DBG_INIT,"p ATBM_FALSE\n");
	...
	if (data.capabilities & ATBM_WPA_CAPABILITY_MFPC && (config->ieee80211w != 0))
	...
	for (i = 0; i < data.num_pmkid; i++) {
		atbm_wpa_hexdump(MSG_DEBUG, "RSN IE: STA PMKID",
			    &data.pmkid[i * ATBM_PMKID_LEN], ATBM_PMKID_LEN);
		sm->pmksa = atbm_pmksa_cache_get(hapd->pmksa, sm->addr,
...
atbm_void atbm_wpa_auth_sta_no_wpa(struct atbmwifi_wpa_state_machine *sm)
{
...
#if CONFIG_SAE
int atbm_wpa_auth_uses_sae(struct atbmwifi_wpa_state_machine *sm)
{
	if (sm == NULL)
		return 0;
...
int atbm_wpa_auth_uses_ft_sae(struct atbmwifi_wpa_state_machine *sm)
{
	if (sm == NULL)
		return 0;
...
int atbm_wpa_auth_pmksa_add_sae(struct hostapd_data *hapd, const atbm_uint8 *addr,			   const atbm_uint8 *pmk, const atbm_uint8 *pmkid)
{

	atbm_wpa_hexdump_key(MSG_DEBUG, "RSN: Cache PMK from SAE", pmk, ATBM_PMK_LEN);
	if (atbm_pmksa_cache_add(hapd->pmksa, pmk, ATBM_PMK_LEN, pmkid,
				NULL, 0,
...
void atbm_wpa_auth_add_sae_pmkid(struct atbmwifi_wpa_state_machine *sm, const atbm_uint8 *pmkid)
{
...
atbm_wpa_auth_sta_get_pmksa(struct atbmwifi_wpa_state_machine *sm)
{
			   ...
			   struct atbmwifi_ieee802_11_elems *elems, int reassoc)
{
	...
	if (config->wpa && wpa_ie) {
		...
		if (sta->atbmwifi_wpa_sm == ATBM_NULL)
			sta->atbmwifi_wpa_sm = atbm_wpa_auth_sta_init(config,sta);

		if (sta->atbmwifi_wpa_sm == ATBM_NULL) {			
			return ATBM_WLAN_STATUS_UNSPECIFIED_FAILURE;
		}
		resp = atbm_wpa_validate_wpa_ie(priv, sta->atbmwifi_wpa_sm,
					  wpa_ie, wpa_ie_len);
		if (resp != ATBM_WLAN_STATUS_SUCCESS)
			return resp;
		...
		if ((sta->flags & (WLAN_STA_HT | WLAN_STA_VHT)) &&
		    sta->atbmwifi_wpa_sm_bufs.pairwise == ATBM_WPA_CIPHER_TKIP) {
			return ATBM_WLAN_STATUS_UNSPECIFIED_FAILURE;
		}
#endif /* CONFIG_IEEE80211N */
#if CONFIG_SAE
		if (atbm_wpa_auth_uses_sae(&sta->atbmwifi_wpa_sm_bufs) &&
			sta->sae.state == SAE_ACCEPTED)
			atbm_wpa_auth_add_sae_pmkid(&sta->atbmwifi_wpa_sm_bufs, sta->sae.pmkid);

		if (atbm_wpa_auth_uses_sae(&sta->atbmwifi_wpa_sm_bufs) &&
			sta->auth_alg == ATBM_WLAN_AUTH_OPEN) {
			struct rsn_pmksa_cache_entry *sa;
			sa = atbm_wpa_auth_sta_get_pmksa(&sta->atbmwifi_wpa_sm_bufs);
		...
		} else if (atbm_wpa_auth_uses_sae(&sta->atbmwifi_wpa_sm_bufs) &&
				 sta->auth_alg != ATBM_WLAN_AUTH_SAE &&
				!(sta->auth_alg == ATBM_WLAN_AUTH_FT &&
				atbm_wpa_auth_uses_ft_sae(&sta->atbmwifi_wpa_sm_bufs))) {
		...
		atbm_wpa_auth_sta_no_wpa(sta->atbmwifi_wpa_sm);
	}
	return ATBM_WLAN_STATUS_SUCCESS;
}

atbm_uint8 * atbm_wpa_add_kde(atbm_uint8 *pos, atbm_uint32 kde, const atbm_uint8 *data, atbm_size_t data_len,
...
#endif
	pos = atbm_wpa_add_kde(pos, ATBM_RSN_KEY_DATA_IGTK,
...
//		wifi_printk(WIFI_DBG_MSG,"ptk len err \n\r");
	...
	if (atbm_wpa_eapol_key_mic(PTK->kck, key_info & ATBM_WPA_KEY_INFO_TYPE_MASK,
...
/**
 * atbm_wpa_parse_kde_ies - Parse EAPOL-Key Key Data IEs
 * @buf: Pointer to the Key Data buffer
 * @len: Key Data Length
 * @ie: Pointer to parsed IE data
 * Returns: 0 on success, -1 on failure
 */
int atbm_wpa_parse_kde_ies(const atbm_uint8 *buf, atbm_size_t len, struct wpa_eapol_ie_parse *ie)
{
	...
	for (pos = buf, end = pos + len; pos + 1 < end; pos += 2 + pos[1]) {
		...
		if (pos + 2 + pos[1] > end) {
			...
			atbm_wpa_hexdump_key(MSG_DEBUG, "WPA: Key Data",
					buf, len);
#endif
			ret = -1;
			break;
			  ...
			  struct atbmwifi_wpa_ptk *ptk)
{
	...
	atbm_wpa_pmk_to_ptk(pmk, ATBM_PMK_LEN, "Pairwise key expansion",
	...
	if (sm->wpa == ATBM_WPA_VERSION_WPA2 && atbmwifi_wpa_key_mgmt_ft(sm->wpa_key_mgmt)) {
		...
		if (atbm_memcmp(sm->sup_pmk_r1_name, sm->pmk_r1_name,
			...
			wpa_auth_logger(sm->wpa_auth, sm->addr, LOGGER_DEBUG,
					...
					"handshake");
			atbm_wpa_hexdump(MSG_DEBUG, "FT: PMKR1Name from "
				    "Supplicant",
				    sm->sup_pmk_r1_name, ATBM_WPA_PMK_NAME_LEN);
			atbm_wpa_hexdump(MSG_DEBUG, "FT: Derived PMKR1Name",
...
static int hostapd_send_4_of_3_msg(struct atbmwifi_vif *priv,struct atbmwifi_wpa_state_machine *sm)
{
	...
	*/
	...
	if (gtk) {
		...
		hdr[0] = keyidx & 0x03;
		hdr[1] = 0;
		pos = atbm_wpa_add_kde(pos, ATBM_RSN_KEY_DATA_GROUPKEY, hdr, 2,
				  gtk, gtk_len);
	}
	...
	wpa_send_eapol(config, sm,
		       ...
		       _rsc, sm->ANonce, kde, pos - kde, keyidx, encr, ATBM_TRUE);
	atbm_kfree(kde);
	wifi_printk(WIFI_DBG_MSG,"hostapd 3/4\n\r");
	return 0;
}
atbm_void atbm_wpa_receive(struct atbmwifi_vif *priv,
		 struct atbmwifi_wpa_state_machine *sm,
...
//	wifi_printk(WIFI_DBG_MSG, "atbm_wpa_receive:++\n");
...
//		msgtxt = "2/4 Pairwise";
	...
	if (!(key_info & ATBM_WPA_KEY_INFO_REQUEST) &&
		    ...
		    atbm_memcmp(sm->SNonce, key->key_nonce, ATBM_WPA_NONCE_LEN) != 0)
		{
			...
			wifi_printk(WIFI_DBG_MSG,"atbm_wpa_receive:""STA"
						"EAPOL-Key1/4\n\r");
			sm->update_snonce = 1;
		...
		for (i = 0; i < RSNA_MAX_EAPOL_RETRIES; i++) {
			if (!sm->key_replay[i].valid)
				break;
		#ifdef LINUX_DEBUG
			atbm_wpa_hexdump(MSG_DEBUG, "pending replay counter",
				    sm->key_replay[i].counter,
				    ATBM_WPA_REPLAY_COUNTER_LEN);
		#endif
		}
#ifdef LINUX_DEBUG
		atbm_wpa_hexdump(MSG_DEBUG, "received replay counter",
...
//		random_add_randomness(key->key_nonce, ATBM_WPA_NONCE_LEN);
		if (atbm_wpa_parse_kde_ies((atbm_uint8 *) (key + 1), key_data_length,
				      &kde) < 0) {
#if 0
			wpa_auth_vlogger(wpa_auth, sm->addr, LOGGER_INFO,
...
//	wifi_printk(WIFI_DBG_MSG,"received EAPOL-Key frame(%s)\n\r",msgtxt);
	...
	if (key_info & ATBM_WPA_KEY_INFO_ACK) {		
		wifi_printk(WIFI_DBG_ERROR,"atbm_wpa_receive:Key Ack set\n\r");
		return;
	}
	...
	if (sm->PTK_valid && !sm->update_snonce) {
		if (wpa_verify_key_mic(&sm->PTK, data, data_len)) {
			wifi_printk(WIFI_DBG_ERROR,"atbm_wpa_receive:MIC err(1)\n\r");
			return;
		}
		sm->MICVerified = ATBM_TRUE;
		...
		atbmwifi_eloop_cancel_timeout(wpa_send_eapol_timeout,  (atbm_void *)sm,ATBM_NULL);
		sm->pending_1_of_4_timeout = 0;
		wifi_printk(WIFI_DBG_ERROR,"atbm_wpa_receive: MIC OK\n\r");
	}
	...
	if (key_info & ATBM_WPA_KEY_INFO_REQUEST) {
		...
		if (msg == SMK_ERROR) {
			...
			wpa_smk_m1(wpa_auth, sm, key);
			   ...
			   atbm_wpa_parse_kde_ies((const atbm_uint8 *) (key + 1),
	...
	wifi_printk(WIFI_DBG_ERROR,"atbm_wpa_receive %x,%x \n\r",priv->appdata, sm);
	eloop_register_task((atbm_void *)priv,atbm_ap_get_sta((struct hostapd_data *)priv->appdata,sm->addr));

}

struct hostapd_sta_info * atbm_ap_get_sta(struct hostapd_data * hapd, const atbm_uint8 *sta)
{
...
struct hostapd_sta_info * atbm_ap_sta_add(struct hostapd_data *hapd, const atbm_uint8 *addr,atbm_uint8 linkid)
{
	...
	for(id=0;id<ATBMWIFI__MAX_STA_IN_AP_MODE;id++){
		...
		if(atbm_memcmp(addr, sta->addr, ATBM_ETH_ALEN) == 0)	{
			if((linkid-1) != id){
				//return tmp;
	...
	//	return ATBM_NULL;
	//}
	sta = &hapd->sta_table[linkid-1];
	memset(sta,0,sizeof(struct hostapd_sta_info));
...
atbm_void ap_sta_del(struct hostapd_data *hostapd,struct hostapd_sta_info *sta)
{
	...
	atbmwifi_eloop_cancel_timeout(wpa_send_eapol_timeout,sm,ATBM_NULL);
	sta->atbmwifi_wpa_sm=ATBM_NULL;
	if(sm)
	{
		if (sm->wpa_ie)
		...
		{
			atbm_kfree(sm->last_rx_eapol_key);
			sm->last_rx_eapol_key=ATBM_NULL; 
		}
...
{
	struct hostapd_data *hostapd = (struct hostapd_data *)priv->appdata;
	struct hostapd_sta_info *sta = atbm_ap_get_sta(hostapd,staMacAddr);	
 ...
 int atbm_hostapd_get_aid(struct hostapd_data *hapd, struct hostapd_sta_info *sta)
{
 ...
 /**
  * atbm_ieee802_1x_receive - Process the EAPOL frames from the Supplicant
 ...
 atbm_void atbm_ieee802_1x_receive(struct atbmwifi_vif *priv,
 ...
 //  int key_mgmt;
	 ...
	 sta = atbm_ap_get_sta((struct hostapd_data *)priv->appdata, sa);
	 if(ATBM_NULL == sta)
	 {
		 return;
	 }
	 ...
	 if (datalen >= sizeof(struct atbmwifi_ieee802_1x_eapol_key) &&
		 ...
		 atbm_wpa_receive(priv, sta->atbmwifi_wpa_sm, (atbm_uint8 *) hdr,
				 sizeof(*hdr) + datalen);
		 return;
	 }
#if CONFIG_WPS
 ...
 int hostapd_rx_assoc_req(struct atbmwifi_vif *priv,struct atbm_buff *skb)
 {
	...
	if(sta == ATBM_NULL)
	{
		sta = atbm_ap_sta_add(hapd, mgmt->sa,tx_info->link_id);
		if(sta == ATBM_NULL)
		{	 
	...
	if (atbm_hostapd_get_aid(hapd, sta) <= 0) {
...
fail:
	if (resp != ATBM_WLAN_STATUS_SUCCESS){
		...
		hostapd_link_sta_sm(priv,atbmwifi_sta_find_form_linkid(priv,tx_info->link_id),mgmt->sa);
	}
	return resp;
 }
		      ...
		      int keyidx, int encr, int force_version, ATBM_BOOL noenc)
{
	...
	if (key_info & ATBM_WPA_KEY_INFO_MIC) {
		...
		atbm_wpa_eapol_key_mic(sm->PTK.kck, version, (atbm_uint8 *) hdr, len,
				  key->key_mic);
	}
//	wifi_printk(WIFI_DBG_MSG,"atbm___wpa_send_eapol len+kde(%d)\n\r",len);
	wifi_printk(WIFI_CONNECT,"hostap_tx_eap\n");
	ret =hostapd_send_eapol((struct atbmwifi_vif * )atbmwifi_config_get_priv(config),sm->addr,ATBM_ETH_P_EAPOL,(atbm_uint8 *)hdr,len,noenc);
	atbm_kfree(hdr);
	return ret;
}
#if CONFIG_WPS
							...
							struct atbmwifi_ieee802_1x_hdr *hdr, atbm_uint16 datalen)
{
	...
	atbm_wpabuf_free(hostapd->wps_last_rx_data);
	hostapd->wps_last_rx_data = atbm_wpabuf_alloc_copy(hdr, datalen + sizeof(*hdr));
...
atbm_void hostapd_setup_4_way_handshake(struct atbmwifi_vif *priv,atbm_uint8 *da)
{
	...
	atbmwifi_wpa_event_queue((atbm_wpa_event_data)priv,(atbm_wpa_event_data)&priv->link_id_db[link_id-1],(atbm_wpa_event_data)ATBM_NULL,WPA_EVENT__HOSTAPD_STA_HANDSHAKE_START,ATBM_WPA_EVENT_NOACK);
}
int hostapd_send_4_of_1_msg(struct atbmwifi_vif *priv,struct atbmwifi_wpa_state_machine *sm)
{
	...
	wpa_send_eapol(atbmwifi_get_config(priv), sm,
		       ...
		       sm->ANonce,pmkid,pmkid_len, 0, 0, ATBM_TRUE);
	return 0;
...
{	
	wifi_printk(WIFI_WPA,"handshake_over\n");
	sm->EAPOLKeyReceived = ATBM_FALSE;
	if (sm->Pair) {
...
int hostapd_4_way_handshake_process(struct atbmwifi_vif *priv,struct atbmwifi_wpa_state_machine *sm)
{
	...
	switch(sm->wpa_ptk_state)
	{
		...
		case ATBM_WPA_PTK_PTKSTART:
			...
			wifi_printk(WIFI_ALWAYS,"hostapd_send_4_of_1_msg res = %d\n",res);
			break;
			
		case ATBM_WPA_PTK_PTKINITNEGOTIATING:	
			res = hostapd_send_4_of_3_msg(priv,sm);
			wifi_printk(WIFI_ALWAYS,"hostapd_send_4_of_3_msg res = %d \n",res);
			break;
			
		case ATBM_WPA_PTK_PTKINITDONE:
			hostapd_handshake_over(priv,sm);
			wifi_printk(WIFI_ALWAYS,"hostapd_handshake_over \n");
			res = -1;
			break;
		default:
			res = -1;
...
int hostapd_send_1_of_group(struct atbmwifi_vif *priv,struct atbmwifi_wpa_state_machine *sm)
{
	...
	wpa_send_eapol(atbmwifi_get_config(priv), sm,
		       ...
		       rsc, gsm->GNonce, kde, pos - kde, gsm->GN, 1, ATBM_FALSE);
...
int hostapd_2_way_group_process(struct atbmwifi_vif *priv,struct atbmwifi_wpa_state_machine *sm)
{
	...
	wifi_printk(WIFI_WPA,"hostapd_2_way_group\n");
	if (sm->GTimeoutCtr > (int) dot11RSNAConfigGroupUpdateCount) {
					/* No point in sending the EAPOL-Key - we will disconnect
					 * immediately following this. */
			...
			switch(hdr->type){
				...
				case ATBM_IEEE802_1X_TYPE_EAPOL_START:
					...
					*(atbm_uint8 *)(eap_headr + 1) = ATBM_EAP_TYPE_IDENTITY;
					...
					hostapd_send_eapol(priv, sta->addr, ATBM_ETH_P_EAPOL, (atbm_uint8 *)hdr, len, ATBM_TRUE);
					atbm_kfree(hdr);
					hdr = ATBM_NULL;
					break;
				default:
...
wps_out:
	...
	switch(sta->timeout)
	{
		case STA_START:
		{	
			struct atbmwifi_wpa_state_machine *sm = sta->atbmwifi_wpa_sm;
			if(sm){
				wifi_printk(WIFI_WPA,"hostapd handshake %x\n",sta->atbmwifi_wpa_sm);
				sm->linkid = sta->aid;
				hostapd_run_handshake(priv,sm);
			}
			break;
		}
		case STA_DISASSOC:
		...
		{	
			...
			atbmwifi_ap_deauth(priv, sta->addr, ATBM_TRUE);
			break;
		}
		default:
		{
		  ...
		  struct atbmwifi_sta_priv *sta_priv,atbm_uint8* mac)
{
	...
	sta = atbm_ap_get_sta((struct hostapd_data *)priv->appdata, mac);
...
#if CONFIG_IEEE80211W
	if(sta->atbmwifi_wpa_sm && sta->atbmwifi_wpa_sm->mgmt_frame_prot){
		sta_priv->ieee_80211w = 1;
	}
#endif
 ...
 *       1. 释放 WPS 资源 (如果使能 WPS)
 *       2. 释放 PMKSA 缓存 (如果使能 SAE)
 *       3. 释放 hostapd 数据结构
 *       4. 清空全局 hostapd 指针
 *
 * @see atbmwifi_stop_ap() - 调用该函数释放 hostapd
 * @see init_hostapd() - 初始化 hostapd
 */
atbm_void free_hostapd(struct atbmwifi_vif *priv)
{
	struct hostapd_data *hostapd = NULL;
	...
	if(priv->appdata){
		...
		/* 步骤 1: 释放 WPS 资源 */
#if CONFIG_WPS
		atbm_hostapd_deinit_wps(priv->appdata);
#endif

		/* 步骤 2: 释放 PMKSA 缓存 */
#if CONFIG_SAE
		atbm_pmksa_cache_deinit(hostapd->pmksa);
#endif

		/* 步骤 3: 释放 hostapd 数据结构 */
		atbm_kfree(priv->appdata);
		priv->appdata = ATBM_NULL;

		/* 步骤 4: 清空全局 hostapd 指针 */
		g_hostapd = ATBM_NULL;
	}
}
 ...
 int hostapd_start(struct atbmwifi_vif *priv)
{
		...
		case ATBM_KEY_MIX:
			{
				...
				config->wpa = ATBM_WPA_PROTO_RSN | ATBM_WPA_PROTO_WPA;//can do it				
				break;
			}
		case ATBM_KEY_NONE:
			{
	...
	if(config->max_sta_count <= 0 || config->max_sta_count > ATBMWIFI__MAX_STA_IN_AP_MODE)
		config->max_sta_count = ATBMWIFI__MAX_STA_IN_AP_MODE;
	...
	wifi_printk(WIFI_ALWAYS,"%s %d ,%d maxstacnt = %d\n",__func__,__LINE__,config->max_sta_count,config->max_sta_count);
	
	wifi_printk(WIFI_WPA,"hostapd(%s)(%d %d)(%x) start to beacon\n",
			priv->ssid,config->key_mgmt,config->group_cipher,hostapd->group);

	wpa_group_init(config,&hostapd->group,0);

#if CONFIG_5G_SUPPORT
	if(config->channel_index >= 36){
		ATBM_BUG_ON(!atbmwifi_5g_supported(priv->hw_priv->chip_priv));
		config->band = ATBM_IEEE80211_BAND_5GHZ;
	}else
#endif
		config->band = ATBM_IEEE80211_BAND_2GHZ;
	priv->bss.sta_priv.band = (enum atbmwifi_ieee80211_band)config->band;
	atbm_start_ap(priv);
...
#if CONFIG_WPS
	atbm_hostapd_init_wps(priv);
#endif
#if CONFIG_SAE
	{
		int sae_groups[4] = {19, 20, 21, 0};
		hostapd->wpa_passphrase = (char *)priv->config.password;
		hostapd->sae_sync = 5;
		hostapd->sae_anti_clogging_threshold = 5;
		atbm_memcpy(&hostapd->sae_groups, sae_groups, sizeof(sae_groups));
		hostapd->pmksa = atbm_pmksa_cache_init(wpa_auth_pmksa_free_cb, ATBM_NULL, ATBM_NULL);
	}
#endif
	...
	if(config->key_mgmt == ATBM_WPA_KEY_MGMT_WEP){
		wpa_common_install_wepkey(priv,(char *)priv->config.password,priv->config.group_cipher,priv->config.key_id/*KeyIndex 0...3*/,1);
		atbmwifi_wep_key_work(priv);
	}
	char da[6]={0xff,0xff,0xff,0xff,0xff,0xff};
	atbmwifi_ieee80211_tx_mgmt_deauth(priv,da,priv->bssid,3);
	return 0;
}
