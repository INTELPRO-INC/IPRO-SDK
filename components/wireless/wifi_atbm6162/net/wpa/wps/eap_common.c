...
/**
 * atbm_eap_hdr_len_valid - Validate EAP header length field
 ...
 */
 atbm_int32 atbm_eap_hdr_len_valid(const struct wpabuf *msg, atbm_size_t min_payload)
{
...
/**
 * atbm_eap_hdr_validate - Validate EAP header
 ...
 * not.
 */
 const atbm_uint8 * atbm_eap_hdr_validate(EapVenType vendor, EapType eap_type,
			    const struct wpabuf *msg, atbm_size_t *plen)
{
	...
	if (!atbm_eap_hdr_len_valid(msg, 1))
		return NULL;
...
/**
 * atbm_eap_msg_alloc - Allocate a buffer for an EAP message
 ...
 */
struct wpabuf * atbm_eap_msg_alloc(EapVenType vendor, EapType type, atbm_uint32 payload_len,
			      EapCodeType code, atbm_uint8 identifier)
{
	...
	hdr = atbm_wpabuf_put(buf, sizeof(*hdr));
	hdr->code = code;
...
/**
 * atbm_eap_update_len - Update EAP header length
 * @msg: EAP message from atbm_eap_msg_alloc
 *
 * This function updates the length field in the EAP header to match with the
 * current length for the buffer. This allows atbm_eap_msg_alloc() to be used to
 * allocate a larger buffer than the exact message length (e.g., if exact
 * message length is not yet known).
 */
 atbm_void atbm_eap_update_len(struct wpabuf *msg)
{
	struct atbm_eap_hdr *hdr;
...
/**
 * atbm_eap_get_id - Get EAP Identifier from wpabuf
 * @msg: Buffer starting with an EAP header
 * Returns: The Identifier field from the EAP header
 */
 atbm_uint8 atbm_eap_get_id(const struct wpabuf *msg)
{
...
/**
 * atbm_eap_get_id - Get EAP Type from wpabuf
 * @msg: Buffer starting with an EAP header
 * Returns: The EAP Type after the EAP header
 */
 EapType atbm_eap_get_type(const struct wpabuf *msg)
{
	if (wpabuf_len(msg) < sizeof(struct atbm_eap_hdr) + 1)
		return ATBM_EAP_TYPE_NONE;

	return (EapType)((const atbm_uint8 *) wpabuf_head(msg))[sizeof(struct atbm_eap_hdr)];
}
 ...
 static atbm_void eap_identity_process(struct hostapd_data *hapd,struct hostapd_sta_info *sta, struct wpabuf *respData)
{
	...
	pos = atbm_eap_hdr_validate(ATBM_EAP_VENDOR_IETF, ATBM_EAP_TYPE_IDENTITY,
			       respData, &len);
	if (pos == NULL)
				...
				atbm_size_t len)
{
	...
	buf = atbm_wpabuf_alloc_copy(eap, len);
	switch(type){
