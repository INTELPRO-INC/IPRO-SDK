...
extern int atbm_uuid_str2bin(const char *str, atbm_uint8 *bin);
extern int atbm_wps_registrar_add_pin(struct wps_registrar *reg,const atbm_uint8 *addr);
 ...
 static atbm_void hostapd_free_wps(struct wps_context *wps)
{
	...
	for (i = 0; i < MAX_WPS_VENDOR_EXTENSIONS; i++)
		atbm_wpabuf_free(wps->dev.vendor_ext[i]);
	//atbm_wps_device_data_free(&wps->dev);
...
/**
 * atbm_hostapd_init_wps - init wps AP structure
 * Returns: 0 on success, -1 on failure
 *
 * This function is called at program initialization to enable wps AP.
 */
 int atbm_hostapd_init_wps(struct atbmwifi_vif *priv)
{
	...
	wifi_printk(WIFI_DBG_ERROR, "atbm_hostapd_init_wps\n");
	...
	if (wps == NULL)
	{
		wpa_printf(MSG_ERROR, "atbm_hostapd_init_wps: invalid wps\n");
		return -1;
	}
	...
	if (atbm_uuid_str2bin("12345678-9abc-def0-1234-56789abcdef0", wps->uuid)) {
		wpa_printf(MSG_ERROR, "atbm_hostapd_init_wps: invalid UUID\n");	
		goto fail;
	}
	...
	{
		wpa_printf(MSG_ERROR, "atbm_hostapd_init_wps: invalid device_type\n");	
		goto fail;
	}
	if (atbmwifi_hexstr2bin(os_version, "01020300", 4)) {
		wpa_printf(MSG_ERROR, "atbm_hostapd_init_wps: invalid os_version\n");
		goto fail;
	}
	...
	wps->registrar = atbm_wps_registrar_init(wps, &cfg);
...
fail:
 ...
 int atbm_hostapd_init_wps_complete(struct hostapd_data *hapd)
{
	...
	if (hostapd_wps_upnp_init(hapd, wps) < 0) {
		...
		atbm_wps_registrar_deinit(wps->registrar);
		hostapd_free_wps(wps);
		hapd->wps = NULL;
		return -1;
	}
...
/**
 * atbm_hostapd_deinit_wps - destrory wps AP structure
 * Returns: NULL
 *
 * This function is called at program over run to destroy wps AP.
 */
 atbm_void atbm_hostapd_deinit_wps(struct hostapd_data *hapd)
{
...
#endif /* CONFIG_WPS_UPNP */
	atbm_wps_registrar_deinit(hapd->wps->registrar);
 ...
 int wps_add_pin(struct hostapd_data *hapd, atbm_void *ctx)
{
	...
	if (hapd->wps == NULL)
		return 0;
	ret = atbm_wps_registrar_add_pin(hapd->wps->registrar, 0);
	return ret;
}

 int atbm_hostapd_wps_button_pushed(struct hostapd_data *hapd, atbm_void *ctx)
{
	const atbm_uint8 *p2p_dev_addr = ctx;
	if (hapd->wps == NULL)
		return -1;
	return atbm_wps_registrar_button_pushed(hapd->wps->registrar, p2p_dev_addr);
}
 ...
 int hostapd_cancel_wps(struct hostapd_data *hapd)
{
	...
	atbm_wps_registrar_wps_cancel(hapd->wps->registrar);
	//ap_for_each_sta(hapd, ap_sta_wps_cancel, NULL);

	return 0;
}

 atbm_void atbm_hostapd_wps_ap_pin_disable(struct hostapd_data *hapd)
{
 ...
 static atbm_void hostapd_wps_ap_pin_timeout(void *eloop_data, atbm_void *user_ctx)
{
	...
	atbm_hostapd_wps_ap_pin_disable(hapd);
...
/* static int wps_ap_pin_disable(struct hostapd_data *hapd, atbm_void *ctx)
{
 ...
 int atbm_hostapd_wps_config_ap(struct hostapd_data *hapd, char *ssid,
			  const char *auth, const char *encr, char *key)
{
	...
	return atbm_wps_registrar_config_ap(hapd->wps->registrar, &cred);
}
#endif
