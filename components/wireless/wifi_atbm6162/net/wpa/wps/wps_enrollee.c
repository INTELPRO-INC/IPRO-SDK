...
extern int atbm_wps_build_device_attrs(struct wps_device_data *dev, struct wpabuf *msg);
extern int atbm_wps_build_rf_bands(struct wps_device_data *dev, struct wpabuf *msg);
extern int atbm_wps_build_os_version(struct wps_device_data *dev, struct wpabuf *msg);
extern int atbm_wps_build_vendor_ext_m1(struct wps_device_data *dev, struct wpabuf *msg);
extern int atbm_wps_process_device_attrs(struct wps_device_data *dev,
 ...
 static int wps_build_e_hash(struct wps_data *wps, struct wpabuf *msg)
{
	...
	if (random_get_bytes(wps->snonce, 2 * WPS_SECRET_NONCE_LEN) < 0)
		return -1;
	atbm_wpa_hexdump(MSG_DEBUG, "WPS: E-S1", wps->snonce, WPS_SECRET_NONCE_LEN);
	atbm_wpa_hexdump(MSG_DEBUG, "WPS: E-S2",
	...
	atbm_wpa_hexdump(MSG_DEBUG, "WPS: E-Hash1", hash, SHA256_MAC_LEN);
	...
	addr[1] = wps->psk2;
	atbmwifi_hmac_sha256_vector(wps->authkey, WPS_AUTHKEY_LEN, 4, addr, len, hash);
	atbm_wpa_hexdump(MSG_DEBUG, "WPS: E-Hash2", hash, SHA256_MAC_LEN);
 ...
 static struct wpabuf * wps_build_m1(struct wps_data *wps)
{
	...
	if (random_get_bytes(wps->nonce_e, WPS_NONCE_LEN) < 0)
		return NULL;
	atbm_wpa_hexdump(MSG_DEBUG, "WPS: Enrollee Nonce",
	...
	if (atbm_wps_build_version(msg) ||
	    atbm_wps_build_msg_type(msg, WPS_M1) ||
	    atbm_wps_build_uuid_e(msg, wps->uuid_e) ||
	    ...
	    atbm_wps_build_config_methods(msg, config_methods) ||
	    ...
	    atbm_wps_build_vendor_ext_m1(&wps->wps->dev, msg)) {
 ...
 static struct wpabuf * wps_build_m3(struct wps_data *wps)
{
	...
	if (atbm_wps_build_version(msg) ||
	    atbm_wps_build_msg_type(msg, WPS_M3) ||
 ...
 static struct wpabuf * wps_build_m5(struct wps_data *wps)
{
	...
	if (atbm_wps_build_version(msg) ||
	    atbm_wps_build_msg_type(msg, WPS_M5) ||
	    atbm_wps_build_registrar_nonce(wps, msg) ||
 ...
 static struct wpabuf * wps_build_m7(struct wps_data *wps)
{
	...
	if (atbm_wps_build_version(msg) ||
	    atbm_wps_build_msg_type(msg, WPS_M7) ||
	    atbm_wps_build_registrar_nonce(wps, msg) ||
 ...
 static struct wpabuf * wps_build_wsc_done(struct wps_data *wps)
{
	...
	if (atbm_wps_build_version(msg) ||
	    atbm_wps_build_msg_type(msg, WPS_WSC_DONE) ||
	    atbm_wps_build_enrollee_nonce(wps, msg) ||
	    atbm_wps_build_registrar_nonce(wps, msg) ||
	    atbm_wps_build_wfa_ext(msg, 0, NULL, 0)) {
		atbm_wpabuf_free(msg);
			      ...
			      atbm_size_t pk_len)
{
	...
	atbm_wpabuf_free(wps->dh_pubkey_r);
	wps->dh_pubkey_r = atbm_wpabuf_alloc_copy(pk, pk_len);
 ...
 static int wps_process_r_hash1(struct wps_data *wps, const atbm_uint8 *r_hash1)
{
	...
	atbm_wpa_hexdump(MSG_DEBUG, "WPS: R-Hash1", wps->peer_hash1, WPS_HASH_LEN);
 ...
 static int wps_process_r_hash2(struct wps_data *wps, const atbm_uint8 *r_hash2)
{
	...
	atbm_wpa_hexdump(MSG_DEBUG, "WPS: R-Hash2", wps->peer_hash2, WPS_HASH_LEN);
 ...
 static int wps_process_r_snonce1(struct wps_data *wps, const atbm_uint8 *r_snonce1)
{
	...
	//atbm_wpa_hexdump_key(MSG_DEBUG, "WPS: R-SNonce1", r_snonce1,
	//		WPS_SECRET_NONCE_LEN);
 ...
 static int wps_process_r_snonce2(struct wps_data *wps, const atbm_uint8 *r_snonce2)
{
	...
	//atbm_wpa_hexdump_key(MSG_DEBUG, "WPS: R-SNonce2", r_snonce2,
	//		WPS_SECRET_NONCE_LEN);
 ...
 static enum wps_process_res wps_process_m8_preHandle(struct wps_data *wps, const struct wpabuf *msg, struct wps_parse_attr *attr)
{
	...
	proc_msg->msg = atbm_wpabuf_dup(msg);
						 ...
						 const struct wpabuf *msg)
{
	...
	{
		wpa_printf(MSG_ERROR, "WPS: Mismatch in registrar nonce");
		atbm_wpa_hexdump(MSG_ERROR, "WPS: Received Registrar Nonce",
			    attr->registrar_nonce, WPS_NONCE_LEN);
		atbm_wpa_hexdump(MSG_ERROR, "WPS: Expected Registrar Nonce",
			    wps->nonce_r, WPS_NONCE_LEN);
		ret = WPS_FAILURE;
		goto end_process;
	}
	...
	if (attr->enrollee_nonce == NULL ||
	    atbm_memcmp(wps->nonce_e, attr->enrollee_nonce, WPS_NONCE_LEN) != 0) {
		wpa_printf(MSG_ERROR, "WPS: Mismatch in enrollee nonce");
		atbm_wpa_hexdump(MSG_ERROR, "WPS: Received Enrollee Nonce",
			    attr->enrollee_nonce, WPS_NONCE_LEN);
		atbm_wpa_hexdump(MSG_ERROR, "WPS: Expected Enrollee Nonce",
			    wps->nonce_e, WPS_NONCE_LEN);
		ret = WPS_FAILURE;
		goto end_process;
	}
					      ...
					      const struct wpabuf *msg)
{
	...
	if (op_code == WSC_UPnP) {
		...
		if (atbm_wps_parse_msg(msg, attr) == 0 && attr->msg_type) {
			if (*attr->msg_type == WPS_WSC_ACK)
				op_code = WSC_ACK;
