 ...
 static atbm_void wps_free_devices(struct wps_registrar_device *dev)
{
	...
	while (dev) {
		prev = dev;
		dev = dev->next;
		atbm_wps_device_data_free(&prev->dev);
		atbm_kfree(prev);
	}
}
...
#define WPS_STRDUP(n) \
	atbm_kfree(dst->n); \
	dst->n = src->n ? atbm_strdup(src->n) : NULL
		     ...
		     struct wps_device_data *dev, const atbm_uint8 *uuid)
{
 ...
 int atbm_wps_registrar_pbc_overlap(struct wps_registrar *reg,
					...
					struct wpabuf *msg)
{
	...
	if (id != DEV_PW_PUSHBUTTON || !reg->dualband)
		return 0;
	return atbm_wps_build_uuid_e(msg, reg->wps->uuid);
}
*/
...
//#endif /* CONFIG_WPS2 */
 ...
 static int wps_build_config_methods_r(struct wps_registrar *reg,
				      struct wpabuf *msg)
{
	return atbm_wps_build_config_methods(msg, reg->wps->config_methods);
}



/**
 * atbm_wps_registrar_init - Initialize WPS Registrar data
 ...
 * responsible for freeing the returned data with atbm_wps_registrar_deinit() when
 * Registrar functionality is not needed anymore.
 */
 struct wps_registrar *
atbm_wps_registrar_init(struct wps_context *wps,struct wps_registrar_config *cfg)
{
	...
	if (cfg->extra_cred) {
		reg->extra_cred = atbm_wpabuf_alloc_copy(cfg->extra_cred,
						    cfg->extra_cred_len);
...
/**
 * atbm_wps_registrar_deinit - Deinitialize WPS Registrar data
 * @reg: Registrar data from atbm_wps_registrar_init()
 */
 atbm_void atbm_wps_registrar_deinit(struct wps_registrar *reg)
{
	if (reg == NULL)
		return;
	...
	atbm_wpabuf_free(reg->extra_cred);
	atbm_kfree(reg);
}


 int atbm_wps_registrar_add_pin(struct wps_registrar *reg,const atbm_uint8 *addr)
{
	//wpa_printf(MSG_DEBUG, "WPS: A new PIN configured ");
	//atbm_wpa_hexdump(MSG_DEBUG, "WPS: UUID", uuid, WPS_UUID_LEN);
	//atbm_wpa_hexdump_ascii_key(MSG_DEBUG, "WPS: PIN", pin, pin_len);
	reg->selected_registrar = 1;
	reg->pbc = 0;
	atbm_wps_registrar_selected_registrar_changed(reg, 0);
	atbmwifi_eloop_cancel_timeout(wps_registrar_set_selected_timeout, reg, NULL);
...
/**
 * atbm_wps_registrar_button_pushed - Notify Registrar that AP button was pushed
 * @reg: Registrar data from atbm_wps_registrar_init()
 ...
 */
 int atbm_wps_registrar_button_pushed(struct wps_registrar *reg,
				const atbm_uint8 *p2p_dev_addr)
{
	if (p2p_dev_addr == NULL &&
	    atbm_wps_registrar_pbc_overlap(reg, NULL, NULL)) {
 ...
 static atbm_void wps_registrar_pin_completed(struct wps_registrar *reg)
{
 ...
 atbm_void atbm_wps_registrar_complete(struct wps_registrar *registrar, const atbm_uint8 *uuid_e,
...
/*
	if (dev_pw &&
	    wps_registrar_invalidate_wildcard_pin(registrar, dev_pw,
						  dev_pw_len) == 0) {
		atbm_wpa_hexdump_key(MSG_DEBUG, "WPS: Invalidated wildcard PIN",
				dev_pw, dev_pw_len);
	}*/
}


 int atbm_wps_registrar_wps_cancel(struct wps_registrar *reg)
{
	if (reg->pbc) {
...
//		goto out;
	...
	if (atbm_wps_build_version(beacon) ||
	  ...
	  //  wps_build_sel_pbc_reg_uuid_e(reg, beacon) ||
	   // (reg->dualband && atbm_wps_build_rf_bands(&reg->wps->dev, beacon)) ||
	   // atbm_wps_build_wfa_ext(beacon, 0, auth_macs, count) ||
	    atbm_wps_build_vendor_ext(&reg->wps->dev, beacon)) {
		atbm_wpabuf_free(beacon);
		atbm_wpabuf_free(probe);
		return -1;
	}

#if CONFIG_P2P
	if (atbm_wps_build_dev_name(&reg->wps->dev, beacon) ||
	    atbm_wps_build_primary_dev_type(&reg->wps->dev, beacon)) {
		atbm_wpabuf_free(beacon);
		atbm_wpabuf_free(probe);
		return -1;
	}
	...
	if (atbm_wps_build_version(probe) ||
	    ...
	    atbm_wps_build_resp_type(probe, reg->wps->ap ? WPS_RESP_AP :
				WPS_RESP_REGISTRAR) ||
	    atbm_wps_build_uuid_e(probe, reg->wps->uuid) ||
	    atbm_wps_build_device_attrs(&reg->wps->dev, probe) ||
	    wps_build_probe_config_methods(reg, probe) ||
	    atbm_wps_build_rf_bands(&reg->wps->dev, probe) ||
	   // atbm_wps_build_wfa_ext(probe, 0, auth_macs, count) ||
	    atbm_wps_build_vendor_ext(&reg->wps->dev, probe)) {
		atbm_wpabuf_free(beacon);
		atbm_wpabuf_free(probe);
		return -1;
	}

	beacon = atbm_wps_ie_encapsulate(beacon);
	probe = atbm_wps_ie_encapsulate(probe);
	...
	atbm_wpabuf_free(hapd->wps_beacon_ie);
	...
	atbm_wpabuf_free(hapd->wps_probe_resp_ie);
 ...
 static int wps_build_r_hash(struct wps_data *wps, struct wpabuf *msg)
{
	...
	if (atbmwifi_os_get_random(wps->snonce, 2 * WPS_SECRET_NONCE_LEN) < 0)
		return -1;
	atbm_wpa_hexdump(MSG_DEBUG, "WPS: R-S1", wps->snonce, WPS_SECRET_NONCE_LEN);
	atbm_wpa_hexdump(MSG_DEBUG, "WPS: R-S2",
	...
	hash = atbm_wpabuf_put(msg, SHA256_MAC_LEN);
	...
	atbm_wpa_hexdump(MSG_DEBUG, "WPS: R-Hash1", hash, SHA256_MAC_LEN);
	...
	addr[1] = wps->psk2;
	atbmwifi_hmac_sha256_vector(wps->authkey, WPS_AUTHKEY_LEN, 4, addr, len, hash);
	atbm_wpa_hexdump(MSG_DEBUG, "WPS: R-Hash2", hash, SHA256_MAC_LEN);
			       ...
			       const struct wps_credential *cred)
{
	...
	atbm_wpa_hexdump_ascii(MSG_DEBUG, "WPS: SSID for Credential",
				      ...
				      const struct wps_credential *cred)
{
	wpa_printf(MSG_DEBUG, "WPS:  * Network Key (len=%d)",
		   (int) cred->key_len);
	atbm_wpa_hexdump_key(MSG_DEBUG, "WPS: Network Key",
				...
				const struct wps_credential *cred)
{
 ...
 int atbm_wps_build_cred(struct wps_data *wps, struct wpabuf *msg)
{
	struct wpabuf *cred;
	...
	if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->wps->ap &&
		...
		while (wps->new_psk_len &&
			...
			wps->new_psk_len--;
		atbm_wpa_hexdump_ascii_key(MSG_DEBUG, "WPS: Generated passphrase",
		...
		atbm_wpa_hexdump_key(MSG_DEBUG, "WPS: Generated per-device PSK",
				wps->new_psk, wps->new_psk_len);
 ...
 static struct wpabuf * wps_build_ap_cred(struct wps_data *wps)
{
	...
	if (plain == NULL) {
		atbm_wpabuf_free(msg);
		return NULL;
	}

	if (wps_build_ap_settings(wps, plain)) {
		atbm_wpabuf_free(plain);
		atbm_wpabuf_free(msg);
		return NULL;
	}
 ...
 static struct wpabuf * wps_build_m2(struct wps_data *wps)
{
	...
	if (atbmwifi_os_get_random(wps->nonce_r, WPS_NONCE_LEN) < 0)
		return NULL;
	//atbm_wpa_hexdump(MSG_DEBUG, "WPS: Registrar Nonce",
	//	    wps->nonce_r, WPS_NONCE_LEN);
	//atbm_wpa_hexdump(MSG_DEBUG, "WPS: UUID-R", wps->uuid_r, WPS_UUID_LEN);
	...
	if (atbm_wps_build_version(msg) ||
	    atbm_wps_build_msg_type(msg, WPS_M2) ||
	    atbm_wps_build_enrollee_nonce(wps, msg) ||
	    atbm_wps_build_registrar_nonce(wps, msg) ||
	    wps_build_uuid_r(wps, msg) ||
	    atbm_wps_build_public_key(wps, msg) ||
	    wps_derive_keys(wps) ||
	    atbm_wps_build_auth_type_flags(wps, msg) ||
	    atbm_wps_build_encr_type_flags(wps, msg) ||
	    atbm_wps_build_conn_type_flags(wps, msg) ||
	    wps_build_config_methods_r(wps->wps->registrar, msg) ||
	    atbm_wps_build_device_attrs(&wps->wps->dev, msg) ||
	    atbm_wps_build_rf_bands(&wps->wps->dev, msg) ||
	    atbm_wps_build_assoc_state(wps, msg) ||
	    atbm_wps_build_config_error(msg, WPS_CFG_NO_ERROR) ||
	    atbm_wps_build_dev_password_id(msg, wps->dev_pw_id) ||
	    atbm_wps_build_os_version(&wps->wps->dev, msg) ||
	    atbm_wps_build_wfa_ext(msg, 0, NULL, 0) ||
	    atbm_wps_build_authenticator(wps, msg)) {
		atbm_wpabuf_free(msg);
		return NULL;
	}
 ...
 static struct wpabuf * wps_build_m2d(struct wps_data *wps)
{
	...
	if (atbm_wps_build_version(msg) ||
	    atbm_wps_build_msg_type(msg, WPS_M2D) ||
	    atbm_wps_build_enrollee_nonce(wps, msg) ||
	    atbm_wps_build_registrar_nonce(wps, msg) ||
	    wps_build_uuid_r(wps, msg) ||
	    atbm_wps_build_auth_type_flags(wps, msg) ||
	    ...
	    atbm_wps_build_rf_bands(&wps->wps->dev, msg) ||
	    atbm_wps_build_assoc_state(wps, msg) ||
	    atbm_wps_build_config_error(msg, err) ||
	    atbm_wps_build_os_version(&wps->wps->dev, msg) ||
	    atbm_wps_build_wfa_ext(msg, 0, NULL, 0)) {
		atbm_wpabuf_free(msg);
		return NULL;
 ...
 static struct wpabuf * wps_build_m4(struct wps_data *wps)
{
	...
	if (atbm_wps_build_version(msg) ||
	    atbm_wps_build_msg_type(msg, WPS_M4) ||
	    atbm_wps_build_enrollee_nonce(wps, msg) ||
	    wps_build_r_hash(wps, msg) ||
	    wps_build_r_snonce1(wps, plain) ||
	    atbm_wps_build_key_wrap_auth(wps, plain) ||
	    atbm_wps_build_encr_settings(wps, msg, plain) ||
	    atbm_wps_build_wfa_ext(msg, 0, NULL, 0) ||
	    atbm_wps_build_authenticator(wps, msg)) {
 ...
 static struct wpabuf * wps_build_m6(struct wps_data *wps)
{
	...
	if (atbm_wps_build_version(msg) ||
	    atbm_wps_build_msg_type(msg, WPS_M6) ||
	    atbm_wps_build_enrollee_nonce(wps, msg) ||
 ...
 static struct wpabuf * wps_build_m8(struct wps_data *wps)
{
	...
	if (atbm_wps_build_version(msg) ||
	    atbm_wps_build_msg_type(msg, WPS_M8) ||
	    atbm_wps_build_enrollee_nonce(wps, msg) ||
	    ((wps->wps->ap || wps->er) && atbm_wps_build_cred(wps, plain)) ||
...
//	wps->state = RECV_DONE;
 ...
 struct wpabuf * atbm_wps_registrar_get_msg(struct wps_data *wps,
				      enum wsc_op_code *op_code)
{
	...
	if (*op_code == WSC_MSG && msg) {
		 ...
		 */
		atbm_wpabuf_free(wps->last_msg);
		wps->last_msg = atbm_wpabuf_dup(msg);
	}
 ...
 static int wps_process_uuid_e(struct wps_data *wps, const atbm_uint8 *uuid_e)
{
	...
	//atbm_wpa_hexdump(MSG_DEBUG, "WPS: UUID-E", wps->uuid_e, WPS_UUID_LEN);
 ...
 static int wps_process_e_hash1(struct wps_data *wps, const atbm_uint8 *e_hash1)
{
	...
	atbm_wpa_hexdump(MSG_DEBUG, "WPS: E-Hash1", wps->peer_hash1, WPS_HASH_LEN);
 ...
 static int wps_process_e_hash2(struct wps_data *wps, const atbm_uint8 *e_hash2)
{
	...
	atbm_wpa_hexdump(MSG_DEBUG, "WPS: E-Hash2", wps->peer_hash2, WPS_HASH_LEN);
 ...
 static int wps_process_e_snonce1(struct wps_data *wps, const atbm_uint8 *e_snonce1)
{
	...
	atbm_wpa_hexdump_key(MSG_DEBUG, "WPS: E-SNonce1", e_snonce1,
			WPS_SECRET_NONCE_LEN);
 ...
 static int wps_process_e_snonce2(struct wps_data *wps, const atbm_uint8 *e_snonce2)
{
	...
	atbm_wpa_hexdump_key(MSG_DEBUG, "WPS: E-SNonce2", e_snonce2,
			WPS_SECRET_NONCE_LEN);
			      ...
			      atbm_size_t pk_len)
{
	...
	atbm_wpabuf_free(wps->dh_pubkey_e);
	wps->dh_pubkey_e = atbm_wpabuf_alloc_copy(pk, pk_len);
					   ...
					   struct wps_parse_attr *attr)
{
	...
	if (wps_process_uuid_e(wps, attr->uuid_e) ||
	    ...
	    atbm_wps_process_device_attrs(&wps->peer_dev, attr) ||
	    atbm_wps_process_rf_bands(&wps->peer_dev, attr->rf_bands) ||
	    ...
	    atbm_wps_process_os_version(&wps->peer_dev, attr->os_version))
		return WPS_FAILURE;
	...
	/*if (wps->dev_pw_id == DEV_PW_PUSHBUTTON) {
		if ((wps->wps->registrar->force_pbc_overlap ||
		     atbm_wps_registrar_pbc_overlap(wps->wps->registrar,
					   ...
					   struct wps_parse_attr *attr)
{
	...
	if (atbm_wps_parse_msg(decrypted, eattr) < 0 ||
						...
						const struct wpabuf *msg)
{
	...
	if (atbm_wps_parse_msg(msg, attr) < 0)
	{
		atbm_kfree(attr);
		return WPS_FAILURE;
	}
						 ...
						 const struct wpabuf *msg)
{
	...
	if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->new_psk &&
		...
		atbm_wpa_hexdump_ascii_key(MSG_DEBUG,
 ...
 enum wps_process_res atbm_wps_registrar_process_msg(struct wps_data *wps,
					       enum wsc_op_code op_code,
					       const struct wpabuf *msg)
{
 ...
 int atbm_wps_registrar_update_ie(struct wps_registrar *reg)
{
...
/**
 * atbm_wps_registrar_selected_registrar_changed - SetSelectedRegistrar change
 * @reg: Registrar data from atbm_wps_registrar_init()
 *
 * This function is called when selected registrar state changes, e.g., when an
 * AP receives a SetSelectedRegistrar UPnP message.
 */
 atbm_void atbm_wps_registrar_selected_registrar_changed(struct wps_registrar *reg, atbm_uint16 dev_pw_id)
{
			   ...
			   char *buf, atbm_size_t buflen)
{
	...
	if (d == NULL)
		return 0;
	if (atbm_uuid_bin2str(d->uuid, uuid, sizeof(uuid)))
		return 0;
 ...
 int atbm_wps_registrar_config_ap(struct wps_registrar *reg,
			    struct wps_credential *cred)
{
#if CONFIG_WPS2
