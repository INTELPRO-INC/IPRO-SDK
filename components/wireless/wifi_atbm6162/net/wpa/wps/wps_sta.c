...
//	struct atbmwifi_vif *priv;
	...
	if(cred->key_len == 64){
		atbm_wpa_hexdump(MSG_ALWAYS, "WPS: cred, Network Key \n", cred->key, cred->key_len);
	}else{
...
/**
 * atbm_wps_deinit - Deinitialize WPS Registration protocol data
 * @data: WPS Registration protocol data from wps_init()
 */
  atbm_void atbm_wps_deinit(struct wps_data *data)
{
	...
	atbm_wpabuf_free(data->dh_pubkey);
	data->dh_pubkey=NULL;

	atbm_wpabuf_free(data->dh_privkey);
	data->dh_privkey=NULL;

	atbm_wpabuf_free(data->dh_pubkey_e);
	data->dh_pubkey_e=NULL;

	atbm_wpabuf_free(data->dh_pubkey_r);
	data->dh_pubkey_r=NULL;
	
	
	atbm_wpabuf_free(data->last_msg);
	data->last_msg=NULL;
	...
	atbm_wps_device_data_free(&data->peer_dev);
 ...
 * registration run has been completed by calling atbm_wps_deinit().
 */
 static struct wps_data * wps_init(struct wpa_supplicant *wpa_s)
{
	...
	switch(wpa_s->wps_mode){
		case WPS_MODE_PIN:
			...
			data->dev_password = (atbm_uint8 *) atbm_strdup((char *)wpa_s->pin);
		...
		case WPS_MODE_PBC:
			...
			data->dev_password = (atbm_uint8 *) atbm_strdup("00000000");
			if (data->dev_password == NULL) {
			      ...
			      struct wps_context *wps)
{
	...
	//atbm_wpa_hexdump(MSG_DEBUG, "WPS: UUID based on MAC address\n", wps->uuid, WPS_UUID_LEN);
}
...
__error__:
 ...
 int atbm_wpas_wps_start_pbc(struct wpa_supplicant *wpa_s, const atbm_uint8 *bssid, int p2p_group)
{
	int ret = 0;
	struct atbmwifi_vif *priv = wpa_s->priv;
	...
	if(ret < 0){
		wpa_printf(MSG_ERROR, "WPS: atbm_wpas_wps_start_pbc, wpas init wps failed.");
		goto __error__;
	}
...
__error__:
 ...
 int atbm_wpas_wps_start_pin(struct wpa_supplicant *wpa_s, const atbm_uint8 *bssid,
...
//	atbm_wifi_disconnect();
	...
	if(wpa_s->pin == NULL){
		wpa_printf(MSG_ERROR, "WPS: atbm_wpas_wps_start_pin, pin code malloc is NULL.");
		ret = -1;
		goto __error__;
	}
	...
	if (pin){
		if(strlen(pin) != PIN_CODE_LENGTH){
			wpa_printf(MSG_ERROR, "WPS: atbm_wpas_wps_start_pin, pin code length invalid,[%s] len %d", pin,strlen(pin));
			ret = -1;
			goto __error__;
		}
	...
	wpa_printf(MSG_DEBUG, "WPS: atbm_wpas_wps_start_pin, pin code is %s", wpa_s->pin);

	wpa_s->wps_mode = WPS_MODE_PIN;

	ret = wpas_init_wps(wpa_s);
	if(ret < 0){
		wpa_printf(MSG_ERROR, "WPS: atbm_wpas_wps_start_pin, wpas init wps failed.");
		goto __error__;
	}
