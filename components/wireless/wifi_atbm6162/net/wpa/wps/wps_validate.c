				   ...
				   int mandatory)
{
	...
	if (len > 0 && model_name[len - 1] == 0) {
		//atbm_wpa_hexdump_ascii(MSG_ERROR, "WPS-STRICT: Invalid Model Name "
				     ...
				     int mandatory)
{
	...
	if (len > 0 && model_number[len - 1] == 0) {
		//atbm_wpa_hexdump_ascii(MSG_ERROR, "WPS-STRICT: Invalid Model Number "
				      ...
				      int mandatory)
{
	...
	if (len > 0 && serial_number[len - 1] == 0) {
		//atbm_wpa_hexdump_ascii(MSG_ERROR, "WPS-STRICT: Invalid Serial "
				 ...
				 int mandatory)
{
	...
	if (len > 0 && dev_name[len - 1] == 0) {
		//atbm_wpa_hexdump_ascii(MSG_ERROR, "WPS-STRICT: Invalid Device Name "
					...
					int mandatory)
{
	...
	if (len > 30 && (len % ETH_ALEN) != 0) {
		atbm_wpa_hexdump(MSG_ERROR, "WPS-STRICT: Invalid Authorized "
				    ...
				    const atbm_uint8 *encr_type, int mandatory)
{
	...
	if (((encr_type == NULL || ATBM_WPA_GET_BE16(encr_type) != WPS_ENCR_WEP) &&
	    ...
	    key_len > 64) {
		//atbm_wpa_hexdump_ascii_key(MSG_ERROR, "WPS-STRICT: Invalid Network "
 ...
 static int wps_validate_cred(const atbm_uint8 *cred, atbm_size_t len)
{
	...
	if (atbm_wps_parse_msg(&buf, attr) < 0) {
 ...
 int wps_validate_beacon(const struct wpabuf *wps_ie)
{
	...
	if (atbm_wps_parse_msg(wps_ie, attr) < 0) {
 ...
 int wps_validate_m1(const struct wpabuf *tlvs)
{
	...
	if (atbm_wps_parse_msg(tlvs, attr) < 0) {
