...
extern struct wpabuf * atbm_wps_get_msg(struct wps_data *wps, enum wsc_op_code *op_code);
extern atbm_void atbm_wps_deinit(struct wps_data *data);
extern enum wps_process_res atbm_wps_process_msg(struct wps_data *wps,
 ...
 atbm_void hostapd_eap_wsc_init(struct atbmwifi_vif *priv,atbm_uint8 *da)
{
	...
	if (config != NULL){
		if (config->dev_password_len > 0) {
			...
			data->dev_password = (atbm_uint8 *)atbm_strdup((char *)config->dev_password);
 ...
 atbm_void eap_wsc_dinit(struct atbmwifi_vif *priv)
{
	...
	if (hapd->wps_last_rx_data)
		atbm_wpabuf_free(hapd->wps_last_rx_data);
	hapd->wps_last_rx_data = NULL;
}
 ...
 struct wpabuf * eap_wsc_build_start(atbm_uint8 id)
{
	...
	req = atbm_eap_msg_alloc(ATBM_EAP_VENDOR_WFA,(EapType)EAP_VENDOR_TYPE_WSC, 2,EAP_CODE_REQUEST,id);
	if (req == NULL) {
 ...
 static struct wpabuf * eap_wsc_build_msg(struct eap_wsc_data *data, atbm_uint8 id)
{
	...
	if (flags & WSC_FLAGS_LF)
		plen += 2;
	req = atbm_eap_msg_alloc(ATBM_EAP_VENDOR_WFA, (EapType)EAP_VENDOR_TYPE_WSC, plen,
			    EAP_CODE_REQUEST, id);
	if (req == NULL) {
		wpa_printf(MSG_ERROR, "EAP-WSC: Failed to allocate memory for "
			   "request");
 ...
 static struct wpabuf * eap_wsc_build_msg_fail(atbm_uint8 id)
{
	...
	hdr = atbm_wpabuf_put(req, sizeof(*hdr));
 ...
 atbm_void wps_send_eapol_timeout(void *data1, void *data2)
{
	...
	if(g_hostapd && sta){
		...
		hostapd_send_eapol(wpa_get_driver_priv(g_hostapd->priv), sta->addr, ATBM_ETH_P_EAPOL, (atbm_uint8*)g_hostapd->wps_tx_hdr, g_hostapd->wps_tx_hdr_len, ATBM_TRUE);
	}
}
 ...
 atbm_void hostapd_eap_wsc_process(struct hostapd_data *hapd,struct hostapd_sta_info *sta,struct wpabuf *respData)
{
	...
	pos = atbm_eap_hdr_validate(ATBM_EAP_VENDOR_WFA, (EapType)EAP_VENDOR_TYPE_WSC,
			       respData, &len);
	if (pos == NULL || len < 2)
	...
	res = atbm_wps_process_msg(hapd->wpsdata, (enum wsc_op_code )op_code,  &tmpbuf);
	switch (res) {
	case WPS_DONE:
...
//		hapd->wpsdata->state = RECV_DONE;
	...
	data.out_buf = atbm_wps_get_msg(hapd->wpsdata,
						    &data.out_op_code);
