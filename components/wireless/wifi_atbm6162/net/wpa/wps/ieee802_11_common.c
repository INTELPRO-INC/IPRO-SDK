...
#include "atbm_hal.h"
 ...
 enum atbm_hostapd_hw_mode atbm_ieee80211_freq_to_chan(atbm_int32 freq, atbm_uint8 *channel)
{
 ...
 static atbm_int32 country_match(const char *cc[], const char *country)
{
 ...
 static atbm_int32 atbm_ieee80211_chan_to_freq_us(atbm_uint8 op_class, atbm_uint8 chan)
{
	switch (op_class) {
 ...
 static atbm_int32 atbm_ieee80211_chan_to_freq_eu(atbm_uint8 op_class, atbm_uint8 chan)
{
	switch (op_class) {
 ...
 static atbm_int32 atbm_ieee80211_chan_to_freq_jp(atbm_uint8 op_class, atbm_uint8 chan)
{
	switch (op_class) {
 ...
 static atbm_int32 atbm_ieee80211_chan_to_freq_cn(atbm_uint8 op_class, atbm_uint8 chan)
{
	switch (op_class) {
 ...
 static atbm_int32 atbm_ieee80211_chan_to_freq_global(atbm_uint8 op_class, atbm_uint8 chan)
{
...
/**
 * atbm_ieee80211_chan_to_freq - Convert channel info to frequency
 * @country: Country code, if known; otherwise, global operating class is used
 * @op_class: Operating class
 * @chan: Channel number
 * Returns: Frequency in MHz or -1 if the specified channel is unknown
 */
 atbm_int32 atbm_ieee80211_chan_to_freq(const char *country, atbm_uint8 op_class, atbm_uint8 chan)
{
	atbm_int32 freq;

	if (country_match(us_op_class_cc, country)) {
		freq = atbm_ieee80211_chan_to_freq_us(op_class, chan);
		if (freq > 0)
			return freq;
	}

	if (country_match(eu_op_class_cc, country)) {
		freq = atbm_ieee80211_chan_to_freq_eu(op_class, chan);
		if (freq > 0)
			return freq;
	}

	if (country_match(jp_op_class_cc, country)) {
		freq = atbm_ieee80211_chan_to_freq_jp(op_class, chan);
		if (freq > 0)
			return freq;
	}

	if (country_match(cn_op_class_cc, country)) {
		freq = atbm_ieee80211_chan_to_freq_cn(op_class, chan);
		if (freq > 0)
			return freq;
	}

	return atbm_ieee80211_chan_to_freq_global(op_class, chan);
}


 atbm_int32 atbm_ieee80211_is_dfs(atbm_int32 freq)
{
