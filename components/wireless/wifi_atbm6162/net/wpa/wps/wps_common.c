 ...
 atbm_int32 wps_derive_keys(struct wps_data *wps)
{
	...
	dh5_free(wps->dh_ctx);
	wps->dh_ctx = NULL;
	dh_shared = atbm_wpabuf_zeropad(dh_shared, 192);
	if (dh_shared == NULL) {
	...
	//atbm_wpa_hexdump_key(MSG_DEBUG, "WPS: DHKey", dhkey, SHA256_MAC_LEN);
	atbm_wpabuf_free(dh_shared);
	...
	//atbm_wpa_hexdump_key(MSG_DEBUG, "WPS: KDK", kdk, SHA256_MAC_LEN);
	...
	//atbm_wpa_hexdump_key(MSG_DEBUG, "WPS: AuthKey",
	//		wps->authkey, WPS_AUTHKEY_LEN);
	//atbm_wpa_hexdump_key(MSG_DEBUG, "WPS: KeyWrapKey",
	//		wps->keywrapkey, WPS_KEYWRAPKEY_LEN);
	//atbm_wpa_hexdump_key(MSG_DEBUG, "WPS: EMSK", wps->emsk, WPS_EMSK_LEN);
	atbm_kfree(dhkey);
	atbm_kfree(keys);
	atbm_kfree(kdk);
		    ...
		    atbm_size_t dev_passwd_len)
{
	...
	atbm_wpa_hexdump_ascii_key(MSG_DEBUG, "WPS: Device Password",
			      dev_passwd, dev_passwd_len);
	atbm_wpa_hexdump_key(MSG_DEBUG, "WPS: PSK1", wps->psk1, WPS_PSK_LEN);
	atbm_wpa_hexdump_key(MSG_DEBUG, "WPS: PSK2", wps->psk2, WPS_PSK_LEN);
}
					  ...
					  atbm_size_t encr_len)
{
	...
	atbm_wpa_hexdump(MSG_MSGDUMP, "WPS: Encrypted Settings", encr, encr_len);
...
/**
 * atbm_wps_build_assoc_resp_ie - Build WPS IE for (Re)Association Response
 * Returns: WPS IE or %NULL on failure
 *
 * The caller is responsible for freeing the buffer.
 */
 struct wpabuf * atbm_wps_build_assoc_resp_ie(atbm_void)
{
	struct wpabuf *ie;
	atbm_uint8 *len;
	...
	len = atbm_wpabuf_put(ie, 1);
	wpabuf_put_be32(ie, WPS_DEV_OUI_WFA);

	if (atbm_wps_build_version(ie) ||
	    atbm_wps_build_resp_type(ie, WPS_RESP_AP) ||
	    atbm_wps_build_wfa_ext(ie, 0, NULL, 0)) {
		atbm_wpabuf_free(ie);
		return NULL;
	}
 ...
 struct wpabuf * wps_build_wsc_ack(struct wps_data *wps)
{
	...
	if (atbm_wps_build_version(msg) ||
	    atbm_wps_build_msg_type(msg, WPS_WSC_ACK) ||
 ...
 struct wpabuf * wps_build_wsc_nack(struct wps_data *wps)
{
	...
	if (atbm_wps_build_version(msg) ||
	    atbm_wps_build_msg_type(msg, WPS_WSC_NACK) ||
	    atbm_wps_build_enrollee_nonce(wps, msg) ||
	    atbm_wps_build_registrar_nonce(wps, msg) ||
	    atbm_wps_build_config_error(msg, wps->config_error) ||
	    atbm_wps_build_wfa_ext(msg, 0, NULL, 0)) {
		atbm_wpabuf_free(msg);
		return NULL;
	}
