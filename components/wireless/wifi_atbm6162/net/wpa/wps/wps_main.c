...
extern struct wpabuf * atbm_eap_msg_alloc(EapVenType vendor, EapType type, atbm_uint32 payload_len,
			      EapCodeType code, atbm_uint8 identifier);
extern const atbm_uint8 * atbm_eap_hdr_validate(EapVenType vendor, EapType eap_type,
...
extern atbm_uint8 atbm_eap_get_id(const struct wpabuf *msg);
extern int wps_add_pin(struct hostapd_data *hapd, atbm_void *ctx);

/**
 * atbm_wps_build_probe_req_ie - Build WPS IE for Probe Request
 ...
 */
 struct wpabuf * atbm_wps_build_probe_req_ie(atbm_uint16 pw_id, struct wps_device_data *dev,
				       ...
				       const atbm_uint8 *req_dev_types)
{
	...
	if (atbm_wps_build_version(ie) ||
	    atbm_wps_build_req_type(ie, req_type) ||
	    atbm_wps_build_config_methods(ie, dev->config_methods) ||
	    atbm_wps_build_uuid_e(ie, uuid) ||
	    atbm_wps_build_primary_dev_type(dev, ie) ||
	    atbm_wps_build_rf_bands(dev, ie) ||
	    atbm_wps_build_assoc_state(NULL, ie) ||
	    atbm_wps_build_config_error(ie, WPS_CFG_NO_ERROR) ||
	    atbm_wps_build_dev_password_id(ie, pw_id) ||
#if CONFIG_WPS2
	    atbm_wps_build_manufacturer(dev, ie) ||
	    atbm_wps_build_model_name(dev, ie) ||
	    atbm_wps_build_model_number(dev, ie) ||
	    atbm_wps_build_dev_name(dev, ie) ||
	    atbm_wps_build_wfa_ext(ie, req_type == WPS_REQ_ENROLLEE, NULL, 0) ||
#endif /* CONFIG_WPS2 */
	    atbm_wps_build_req_dev_type(dev, ie, num_req_dev_types, req_dev_types)
	    ||
	    atbm_wps_build_secondary_dev_type(dev, ie)
		) {
		atbm_wpabuf_free(ie);
		return NULL;
	}
...
#if (CONFIG_WPS2 == 0)
	if (dev->p2p && atbm_wps_build_dev_name(dev, ie)) {
		atbm_wpabuf_free(ie);
		return NULL;
	}
#endif /* CONFIG_WPS2 */

	return atbm_wps_ie_encapsulate(ie);
}
 ...
 atbm_uint8 * atbmwifi_ieee80211_add_preq_wps_ie(struct atbmwifi_vif *priv,atbm_uint8 * pos)
{
	...
	if (wpa_s->wps_mode != WPS_MODE_UNKNOWN) {
		...
		wps_ie = atbm_wps_build_probe_req_ie(wpa_s->wps_mode == WPS_MODE_PBC ? DEV_PW_PUSHBUTTON :
						DEV_PW_DEFAULT,
						&wps_ctx->dev,
 ...
 atbm_uint8 * atbmwifi_ieee80211_add_assocreq_wps_ie(struct atbmwifi_vif *priv,atbm_uint8 * pos)
{
	...
	if (atbm_wps_build_version(wps_ie) ||
	    atbm_wps_build_req_type(wps_ie, WPS_REQ_ENROLLEE) ||
	    atbm_wps_build_wfa_ext(wps_ie, 0, NULL, 0)){
		atbm_wpabuf_free(wps_ie);
		return pos;
	}

	//atbm_wps_ie_encapsulate(wps_ie);
 ...
 int wps_parse_msg_attr(const atbm_uint8 *msg, atbm_uint8 length, struct wps_parse_attr *attr)
{
 ...
 int atbm_wps_is_addr_authorized(struct wps_parse_attr *attr, const atbm_uint8 *addr)
 {
	 unsigned int i;
...
int check_valid_wps_ap(struct atbmwifi_vif *priv, struct wps_parse_attr *attr, atbm_uint8 *addr){
	...
	if(priv->pin){
		if(atbm_wps_is_addr_authorized(attr, addr)){
			return 1;
		}
		/*for wps Interoperability Test(5.4.5) wsc 1.0 ap with external registar*/ 
#if 0
		if(wpa_s->scan_runs >= WPS_PIN_SCAN_IGNORE_SEL_REG ||
			((atbm_GetOsTime() - wpa_s->wps_pin_start_time >= WPS_PIN_TIME_IGNORE_SEL_REG * ATBM_HZ)){
			return 1;
		}
#endif
	}
...
/**
 * atbm_wps_get_msg - Build a WPS message
 ...
 * atbm_wps_process_msg(). The caller is responsible for freeing the buffer.
 */
 struct wpabuf * atbm_wps_get_msg(struct wps_data *wps, enum wsc_op_code *op_code)
{
	if (wps->registrar)
		return atbm_wps_registrar_get_msg(wps, op_code);
	else
		return wps_enrollee_get_msg(wps, op_code);
}

/**
 * atbm_wps_process_msg - Process a WPS message
 * @wps: WPS Registration protocol data from wps_init()
 ...
 * Response to this message is built by calling atbm_wps_get_msg().
 */
 enum wps_process_res atbm_wps_process_msg(struct wps_data *wps,
				     enum wsc_op_code op_code,
				     const struct wpabuf *msg)
{
	if (wps->registrar)
		return atbm_wps_registrar_process_msg(wps, op_code, msg);
	else
		return wps_enrollee_process_msg(wps, op_code, msg);
}

 struct wpabuf * atbm_eap_wsc_build_frag_ack(atbm_uint8 id, atbm_uint8 code)
{
	struct wpabuf *msg;

	msg = atbm_eap_msg_alloc(ATBM_EAP_VENDOR_WFA, (EapType)EAP_VENDOR_TYPE_WSC, 2, (EapCodeType)code, id);
	if (msg == NULL) {
						...
						const atbm_uint8 *buf, atbm_size_t len)
{
	...
	return atbm_eap_wsc_build_frag_ack(id, EAP_CODE_RESPONSE);
}
 ...
 static struct wpabuf * eap_wsc_build_msg(struct eap_wsc_data *data,atbm_uint8 id)
{
	...
	if (flags & WSC_FLAGS_LF)
		plen += 2;
	resp = atbm_eap_msg_alloc(ATBM_EAP_VENDOR_WFA, (EapType)EAP_VENDOR_TYPE_WSC, plen,
 ...
 struct wpabuf * wpas_eap_wsc_process(struct wpa_supplicant *wpa_s, const struct wpabuf *reqData)
{
	...
	pos = atbm_eap_hdr_validate(ATBM_EAP_VENDOR_WFA, (EapType)EAP_VENDOR_TYPE_WSC, reqData,&len);
	if (pos == NULL || len < 2) {
		return NULL;
	}

	id = atbm_eap_get_id(reqData);
	...
	res = atbm_wps_process_msg(data->wps, (enum wsc_op_code)op_code, data->in_buf);
	switch (res) {
	case WPS_DONE:
	...
	if (data->in_buf != &tmpbuf)
		atbm_wpabuf_free(data->in_buf);
	data->in_buf = NULL;

send_msg:
	if (data->out_buf == NULL) {
		data->out_buf = atbm_wps_get_msg(data->wps, &data->out_op_code);
		if (data->out_buf == NULL) {
 ...
 int atbmwps_start_pbc(struct atbmwifi_vif *priv, atbm_uint8 *p2p_info)
{
	...
	switch(priv->iftype){
		...
		case ATBM_NL80211_IFTYPE_AP:
			...
			priv->pbc = 1;
			priv->pin = 0;
			ret = atbm_hostapd_wps_button_pushed(priv->appdata, NULL); 
		...
		case ATBM_NL80211_IFTYPE_STATION:
			...
			ret = atbm_wpas_wps_start_pbc(priv->appdata, NULL, p2p_group);
 ...
 int atbmwps_start_pin(struct atbmwifi_vif *priv, const char *pin, atbm_uint8* buf, atbm_uint32 info)
{
	...
	switch(priv->iftype){
		...
		case ATBM_NL80211_IFTYPE_STATION:
			...
			dev_pw_id = info;
			ret = atbm_wpas_wps_start_pin(priv->appdata, priv->bssid, pin, p2p_group, dev_pw_id);
...
int atbmwps_init(struct atbmwifi_vif *priv)
{
	...
	if(atbmwifi_is_ap_mode(priv->iftype)){
		ret = atbm_hostapd_init_wps(priv->appdata);
	}else if(atbmwifi_is_sta_mode(priv->iftype)){
