...
#include "wpa_debug.h"
 ...
 atbm_int32 atbm_wps_build_public_key(struct wps_data *wps, struct wpabuf *msg)
{
	struct wpabuf *pubkey;

	wpa_printf(MSG_DEBUG, "WPS:  * Public Key");
	//atbm_wpabuf_free(wps->dh_privkey);
	//wps->dh_privkey = NULL;
	...
	if (wps->dev_pw_id != DEV_PW_DEFAULT && wps->dh_privkey &&
		...
		//wps->dh_privkey = atbm_wpabuf_dup(wps->wps->dh_privkey);
		//wps->dh_ctx = wps->wps->dh_ctx;
		//wps->wps->dh_ctx = NULL;
		pubkey = atbm_wpabuf_dup(wps->dh_pubkey);
	} else {
		...
		pubkey = atbm_wpabuf_zeropad(pubkey, 192);
		#else
 ...
 atbm_int32 atbm_wps_build_req_type(struct wpabuf *msg, enum wps_request_type type)
{
 ...
 atbm_int32 atbm_wps_build_resp_type(struct wpabuf *msg, enum wps_response_type type)
{
 ...
 atbm_int32 atbm_wps_build_config_methods(struct wpabuf *msg, atbm_uint16 methods)
{
...
*/
 atbm_int32 atbm_wps_build_uuid_e(struct wpabuf *msg, const atbm_uint8 *uuid)
{
 ...
 atbm_int32 atbm_wps_build_dev_password_id(struct wpabuf *msg, atbm_uint16 id)
{
 ...
 atbm_int32 atbm_wps_build_config_error(struct wpabuf *msg, atbm_uint16 err)
{
 ...
 atbm_int32 atbm_wps_build_authenticator(struct wps_data *wps, struct wpabuf *msg)
{
 ...
 atbm_int32 atbm_wps_build_version(struct wpabuf *msg)
{
	/*
 ...
 atbm_int32 atbm_wps_build_wfa_ext(struct wpabuf *msg, atbm_int32 req_to_enroll,
		      const atbm_uint8 *auth_macs, atbm_size_t auth_macs_count)
{
	...
	len = atbm_wpabuf_put(msg, 2); /* to be filled */
	...
	ATBM_WPA_PUT_BE16(len, (atbm_uint8 *) atbm_wpabuf_put(msg, 0) - len - 2);
 ...
 atbm_int32 atbm_wps_build_msg_type(struct wpabuf *msg, enum wps_msg_type msg_type)
{
 ...
 atbm_int32 atbm_wps_build_enrollee_nonce(struct wps_data *wps, struct wpabuf *msg)
{
 ...
 atbm_int32 atbm_wps_build_registrar_nonce(struct wps_data *wps, struct wpabuf *msg)
{
 ...
 atbm_int32 atbm_wps_build_auth_type_flags(struct wps_data *wps, struct wpabuf *msg)
{
 ...
 atbm_int32 atbm_wps_build_encr_type_flags(struct wps_data *wps, struct wpabuf *msg)
{
 ...
 atbm_int32 atbm_wps_build_conn_type_flags(struct wps_data *wps, struct wpabuf *msg)
{
 ...
 atbm_int32 atbm_wps_build_assoc_state(struct wps_data *wps, struct wpabuf *msg)
{
 ...
 atbm_int32 atbm_wps_build_key_wrap_auth(struct wps_data *wps, struct wpabuf *msg)
{
	atbm_uint8 hash[SHA256_MAC_LEN];
 ...
 atbm_int32 atbm_wps_build_encr_settings(struct wps_data *wps, struct wpabuf *msg,
			    struct wpabuf *plain)
{
	atbm_size_t pad_len;
...
/* Encapsulate WPS IE data with one (or more, if needed) IE headers */
 struct wpabuf * atbm_wps_ie_encapsulate(struct wpabuf *data)
{
	struct wpabuf *ie;
...
//}
 ...
 atbm_int32 atbm_wps_build_rf_bands_attr(struct wpabuf *msg, atbm_uint8 rf_bands)
{
 ...
 atbm_int32 atbm_wps_build_ap_channel(struct wpabuf *msg, atbm_uint16 ap_channel)
{
