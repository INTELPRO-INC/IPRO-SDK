 ...
 *
 * 文件说明:
 *   本文件实现 WPA 事件队列机制，用于处理 WiFi 连接过程中的各种异步事件。
 *   事件队列采用生产者-消费者模式，将事件处理从中断上下文转移到线程上下文。
 *
 * 主要功能:
 *   - WPA 事件队列管理（添加、处理、释放）
 *   - WPA 事件线程（后台处理事件）
 *   - 支持的事件类型包括：扫描、连接、认证、关联、EAPOL 等
 *
 * 事件处理流程:
 *   1. 调用 atbmwifi_wpa_event_queue() 将事件加入队列
 *   2. 唤醒 WPA 事件线程
 *   3. 事件线程从队列取出事件并处理
 *   4. 处理完成后释放事件资源
 *
 * 支持的事件类型:
 *   - WPA_EVENT__SUPPLICANT_START_CONNECT: 开始连接
 *   - WPA_EVENT__SUPPLICANT_SCAN: 扫描
 *   - WPA_EVENT__SUPPLICANT_SCAN_END: 扫描完成
 *   - WPA_EVENT__SUPPLICANT_AUTH: 认证
 *   - WPA_EVENT__SUPPLICANT_ASSOC: 关联
 *   - WPA_EVENT__SUPPLICANT_CONNECT_FAIL: 连接失败
 *   - WPA_EVENT__SUPPLICANT_EAPOL: EAPOL 帧处理
 *   - WPA_EVENT__HOSTAPD_START: AP 模式启动
 *   - WPA_EVENT__HOSTAPD_STA_HANDSHAKE: AP 模式 4 次握手
 *
*****************************************************************************************************************/
...
extern atbm_void atbm_ieee802_1x_receive(struct atbmwifi_vif *priv,
...
static int atbmwifi_wpa_event_bh(atbm_void *arg, int wakeup_state);
...
struct atbm_wpa_event {
	...
	atbm_wpa_event_data data1;
	atbm_wpa_event_data data2;
	atbm_wpa_event_data data3;
	atbm_uint8 ack_en;
};
...
#define atbm_wpa_event_bh_wakeup()			do{ \
												if(!atbm_wpa_event_is_running()){ \
													atbm_wakeupThreadInternal(atbm_wpa_event_bh); \
												} \
											}while(0);
...
#define atbm_wpa_wait_ack(wait_ack)																			\
		do{ 																								\
			if(wait_ack==ATBM_WPA_EVENT_ACK){															\
...
{	
	...
	atbm_wpa_event_bh = atbm_createThreadInternal("atbm_hif",
									atbmwifi_wpa_event_bh,
									(atbm_void*)ATBM_NULL,
									HIF_TASK_PRIO,
									100*ATBM_HZ);
...
atbm_uint32 atbmwifi_wpa_event_destory(atbm_void)
{
	...
	atbm_stopThreadInternal(atbm_wpa_event_bh);
	...
	switch(event->event){
		...
		*/
		...
		case WPA_EVENT__SUPPLICANT_DEAUTHEN:
		{
			struct atbmwifi_vif *priv = (struct atbmwifi_vif *)event->data1;
			atbm_uint16 linkid = (atbm_uint16)event->data2;		
			if((linkid > ATBMWIFI__MAX_STA_IN_AP_MODE) || ( linkid<=0)){
				wifi_printk(WIFI_WPA,"event_process%s linkid[%d] err\n",event_string,linkid);
				break;
			}
			wifi_printk(WIFI_WPA,"SUPPLICANT_DEAUTHEN %d\n",linkid);
			atbmwifi_ap_deauth(priv, priv->link_id_db[linkid-1].mac, ATBM_FALSE);
			break;
		}
		case WPA_EVENT__SUPPLICANT_CONNECT_FAIL:
		{
		...
		*/
		...
		case WPA_EVENT__HOSTAPD_STA_HANDSHAKE:
		{
			...
			wifi_printk(WIFI_WPA,"STA_HANDSHAKE %x ssta %x\n",priv,sta);
...
/**************************************************************************************************************
 * 函数名: atbmwifi_wpa_event_queue
 *
 * 功能描述:
 *   将 WPA 事件加入事件队列。该函数是事件驱动架构的核心，负责将各种 WiFi 事件
 *   （扫描、连接、认证、EAPOL 等）加入队列，由后台线程异步处理。
 *
 * 在连接流程中的位置:
 *   事件队列层
 *   在整个连接流程中被多次调用，用于触发各种事件处理
 *
 * 参数说明:
 *   @user_data1: 事件数据 1，通常是 priv 指针
 *   @user_data2: 事件数据 2，根据事件类型不同含义不同
 *   @user_data3: 事件数据 3，根据事件类型不同含义不同
 *   @event_id:   事件 ID，参见 atbm_wpa_event_id 枚举
 *   @wait_ack:   是否等待确认:
 *                - ATBM_WPA_EVENT_ACK: 等待事件处理完成
 *                - ATBM_WPA_EVENT_NOACK: 不等待，立即返回
 *
 * 返回值:
 *   0:  成功
 *   -1: 失败（事件 ID 无效或队列已满）
 ...
 *
 * 注意事项:
 *   - 队列大小有限（ATBM_WPA_EVENT_QUEUE_MAX），队列满时会失败
 *   - wait_ack 模式会阻塞调用线程直到事件处理完成
 *   - 事件处理在独立线程中进行，不会阻塞调用者
 *
 * 相关函数:
 *   - atbmwifi_wpa_event_bh(): 事件处理线程
 *   - atbmwifi_wpa_event_process(): 事件处理函数
 *
 **************************************************************************************************************/
atbm_int32 atbmwifi_wpa_event_queue(atbm_wpa_event_data user_data1,atbm_wpa_event_data user_data2,atbm_wpa_event_data user_data3,
...
//	atbm_uint32 irq_flag = 0;
	...
	if(ret == -1){
		wifi_printk(WIFI_DBG_ERROR, "atbm_wpa_event_free_list table is empty\n");
	}
	else {
		atbm_wpa_event_bh_wakeup();
		atbm_wpa_wait_ack(wait_ack);
	}
	return ret;
}
int eloop_register_task(atbm_void *user_data1,atbm_void *user_data2)
{		
	return atbmwifi_wpa_event_queue((atbm_wpa_event_data)user_data1,(atbm_wpa_event_data)user_data2,(atbm_wpa_event_data)ATBM_NULL,WPA_EVENT__HOSTAPD_STA_HANDSHAKE,ATBM_WPA_EVENT_NOACK);
}
