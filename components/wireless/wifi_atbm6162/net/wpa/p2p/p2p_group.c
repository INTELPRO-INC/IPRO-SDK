...
{
	atbm_wpabuf_free(m->wfd_ie);
	atbm_wpabuf_free(m->p2p_ie);
	atbm_wpabuf_free(m->client_info);
	atbm_kfree(m);
}
...
void p2p_group_deinit(struct p2p_group *group)
{
	...
	atbm_wpabuf_free(group->noa);
	atbm_wpabuf_free(group->wfd_ie);
	atbm_kfree(group);
}
...
wifi_display_build_go_ie(struct p2p_group *group)
{
	...
	len = atbm_wpabuf_put(wfd_subelems, 2);
	m = group->members;
	while (m) {
	...
	if (count == 0) {
		...
		ATBM_WPA_PUT_BE16(len, (atbm_uint8 *) atbm_wpabuf_put(wfd_subelems, 0) - len -
			     2);
			    ...
			    int max_clients)
{
	...
	group_info = atbm_wpabuf_put(buf, 0);
	...
	ATBM_WPA_PUT_LE16(group_info + 1,
		     (atbm_uint8 *) atbm_wpabuf_put(buf, 0) - group_info - 3);
}
...
static struct wpabuf * p2p_group_build_probe_resp_ie(struct p2p_group *group)
{
	...
	atbm_wpabuf_free(p2p_subelems);

	if (group->p2p->vendor_elem &&
	    group->p2p->vendor_elem[VENDOR_ELEM_PROBE_RESP_P2P_GO]) {
		struct wpabuf *extra;
		extra = atbm_wpabuf_dup(group->p2p->vendor_elem[VENDOR_ELEM_PROBE_RESP_P2P_GO]);
		ie = atbm_wpabuf_concat(extra, ie);
	}

#ifdef CONFIG_WIFI_DISPLAY
	if (group->wfd_ie) {
		struct wpabuf *wfd = atbm_wpabuf_dup(group->wfd_ie);
		ie = atbm_wpabuf_concat(wfd, ie);
	}
#endif /* CONFIG_WIFI_DISPLAY */
			...
			atbm_size_t noa_len)
{
	if (noa == NULL) {
		...
		if (!group->noa) {
			group->noa = atbm_wpabuf_alloc_copy(noa, noa_len);
			if (group->noa == NULL)
				return -1;
		}
	}
