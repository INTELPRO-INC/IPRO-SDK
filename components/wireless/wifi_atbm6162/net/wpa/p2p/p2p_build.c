...
atbm_uint8 * p2p_buf_add_ie_hdr(struct wpabuf *buf)
{
	...
	len = atbm_wpabuf_put(buf, 1); /* IE length to be filled */
	wpabuf_put_be32(buf, P2P_IE_VENDOR_TYPE);
...
void p2p_buf_update_ie_hdr(struct wpabuf *buf, atbm_uint8 *len)
{
	...
	*len = (atbm_uint8 *) atbm_wpabuf_put(buf, 0) - len - 1;
}
			      ...
			      struct p2p_channels *chan)
{
	...
	len = atbm_wpabuf_put(buf, 2); /* IE length to be filled */
	...
	ATBM_WPA_PUT_LE16(len, (atbm_uint8 *) atbm_wpabuf_put(buf, 0) - len - 2);
	p2p_hexdump(MSG_DEBUG, "P2P: * Channel List",
		    len + 2, (atbm_uint8 *) atbm_wpabuf_put(buf, 0) - len - 2);
}
				  ...
				  struct p2ps_advertisement *adv_list)
{
	...
	attr_len = atbm_wpabuf_put(tmp_buf, sizeof(atbm_uint16));
	ATBM_WPA_PUT_LE16(attr_len, (atbm_uint16) total_len);
	p2p_buf_update_ie_hdr(tmp_buf, ie_len);
	pos = atbm_wpabuf_put(tmp_buf, 0);
		     ...
		     int all_attr)
{
	...
	if (wpabuf_tailroom(buf) < 6)
		return -1;
	wpabuf_put_u8(buf, ATBM_WLAN_EID_VENDOR_SPECIFIC);
	len = atbm_wpabuf_put(buf, 1);
	wpabuf_put_be32(buf, WPS_DEV_OUI_WFA);

	if (atbm_wps_build_version(buf) < 0)
		return -1;
	...
	if (all_attr) {
		...
		if (atbm_wps_build_uuid_e(buf, p2p->cfg->uuid) < 0 ||
	...
	if (atbm_wps_build_wfa_ext(buf, 0, NULL, 0) < 0)
		return -1;
