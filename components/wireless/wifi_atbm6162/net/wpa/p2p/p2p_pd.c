					       ...
					       int join)
{
	...
	if (p2p->p2ps_prov)
		extra += os_strlen(p2p->p2ps_prov->info) + 1 +
			offsetof(struct p2ps_provision, info);
				...
				const atbm_uint8 *session_mac, const atbm_uint8 *adv_mac)
{
	...
	if (!p2p->p2ps_prov) {
		p2p->p2ps_prov = (struct p2ps_provision *)atbm_kzalloc(offsetof(struct p2ps_provision, info) + 1, GFP_KERNEL);
		if (!p2p->p2ps_prov)
			return -1;
	} else {
		atbm_memset(p2p->p2ps_prov, 0, offsetof(struct p2ps_provision, info) + 1);
	}
				...
				atbm_uint8 *data, atbm_size_t len)
{
	...
	if ((status == P2P_SC_SUCCESS || status == P2P_SC_SUCCESS_DEFERRED) &&
		 ...
		 * fail. Same is ATBM_TRUE for the operating channel.
		 */
...
out:
	 ...
	 * 2. P2PS, advertiser method: DISPLAY, autoaccept: ATBM_TRUE,
