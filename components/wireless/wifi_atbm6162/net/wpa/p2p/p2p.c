...
void p2p_go_neg_failed(struct p2p_data *p2p, int status)
{
	...
	peer->oob_pw_id = 0;
	atbm_wpabuf_free(peer->go_neg_conf);
					 ...
					 atbm_size_t ies_len)
{
	...
	atbm_wpabuf_free(dev->info.vendor_elems);
	...
	for (pos = ies; end - pos > 1; pos += len) {
		...
		if (atbm_wpabuf_resize(&dev->info.vendor_elems, 2 + len) < 0)
			break;
		   ...
		   atbm_size_t ies_len, int scan_res)
{
	...
	atbm_wpabuf_free(dev->info.p2ps_instance);
	dev->info.p2ps_instance = NULL;
	if (msg.adv_service_instance && msg.adv_service_instance_len)
		dev->info.p2ps_instance = atbm_wpabuf_alloc_copy(
	...
	for (i = 0; i < P2P_MAX_WPS_VENDOR_EXT; i++) {
		atbm_wpabuf_free(dev->info.wps_vendor_ext[i]);
	...
	for (i = 0; i < P2P_MAX_WPS_VENDOR_EXT; i++) {
		if (msg.wps_vendor_ext[i] == NULL)
			break;
		dev->info.wps_vendor_ext[i] = atbm_wpabuf_alloc_copy(
	...
	if (wfd_changed) {
		atbm_wpabuf_free(dev->info.wfd_subelems);
		if (msg.wfd_subelems)
			dev->info.wfd_subelems = atbm_wpabuf_dup(msg.wfd_subelems);
		else
...
static void p2p_device_free(struct p2p_data *p2p, struct p2p_device *dev)
{
	...
	atbm_wpabuf_free(dev->info.wfd_subelems);
	atbm_wpabuf_free(dev->info.vendor_elems);
	atbm_wpabuf_free(dev->go_neg_conf);
	atbm_wpabuf_free(dev->info.p2ps_instance);
	     ...
	     atbm_uint8 seek_count, const char **seek, int freq)
{
	...
	if (req_dev_types && num_req_dev_types) {
		p2p->req_dev_types = atbm_memdup(req_dev_types,
					       num_req_dev_types *
					       WPS_DEV_TYPE_LEN);
...
int p2p_match_dev_type(struct p2p_data *p2p, struct wpabuf *wps)
{
	...
	if (atbm_wps_parse_msg(wps, &attr))
				    ...
				    const atbm_uint8 *addr, int rx_freq)
{
	...
	resp = atbm_wpabuf_put(buf, offsetof(struct atbmwifi_ieee80211_mgmt,
			...
			const char *svc_info, const atbm_uint8 *cpt_priority)
{
	...
	adv_data_len = offsetof(struct p2ps_advertisement, svc_name) + adv_len + 1 +
		info_len + 1;
...
{
#ifdef CONFIG_WIFI_DISPLAY
	atbm_wpabuf_free(p2p->wfd_ie_beacon);
	atbm_wpabuf_free(p2p->wfd_ie_probe_req);
	atbm_wpabuf_free(p2p->wfd_ie_probe_resp);
	atbm_wpabuf_free(p2p->wfd_ie_assoc_req);
	atbm_wpabuf_free(p2p->wfd_ie_invitation);
	atbm_wpabuf_free(p2p->wfd_ie_prov_disc_req);
	atbm_wpabuf_free(p2p->wfd_ie_prov_disc_resp);
	atbm_wpabuf_free(p2p->wfd_ie_go_neg);
	atbm_wpabuf_free(p2p->wfd_dev_info);
	atbm_wpabuf_free(p2p->wfd_assoc_bssid);
	atbm_wpabuf_free(p2p->wfd_coupled_sink_info);
	atbm_wpabuf_free(p2p->wfd_r2_dev_info);
#endif /* CONFIG_WIFI_DISPLAY */
...
void p2p_remove_wps_vendor_extensions(struct p2p_data *p2p)
{
	...
	for (i = 0; i < P2P_MAX_WPS_VENDOR_EXT; i++) {
		atbm_wpabuf_free(p2p->wps_vendor_ext[i]);
				 ...
				 const struct wpabuf *vendor_ext)
{
	...
	p2p->wps_vendor_ext[i] = atbm_wpabuf_dup(vendor_ext);
	if (p2p->wps_vendor_ext[i] == NULL)
		return -1;
...
int p2p_get_oper_freq(struct p2p_data *p2p, const atbm_uint8 *iface_addr)
{
	struct p2p_device *dev = p2p_get_device_interface(p2p, iface_addr);
	if (dev == NULL){
		wifi_printk(WIFI_ALWAYS,"no found dev\n");
		return -1;
	}
		
	if (dev->oper_freq <= 0){
		wifi_printk(WIFI_ALWAYS,"dev oper_freq err\n");
		return -1;
	}
...
int p2p_set_wfd_dev_info(struct p2p_data *p2p, const struct wpabuf *elem)
{
	atbm_wpabuf_free(p2p->wfd_dev_info);
	if (elem) {
		p2p->wfd_dev_info = atbm_wpabuf_dup(elem);
...
int p2p_set_wfd_r2_dev_info(struct p2p_data *p2p, const struct wpabuf *elem)
{
	atbm_wpabuf_free(p2p->wfd_r2_dev_info);
	if (elem) {
		p2p->wfd_r2_dev_info = atbm_wpabuf_dup(elem);
...
int p2p_set_wfd_assoc_bssid(struct p2p_data *p2p, const struct wpabuf *elem)
{
	atbm_wpabuf_free(p2p->wfd_assoc_bssid);
	if (elem) {
		p2p->wfd_assoc_bssid = atbm_wpabuf_dup(elem);
				  ...
				  const struct wpabuf *elem)
{
	atbm_wpabuf_free(p2p->wfd_coupled_sink_info);
	if (elem) {
		p2p->wfd_coupled_sink_info = atbm_wpabuf_dup(elem);
