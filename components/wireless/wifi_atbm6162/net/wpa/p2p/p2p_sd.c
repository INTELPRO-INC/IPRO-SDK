		     ...
		     atbm_uint8 dialog_token, const struct wpabuf *resp_tlvs)
{
	...
	if (wpabuf_len(resp_tlvs) > max_len) {
		...
		p2p->sd_resp = atbm_wpabuf_dup(resp_tlvs);
		if (p2p->sd_resp == NULL) {
			     ...
			     const atbm_uint8 *data, atbm_size_t len, int rx_freq)
{
	...
	if (comeback_delay) {
		...
		if (p2p->sd_rx_resp) {
			...
			atbm_wpabuf_free(p2p->sd_rx_resp);
			p2p->sd_rx_resp = NULL;
		}
...
skip_nqp_header:
	if (atbm_wpabuf_resize(&p2p->sd_rx_resp, end - pos) < 0)
		return;
