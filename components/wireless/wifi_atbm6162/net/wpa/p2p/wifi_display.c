...
void wifi_display_deinit(struct atbmwifi_vif *priv)
{
	...
	for (i = 0; i < MAX_WFD_SUBELEMS; i++) {
		atbm_wpabuf_free(priv->wfd_subelem[i]);
...
int wifi_display_subelem_set(struct atbmwifi_vif *priv, int subelem, char *val)
{
	...
	if (len == 0) {
		...
		if (atbmwifi_hexstr2bin(atbm_wpabuf_put(e, len), val, len) < 0) {
			atbm_wpabuf_free(e);
			return -1;
		}
	...
	atbm_wpabuf_free(priv->wfd_subelem[subelem]);
				      ...
				      struct wpabuf *ie)
{
	...
	while (end > pos) {
		...
		if (subelem < MAX_WFD_SUBELEMS && subelements[subelem] == 0) {
			e = atbm_wpabuf_alloc_copy(pos, len);
			if (e == NULL)
				return -1;
