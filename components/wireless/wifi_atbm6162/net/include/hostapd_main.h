...
#define WPA_SEND_EAPOL_TIMEOUT 0 //second
#define WPA_SEND_EAPOL_TIMEOUT_MS 500 //ms
...
struct hostapd_data {
	...
	struct hostapd_sta_info *sta_list[ATBMWIFI__MAX_STA_IN_AP_MODE+1]; /* STA info list head */
	struct hostapd_sta_info sta_table[ATBMWIFI__MAX_STA_IN_AP_MODE+1]; /* STA info list head */
...
#define hostapd_send_eapol(priv,da,proto,buf,len,noenc)  wpa_drv_send_eapol(priv,da,proto,buf,len,noenc)
