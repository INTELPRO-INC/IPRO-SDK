...
static int sme_check_sa_query_timeout(struct wpa_supplicant *wpa_s)
{
	...
	if (1000 < passed) {
		...
		atbm_sta_deauth(wpa_s->priv, ATBM_FALSE);
		return 1;
	}
...
static void sme_sa_query_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	nbuf = (atbm_uint8 *)atbm_realloc_array(wpa_s->sa_query_trans_id,
...
{	
	...
	case ATBM_WLAN_CATEGORY_BACK:
		{
			...
			if(priv->hw_priv->driver_setup_ba){
				...
				wifi_printk(WIFI_BASETUP, "RX BA action %d sta_priv %x\n", mgmt->u.action.u.addba_req.action_code,sta_priv);
	...
	case ATBM_WLAN_CATEGORY_SPECTRUM_MGMT:
		...
		if(ATBM_WLAN_ACTION_SPCT_CHL_SWITCH == mgmt->u.action.u.chan_switch.action_code)
		{
			...
			atbm_ieee80211_sta_process_chanswitch(priv,&priv->bss,&sw_packed_ie,atbm_GetOsTime());
		}
		break;
	...
	if(priv->scan.ApScan_in_process){
		if((priv->scan_ret.info)
			...
			if(atbmwifi_rx_scan_is_valid(priv,ie)){					
				...
				info->channel = pelems->ds_params ? pelems->ds_params[0] : atbmwifi_ieee80211_frequency_to_channel(hwhdr->freq);
				if(pelems->tim)
...
struct ie_list{
	struct ie_list *next, *prev;
	atbm_uint8 *ie;
	int ielen;
	atbm_uint16 capability;
	atbm_uint8 max_bssid_indicator;
	atbm_uint8 bssid_index;
	atbm_uint8 nontransmit;
	atbm_uint8 bssid[ATBM_ETH_ALEN];
};

#if CONFIG_HE
 static const struct element
 *atbmwifi_get_profile_continuation(const atbm_uint8 *ie, atbm_size_t ielen,
					const struct element *mbssid_elem,
					const struct element *sub_elem)
 {
	 const atbm_uint8 *mbssid_end = mbssid_elem->data + mbssid_elem->datalen;
	 const struct element *next_mbssid;
	 const struct element *next_sub;
 
	 next_mbssid = (const struct element *)atbmwifi_find_ie(ATBM_WLAN_EID_MULTIPLE_BSSID,
					  mbssid_end,
					  ielen - (mbssid_end - ie));
	 ...
	 */
	  ...
	  * of a new profile
	  */
	 return next_sub->data[0] == ATBM_WLAN_EID_NONTRANSMITTED_BSSID_CAPA ?
			NULL : next_mbssid;
 }
 
 atbm_size_t atbmwifi_merge_profile(const atbm_uint8 *ie, atbm_size_t ielen,
				   const struct element *mbssid_elem,
				   const struct element *sub_elem,
				   atbm_uint8 *merged_ie, atbm_size_t max_copy_len)
 {
	 atbm_size_t copied_len = sub_elem->datalen;
	 const struct element *next_mbssid;
 
	 if (sub_elem->datalen > max_copy_len)
		 return 0;
 
	 atbm_memcpy(merged_ie, sub_elem->data, sub_elem->datalen);
 
	 while ((next_mbssid = atbmwifi_get_profile_continuation(ie, ielen,
								 mbssid_elem,
								 sub_elem))) {
		 ...
		 if (copied_len + next_sub->datalen > max_copy_len)
			 break;
		 atbm_memcpy(merged_ie + copied_len, next_sub->data,
				next_sub->datalen);
...
static atbm_size_t atbmwifi_gen_new_ie(const atbm_uint8 *ie, atbm_size_t ielen,
			   const atbm_uint8 *subelement, atbm_size_t subie_len,
			   atbm_uint8 *new_ie, atbm_uint32 gfp)
{
 atbm_uint8 *pos, *tmp;
 const atbm_uint8 *tmp_old, *tmp_new;
  ...
  */
 sub_copy = (atbm_uint8 *)atbm_memdup(subelement, subie_len);
 if (!sub_copy)
	 return 0;
 ...
 /* set new ssid */
 tmp_new = atbmwifi_find_ie(ATBM_WLAN_EID_SSID, sub_copy, subie_len);
 if (tmp_new) {
	 atbm_memcpy(pos, tmp_new, tmp_new[1] + 2);
	 pos += (tmp_new[1] + 2);
 }

 /* get non inheritance list if exists */
 non_inherit_elem =
	 (const struct element *)atbmwifi_find_ext_elem(ATBM_WLAN_EID_EXT_NON_INHERITANCE,
					sub_copy, subie_len);

 /* go through IEs in ie (skip SSID) and subelement,
  * merge them into new_ie
  */
 tmp_old = atbmwifi_find_ie(ATBM_WLAN_EID_SSID, ie, ielen);
 ...
 while (tmp_old + tmp_old[1] + 2 - ie <= ielen) {
	 ...
	 if (tmp_old[0] == ATBM_WLAN_EID_EXTENSION)
		 tmp = (atbm_uint8 *)atbmwifi_find_ext_ie(tmp_old[2], sub_copy,
						  subie_len);
	 else
		 tmp = (atbm_uint8 *)atbmwifi_find_ie(tmp_old[0], sub_copy,
						  subie_len);
	 ...
	 if (!tmp) {
		 ...
		 if (atbmwifi_is_element_inherited(old_elem,
						   non_inherit_elem)) {
			 atbm_memcpy(pos, tmp_old, tmp_old[1] + 2);
			 pos += tmp_old[1] + 2;
		 }
	 } else {
		 ...
		 if (tmp_old[0] == ATBM_WLAN_EID_VENDOR_SPECIFIC) {
			 if (!atbm_memcmp(tmp_old + 2, tmp + 2, 5)) {
				 /* same vendor ie, copy from
				  * subelement
				  */
				 atbm_memcpy(pos, tmp, tmp[1] + 2);
				 pos += tmp[1] + 2;
 ...
 while (tmp_new + tmp_new[1] + 2 - sub_copy <= subie_len) {
	 if (!(tmp_new[0] == ATBM_WLAN_EID_NONTRANSMITTED_BSSID_CAPA ||
 ...
 atbm_void atbmwifi_rx_probe_resp(struct atbmwifi_vif *priv,struct atbm_buff *skb)
{
...
#endif
	atbm_uint8 *ie;
	struct ie_list *ielist, *ielisthead = ATBM_NULL;
#if CONFIG_HE
	atbm_uint8 *mbssid_ie, *mbssid_index_ie, *profile, *new_ie;
	...
	ielisthead = (struct ie_list *)atbm_kzalloc(sizeof(struct ie_list), GFP_KERNEL);
	ielisthead->ie = ie;
	ielisthead->ielen = len;
#if CONFIG_HE
	if(priv->hw_priv->chip_version != OCEANUS_NO_WIFI6){
		ielisthead->prev = ielisthead;
		mbssid_ie = atbmwifi_find_ie(ATBM_WLAN_EID_MULTIPLE_BSSID,data,len);
		if(mbssid_ie){
			profile = atbm_kmalloc(len, GFP_KERNEL);
			for_each_element(sub, mbssid_ie + 3, mbssid_ie[1]-1){
				atbm_uint8 profile_len;
				struct ie_list *ielist_tmp = (struct ie_list *)atbm_kzalloc(sizeof(struct ie_list), GFP_KERNEL);
				
				if (sub->id != 0 || sub->datalen < 4) {
					/* not a valid BSS profile */
					continue;
				}
				if (sub->data[0] != ATBM_WLAN_EID_NONTRANSMITTED_BSSID_CAPA ||
					sub->data[1] != 2) {
				...
				atbm_memset(profile, 0, len);
				profile_len = atbmwifi_merge_profile(ie, len,
								     (const struct element *)mbssid_ie,
								     sub,
								     profile,
								     len);

				/* found a Nontransmitted BSSID Profile */
				mbssid_index_ie = atbmwifi_find_ie
					(ATBM_WLAN_EID_MULTIPLE_BSSID_INDEX,
					 profile, profile_len);
				...
				ielist_tmp->bssid_index = mbssid_index_ie[2];
				ielist_tmp->max_bssid_indicator = mbssid_ie[2];

				ielist_tmp->ie = atbm_kmalloc(ATBM_IEEE80211_MAX_DATA_LEN, GFP_KERNEL);

				atbmwifi_gen_new_bssid(mgmt->bssid,
										   ielist_tmp->max_bssid_indicator,
										   ielist_tmp->bssid_index,
										   ielist_tmp->bssid);

				ielist_tmp->ielen = atbmwifi_gen_new_ie(ie, len,
								 profile,
								 profile_len, ielist_tmp->ie,
								 GFP_KERNEL);
				if (!ielist_tmp->ielen){
					atbm_kfree(ielist_tmp->ie);
					continue;
				}
				//wifi_printk(WIFI_ALWAYS, "add multibss[%d] [%d]to list!!\n", ielist_tmp->ie[1], ielist_tmp->ielen);
				ielist_tmp->capability = atbm_le16_to_cpu(*((atbm_uint16*)(profile + 2)));
				ielist_tmp->nontransmit = 1;
				ielisthead->prev->next = ielist_tmp;
				ielisthead->prev = ielist_tmp;
			}
			atbm_kfree(profile);
		}
	}
	...
	do{
		 ...
		 if(priv->scan.in_progress||priv->scan.ApScan_in_process){
#if CONFIG_P2P
			if(priv->p2p_scan){
				atbm_p2p_prash_ie(priv, mgmt->bssid, hwhdr->freq, hwhdr->signal, ielist->ie, len);
			}else
#endif
#if CONFIG_WPS
			if(wpa_s->wps_mode != WPS_MODE_UNKNOWN){
				...
				atbm_ieee802_11_parse_elems(ielist->ie, ielist->ielen, pelems);
				...
				if(check_valid_wps_ap(priv, attr, wpa_s->own_addr)){
					if( pelems->ds_params!=ATBM_NULL){
						...
						priv->bss.channel_num = atbmwifi_ieee80211_frequency_to_channel((int)hwhdr->freq);
					}
#if CONFIG_5G_SUPPORT
					if(priv->bss.channel_num >= 36){
						ATBM_WARN_ON(!atbmwifi_5g_supported(priv->hw_priv->chip_priv));
					...
					if(pelems->ht_cap_elem){
						atbmwifi_ieee80211_ht_cap_ie_to_sta_ht_cap(priv->hw_priv->bands[priv->bss.sta_priv.band], pelems->ht_cap_elem, &priv->bss.sta_priv.rate.ht_cap);
					}
					...
					atbmwifi_ieee80211_get_sta_rateinfo(&priv->bss.sta_priv.rate,
							priv->hw_priv->bands[priv->bss.sta_priv.band],
			...
			if((priv->scan_ret.info)
				&&(priv->scan_no_connect)
				&&(priv->scan_ret.len < MAX_SCAN_INFO_NUM)){
				if(atbmwifi_rx_scan_is_valid(priv,ielist->ie)){					
					struct atbmwifi_scan_result_info *info = priv->scan_ret.info + priv->scan_ret.len;
					atbm_ieee802_11_parse_elems(ielist->ie,ielist->ielen,pelems);
					atbm_memcpy(info->ssid ,pelems->ssid,pelems->ssid_len);
#if CONFIG_HE
				if(priv->hw_priv->chip_version == OCEANUS_NO_WIFI6){
					...
					if(ielist->nontransmit){
						atbm_memcpy(info->BSSID,ielist->bssid,6);
					}else
#endif
					{
						atbm_memcpy(info->BSSID,mgmt->bssid,6);
			...
			if(!priv->scan_no_connect
				...
				&&(ielist->ie[1] == priv->config.ssid_len)
				&&(atbm_memcmp(&ielist->ie[2],priv->config.ssid,priv->config.ssid_len)==0)){	
				ret = 0;
				...
				if (pelems->ht_info_elem){
					if(!(priv->bss.sta_priv.rate.ht_cap.cap & ATBM_IEEE80211_HT_CAP_40MHZ_INTOLERANT)
								&& priv->bss.sta_priv.rate.ht_cap.cap & ATBM_IEEE80211_HT_CAP_SUP_WIDTH_20_40){
						switch(pelems->ht_info_elem->ht_param & ATBM_IEEE80211_HT_PARAM_CHA_SEC_OFFSET) {
...
#if CONFIG_VHT
				if(atbmwifi_vht_supported(priv->hw_priv->chip_priv)){
					if (pelems->vht_cap_elem){
						atbmwifi_ieee80211_vht_cap_ie_to_sta_vht_cap(priv->hw_priv->bands[priv->bss.sta_priv.band],
							pelems->vht_cap_elem, &priv->bss.sta_priv.rate.vht_cap);
...
#if CONFIG_HE
			if(priv->hw_priv->chip_version == OCEANUS_NO_WIFI6){
				priv->bss.sta_priv.he_used = ATBM_FALSE;
			}else
				if (pelems->he_operation && pelems->he_cap) {
					priv->bss.is_wifi6_ap = ATBM_TRUE;
					atbmwifi_ieee80211_he_cap_ie_to_sta_he_cap(priv->hw_priv->bands[priv->bss.sta_priv.band],
						pelems->he_cap,
						pelems->he_cap_len,
						&priv->bss.sta_priv.rate.he_cap);
					priv->bss.sta_priv.he_used = priv->bss.sta_priv.rate.he_cap.has_he;
					priv->bss.sta_priv.rate.he_cap.he_operation.he_oper_params = pelems->he_operation->he_oper_params;
#ifdef WLAN_RSNX_CAPA_PROTECTED_TWT
					if (elems.rsnx && elems.rsnx_len &&
						(elems.rsnx[0] & WLAN_RSNX_CAPA_PROTECTED_TWT) &&
						wiphy_ext_feature_isset(local->hw.wiphy,
							NL80211_EXT_FEATURE_PROTECTED_TWT))
						bss_conf->twt_protected = ATBM_TRUE;
					else
						bss_conf->twt_protected = ATBM_FALSE;
					...
					priv->bss.sta_priv.he_used = ATBM_FALSE;
#ifdef WLAN_RSNX_CAPA_PROTECTED_TWT
					bss_conf->twt_requester = ATBM_FALSE;
					bss_conf->twt_protected = ATBM_FALSE;
#endif // WLAN_RSNX_CAPA_PROTECTED_TWT
				}
#ifdef WLAN_RSNX_CAPA_PROTECTED_TWT		
				...
				if (priv->bss.sta_priv.he_used) {
					priv->bss.he_bss_color.color =
						atbm_le32_get_bits(pelems->he_operation->he_oper_params,
							ATBM_IEEE80211_HE_OPERATION_BSS_COLOR_MASK);
					priv->bss.he_bss_color.partial =
						atbm_le32_get_bits(pelems->he_operation->he_oper_params,
							ATBM_IEEE80211_HE_OPERATION_PARTIAL_BSS_COLOR);
					priv->bss.he_bss_color.enabled =
						!atbm_le32_get_bits(pelems->he_operation->he_oper_params,
							ATBM_IEEE80211_HE_OPERATION_BSS_COLOR_DISABLED);
			
					priv->bss.htc_trig_based_pkt_ext =
						atbm_le32_get_bits(pelems->he_operation->he_oper_params,
							ATBM_IEEE80211_HE_OPERATION_DFLT_PE_DURATION_MASK);
					priv->bss.frame_time_rts_th =
						atbm_le32_get_bits(pelems->he_operation->he_oper_params,
							ATBM_IEEE80211_HE_OPERATION_RTS_THRESHOLD_MASK);
			
					priv->bss.uora_exists = !!pelems->uora_element;
					if (pelems->uora_element)
						priv->bss.uora_ocw_range = pelems->uora_element[0];
			
					ieee80211_he_op_ie_to_bss_conf(priv, pelems->he_operation);
					ieee80211_he_spr_ie_to_bss_conf(priv, pelems->he_spr);
...
#if CONFIG_HE
			if(priv->hw_priv->chip_version == OCEANUS_NO_WIFI6){
				atbm_memcpy(priv->bssid,mgmt->bssid,ATBM_ETH_ALEN);
				atbm_memcpy(priv->bss.bssid, mgmt->bssid,ATBM_ETH_ALEN);
				atbm_memcpy(priv->daddr,mgmt->bssid,ATBM_ETH_ALEN);
			}else
				if(ielist->nontransmit){
					priv->bss.nontransmit = 1;
					priv->bss.bssid_index = ielist->bssid_index;
					priv->bss.max_bssid_indicator = ielist->max_bssid_indicator;
					atbm_memcpy(priv->bss.transbssid, mgmt->bssid, ATBM_ETH_ALEN);
					atbm_memcpy(priv->bssid,ielist->bssid,ATBM_ETH_ALEN);
					atbm_memcpy(priv->bss.bssid, ielist->bssid,ATBM_ETH_ALEN);
					atbm_memcpy(priv->daddr,ielist->bssid,ATBM_ETH_ALEN);
					wifi_printk(WIFI_DBG_ERROR, "nontransmit:%d "MACSTR"\n", ielist->nontransmit,
																	MAC2STR(ielist->bssid));
				}else
#endif
				{
					atbm_memcpy(priv->bssid,mgmt->bssid,ATBM_ETH_ALEN);
...
#if CONFIG_ATBM_SUPPORT_CSA
atbm_void atbmwifi_switch_channletpye(struct atbmwifi_vif *priv,struct atbmwifi_ieee802_11_elems *elems, atbm_uint64 timestamp)
{
	struct atbmwifi_ieee80211_channel_sw_packed_ie sw_packed_ie;

	sw_packed_ie.chan_sw_ie = NULL;
...
//	struct atbmwifi_common *hw_priv = _atbmwifi_vifpriv_to_hwpriv(priv);
	...
	baselen = (atbm_uint8 *) mgmt->u.beacon.variable - (atbm_uint8 *) mgmt;
	if(priv->assoc_ok == 0 || atbm_memcmp(mgmt->sa, priv->bssid, ATBM_ETH_ALEN)){
		return;
	}else{
		atbm_ieee802_11_parse_elems(mgmt->u.beacon.variable, len - baselen, &elems);
		/*parse 40M ies*/
#if CONFIG_ATBM_SUPPORT_CSA
		atbmwifi_switch_channletpye(priv,&elems, atbm_GetOsTime());
#endif
		/*parse wmm param*/
		if (elems.wmm_param){
			atbmwifi_ieee80211_sta_wmm_params(priv,elems.wmm_param,elems.wmm_param_len, elems.mu_edca_param_set);
		}
		/*parse ps mode */
		//
		...
		if (elems.ht_cap_elem && elems.ht_info_elem && elems.wmm_param &&
			...
			changed |= atbmwifi_ieee80211_enable_ht(sband, elems.ht_info_elem,
...
#if (SUPPORT_LIGHT_SLEEP) || (CONFIG_ATBM_SUPPORT_CSA)
		...
		case ATBM_IEEE80211_STYPE_DISASSOC:
			if(priv->assoc_ok || priv->join_status == ATBMWIFI__JOIN_STATUS_STA){
				...
				if(priv->auth_retry && !priv->assoc_ok){
					priv->auth_retry = 0;
					atbmwifi_wpa_event_queue((atbm_wpa_event_data)priv,(atbm_wpa_event_data)ATBM_NULL,(atbm_wpa_event_data)ATBM_NULL,WPA_EVENT__SUPPLICANT_AUTHEN,ATBM_WPA_EVENT_NOACK);
					break;
				}
#endif
				atbm_sta_deauth(priv, ATBM_FALSE);
			...
			const atbm_uint8 *sa)
{
	...
	if (auth_transaction == 1 &&
		...
		atbm_wpabuf_free(wpa_s->sae_token);
		wpa_s->sae_token = atbm_wpabuf_alloc_copy(data + sizeof(atbm_uint16),
 ...
 // 				 wpa_s->current_ssid, 0);
		wpa_prepare_auth_direct(wpa_s->priv);
		return 0;
	 } else if (auth_transaction == 2) {
		wpa_printf(MSG_DEBUG, "SME SAE confirm %d\n", wpa_s->sae.state);
...
//	atbm_uint16 resp = ATBM_WLAN_STATUS_SUCCESS;
	...
	else if((auth_alg == ATBM_WLAN_AUTH_SHARED_KEY))
	{
		...
		atbm_wpa_sm_set_pmk(wpa_s->wpa, wpa_s->sae.pmk, ATBM_PMK_LEN,
			     ...
			     const atbm_uint8 *ie, atbm_size_t ie_len,atbm_uint8 channel)
{
...
#endif
	int special_ie_len = priv->special_extra_ie_len;
	atbm_uint8 *special_ie = priv->special_extra_ie;
	pos = buffer;
	...
	*pos++ = 0;
	...
	if(atbmwifi_vht_supported(hw_priv->chip_priv)){
		pos = atbmwifi_ieee80211_ie_build_vht_cap(&sband->vht_cap, pos,
				(sband->vht_cap.cap >>ATBM_IEEE80211_VHT_CAP_BEAMFORMEE_STS_SHIFT) & 7);
	}
#endif

#if CONFIG_HE
	if(priv->hw_priv->chip_version != OCEANUS_NO_WIFI6){
		pos = atbmwifi_ieee80211_ie_build_he_cap(&sband->iftype_data.he_cap, pos);
	}
#endif
	...
	if ((special_ie_len > 0) && (special_ie_len)) {
		atbm_memcpy(pos,special_ie, special_ie_len);
		pos += special_ie_len;
	}
	return pos - buffer;
}
...
struct atbm_buff * atbmwifi_ieee80211_send_assoc_req(struct atbmwifi_vif *priv)
{
...
#if CONFIG_VHT
				2 + sizeof(struct atbm_ieee80211_vht_cap) + /* VHT */
				3 + /* Operation mode notification */
#endif
#if CONFIG_HE
	...
	*pos++ = priv->ssid_length;
	...
	band = priv->hw_priv->bands[priv->bss.sta_priv.band];
	if(priv->bss.sta_priv.ht){
		pos = atbmwifi_ieee80211_add_ht_ie(priv,band, pos);
	}
#if CONFIG_VHT
	if(atbmwifi_vht_supported(priv->hw_priv->chip_priv)){
		if(priv->bss.sta_priv.vht){
			if(priv->bss.information_elements && priv->bss.len_information_elements){
				struct atbm_ieee80211_vht_cap *ap_vht_cap =  (struct atbm_ieee80211_vht_cap *)atbmwifi_find_ie(ATBM_WLAN_EID_VHT_CAP,
					priv->bss.information_elements, priv->bss.len_information_elements);
				if(ap_vht_cap){
					atbm_uint8 band_width = 0;
					if(priv->bss.channel_type >= ATBM_NL80211_CHAN_HT40MINUS)
						band_width = 1;
					pos = ieee80211_add_vht_ie(band, ap_vht_cap, pos);
					pos = ieee80211_add_op_mode_ie(band, band_width, pos);
				}
			}
		}
	}
	...
	if(priv->hw_priv->chip_version != OCEANUS_NO_WIFI6){
		if (priv->bss.sta_priv.he_used){
			wifi_printk(WIFI_ALWAYS, "assoc.use_he ++\n");
			pos = atbmwifi_ieee80211_ie_build_he_cap(&band->iftype_data.he_cap, pos);
		}
		if(priv->bss.nontransmit){
			pos = atbmwifi_ieee80211_ie_build_ext_capab(pos, wpa_s->extended_capa, wpa_s->extended_capa_len);
		}
	}
#endif
...
{
	struct atbmwifi_common *hw_priv = _atbmwifi_vifpriv_to_hwpriv(priv);
	struct hostapd_sta_info *sta = atbm_ap_get_sta((struct hostapd_data *)priv->appdata, da);
				...
				const atbm_uint8 *bssid, int update)
{
	...
	atbm_wpabuf_free(hapd->sae_data);
						...
						int group, const atbm_uint8 *addr)
{
	...
	if (!(hapd->last_sae_token_key_update) ||
		...
		if (random_get_bytes(hapd->sae_token_key,
			...
			return NULL;
		atbm_wpa_hexdump(MSG_DEBUG, "SAE: Updated token key",
	...
	token = atbm_wpabuf_put(buf, SHA256_MAC_LEN);
	addrs[0] = addr;
	len[0] = ATBM_ETH_ALEN;
	addrs[1] = idx;
	len[1] = sizeof(idx);
...
extern int atbm_wpa_auth_pmksa_add_sae(struct hostapd_data *hapd, const atbm_uint8 *addr,			   const atbm_uint8 *pmk, const atbm_uint8 *pmkid);
	 atbm_wpa_auth_pmksa_add_sae(hapd, sta->addr,
 ...
 reply:
	 if (!sta_removed && resp != ATBM_WLAN_STATUS_SUCCESS) {
		...
		if (resp == ATBM_WLAN_STATUS_FINITE_CYCLIC_GROUP_NOT_SUPPORTED &&
			...
			hapd->sae_data = atbm_wpabuf_alloc_copy(pos, 2);
	
 		wifi_printk(WIFI_DBG_ERROR, "SAE reply auth_trans:%d resp:%d \n", auth_transaction, resp);
...
remove_sta:
	if (!sta_removed &&
		...
		atbmwifi_sta_del(hapd->priv, sta->addr, ATBM_TRUE);
	}
...
atbm_void atbmwifi_rx_ap_auth(struct atbmwifi_vif *priv,struct atbm_buff *skb, int linkid)
{
...
#endif /* CONFIG_SAE */
	//if one sta is connecting, not allow new one
	if((priv->connect_timer_linkid!=0) && (linkid !=priv->connect_timer_linkid )){
	...
	if(auth_alg == ATBM_WLAN_AUTH_SAE){
		sta = atbm_ap_get_sta(hapd, mgmt->sa);
		if(sta == ATBM_NULL)
		{
			sta = atbm_ap_sta_add(hapd, mgmt->sa, linkid);
			if(sta == ATBM_NULL)
			{	 
 ...
 int atbmwifi_rx_assoc_req(struct atbmwifi_vif *priv, struct atbm_buff *skb, struct atbmwifi_ieee80211_tx_info * tx_info)
{
	...
	atbm_memset(&sta_priv->rate, 0, sizeof(struct atbmwifi_cfg80211_rate));
	...
	{
		wpaBuff = atbm_wps_build_assoc_resp_ie();
		if(!wpaBuff){
	...
	if(atbmwifi_vht_supported(priv->hw_priv->chip_priv)){
		if(elems.vht_cap_elem){
			atbmwifi_ieee80211_vht_cap_ie_to_sta_vht_cap(priv->hw_priv->bands[priv->bss.sta_priv.band],
				elems.vht_cap_elem, &sta_priv->rate.vht_cap);
			sta_priv->vht = 1;
		}
	}
#endif
	...
	if(priv->hw_priv->chip_version != OCEANUS_NO_WIFI6){
		if (elems.he_cap) {
			atbmwifi_ieee80211_he_cap_ie_to_sta_he_cap(priv->hw_priv->bands[priv->bss.sta_priv.band],
				elems.he_cap,
				elems.he_cap_len,
				&sta_priv->rate.he_cap);
			sta_priv->he_used = sta_priv->rate.he_cap.has_he;
			//sta_priv->rate.he_cap.he_operation.he_oper_params = elems.he_operation->he_oper_params;
		}
	}
#endif

	wifi_printk(WIFI_WPA,"atbm_is_40m [%x],sta_supp_40m [%x],sta_chtype[%x]\n",atbm_is_40m,sta_supp_40m,sta_priv->rate.channel_type);
	atbmwifi_ieee80211_get_sta_rateinfo(&sta_priv->rate,
			priv->hw_priv->bands[priv->bss.sta_priv.band],
...
fail:
	{
		...
		if(wps_ie && wps_ie_len){
			...
			atbm_wpabuf_free(wpaBuff);
		}else
#endif
		{
			skb = atbmwifi_ieee80211_send_assoc_resp(priv,resp,type == ATBM_IEEE80211_STYPE_REASSOC_REQ ? 1:0,ATBM_NULL,0,tx_info->link_id);
		}
		atbmwifi_tx(priv->hw_priv,skb,priv);
     ...
     switch(stype){
		...
		case ATBM_IEEE80211_STYPE_AUTH:	
		   {	
				...
				if(linkid < 0){
					wifi_printk(WIFI_DBG_ERROR,"[ap]: linkid is err \n");
					break;
				}

				if(priv->whitelist_enable) {
					for (i = 0; i < ATBMWIFI__MAX_STA_IN_AP_MODE; i++){
						if((priv->whitelist_enable & (1<<i)) && atbm_memcmp(&priv->whitelist[i], hdr->addr2, 6) == 0)
							break;
					}
					if(i == ATBMWIFI__MAX_STA_IN_AP_MODE) {
						wifi_printk(WIFI_WPA,"[ap]: sta not in whitelist \n");
						for (i = 0; i < ATBMWIFI__MAX_STA_IN_AP_MODE; i++){
							if(priv->whitelist_enable & (1<<i))
								wifi_printk(WIFI_WPA,"[ap]: whitelist[%d] = %02x%02x%02x%02x%02x%02x\n",i,priv->whitelist[i][0],
									priv->whitelist[i][1],priv->whitelist[i][2],priv->whitelist[i][3],priv->whitelist[i][4],priv->whitelist[i][5]);
						}
						break;
					}
				}
				atbmwifi_rx_ap_auth(priv,skb,linkid);
		...
		case ATBM_IEEE80211_STYPE_DISASSOC:
			...
			if(priv->link_id_db[tx_info->link_id-1].status != ATBMWIFI__LINK_SOFT) {
				...
				atbmwifi_wpa_event_queue((atbm_wpa_event_data)priv,(atbm_wpa_event_data)tx_info->link_id,(atbm_wpa_event_data)ATBM_NULL,WPA_EVENT__SUPPLICANT_DEAUTHEN,ATBM_WPA_EVENT_NOACK);
				//atbmwifi_ap_deauth(priv,atbmwifi_ieee80211_get_SA(hdr), ATBM_FALSE);
			}
			break;
		default:
			break;
			    ...
			    atbm_size_t ies_len,atbm_uint16 aid)
{
	...
	atbm_uint16 capab=0;
	struct atbmwifi_cfg *config = atbmwifi_get_config(priv);
	struct atbmwifi_sta_priv *sta_priv = atbmwifi_sta_find_form_linkid(priv, aid);
	...
	send_len = offsetof(struct atbmwifi_ieee80211_mgmt, u.assoc_resp.variable);
	...
	reply->u.assoc_resp.aid = atbm_cpu_to_le16(aid/*aid=1*/   | ATBM_BIT(14) | ATBM_BIT(15));
	/* Supported rates */
	...
	p = atbmwifi_ieee80211_add_ht_ie(priv,priv->hw_priv->bands[sta_priv->band],p);
	...
	if(atbmwifi_vht_supported(priv->hw_priv->chip_priv)){
		if(sta_priv->vht){
			int nsts = 0, sta_nsts;
			struct atbm_ieee80211_sta_vht_cap *vht_cap = &priv->hw_priv->bands[sta_priv->band]->vht_cap;
			
			nsts = (vht_cap->cap >>
				ATBM_IEEE80211_VHT_CAP_BEAMFORMEE_STS_SHIFT) & 7;
			sta_nsts = (sta_priv->rate.vht_cap.cap >>
				ATBM_IEEE80211_VHT_CAP_BEAMFORMEE_STS_SHIFT) & 7;
			...
			p = atbmwifi_ieee80211_ie_build_vht_cap(vht_cap, p, nsts);
			p = atbmwifi_ieee80211_ie_build_vht_oper(priv, p);
		}
	}
#endif
#if CONFIG_HE
	if(priv->hw_priv->chip_version != OCEANUS_NO_WIFI6){
		if(sta_priv->he_used){
			p = atbmwifi_ieee80211_ie_build_he_cap(&priv->hw_priv->bands[sta_priv->band]->iftype_data.he_cap, p);
			p = ieee80211_ie_build_he_oper(p);
		}
	}
#endif
...
{	
	...
	send_len = offsetof(struct atbmwifi_ieee80211_mgmt, u.probe_resp.variable);
	...
	*p++ = priv->ssid_length;
	...
	/* Supported rates */	
	/* Extended supported rates */
	if(priv->config.disable_ofdm){
		p = atbmwifi_ieee80211_add_rate_ie(p ,(config->band==WSM_PHY_BAND_5G),0xf);
	}else{
		p = atbmwifi_ieee80211_add_rate_ie(p ,(config->band==WSM_PHY_BAND_5G),~0);
	}
	...
	*p++ = priv->config.channel_index;
	...
	band = priv->hw_priv->bands[priv->config.band];
#if CONFIG_IEEE80211N
	if(!priv->config.disable_ht){
		p = atbmwifi_ieee80211_add_ht_ie(priv,band,p);
		p = atbmwifi_ieee80211_add_ht_operation(priv,p);
	}
#endif /* CONFIG_IEEE80211N */

#if CONFIG_VHT
	if(!priv->config.disable_vht && atbmwifi_vht_supported(priv->hw_priv->chip_priv)){
		p = atbmwifi_ieee80211_ie_build_vht_cap(&band->vht_cap, p, 
			(band->vht_cap.cap >>ATBM_IEEE80211_VHT_CAP_BEAMFORMEE_STS_SHIFT) & 7);
		p = atbmwifi_ieee80211_ie_build_vht_oper(priv, p);
	}
#endif
	...
	if(priv->hw_priv->chip_version != OCEANUS_NO_WIFI6){
		...
		if(!priv->config.disable_he){
			p = atbmwifi_ieee80211_ie_build_he_cap(&band->iftype_data.he_cap, p);
			p = ieee80211_ie_build_he_oper(p);
		}
	}
#endif
	...
	if (special_ie && (special_ie_len > 0)) {
		wifi_printk(WIFI_DBG_MSG, "set special_probe resp_ie \n");
		atbm_memcpy(p,special_ie, special_ie_len);
		p += (special_ie_len);
	}
#if CONFIG_WPS
 ...
 struct atbm_buff *atbmwifi_ieee80211_send_beacon(struct atbmwifi_vif  *priv,const atbm_uint8 *ies, atbm_size_t ies_len)
{
	...
	send_len = offsetof(struct atbmwifi_ieee80211_mgmt, u.beacon.variable);
	...
	*p++ =ATBM_BIT(2);
	...
	band = priv->hw_priv->bands[config->band];
#if CONFIG_IEEE80211N
	if(!config->disable_ht){
		p = atbmwifi_ieee80211_add_ht_ie(priv,band, p);
		p = atbmwifi_ieee80211_add_ht_operation(priv,p);
	}
#endif /* CONFIG_IEEE80211N */
...
#if CONFIG_VHT
	if(!config->disable_vht && 	atbmwifi_vht_supported(priv->hw_priv->chip_priv)){
		p = atbmwifi_ieee80211_ie_build_vht_cap(&band->vht_cap, p, 
			(band->vht_cap.cap >>ATBM_IEEE80211_VHT_CAP_BEAMFORMEE_STS_SHIFT) & 7);
		p = atbmwifi_ieee80211_ie_build_vht_oper(priv, p);
	}
...
#if CONFIG_HE
	if(priv->hw_priv->chip_version != OCEANUS_NO_WIFI6){
		if(!config->disable_he){
			p = atbmwifi_ieee80211_ie_build_he_cap(&priv->hw_priv->bands[priv->config.band]->iftype_data.he_cap, p);
			p = ieee80211_ie_build_he_oper(p);
		}
	}
#endif
	...
	if (special_ie && special_ie_len > 0) {
		wifi_printk(WIFI_DBG_ERROR, "set special_beacon_ie \n");
		atbm_memcpy(p,special_ie,special_ie_len);
		p += special_ie_len;
	}
#if CONFIG_WPS
			 ...
			 atbm_uint16 transaction, atbm_uint16 auth_alg,  const atbm_uint8 *da,const atbm_uint8 *bssid,atbm_uint16 resp)
{
	...
	if(extra_sae_len){
		if(atbmwifi_is_sta_mode(priv->iftype)){
			atbm_memcpy(atbm_skb_put(skb, extra_sae_len), (void*)((char *)wpabuf_head(wpa_s->sae_data) + 4), extra_sae_len);
		}else{
			atbm_memcpy(atbm_skb_put(skb, extra_sae_len), (void*)wpabuf_head(hapd->sae_data), extra_sae_len);
		}
	}
#endif
			  ...
			  atbm_uint16 agg_size, atbm_uint16 timeout)
{
	...
	atbm_skb_put(skb, 1 + sizeof(mgmt->u.action.u.addba_req) - ATBM_ZEROSIZE);
...
#if CONFIG_HE
static void ieee80211_add_addbaext(struct atbmwifi_sta_priv *sta_priv,
				   struct atbm_buff *skb,
				   const struct atbm_ieee80211_addba_ext_ie *req)
{
	...
	const struct atbm_ieee80211_sta_he_cap *he_cap;
	atbm_uint8 frag_level, cap_frag_level;
	atbm_uint8 *pos;

	if (sta_priv->he_used)
		return;

	he_cap = &sta_priv->rate.he_cap;

	pos = atbm_skb_put(skb, 2 + sizeof(struct atbm_ieee80211_addba_ext_ie));
	*pos++ = ATBM_WLAN_EID_ADDBA_EXTENSION;
	*pos++ = sizeof(struct atbm_ieee80211_addba_ext_ie);
	...
	frag_level = atbm_le32_get_bits(req->data,
				  ATBM_IEEE80211_ADDBA_EXT_FRAG_LEVEL_MASK);
	cap_frag_level = atbm_le32_get_bits(he_cap->he_cap_elem.mac_cap_info[0],
					   ...
					   const struct atbm_ieee80211_addba_ext_ie *addbaext)
{
	...
	capab = (atbm_uint16)(policy << 1|ATBM_BIT(0)); /* bit 1 aggregation policy */
...
#if CONFIG_HE
	if(priv->hw_priv->chip_version != OCEANUS_NO_WIFI6){
		if (sta_priv->rate.he_cap.has_he && addbaext)
			ieee80211_add_addbaext(sta_priv, skb, addbaext);
	}
#endif //CONFIG_HE
...
atbm_void atbmwifi_ieee80211_send_htc_qosnullfunc(struct atbmwifi_vif *priv, atbm_uint32 htc)
{
	...
	skb = atbm_dev_alloc_skb(sizeof(struct atbmwifi_ieee80211_qos_hdr) + 4);
	if (!skb)
	{
		wifi_printk(WIFI_TX,"<ERROR> send_auth alloc skb \n");
		return;
	...
	QosNulldata = (struct atbmwifi_ieee80211_qos_hdr *) atbm_skb_put(skb, 30);
	...
	QosNulldata->frame_control = atbm_cpu_to_le16(ATBM_IEEE80211_FTYPE_DATA|
				   ...
				   ATBM_IEEE80211_FCTL_ORDER);
	atbm_memcpy(QosNulldata->addr1, priv->bssid, ATBM_ETH_ALEN);
	atbm_memcpy(QosNulldata->addr2, priv->mac_addr, ATBM_ETH_ALEN);
	atbm_memcpy(QosNulldata->addr3, priv->bssid, ATBM_ETH_ALEN);
	QosNulldata->qos_ctrl = 0;

	memcpy(skb->abuf + sizeof(struct atbmwifi_ieee80211_qos_hdr), &htc, 4);
	ATBM_IEEE80211_SKB_TXCB(skb)->flags |= ATBM_IEEE80211_TX_INTFL_DONT_ENCRYPT|ATBM_IEEE80211_TX_CTL_USE_MINRATE;
	atbmwifi_tx(priv->hw_priv,skb,priv);
}
