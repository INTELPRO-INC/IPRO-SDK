...
void hmac_tx_statu(void * priv, struct atbm_ieee80211_sta * sta, void * priv_sta)
{
	...
	{
		//atbm_printk_rc("[%d]:%d status %x %x\n", i, status->txcnt[i], status, &status->txcnt[i]);
		if (status->txcnt[i] > 0)
		{
	...
	if ((rates_info->not_sample_cnt > 4)) //11g->11n,11ax
	{
     ...
     Lite:   snd_idle_r |main_idle_r + tx_ratio     
     */

	main_idle_ratio = (u16) (sta->txs_retrys[RATE_DESC_INDEX_MAX * 2]);
	snd_idle_ratio		= (u16) (sta->txs_retrys[RATE_DESC_INDEX_MAX * 2 + 1]);
	reservedL16 	= (u16) (sta->txs_retrys[RATE_DESC_INDEX_MAX * 2 + 2]);

	atbm_printk_rc("##### chip_category:%d main_idle_ratio:0x%x, snd_idle_ratio:0x%x, reservedL16:0x%x\n",rates_info->chip_category , main_idle_ratio,snd_idle_ratio, reservedL16);
	 ...
	 if (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_40_MHZ_WIDTH)
			 ...
			 (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_20_MHZ_WIDTH)) //support 40M&20M
		{
		 ...
		 //atbm_printk_err("current tick:%d  updatetick:%d\n",atbm_GetOsTick(), rates_info->bw_total_pkt_update_time);
			...
			if (time_after(atbm_GetOsTick(), rates_info->bw_total_pkt_update_time + (2000 * ATBM_HZ) / 1000))
			{
				...
				{ //have valid idle ratiorates_info->snd_idle_ratio
					...
					rates_info->bw_total_pkt_cnt =
						 (rates_info->bw_total_pkt_cnt * ATBM_HZ) / (atbm_GetOsTick() - rates_info->bw_total_pkt_update_time);
...
//	strRateIndexPhy[rates_info->max_tp_rate_idx],sum_tp, sum_tx_cnt, main_idle_ratio, rates_info->average_throughput);
	if (time_after(atbm_GetOsTick(), rates_info->tx_pkt_count_update_time + (2000 * ATBM_HZ) / 1000))
		{
	...
	atbm_printk_rc("mtp[%d]:%s %d  mean_rssi:%d  rc_flags:%x\n\n", rates_info->max_tp_rate_idx, strRateIndexPhy[rates_info->max_tp_rate_idx], max_tp,
...
u32 rate_info_to32bit(RATE_SET_T * prate_set)
{
	...
	rate_set.max_suc_rate_index = rate_index_to_wsm_rate(prate_set->max_suc_rate_index, (RATE_MODE_TYPE)prate_set->rate_mode);
	rate_set.down_rate_index = rate_index_to_wsm_rate(prate_set->down_rate_index, (RATE_MODE_TYPE)prate_set->rate_mode);
	rate_set.lowest_rate_index = rate_index_to_wsm_rate(prate_set->lowest_rate_index, (RATE_MODE_TYPE)prate_set->rate_mode);
...
void get_max_assoc_rate(struct atbm_ieee80211_sta * sta, struct RATE_CONTROL_RATE_S * rates_info)
{
	...
	if(atbm_hweight32(rate_mask) < 3){
               	atbm_printk_rc("Single Rate mode Ht rate_mask:0x%x \n", rate_mask);
                rates_info->n_mode = 1;
		rates_info->tx_rc_flags |= ATBM_IEEE80211_TX_CTL_USE_MINRATE;
	}
#endif
		}		
	...
	atbm_printk_rc(" supp_rates band0:0x%x band1 0x%x \n",  sta->supp_rates[0],   sta->supp_rates[1]);
	  ...
	  rates_info->is_2G_flag = 1; //default set work on 2.4G


       if(rates_info->center_freq >2600)
	{
          u8band = 1;
		  rates_info->is_2G_flag = 0; 
	}
...
#if TEST_ON_FIXED_RATE	
	if(atbm_hweight32(rate_mask) < 3){
         atbm_printk_rc("Single Rate mode  legacy rate_mask:0x%x \n", rate_mask);
		 rates_info->tx_rc_flags |= ATBM_IEEE80211_TX_CTL_USE_MINRATE;
	}
#endif
...
s8 lite_idle_ratio_select(u16 main_snd_idle_ratio, u16 main_idle_ratio,  s8 bw_40m_cn_in)
{
    ...
    */
    ...
    atbm_printk_rc("main_snd_idle_ratio:%d main_idle_ratio:%d bw_40m_cn in:%d out:%d\n",main_snd_idle_ratio/20, main_idle_ratio, bw_40m_cn_in,bw_40m_cn);
	return bw_40m_cn;
}
...
unsigned char bw_select_40m_lite(struct RATE_CONTROL_RATE_S * rates_info)
{
   ...
   // atbm_printk_rc("rssi:%d  snd_ratio:%d  m_sndr:%d m_tp:%d mtp_prob:%d bw20:%d\n",rssi, snd_idle_ratio,((main_idle_ratio>>8)&0xff), m_tp_rate, mtp_prob, cur_band40_2_20m);
	...
	if((good_sig == 0)&&(bw_40m_select == 1)) 
	{
        ...
        ||((m_tp_rate < RATE_INDEX_HE_MCS2) && (m_tp_rate >= RATE_INDEX_HE_MCS0))) && (rates_info->is_2G_flag == 1))
		{
			bw_40m_select = 0;
		}
		else if(((m_tp_rate <= RATE_INDEX_N_39M) ||((m_tp_rate < RATE_INDEX_VHT_MCS4) && (m_tp_rate >= RATE_INDEX_VHT_MCS0))
        ||((m_tp_rate < RATE_INDEX_HE_MCS5) && (m_tp_rate >= RATE_INDEX_HE_MCS0))) && (rates_info->is_2G_flag == 1))
		{
...
unsigned char bw_select_40m(struct RATE_CONTROL_RATE_S * rates_info)
{
  ...
  if((rates_info->chip_category == ATBM_OCEANUS_CATEGORY)||(rates_info->chip_category == ATBM_CLITE_CATEGORY)
  		|| (rates_info->chip_category == ATBM_OLITE_CATEGORY) || (rates_info->chip_category == ATBM_DEMETER_CATEGORY))
  	{
...
unsigned char bw_select_40m_simple(struct RATE_CONTROL_RATE_S * rates_info)
{
   ...
   u8  max_tp_idx_desc;
	...
	if(bw_40m_select == 0){
		atbm_printk_err("rssi:%d  m_tp:%d mtp_prob:%d bw_pkt_cnt:%d	bw20:%d Select40:%d\n", rssi,  m_tp_rate, mtp_prob, bw_pkt_cnt, cur_band40_2_20m, bw_40m_select);
	}
...
void set_lowest_rate(u8 rateidx, struct atbm_ieee80211_tx_rate_control * txrc)
{
	...
	wsm_rate_id 		= rate_index_to_wsm_rate(rate_set.max_tp_rate_index, (RATE_MODE_TYPE)rate_set.rate_mode);
	...
	struct atbm_ieee80211_tx_rate_control * txrc)
{
	...
	//atbm_printk_rc("%s:hmac_get_rate!\n", __func__);

#if TEST_ON_FIXED_RATE
	if ((sta != NULL) && (rates_info != NULL))
	{
		if(rates_info->tx_rc_flags & ATBM_IEEE80211_TX_CTL_USE_MINRATE)
		{						  
	...
	if (txrc->tx_rc_in->manual)
	{
		...
		if (txrc->tx_rc_in->manual & TX_RATE_FIXED_STATIC)
		{
			if ((sta != NULL) && (rates_info != NULL))
			{
				if (time_after(atbm_GetOsTick(), rates_info->last_update_time + (rate_update_interval * ATBM_HZ) / 1000))
				{
	...
	rate_index_to_wsm_rate_mode(prate_set->max_tp_rate_index, &rate_mode);
	wsm_rate_id 		= rate_index_to_wsm_rate(prate_set->max_tp_rate_index, (RATE_MODE_TYPE)prate_set->rate_mode);
	   ...
	   u32 changed, enum atbm_nl80211_channel_type oper_chan_type)
{
	...
	atbm_printk_rc("%s chip_cat:%d sta_vht_cap:%x rcaddr:0x%x he_capaddr:0x%x has_he:0x%x\n", __func__, rc_priv->chip_cat, sta->vht_cap, rates_info, &rates_info->sta.he_cap, rates_info->sta.he_cap.has_he);

	//reset rates info struct.
	reset_rate_info(rate_info, is_ap_flag);


	memcpy((u8*)(&rates_info->sta.ht_cap), (u8*)(sta->ht_cap),	sizeof(struct atbmwifi_ieee80211_sta_ht_cap));
	...
	ATBM_IEEE80211_TX_RC_40M_TO_20M		= ATBM_BIT(12)
	*/
...
static void * hmac_alloc_sta(void * priv, struct atbm_ieee80211_sta * sta, gfp_t gfp)
{
	...
	pHmac_rate = (RATE_CONTROL_RATE*)atbm_kzalloc(sizeof(RATE_CONTROL_RATE) +sizeof(struct txrate_status) + 2 * sizeof(u32),
		 gfp);
	...
	atbm_printk_rc("%s	chip_category:%d nrates:%d sta->txs_retrys:0x%x\n", __func__, rc_priv->chip_cat, sta->n_rates, sta->txs_retrys);
...
static void * hmac_alloc(u8 chip_cat)
{
	...
	// hmac_init();
	mp = (struct hmac_rc_priv*)atbm_kzalloc(sizeof(struct hmac_rc_priv), GFP_ATOMIC);
	...
	struct atbmn_ieee80211_rx_status * status)
{
   ...
   //if(ieee80211_is_data(status->frame_ctrl) || ieee80211_is_data_qos(status->frame_ctrl))
	...
	//rc_sta_info->rx_rate_avg = rc_sta_info->rx_rate_avg / 2 + sband->bitrates[status->rate_idx].bitrate / 2;
...
int hmac_get_txrx_status_wh(struct sta_info* sta_info, struct TX_RX_Statistics_S* data)
{
	...
	data->rx_rate_avg = rc_sta_info->rx_rate_avg;
