...
#define RX_SKB__RX_QUEUE_PACKAGE (3)
#define TX_SKB__TX_8023_FRAME (4)
#define TX_SKB__FORWARD_8023_FRAME (5)
#define TX_SKB__INTERNAL_8023_FRAME (6)
/*
...
struct sbus_priv {
	...
	struct atbm_sdio_func	*cmd_func;
	sbus_irq_handler	irq_handler;
	void			*irq_priv;
	atbm_atomic_t   rx_queue_qnum;
#ifdef CONFIG_INF_AGGR_TX
...
#endif  //CONFIG_INF_AGGR_TX
	struct sbus_desc rx_desc[RX_DESC_NUM];
	struct sbus_desc tx_desc[TX_DESC_NUM];
};
...
struct sbus_ops {
	...
	int (*set_block_size)(struct sbus_priv *self, int type, atbm_uint32 size);
