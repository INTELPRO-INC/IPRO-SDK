...
#define WSM_CMD_TIMEOUT		(6 * ATBM_HZ) /* With respect to interrupt loss */
#define WSM_CMD_SCAN_TIMEOUT	(5 * ATBM_HZ) /* With respect to interrupt loss */
#define WSM_CMD_JOIN_TIMEOUT	(8 * ATBM_HZ) /* Join timeout is 5 sec. in FW   */
#define WSM_CMD_START_TIMEOUT	(7 * ATBM_HZ)
#define WSM_CMD_RESET_TIMEOUT	(4 * ATBM_HZ) /* 2 sec. timeout was observed.   */
#define WSM_CMD_DEFAULT_TIMEOUT	(4 * ATBM_HZ)
				...
				struct wsm_buf *buf, int interface_link_id)
{
	...
	if((priv->iftype == ATBM_NL80211_IFTYPE_AP)||(priv->iftype ==ATBM_NL80211_IFTYPE_P2P_GO)){
		...
		if(priv->bss.sta_priv.sta_rc_priv)
			mac80211_ratectrl->tx_status(&priv->bss.sta_priv.rate,priv->bss.sta_priv.sta_rc_priv,&tx_rate_confirm->txstatus);	
	}
...
#define CAPABILITIES_ATBM_PRIVATE_IE 	ATBM_BIT(1)
#define CAPABILITIES_IPC 				ATBM_BIT(2)
#define CAPABILITIES_NO_CONFIRM 		ATBM_BIT(3)
#define CAPABILITIES_WOW		 		ATBM_BIT(4)
#define CAPABILITIES_NO_BACKOFF 		ATBM_BIT(5)
#define CAPABILITIES_CFO 				ATBM_BIT(6)
#define CAPABILITIES_CHIP_FLAG			ATBM_BIT(7)
#define CAPABILITIES_TXCAL 				ATBM_BIT(8)
#define CAPABILITIES_LMAC_BAB_SETUP		ATBM_BIT(9)
#define CAPABILITIES_FORCE_PM_ACTIVE	ATBM_BIT(10)
#define CAPABILITIES_SMARTCONFIG		ATBM_BIT(11)
#define CAPABILITIES_ETF				ATBM_BIT(12)
#define CAPABILITIES_BLE				ATBM_BIT(13)
#define CAPABILITIES_40M				ATBM_BIT(14)
#define CAPABILITIES_LDPC				ATBM_BIT(15)
#define CAPABILITIES_NAV_ENABLE			ATBM_BIT(16)
#define CAPABILITIES_USB_RECOVERY_BUG	ATBM_BIT(17)
#define CAPABILITIES_VIFADDR_LOCAL_BIT	ATBM_BIT(18)
#define CAPABILITIES_USE_IPC 					ATBM_BIT(19)
#define CAPABILITIES_OUTER_PA 					ATBM_BIT(20)
#define CAPABILITIES_WFA			 			ATBM_BIT(21)
#define CAPABILITIES_EAP_GROUP_REKEY	 		ATBM_BIT(22)
#define CAPABILITIES_RTS_LONG_DURATION 			ATBM_BIT(23)
#define CAPABILITIES_TX_CFO_PPM_CORRECTION 		ATBM_BIT(24)
#define CAPABILITIES_RXCAL			 			ATBM_BIT(25)
#define CAPABILITIES_HW_CHECKSUM  				ATBM_BIT(26)
#define CAPABILITIES_80211_TO_8023			 	ATBM_BIT(27)
#define CAPABILITIES_NEW_CHIPER			 		ATBM_BIT(28)
#define CAPABILITIES_EFUSE8 					ATBM_BIT(29)
#define CAPABILITIES_EFUSEI 					ATBM_BIT(30)
#define CAPABILITIES_EFUSEB 					ATBM_BIT(31)

#define EX_CAPABILITIES_TWO_CHIP_ONE_SOC			ATBM_BIT(0)
#define EX_CAPABILITIES_MANUAL_SET_AC				ATBM_BIT(1)
#define EX_CAPABILITIES_LMAC_BW_CONTROL				ATBM_BIT(2)
#define EX_CAPABILITIES_SUPPORT_TWO_ANTENNA			ATBM_BIT(3)
#define EX_CAPABILITIES_ENABLE_STA_REMAIN_ON_CHANNEL ATBM_BIT(4)
#define EX_CAPABILITIES_ENABLE_PS					ATBM_BIT(5)
#define EX_CAPABILITIES_TX_REQUEST_FIFO_LINK		ATBM_BIT(6)
#define EX_CAPABILITIES_CHIP_TYPE					(ATBM_BIT(7)|ATBM_BIT(8)|ATBM_BIT(9))
#define EX_CAPABILITIES_WAKEUP_HOST_TX				ATBM_BIT(12)
...
*/
	...
	wifi_printk(WIFI_DBG_INIT,"LmacVersion: %d, build: %d,"
		    " api: %d, cap: 0x%.4X Config[%x]  expection %x, ep0 cmd addr %x NumOfHwXmitedAddr[%d]\n",
		hw_priv->wsm_caps.firmwareVersion,
		hw_priv->wsm_caps.firmwareBuildNumber,
		hw_priv->wsm_caps.firmwareApiVer,
		hw_priv->wsm_caps.firmwareCap,Config[0],Config[1],Config[2], Config[3]);

	hw_priv->wsm_caps.firmwareReady = 1;
	hw_priv->max_hw_support_tx_aggs = 128;
	...
	wifi_printk(WIFI_DBG_INIT,"CAPABILITIES_CHIP_FLAG 		[%d]\n" ,!!(hw_priv->wsm_caps.firmwareCap &CAPABILITIES_CHIP_FLAG ));  
	wifi_printk(WIFI_DBG_INIT,"CAPABILITIES_TXCAL 		[%d]\n" ,!!(hw_priv->wsm_caps.firmwareCap &CAPABILITIES_TXCAL ) );  
	wifi_printk(WIFI_DBG_INIT,"CAPABILITIES_LMAC_BAB_SETUP 		[%d]\n" ,!!(hw_priv->wsm_caps.firmwareCap &CAPABILITIES_LMAC_BAB_SETUP) );  
	wifi_printk(WIFI_DBG_INIT,"CAPABILITIES_FORCE_PM_ACTIVE 		[%d]\n" ,!!(hw_priv->wsm_caps.firmwareCap &CAPABILITIES_FORCE_PM_ACTIVE));
	...
	hw_priv->driver_setup_ba = !(hw_priv->wsm_caps.firmwareCap & CAPABILITIES_LMAC_BAB_SETUP);
#endif
					...
					struct atbm_buff **skb_p)
{
	...
	* with interface ID == 2 */
	if (interface_link_id == ATBM_WIFI_GENERIC_IF_ID) {
		...
		fctl = *(atbm_uint16 *)buf->data;
		switch(fctl & atbm_cpu_to_le16(ATBM_IEEE80211_FCTL_STYPE)){
...
void atbm_wsm_event_work(struct atbm_work_struct *work)
{
		 ...
		 atbm_void *arg, atbm_uint16 cmd, long tmo, int if_id)
{
	...
	if (cmd == 0x0006) /* Write MIB */
		wifi_printk(WIFI_WSM, "[wsm_cmd_send] >>> 0x%x [MIB: 0x%x] (%d)\n",
			cmd, __atbm_le16_to_cpu(*((atbm_uint16 *)(((struct wsm_hdr_tx *)buf->begin) + 1))),
			buf_len);
	else
		wifi_printk(WIFI_WSM, "[wsm_cmd_send] >>> 0x%x (%d)\n", cmd, buf_len);
	...
	wsm_h->u.common.total_len = atbm_cpu_to_le16(buf_len);
	wsm_h->u.common.flag = 0;
	wsm_h->len = atbm_cpu_to_le16(buf_len);
	wsm_h->id = atbm_cpu_to_le16(cmd |(if_id << 6) );
	...
	atbm_os_wait_event_timeout(&hw_priv->wsm_cmd_wq,40*ATBM_HZ);	
	atbm_spin_lock(&hw_priv->wsm_cmd.lock);
	ret = hw_priv->wsm_cmd.ret;
	atbm_spin_unlock(&hw_priv->wsm_cmd.lock);
	if(!hw_priv->wsm_cmd.done){
		wifi_printk(WIFI_DBG_ERROR, "send cmd err %x!!\n", cmd);
	#if ATBM_SDIO_BUS
		  ...
		  struct wsm_hdr *wsm, struct atbm_buff **skb_p)
{
	...
	wifi_printk(WIFI_WSM, "[wsm_handle_rx] %d<<< 0x%x (%d)\n",__LINE__, id,
				 ...
				 atbm_uint32 link_id_map,int * total)
{
	...
	//int queued;
	//int i;
	urgent = ATBM_BIT(priv->link_id_after_dtim) | ATBM_BIT(priv->link_id_uapsd);
				     ...
				     int *burst)
{
	...
	if (priv->tx_multicast) {
		tx_allowed_mask = ATBM_BIT(priv->link_id_after_dtim);
	       ...
	       atbm_size_t *tx_len, int *burst, int *vif_selected)
{
	...
	const struct atbmwifi_txpriv *txpriv = ATBM_NULL;
	atbm_uint32 total = 0;
	...
	int first = !(*burst);
	static struct atbm_buff *first_skb;
#endif
		...
		for (;;) {
			...
			* packet needs to be found */
			...
			if (atbmwifi_queue_get(queue,
				...
				if_pending = 0;
				if(first && *burst){
					wifi_printk(WIFI_ALWAYS, "unexpected1 %d %d\n", first, *burst);
				}
				*burst = 0;
				continue;
			}
			...
			if(skb->Type == TX_SKB__TX_8023_FRAME || skb->Type == TX_SKB__FORWARD_8023_FRAME){
				if(!first_skb
					|| (first_skb->priority != skb->priority)
					|| (ATBM_IEEE80211_SKB_TXCB(skb)->sta_priv != ATBM_IEEE80211_SKB_TXCB(first_skb)->sta_priv)
					|| (atbmwifi_is_ap_mode(priv->iftype) && atbm_compare_ether_addr(ATBM_OS_SKB_DATA(skb)+ATBM_ETH_ALEN, ATBM_IEEE80211_SKB_TXCB(first_skb)->smac))
					|| (ATBM_IEEE80211_SKB_TXCB(first_skb)->align != (((atbm_uint32)ATBM_OS_SKB_DATA(skb)) & 0x3))){
					ret = atbmwifi_tx_slow(skb, priv, txpriv);
					if(ret == 0){
						//do nothing
						first_skb = skb;
					}else if(ret < 0){
						atbmwifi_queue_remove(queue, packetID);
						if (if_pending == 1) {
							if_pending = 0;
							continue;
						}
					...
					ret = atbmwifi_tx_fast(skb, first_skb, priv);
					if(ret > 0){
						if (if_pending == 1) {
							if_pending = 0;
							continue;
			...
			}else if(skb->Type == TX_SKB__INTERNAL_8023_FRAME){
				ret = atbmwifi_tx_slow(skb, priv, txpriv);
			}
#endif
			wsm = (struct wsm_tx *)ATBM_OS_SKB_DATA(skb);
			wsm->packetID = packetID;
			...
			*vif_selected = priv->if_id;
			...
			if(priv->pspoll_mask & ATBM_BIT(txpriv->raw_link_id)){
				struct atbmwifi_ieee80211_tx_info *tx_info = ATBM_IEEE80211_SKB_TXCB(skb);
		
				tx_info->flags |= ATBM_IEEE80211_TX_CTL_PS_POLL_RESP;
				wsm->htTxParameters |= atbm_cpu_to_le32(WSM_HT_TX_NEED_CONFIRM);
				priv->pspoll_mask &= ~ATBM_BIT(txpriv->raw_link_id);
			}
			...
			*tx_len = __atbm_le16_to_cpu(wsm->hdr.len);
			...
			hdr=(struct atbmwifi_ieee80211_hdr *)
				&((atbm_uint8 *)wsm)[sizeof(struct wsm_tx)];
			...
			if (total>1) {			
				...
				*  to inform PS STAs */
				...
				priv->link_id_uapsd_mask&= ~ATBM_BIT(txpriv->raw_link_id);
			}
			wifi_printk(WIFI_WSM, "[WSM] >>> 0x%.4X (%d:%d) %p %c %x %d\n",
				0x0004, *tx_len, wsm->hdr.u.common.total_len, *data,
				    ...
				    int if_id)
{
	atbm_uint32 val = 0;
	if (arg->promiscuous)
		val |= atbm_cpu_to_le32(ATBM_BIT(0));
	if (arg->bssid)
		val |= atbm_cpu_to_le32(ATBM_BIT(1));
	if (arg->fcs)
		val |= atbm_cpu_to_le32(ATBM_BIT(2));
	if (arg->probeResponder)
		val |= atbm_cpu_to_le32(ATBM_BIT(3));
	if (arg->keepalive)
		val |= atbm_cpu_to_le32(ATBM_BIT(4));
			      ...
			      int if_id)
{
	...
	if (arg->unprotectedMgmtFramesAllowed)
		val |= atbm_cpu_to_le32(ATBM_BIT(1));
	if (arg->encryptionNoForAuthFrame)
		val |= atbm_cpu_to_le32(ATBM_BIT(2));
...
static  int wsm_get_mac_address(struct atbmwifi_common *hw_priv, atbm_uint8 *mac)
{
 ...
 static inline int	wsm_set_rekey_data(struct atbmwifi_common *hw_priv,struct wsm_rekey_data *arg,int if_id)
 {
...
//	atbm_uint32 len = hw_priv->etf_len;	
	...
	arg.flag = start? ATBM_BIT(WSM_SET_CHANTYPE_FLAGS__ETF_TEST_START):0;		
	...
	if(hw_priv->etf_greedfiled == 1){
		arg.flag |= ATBM_BIT(WSM_SET_CHANTYPE_FLAGS__ETF_GREEDFILED);
	}
...
//	atbm_uint32 len = hw_priv->etf_len;	
	...
	arg.band = 0;			//0:2.4G,1:5G
	arg.flag = start? ATBM_BIT(WSM_SET_CHANTYPE_PRB_TPC):0;		
	arg.channelNumber = hw_priv->etf_channel;// channel number
	arg.channelType =  hw_priv->etf_channel_type;	// channel type
...
#define HW_RESET_REG_CPU   				ATBM_BIT(16)
#define HW_RESET_REG_HIF   				ATBM_BIT(17)
#define HW_RESET_REG_SYS   				ATBM_BIT(18)
#define HW_RESRT_REG_CHIP  				ATBM_BIT(19)
#define HW_RESET_REG_NEED_IRQ_TO_LMAC	ATBM_BIT(20)
...
int wsm_recovery(struct atbmwifi_common *hw_priv)
{
	...
	wifi_printk(WIFI_ALWAYS,"wsm_recovery++,wsm_txframe_num(%d),wsm_txconfirm_num(%d),cmd(%x)\n",
...
void wsm_sync_channl_reset(struct atbm_work_struct *work)
{
	...
	wsm_tx->u.common.flag=atbm_cpu_to_le32(wsm_flag_atbm_uint32 );
	wsm_tx->len=0x10;
	wsm_tx->id=0x55aa;
	do{
...
int  wsm_recovery_done(struct atbmwifi_common *hw_priv,int type)
{
	int status = 0;
	if(type==OUT_BH){
	    status=atbm_os_wait_event_timeout(&hw_priv->wsm_synchanl_done,120*ATBM_HZ);
	}
    if(status<=0){
		...
		int if_id)
{
	...
	WSM_PUT8(buf,  arg->vifs[0].chantype);
	WSM_PUT16(buf, arg->vifs[0].channel);

	WSM_PUT8(buf,  arg->vifs[1].if_id);
	WSM_PUT8(buf,  arg->vifs[1].chantype);
