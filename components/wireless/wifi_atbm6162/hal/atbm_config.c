...
static atbm_int32 SNR_last = 0;
static atbm_int32 SNR_aver = 0;
...
atbm_int32 atbmwifi_get_SNR_avg(atbm_void)
{
	return -(SNR_aver >> 10);
}

atbm_int32 atbmwifi_get_SNR(atbm_void)
{
	return SNR_last;
}

atbm_void atbmwifi_set_SNR(atbm_int8 rssi)
{
	SNR_last = rssi;
	SNR_aver = SNR_aver ? ((SNR_aver << 3) -  SNR_aver + ((-rssi) << 10)) >> 3 : (-rssi) << 10;
}
...
int atbmwifi_get_Tjroom(struct Tjroom_temperature_t *tjroom)
{
	struct atbmwifi_common *hw_priv = &g_hw_prv;
	int ret;

	ret = wsm_get_Tjroom_temperature(hw_priv, tjroom, sizeof(*tjroom));
	if(ret)
	{
		wifi_printk(WIFI_DBG_ERROR, "get Tjroom  failed\n");
		return ret;
	}
	wifi_printk(WIFI_ALWAYS, "Tjroom:%d, tempC:%d,stempC:%d\n", tjroom->Tjroom,
				tjroom->tempC, tjroom->stempC);
 ...
 int atbmwifi_vif_setup(struct atbmwifi_vif *priv)
{
	...
	WSM_EDCA_SET(&edca, ATBM_IEEE80211_AC_VO /*0*/, wmm_param->aifns, wmm_param->cwMin, wmm_param->cwMax,
			wmm_param->txOpLimit, 0xc8, wmm_param->uapsdEnable, 0, 0, 0, 0); 
	//VI
