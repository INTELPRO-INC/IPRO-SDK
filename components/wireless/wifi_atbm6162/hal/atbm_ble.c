...
{
	thread->thread = atbm_createThreadInternal((char *)thread->name,
										thread->thread_fn, 
										thread, 
										thread->prio,
										10*ATBM_HZ);
...
static int ieee80211_ble_xmit_thread(struct ieee80211_ble_thread *thread, int wake_state)
{
...
static int ieee80211_ble_recv_thread(struct ieee80211_ble_thread *thread, int wake_state)
{
	struct atbm_buff *skb;
	struct atbmwifi_common	*hw_priv = thread->hw_priv;
	struct ieee80211_ble_local *ble_local = &hw_priv->ble_local;
