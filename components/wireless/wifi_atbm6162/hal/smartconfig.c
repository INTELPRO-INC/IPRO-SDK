...
{
	return  smt_magic_channel_mask&ATBM_BIT(channel);
}
...
{
	smt_magic_channel_mask|=ATBM_BIT(channel);
}
	...
	*/	
	...
	{		
		...
		atbmwifi_wpa_event_queue((atbm_wpa_event_data)priv,(atbm_wpa_event_data)ATBM_NULL,
			(atbm_wpa_event_data)ATBM_NULL,WPA_EVENT__SMARTCONFIG_SUCCESS,ATBM_WPA_EVENT_NOACK);
		return 1;
	}
	atbm_kfree(bit2smt_data);
	bit2smt_data = NULL;
...
#define WSM_SET_CHANTYPE_STARTCONFIG		ATBM_BIT(4)
