...
static void ipro_pacs_runtime_reset(void)
{
	/*
	 * Rebuild PACS runtime state when OCRAM-retained .bss makes the module
	 * look registered before this boot has inserted pacs_svc into GATT DB.
	 */
	memset(&pacs, 0, sizeof(pacs));
#if defined(CONFIG_BT_PAC_SRC)
	sys_slist_init(&src_pac_list);
	src_supported_contexts = 0;
#if defined(CONFIG_BT_PAC_SRC_LOC)
	pacs_src_location = 0;
#endif
#endif
#if defined(CONFIG_BT_PAC_SNK)
	sys_slist_init(&snk_pac_list);
	snk_supported_contexts = 0;
#if defined(CONFIG_BT_PAC_SNK_LOC)
	pacs_snk_location = 0;
#endif
#endif
	src_available_contexts = BT_AUDIO_CONTEXT_TYPE_NONE;
	snk_available_contexts = BT_AUDIO_CONTEXT_TYPE_NONE;
	memcpy(pacs_attrs, _pacs_attrs, sizeof(_pacs_attrs));
	pacs_svc = (struct bt_gatt_service)BT_GATT_SERVICE(pacs_attrs);
}
...
int bt_pacs_register(const struct bt_pacs_register_param *param)
{
	...
	if (atomic_test_bit(pacs.flags, PACS_FLAG_REGISTERED)) {
		...
		LOG_WRN("Cleared retained PACS registration state (unregister err %d)", err);
		ipro_pacs_runtime_reset();
	}
	...
	LOG_INF("PACS registered attrs=%u", pacs_svc.attr_count);
