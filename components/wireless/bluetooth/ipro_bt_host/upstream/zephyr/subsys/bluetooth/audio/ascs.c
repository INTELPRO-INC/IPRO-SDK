...
static int ascs_iso_accept(const struct bt_iso_accept_info *info, struct bt_iso_chan **iso_chan)
{
    ...
    IPRO_LOGD(IPRO_LOG_TAG, "iso_accept acl=%p cig=%u cis=%u", (void *)info->acl, info->cig_id, info->cis_id);
    ...
    for (size_t i = 0; i < ARRAY_SIZE(ascs.ase_pool); i++) {
        ...
        state = ase->ep.state;
        IPRO_LOGD(IPRO_LOG_TAG, "iso_accept match ase=%p ep=%p state=%u dir=%u iso=%p stream=%p", (void *)ase, (void *)&ase->ep, state, ase->ep.dir, (void *)ase->ep.iso, (void *)ase->ep.stream);
        if (state != BT_BAP_EP_STATE_ENABLING && state != BT_BAP_EP_STATE_QOS_CONFIGURED) {
            LOG_WRN("ase %p cannot accept ISO connection", ase);
            IPRO_LOGW(IPRO_LOG_TAG, "iso_accept reject state=%u", state);
            break;
        }
        ...
        *iso_chan = chan;
        ...
        IPRO_LOGD(IPRO_LOG_TAG, "iso_accept ok chan=%p", (void *)*iso_chan);

        return 0;
    }

    IPRO_LOGW(IPRO_LOG_TAG, "iso_accept reject no-match");
    return -EACCES;
}
...
{
    struct bt_bap_iso *iso = CONTAINER_OF(chan, struct bt_bap_iso, chan);
    IPRO_LOGD(IPRO_LOG_TAG, "iso_connected chan=%p rx_ep=%p tx_ep=%p", (void *)chan, (void *)iso->rx.ep, (void *)iso->tx.ep);
...
{
    struct bt_bap_iso *iso = CONTAINER_OF(chan, struct bt_bap_iso, chan);
    IPRO_LOGD(IPRO_LOG_TAG, "iso_disconnected chan=%p reason=0x%02x rx_ep=%p tx_ep=%p", (void *)chan, reason, (void *)iso->rx.ep, (void *)iso->tx.ep);
...
static int ase_config(struct bt_ascs_ase *ase, const struct bt_ascs_config *cfg)
{
	...
	if (unicast_server_cb == NULL) {
		...
		if (err == 0) {
			if (!bt_bap_valid_qos_pref(&ase->ep.qos_pref)) {
				LOG_DBG("Invalid QoS preferences");

				/* If the upper layers provide an invalid QoS preferences we reject
				 * the request from the client, as it would not be able to parse the
				 * result as valid anyways
				 */
				err = -EINVAL;
			}
		...
		stream = NULL;
		if (unicast_server_cb->config != NULL) {
		...
		if (err == 0 && stream == NULL) {
			LOG_DBG("Upper layer did not provide a stream object");
			err = -ENOMEM;
		}
	...
	if (err != 0) {
		...
		LOG_DBG("Config failed: err %d, stream %p, code %u, reason %u", err, stream,
			rsp.code, rsp.reason);
		    ...
		    struct bt_bap_qos_cfg *qos, struct bt_bap_ascs_rsp *rsp)
{
	...
	if (unicast_server_cb != NULL && unicast_server_cb->qos != NULL) {
		const int err = unicast_server_cb->qos(stream, qos, rsp);
...
static bool ascs_parse_metadata(struct bt_data *data, void *user_data)
{
	...
	case BT_AUDIO_METADATA_TYPE_STREAM_CONTEXT: {
		...
		if (IS_ENABLED(CONFIG_BT_CAP_ACCEPTOR) &&
		    ...
		    !is_context_available(result->conn, result->ep->dir, context)) {
			LOG_WRN("Context 0x%04x is unavailable", context);
			*result->rsp =
				BT_BAP_ASCS_RSP(BT_BAP_ASCS_RSP_CODE_METADATA_REJECTED, data_type);
			result->err = -EACCES;
			return false;
		}
	...
	case BT_AUDIO_METADATA_TYPE_CCID_LIST: {
		...
		if (IS_ENABLED(CONFIG_BT_CAP_ACCEPTOR) &&
		    !bt_cap_acceptor_ccids_exist(result->ep->stream->conn, data_value, data_len)) {
			LOG_WRN("Metadata contain unknown CCID(s)");
		}
...
static void ipro_ascs_runtime_reset(void)
{
	/*
	 * The IPRO7 port cannot rely on .bss being zero after every reset.
	 * If ascs.registered is retained across boot, the app sees -EALREADY
	 * and skips the real bt_gatt_service_register(), leaving ASCS absent
	 * from the new dynamic GATT DB.  Rebuild the local runtime state before
	 * the first registration attempt in this boot.
	 */
	memset(&ascs, 0, sizeof(ascs));
	memcpy(ascs_attrs, ascs_attrs_backup, sizeof(ascs_attrs_backup));
	ascs_svc = (struct bt_gatt_service)BT_GATT_SERVICE(ascs_attrs);
}
...
int bt_ascs_register(uint8_t snk_cnt, uint8_t src_cnt)
{
	...
	if (ascs.registered) {
		...
		LOG_WRN("Cleared retained ASCS registration state (unregister err %d)", err);
		ipro_ascs_runtime_reset();
	}
	...
	LOG_INF("ASCS registered snk=%u src=%u attrs=%u", snk_cnt, src_cnt,
		ascs_svc.attr_count);
