...
static void ipro_gatt_runtime_clear(void)
{
	/*
	 * Upstream Zephyr assumes .bss starts as zero.  Current IPRO7 reset
	 * paths can retain OCRAM contents, so clear mutable GATT runtime pools
	 * before settings restore tries to find free SC/CF/CCC slots.
	 */
	last_static_handle = 0;
	memset(gatt_flags, 0, sizeof(gatt_flags));
	...
	memset(sc_ccc.cfg, 0, sizeof(sc_ccc.cfg));
	sc_ccc.value = 0;
#endif
#if defined(CONFIG_BT_GATT_CACHING)
