...
#define IPRO_LOG_TAG "BT_SMP"
LOG_MODULE_REGISTER(bt_smp);

#if defined(CONFIG_IPRO_BT_HOST_SMP_TRACE)
/* Longest key material any SMP trace prints (LTK/IRK/CSRK are 16 bytes). */
#define IPRO_SMP_TRACE_HEX_MAX  16U

static void ipro_smp_trace_hex(const char *name, const uint8_t *data, size_t len)
{
	static const char digits[] = "0123456789abcdef";
	char hex[IPRO_SMP_TRACE_HEX_MAX * 2U + 3U];
	size_t count = len < IPRO_SMP_TRACE_HEX_MAX ? len : IPRO_SMP_TRACE_HEX_MAX;
	size_t pos = 0;

	/* One log call rather than a printf per byte: a trace line stays whole
	 * even when another task logs at the same time. */
	for (size_t i = 0; i < count; i++) {
		hex[pos++] = digits[data[i] >> 4];
		hex[pos++] = digits[data[i] & 0x0fU];
	}
	if (len > count) {
		hex[pos++] = '.';
		hex[pos++] = '.';
	...
	hex[pos] = '\0';
	IPRO_LOGD(IPRO_LOG_TAG, "%s=%s", name, hex);
}

static void ipro_smp_trace_addr(const char *name, const bt_addr_le_t *addr)
{
	IPRO_LOGD(IPRO_LOG_TAG, "%s type=%u addr=%02x:%02x:%02x:%02x:%02x:%02x", name, addr->type, addr->a.val[5], addr->a.val[4], addr->a.val[3], addr->a.val[2], addr->a.val[1], addr->a.val[0]);
}

static void ipro_smp_trace_marker(const char *marker)
{
	IPRO_LOGD(IPRO_LOG_TAG, "%s", marker);
}

static void ipro_smp_trace_u32(const char *name, uint32_t value)
{
	IPRO_LOGD(IPRO_LOG_TAG, "%s=%lu", name, (unsigned long)value);
}
#else
#define ipro_smp_trace_hex(name, data, len) do { } while (0)
#define ipro_smp_trace_state(smp, stage) do { } while (0)
#define ipro_smp_trace_marker(marker) do { } while (0)
#define ipro_smp_trace_u32(name, value) do { } while (0)
#endif
...
#if defined(CONFIG_IPRO_BT_HOST_SMP_TRACE)
static void ipro_smp_trace_state(struct bt_smp *smp, const char *stage)
{
	struct bt_conn *conn = smp->chan.chan.conn;

	IPRO_LOGD(IPRO_LOG_TAG, "%s role=%u method=%u flags=0x%08lx passkey=0x%08lx", stage, conn ? conn->role : 0xff, smp->method, (unsigned long)atomic_get(smp->flags), (unsigned long)smp->passkey);
	if (conn) {
		ipro_smp_trace_addr("init_addr", &conn->le.init_addr);
		ipro_smp_trace_addr("resp_addr", &conn->le.resp_addr);
	}
	ipro_smp_trace_hex("preq", smp->preq, sizeof(smp->preq));
	ipro_smp_trace_hex("prsp", smp->prsp, sizeof(smp->prsp));
	ipro_smp_trace_hex("prnd", smp->prnd, sizeof(smp->prnd));
	ipro_smp_trace_hex("rrnd", smp->rrnd, sizeof(smp->rrnd));
	ipro_smp_trace_hex("peer_pkey", smp->pkey, sizeof(smp->pkey));
	if (sc_public_key) {
		ipro_smp_trace_hex("local_pkey", sc_public_key, BT_PUB_KEY_LEN);
	}
	ipro_smp_trace_hex("dhkey", smp->dhkey, sizeof(smp->dhkey));
	ipro_smp_trace_hex("mackey", smp->mackey, sizeof(smp->mackey));
	ipro_smp_trace_hex("ltk", smp->tk, sizeof(smp->tk));
}
#endif
...
static uint8_t sc_smp_send_dhkey_check(struct bt_smp *smp, const uint8_t *e)
{
	...
	ipro_smp_trace_hex("tx_dhkey_check_e", e, 16);
	smp_send(smp, buf, NULL, NULL);
...
static uint8_t compute_and_check_and_send_periph_dhcheck(struct bt_smp *smp)
{
	...
	ipro_smp_trace_state(smp, "periph_dhkey_check");
	ipro_smp_trace_hex("rx_dhkey_check_e", smp->e, 16);
	ipro_smp_trace_hex("calc_remote_re", re, 16);
	ipro_smp_trace_hex("calc_local_e", e, 16);
	ipro_smp_trace_hex("dhcheck_r", r, 16);

	/* compare received E with calculated remote */
	if (memcmp(smp->e, re, 16)) {
		ipro_smp_trace_marker("periph_dhkey_check_mismatch");
...
static uint8_t smp_dhkey_ready(struct bt_smp *smp, const uint8_t *dhkey)
{
	...
	ipro_smp_trace_state(smp, "dhkey_ready");
...
static uint8_t smp_pairing_random(struct bt_smp *smp, struct net_buf *buf)
{
	...
	case PASSKEY_CONFIRM:
		...
		ipro_smp_trace_u32("periph_numeric_compare_passkey", passkey);
		ipro_smp_trace_state(smp, "periph_pairing_random_passkey_confirm");
		atomic_set_bit(smp->flags, SMP_FLAG_USER);
		smp_auth_cb->passkey_confirm(smp->chan.chan.conn, passkey);
		break;
	case JUST_WORKS:
...
static uint8_t smp_dhkey_check(struct bt_smp *smp, struct net_buf *buf)
{
	...
	LOG_DBG("");
	ipro_smp_trace_hex("rx_dhkey_check_req", req->e, sizeof(req->e));
...
{
	/*
	 * Upstream Zephyr assumes .bss is zeroed before bt_enable().  IPRO7
	 * reset/boot paths can retain OCRAM contents, leaving bt_smp_pool[]
	 * with stale L2CAP conn pointers and stale pairing state.  Clear the
	 * runtime pools before fixed-channel accept starts using chan.conn as
	 * the free-slot indicator.
	 */
	memset(bt_smp_pool, 0, sizeof(bt_smp_pool));
#if defined(CONFIG_BT_CLASSIC)
	memset(bt_smp_br_pool, 0, sizeof(bt_smp_br_pool));
#endif
	sc_oobd_present = false;
	legacy_oobd_present = false;
	sc_public_key = NULL;
	bondable = IS_ENABLED(CONFIG_BT_BONDABLE);
