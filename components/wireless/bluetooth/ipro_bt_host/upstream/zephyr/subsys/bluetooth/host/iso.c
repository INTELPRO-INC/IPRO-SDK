...
void hci_le_cis_req(struct net_buf *buf)
{
    ...
    LOG_DBG("acl_handle %u cis_handle %u cig_id %u cis %u", acl_handle, cis_handle, evt->cig_id,
        evt->cis_id);
    IPRO_LOGD(IPRO_LOG_TAG, "CIS_REQ acl=0x%04x cis=0x%04x cig=%u cis_id=%u", acl_handle, cis_handle, evt->cig_id, evt->cis_id);

    if (iso_server == NULL) {
        LOG_DBG("No ISO server registered");
        IPRO_LOGW(IPRO_LOG_TAG, "CIS_REQ reject: no iso server");
    ...
    if (iso) {
        ...
        IPRO_LOGW(IPRO_LOG_TAG, "CIS_REQ reject: duplicate iso handle 0x%04x", cis_handle);
    ...
    if (!acl) {
        ...
        IPRO_LOGW(IPRO_LOG_TAG, "CIS_REQ reject: missing acl handle 0x%04x", acl_handle);
    ...
    if (!iso) {
        ...
        IPRO_LOGW(IPRO_LOG_TAG, "CIS_REQ reject: cannot allocate iso");
    ...
    if (err) {
        ...
        IPRO_LOGW(IPRO_LOG_TAG, "CIS_REQ reject: app err=%d", err);
    ...
    IPRO_LOGW(IPRO_LOG_TAG, "ACCEPT_CIS handle=0x%04x err=%d", cis_handle, err);
    if (err) {
        bt_iso_cleanup_acl(iso);
        bt_conn_unref(iso);
		hci_le_reject_cis(cis_handle, BT_HCI_ERR_INSUFFICIENT_RESOURCES);
