...
/* Small always-on diagnostics for the IPRO BR/EDR HCI boundary.  Keep these
 * as scalar counters: they prove which Packet Boundary flags the controller
 * delivered without enabling high-volume Host logging or trace buffers. */
volatile uint32_t ipro_bt_host_acl_rx_pb_count[4];
volatile uint32_t ipro_bt_host_acl_rx_pb_reject_count;
			...
			uint8_t flags)
{
	...
	if (flags < ARRAY_SIZE(ipro_bt_host_acl_rx_pb_count)) {
		ipro_bt_host_acl_rx_pb_count[flags]++;
	}

	/* Check packet boundary flags */
	switch (flags) {
	case BT_ACL_START_NO_FLUSH:
		/* PB=0 is a valid non-automatically-flushable first fragment for
		 * BR/EDR Controller-to-Host ACL data.  It is reserved on LE-U, so
		 * retain the upstream LE rejection and share only the BR/EDR first
		 * fragment path below.
		 */
		if (!bt_conn_is_br(conn)) {
			ipro_bt_host_acl_rx_pb_reject_count++;
			LOG_ERR("Unexpected LE ACL flags (0x%02x)", flags);
	...
	case BT_ACL_CONT:
		...
		break;
	default:
		/* BT_ACL_COMPLETE is not allowed on LE-U from Controller to Host.
...
static void perform_auto_initiated_procedures(struct bt_conn *conn, void *unused)
{
	 ...
	 */
	if (IS_ENABLED(CONFIG_BT_AUTO_DATA_LEN_UPDATE) && bt_drv_quirk_no_auto_dle()) {
...
int bt_conn_init(void)
{
	...
	/*
	 * Zephyr upstream relies on .bss being zero before Bluetooth host init.
	 * IPRO7 reset/boot paths used by the current SDK do not consistently
	 * clear retained OCRAM, so static host pools can contain stale refcounts,
	 * handles and list nodes.  Clear them here before any lookup/ref path can
	 * observe garbage connection objects.
	 */
	memset(acl_conns, 0, sizeof(acl_conns));
	memset(conn_tx, 0, sizeof(conn_tx));
#if defined(CONFIG_BT_CLASSIC)
	memset(sco_conns, 0, sizeof(sco_conns));
#endif
#if defined(CONFIG_BT_ISO)
	memset(iso_conns, 0, sizeof(iso_conns));
	memset(iso_tx, 0, sizeof(iso_tx));
#endif
#if defined(CONFIG_BT_SMP)
	bt_keys_reset();
#endif
