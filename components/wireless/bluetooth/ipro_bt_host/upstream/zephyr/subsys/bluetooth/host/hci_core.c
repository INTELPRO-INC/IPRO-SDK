...
#define IPRO_LOG_TAG "BT_HCI_CORE"
...
#define BT_HCI_BUS    0
#define BT_HCI_NAME   ""
#define BT_HCI_QUIRKS BT_HCI_QUIRK_NO_AUTO_DLE
#endif
...
/* Frozen, low-cost host/controller command failure context.  Keep only the
 * last non-zero HCI status instead of enabling verbose UART or trace buffers. */
volatile uint32_t ipro_bt_host_hci_error_count;
volatile uint16_t ipro_bt_host_hci_error_opcode;
volatile uint8_t ipro_bt_host_hci_error_status;
			 ...
			 struct net_buf **rsp)
{
	...
	if (!IS_ENABLED(CONFIG_BT_TX_PROCESSOR_THREAD) && k_current_get() == &k_sys_work_q.thread) {
		...
		do {
			...
			if (!success) {
				IPRO_LOGW(IPRO_LOG_TAG, "SELF_PROCESS_TIMEOUT opcode=0x%04x head=%p want=%p ncmd_sem=%p sync_sem=%p", opcode, cmd, buf, (void *)&bt_dev.ncmd_sem, (void *)&sync_sem);
			}
	...
	if (err != 0) {
		IPRO_LOGW(IPRO_LOG_TAG, "SYNC_TIMEOUT opcode=0x%04x err=%d status=0x%02x buf=%p sync_sem=%p rsp=%p", opcode, err, cmd(buf)->status, buf, (void *)&sync_sem, rsp);
	}
	BT_ASSERT_MSG(err == 0,
	...
	if (status) {
		ipro_bt_host_hci_error_opcode = opcode;
		ipro_bt_host_hci_error_status = status;
		ipro_bt_host_hci_error_count++;
...
volatile uint32_t ipro_bt_host_acl_complete_evt_cnt;
volatile uint32_t ipro_bt_host_acl_complete_pkt_cnt;
volatile uint32_t ipro_bt_host_acl_complete_hist[5];
...
static void hci_num_completed_packets(struct net_buf *buf)
{
	...
	for (i = 0; i < evt->num_handles; i++) {
		...
		ipro_bt_host_acl_complete_evt_cnt++;
		ipro_bt_host_acl_complete_pkt_cnt += count;
		ipro_bt_host_acl_complete_hist[(count < 4U) ? count : 4U]++;
...
void bt_hci_le_enh_conn_complete(struct bt_hci_evt_le_enh_conn_complete *evt)
{
	...
	if (IS_ENABLED(CONFIG_BT_PERIPHERAL) &&
		...
		if (!(IS_ENABLED(CONFIG_BT_EXT_ADV) &&
			...
			if (IS_ENABLED(CONFIG_BT_PRIVACY) && adv &&
			    !atomic_test_bit(adv->flags, BT_ADV_USE_IDENTITY)) {
				conn->le.resp_addr.type = BT_ADDR_LE_RANDOM;
				if (!bt_addr_eq(&evt->local_rpa, BT_ADDR_ANY)) {
					bt_addr_copy(&conn->le.resp_addr.a,
						     &evt->local_rpa);
				} else if (!bt_addr_eq(&adv->random_addr.a, BT_ADDR_ANY)) {
					bt_addr_copy(&conn->le.resp_addr.a,
						     &adv->random_addr.a);
				} else if (!bt_addr_eq(&bt_dev.random_addr.a, BT_ADDR_ANY)) {
					bt_addr_copy(&conn->le.resp_addr.a,
						     &bt_dev.random_addr.a);
				} else {
					bt_addr_le_copy(&conn->le.resp_addr,
			...
			IPRO_LOGD(IPRO_LOG_TAG, "periph_resp_addr_fallback type=%u addr=%02x:%02x:%02x:%02x:%02x:%02x local_rpa_any=%u adv=%p", conn->le.resp_addr.type, conn->le.resp_addr.a.val[5], conn->le.resp_addr.a.val[4], conn->le.resp_addr.a.val[3], conn->le.resp_addr.a.val[2], conn->le.resp_addr.a.val[1], conn->le.resp_addr.a.val[0], bt_addr_eq(&evt->local_rpa, BT_ADDR_ANY), (void *)adv);
#endif
		}
...
static bool hci_core_send_cmd(void)
{
	...
	if (!buf) {
		/* bt_tx_irq_raise() can race with another TX work invocation after
		 * process_pending_cmd() observed the FIFO non-empty.  Treat this as
		 * an empty queue, not a host fatal assertion; the ncmd token was
		 * already taken by the caller and must be returned.
		 */
		k_sem_give(&bt_dev.ncmd_sem);
		return false;
	}
...
static int le_set_event_mask(void)
{
	...
	if (IS_ENABLED(CONFIG_BT_CONN)) {
		...
		if (IS_ENABLED(CONFIG_BT_DATA_LEN_UPDATE) &&
		    (BT_FEAT_LE_DLE(bt_dev.le.features) || bt_drv_quirk_no_auto_dle())) {
...
static int set_event_mask(void)
{
	...
	/* Encryption Change is shared by LE and BR/EDR.  A Classic-only
	 * controller has no LE feature bit to test, but still requires this event
	 * to advance conn->encrypt/security after SSP. */
	if (IS_ENABLED(CONFIG_BT_CLASSIC) ||
	    (IS_ENABLED(CONFIG_BT_SMP) && BT_FEAT_LE_ENCR(bt_dev.le.features))) {
...
static int hci_init(void)
{
	...
	/* Zephyr upstream currently assumes every controller is LE capable.  The
	 * IPRO6S controller backend also supports a real BR/EDR-only library
	 * profile, so do not issue LE commands when the Local Supported Features
	 * page says that LE is absent.  Keep the upstream failure for non-LE builds
	 * that did not request the Classic host. */
	if (BT_FEAT_LE(bt_dev.features)) {
		err = le_init();
		if (err) {
			return err;
		...
		LOG_INF("BR/EDR-only controller; skipping LE initialization");
	}
