			   ...
			   const bt_addr_le_t *addr, const char *key)
{
	...
	/*
	 * Only the four identity/address types can be persisted.  In
	 * particular, BT_ADDR_LE_UNRESOLVED and BT_ADDR_LE_ANONYMOUS are
	 * transient HCI values and are not valid settings keys.  Upstream used
	 * an assertion in hex2char() below, turning a bad caller/address into a
	 * fatal bt_work oops.  Reject it at the API boundary instead so pairing
	 * can report the failed store without taking down the complete host.
	 */
	if (addr->type > BT_ADDR_LE_RANDOM_ID) {
		LOG_ERR("Refusing settings key for invalid address type 0x%02x (%s)",
			addr->type, subsys);
		return -EINVAL;
	}
	...
	/* Address type was validated above. */
...
static int commit_settings(void)
{
	...
	/* The IPRO controller derives a per-chip development public address when
	 * eFuse contains an invalid/multicast MAC.  Older firmware used one fixed
	 * fallback address, which may still be present in bt/id after an upgrade.
	 * Keeping that stale host identity makes SMP f5/f6 use a responder address
	 * different from the address used over the air; Numeric Comparison then
	 * fails at DHKey Check even though ECDH itself is correct.
	 *
	 * Public controller addresses are immutable during a boot.  Migrate only a
	 * persisted public identity (never an application-preset/random identity),
	 * remove bonds tied to the obsolete local identity, and let the normal
	 * setup/store path recreate ID + IRK from HCI Read BD_ADDR.
	 */
	if (bt_dev.id_count != 0U &&
	    !atomic_test_bit(bt_dev.flags, BT_DEV_PRESET_ID) &&
	    bt_dev.id_addr[BT_ID_DEFAULT].type == BT_ADDR_LE_PUBLIC) {
		bt_addr_le_t controller_addr;

		if (bt_id_read_public_addr(&controller_addr) != 0U &&
		    !bt_addr_le_eq(&controller_addr,
				    &bt_dev.id_addr[BT_ID_DEFAULT])) {
			char stored_str[BT_ADDR_LE_STR_LEN];
			char controller_str[BT_ADDR_LE_STR_LEN];

			bt_addr_le_to_str(&bt_dev.id_addr[BT_ID_DEFAULT], stored_str,
					  sizeof(stored_str));
			bt_addr_le_to_str(&controller_addr, controller_str,
					  sizeof(controller_str));
			LOG_WRN("Stored public identity %s differs from controller %s; migrating",
				stored_str, controller_str);
			(void)bt_unpair(BT_ID_DEFAULT, NULL);
			(void)bt_settings_delete_id();
			(void)bt_settings_delete_irk();
