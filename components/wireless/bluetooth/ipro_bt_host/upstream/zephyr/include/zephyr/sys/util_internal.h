...
/*
 * IPRO SDK compatibility: the pre-existing CONFIG_BT_CONN setting stores
 * the controller capacity (1, 2, or 4), while Zephyr models the same symbol
 * as a boolean and feeds it to IS_ENABLED()/COND_CODE_1().  Treat every
 * supported non-zero IPRO capacity as enabled; CONFIG_BT_MAX_CONN continues
 * to carry the actual Host array size.
 */
#define _XXXX2 _YYYY,
#define _XXXX4 _YYYY,
