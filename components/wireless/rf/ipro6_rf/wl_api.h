...
/* Canonical retention-memory address for the wl cfg/cal context. On IPRO6
 * this MUST be the managed HBN-RAM WLCAL region (HAL_HBNRAM_WLCAL_ADDR,
 * 0x20090B00): the legacy 0x20010600 is not writable on silicon (cal results
 * silently dropped -> LO never locks) and stale per-file fallbacks to it
 * (e.g. tpc.c) bound wl_cfg to garbage. Define it HERE, once, so every
 * wl_cfg_get() caller resolves the same address. */
#if WL_API_RMEM_EN && !defined(WL_API_RMEM_ADDR)
  #if defined(CONFIG_IPRO6) || defined(IPRO6)
    /* Cannot include ipro6.h here (RF_BASE view conflict); pre-seed the HBN
     * RAM base and let the HAL header compute the managed offset. */
    #ifndef HBN_RAM_BASE
...
uint8_t wl_rf_txcal_temp_optimize(int16_t temperature, uint8_t reset); 
/*
* rf driver api end
*/
