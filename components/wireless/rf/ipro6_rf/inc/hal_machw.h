...
enum
{
    ...
    #if NX_MAC_HE
    /// HE TB TX timeout bit
    HAL_HE_TB_TIMER_BIT = CO_BIT(HAL_HE_TB_TIMER),
    #endif
 ...
 *
 * @param[in] rhd RX desciptor
 * @param[in] len Length to consider
 *
 * @return air time used to transmit @p len bytes of the received frame.
 ****************************************************************************************
 */
__INLINE uint32_t hal_machw_rx_bcn_duration(struct rx_hd *rhd, uint16_t len)
{
    // Ensure that received frame is of type NON_HT
    if (hal_desc_get_rx_format(&rhd->rx_vec_1) != 0)
        return 500;

    // Fill-in the TimeOnAir parameter registers
    nxmac_ppdu_mcs_index_setf(rxv2macrate[hal_desc_get_legacy_rate(&rhd->rx_vec_1)]);
    #if (NX_MAC_VER >= 20)
    nxmac_time_on_air_param_1_pack(0, 0, 0, 0, hal_desc_get_preamble_type(&rhd->rx_vec_1), len);
    #else
    nxmac_time_on_air_param_1_pack(0, 0, 0, hal_desc_get_preamble_type(&rhd->rx_vec_1), 0, len);
    #endif // (NX_MAC_VER >= 20)
...
/**
 ****************************************************************************************
 * @brief Check whether HE is supported by the MAC and PHY HW.
 *
 * @return true if supported, false otherwise
 ****************************************************************************************
 */
__INLINE bool hal_machw_he_support(void)
{
    ...
    return false;
    #else
    return (nxmac_he_getf() && phy_he_supported()) != 0;
    #endif
}
...
/**
 ****************************************************************************************
 * @brief Apply an offset (positive or negative) to the TSF timer.
 * A consequence of the TSF update will be a move of the TBTT. Note that this function
 * should be called while the MAC HW is in IDLE state.
 *
 * @param[in] offset Offset to apply to the TSF
 *
 * @return 0 if the offset is applied, different from 0 otherwise.
 ****************************************************************************************
 */
 ...
 ****************************************************************************************
 * @brief Read TSF
 * @return atomically read TSF
 ****************************************************************************************
 */
uint64_t hal_machw_tsf_read(void);
