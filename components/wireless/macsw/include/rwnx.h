...
/**
 ****************************************************************************************
 * @brief Re-arm the MAC HW timers from the SW queues after a PDS wake.
 *
 * hal_machw_init()'s soft reset returns absTimerReg[*] and TIMERS_INT_UN_MASK to 0
 * while ke_env.queue_timer / mm_timer_env.prog survive PDS in RAM. The static
 * *_hw_set() helpers only fire when a queue head CHANGES, so a timer already
 * pending across the sleep is left armed in SW and dead in HW and never fires.
 * Re-programs both from their queue heads. Call immediately after
 * @ref rwnxl_ps_init. See the definition for why this deliberately does NOT
 * rebase MONOTONIC_COUNTER_2.
 ****************************************************************************************
 */
void rwnxl_restore_mactimer(void);
#endif
...
/**
 ****************************************************************************************
 * @brief This function show reason of why not sleep.
 *
 ****************************************************************************************
 */
void rwnxl_show_not_sleep_reason(void);
 ...
 ****************************************************************************************
 */
int rwnxl_connected_enter_ops(void);

/**
 ****************************************************************************************
 * @brief This function performs save mac critical registers.
 *
 * This function must be called at least 1 time before entering low power.
 ****************************************************************************************
*/
void rwnxl_regs_save_ops(void);
 ...
 *
 * This function must followed by vTaskStepTick.
 * Must and can only be called once after exit low power.
 ****************************************************************************************
 */
