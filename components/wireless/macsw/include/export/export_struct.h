...
struct_func_dcl(sm_status_ind, uint8_t, vif_idx);
...
struct_func_dcl(scanu_start_req, uint32_t, probe_cnt);
...
void *scan_raw_send_req_alloc(void);
...
struct_func_dcl(mm_key_add_req, uint16_t, sta_idx);
...
void *me_set_control_port_cfm_alloc(void);
...
void *me_rc_set_rate_req_alloc(void);
...
#ifdef CFG_LPM
struct_func_dcl(me_config_req, uint8_t, chip_version);
#endif
...
struct_func_dcl(mm_csa_finish_ind, uint8_t, vif_index);
struct_func_dcl(mm_csa_finish_ind, uint8_t, status);
struct_func_dcl(mm_csa_finish_ind, uint8_t, chan_idx);
struct_func_len(mm_csa_finish_ind);
