 ...
 ****************************************************************************************
 */
static inline int phy_freq_to_channel(uint8_t band, uint16_t freq)
{
    ...
    else if ((band == PHY_BAND_5G) && (freq >= 5005) && (freq <= 5885))
    {
...
/**
 ****************************************************************************************
 * @brief Get the index (0..7) of the primary 20MHz channel.
 *
 * The full inline definition lives in the PLF driver phy.h. That header shares the
 * _PHY_H_ include guard with this public header, so TUs that include this file (e.g.
 * via mac.h) never see the inline body. Declare the prototype here so GCC 14 sees the
 * correct unsigned-int return type instead of an implicit int (-Werror). Behaviour is
 * unchanged: PLF-side TUs still inline it; this is purely a declaration for callers.
 ****************************************************************************************
 */
unsigned int phy_primary_index_get(uint16_t freq, uint16_t freq1, uint16_t freq2,
                                   enum mac_chan_bandwidth chantype);
