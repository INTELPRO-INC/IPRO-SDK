...
/**
 ****************************************************************************************
 * @brief Freeze the WiFi task across a PDS sleep/wake.
 *
 * Between PDS entry and the end of rwnxl_resume_wifi() the MAC HW / rxl SW state
 * is inconsistent (the lp_fw blob runs its own beacon RX over the same ring).
 * While frozen, @ref rtos_wifi_task_resume is a no-op, so a stray notification
 * (kmsg push, timer, ISR) cannot let the task process stale RX descriptors.
 * rwnxl_resume_wifi() lifts the freeze once the datapath is re-programmed.
 ****************************************************************************************
 */
void rtos_wifi_task_freeze(void);
void rtos_wifi_task_unfreeze(void);
#endif
