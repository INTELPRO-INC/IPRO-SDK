...
#else
/* Phase 2A.13 Step 2.2: raised from 27 to 28 (above fhost_wifi=27)
 * so rtos_queue_write to queue_desc preempts wifi_task immediately
 * instead of waiting for round-robin tick. Counter L (a2) qdwell was
 * 8.15ms avg with both at 27. */
const int fhost_rx_priority = RTOS_TASK_PRIORITY(28);
#endif
...
{
    BaseType_t res;
    /* Use the native FreeRTOS handle type so &handle matches xTaskCreate's
     * TaskHandle_t* parameter exactly; it converts cleanly to the void*
     * rtos_task_handle on assignment below. */
    TaskHandle_t handle;
...
{
    return xPortIsInsideInterrupt() ? true : false;
}
...
int rtos_mutex_lock_timeout(rtos_mutex mutex, int timeout_ms)
{
    return (xSemaphoreTake(mutex, pdMS_TO_TICKS(timeout_ms)) != pdTRUE) ? 1 : 0;
}
