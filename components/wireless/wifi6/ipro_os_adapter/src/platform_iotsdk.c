...
/* WiFi event base for ipro_event */
...
{
    /* Forward-declared: including the driver headers drags in platform_common.h,
     * whose platform_get_random() prototype conflicts with platform_al.h in
     * this TU. */
    extern int mfg_media_read_macaddr_with_lock(uint8_t mac[6], uint8_t reload);
    extern int Sec_Eng_Trng_Enable(void);
    extern int Sec_Eng_Trng_Get_Random(uint8_t *data, uint32_t len);

    /* Production parts carry a unique factory MAC in efuse/mfg media. */
    if (mfg_media_read_macaddr_with_lock(mac, 1) == 0)
        return 0;

    /* Blank efuse (bench parts): the old code hardcoded 12:23:34:00:45:56 so
     * every DUT booted the same L2 address and collided. The upper-layer random
     * fallback (_mac_use_ramdom) is also useless here because
     * platform_get_random() is an empty stub -> it yields 02:00:00:00:00:00.
     * Pull a unique locally-administered MAC straight from the TRNG instead. */
    Sec_Eng_Trng_Enable();
    if (Sec_Eng_Trng_Get_Random(mac, 6) != 0)
        return -1;                       /* TRNG unavailable -> caller fallback */
...
{
    (void)catalogue;
    /* Post WiFi event using ipro_event system */
    ipro_event_post(WIFI_EVENT, code1, &code2, sizeof(code2), 0);
}
...
platform_event_handler_t platform_add_schedule_event(platform_event_func_t handler, void *arg, uint32_t ms, uint8_t repeat)
{
    ...
    /* platform_event_func_t and TimerCallbackFunction_t are ABI-identical
     * (both void(*)(handle)); cast to satisfy the strict pointer-type check.
     * FreeRTOS invokes the handler with the TimerHandle_t as before. */
    timer_handle = xTimerCreate("schedule_event", pdMS_TO_TICKS(ms), repeat, (void*)0,
