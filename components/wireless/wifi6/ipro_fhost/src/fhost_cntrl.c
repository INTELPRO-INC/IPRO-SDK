...
rtos_task_handle fhost_cntrl_task_handle;

rtos_mutex fhost_cntrl_link_mutex;
...
/// 802.11 status codes, as sm_connect_ind() reports them. A DIFFERENT space from
/// the WLAN_FW_* list above and it overlaps it -- MAC_ST_FAILURE and
/// WLAN_FW_TX_AUTH_FRAME_ALLOCATE_FAIILURE are both 1, so decoding one through
/// the other turns a plain connect failure into an invented frame-allocation
/// error. Only the codes the FW can actually deliver are listed; anything else
/// falls through to the raw value.
static const struct reason_code mac_status_list[] = {
    {MAC_ST_SUCCESSFUL, "MAC_ST_SUCCESSFUL"},
    {MAC_ST_FAILURE, "MAC_ST_FAILURE (unspecified)"},
    {MAC_ST_CAPA_NOT_SUPPORTED, "MAC_ST_CAPA_NOT_SUPPORTED"},
    {MAC_ST_REASSOC_NOT_ASSOC, "MAC_ST_REASSOC_NOT_ASSOC"},
    {MAC_ST_ASSOC_DENIED, "MAC_ST_ASSOC_DENIED"},
    {MAC_ST_AUTH_ALGO_NOT_SUPPORTED, "MAC_ST_AUTH_ALGO_NOT_SUPPORTED"},
    {MAC_ST_AUTH_FRAME_WRONG_SEQ, "MAC_ST_AUTH_FRAME_WRONG_SEQ"},
    {MAC_ST_AUTH_CHALLENGE_FAILED, "MAC_ST_AUTH_CHALLENGE_FAILED"},
    {MAC_ST_AUTH_TIMEOUT, "MAC_ST_AUTH_TIMEOUT"},
    {MAC_ST_ASSOC_TOO_MANY_STA, "MAC_ST_ASSOC_TOO_MANY_STA"},
    {MAC_ST_ASSOC_RATES_NOT_SUPPORTED, "MAC_ST_ASSOC_RATES_NOT_SUPPORTED"},
    {MAC_ST_REFUSED_TEMPORARILY, "MAC_ST_REFUSED_TEMPORARILY"},
    {MAC_ST_INVALID_MFP_POLICY, "MAC_ST_INVALID_MFP_POLICY"},
    {MAC_ST_INVALID_IE, "MAC_ST_INVALID_IE"},
    {MAC_ST_GROUP_CIPHER_INVALID, "MAC_ST_GROUP_CIPHER_INVALID"},
    {MAC_ST_PAIRWISE_CIPHER_INVALID, "MAC_ST_PAIRWISE_CIPHER_INVALID"},
    {MAC_ST_AKMP_INVALID, "MAC_ST_AKMP_INVALID"},
    {MAC_ST_UNSUPPORTED_RSNE_VERSION, "MAC_ST_UNSUPPORTED_RSNE_VERSION"},
    {MAC_ST_INVALID_RSNE_CAPA, "MAC_ST_INVALID_RSNE_CAPA"},
    {MAC_ST_CIPHER_SUITE_REJECTED, "MAC_ST_CIPHER_SUITE_REJECTED"},
};

/// 802.11 reason codes, which is what a disconnection indication actually
/// carries -- its status_code field was removed in release v6.41.2 and the
/// accessor is a stub returning 0.
static const struct reason_code mac_reason_list[] = {
    {MAC_RS_RESERVED, "local/none"},
    {MAC_RS_UNSPECIFIED, "MAC_RS_UNSPECIFIED"},
    {MAC_RS_PREVIOUS_AUTH_NOT_VALID, "MAC_RS_PREVIOUS_AUTH_NOT_VALID"},
    {MAC_RS_DEAUTH_SENDER_LEFT_IBSS_ESS, "MAC_RS_DEAUTH_SENDER_LEFT_IBSS_ESS"},
    {MAC_RS_DIASSOC_INACTIVITY, "MAC_RS_DIASSOC_INACTIVITY"},
    {MAC_RS_DIASSOC_TOO_MANY_STA, "MAC_RS_DIASSOC_TOO_MANY_STA"},
    {MAC_RS_CLASS_2_FORBIDDEN, "MAC_RS_CLASS_2_FORBIDDEN"},
    {MAC_RS_CLASS_3_FORBIDDEN, "MAC_RS_CLASS_3_FORBIDDEN"},
    {MAC_RS_DEAUTH_SENDER_LEFT_BSS, "MAC_RS_DEAUTH_SENDER_LEFT_BSS"},
    {MAC_RS_STA_REQ_ASSOC_NOT_AUTH, "MAC_RS_STA_REQ_ASSOC_NOT_AUTH"},
    {MAC_RS_INVALID_IE, "MAC_RS_INVALID_IE"},
    {MAC_RS_MIC_FAILURE, "MAC_RS_MIC_FAILURE"},
    {MAC_RS_4WAY_HANDSHAKE_TIMEOUT, "MAC_RS_4WAY_HANDSHAKE_TIMEOUT"},
    {MAC_RS_GROUP_KEY_UPDATE_TIMEOUT, "MAC_RS_GROUP_KEY_UPDATE_TIMEOUT"},
    {MAC_RS_BAD_RSNIE, "MAC_RS_BAD_RSNIE"},
    {MAC_RS_GROUP_CIPHER_INVALID, "MAC_RS_GROUP_CIPHER_INVALID"},
    {MAC_RS_PAIRWISE_CIPHER_INVALID, "MAC_RS_PAIRWISE_CIPHER_INVALID"},
    {MAC_RS_AKMP_INVALID, "MAC_RS_AKMP_INVALID"},
    {MAC_RS_802_1X_AUTH_FAIL, "MAC_RS_802_1X_AUTH_FAIL"},
    {MAC_RS_CIPHER_SUITE_REJECTED, "MAC_RS_CIPHER_SUITE_REJECTED"},
    {MAC_RS_TIMEOUT, "MAC_RS_TIMEOUT"},
};

/// Which enumeration a connect-indication status code belongs to. The value
/// travels as a bare uint16_t and the two spaces collide, so the caller has to
/// say which one it is holding -- there is nothing in the value itself to tell.
enum fhost_status_space
{
    /// 802.11 status code, straight from the FW's sm_connect_ind()
    FHOST_STATUS_MAC_ST,
    /// this driver's own WLAN_FW_* action codes, raised locally
    FHOST_STATUS_WLAN_FW,
};
...
static const char* _get_mac_status_str(uint16_t status_code)
{
    return _get_status_code_str(mac_status_list, sizeof(mac_status_list)/sizeof(mac_status_list[0]), status_code);
}

static const char* _get_mac_reason_str(uint16_t reason_code)
{
    return _get_status_code_str(mac_reason_list, sizeof(mac_reason_list)/sizeof(mac_reason_list[0]), reason_code);
}

/// Decode a connect-indication status in the space its caller says it is in.
static const char* _get_connect_status_str(enum fhost_status_space space, uint16_t status_code)
{
    return (space == FHOST_STATUS_MAC_ST) ? _get_mac_status_str(status_code)
...
static int find_ie_ssid(uint8_t *buffer, int len, uint8_t *result, int *ssid_len)
{
    ...
    while (ie->payload < buffer_end && ie->payload + ie->len <= buffer_end) {
#define IE_ID_SSID 0x00
        if (IE_ID_SSID == ie->type && ie->len > 32) {
            return -1;
        } else if (IE_ID_SSID == ie->type) {
            *ssid_len = ie->len;
            memcpy(result, ie->payload, ie->len);
            result[ie->len] = '\0';
            return 0;
        } else {
            /* move to next ie */
            ie = (struct info_element *)(ie->payload + ie->len);
        }
    }
...
#define IE_ID_DS_CHANNEL 0x03
        if (IE_ID_DS_CHANNEL == ie->type && ie->len != 1) {
            return -1;
        } else if (IE_ID_DS_CHANNEL == ie->type) {
            *result = ie->payload[0];
            return 0;
        } else {
            /* move to next ie */
...
static void _rx_handle_multiple_bssid_beacon_save(uint32_t ies, uint32_t ies_len,
                                                  wifi_mgmr_scan_item_t* scan_item)
{
    while (ies_len)
    {
        ...
        max_bssid_ind = co_read8p(mbssid_ie_addr + MAC_MULTI_BSSID_MAX_INDICATOR_OFT);
        ...
        while (subies_len)
        {
            ...
            if (bssid_ies_len > ((bssid_index_ie_addr ? 4 : 0) +
                                 ...
                                 (capa_addr ? 3 : 0)))
            {
                printf("Multiple BSSID has more elements, not supported!\n\r");
            }
            ...
            if (bssid_index_ie_addr && ssid_ie_addr)
            {
                ...
                scan_item->bssid[5] &= ~(CO_BIT(max_bssid_ind)-1);
                scan_item->bssid[5] |= (scan_item->bssid[5] + bssid_index) & (CO_BIT(max_bssid_ind)-1);
                // Get SSID
                memcpy(scan_item->ssid, (void *)(ssid_ie_addr + MAC_SSID_SSID_OFT), ssid_len);
                if (ssid_len < 32) {
                    scan_item->ssid[ssid_len] = 0;
                } else {
                    scan_item->ssid_tail[0] = 0;
                }
                // Save Beacon
...
static void _rx_handle_beacon(void *ind, struct ieee80211_mgmt *mgmt)
{
    ...
    scan_item.auth = WIFI_EVENT_BEACON_IND_AUTH_UNKNOWN;
    ...
    if (-1 == find_ie_ds(mgmt->u.beacon.variable, ind_length, &scan_item.channel)) {
        scan_item.channel = phy_freq_to_channel(rxu_mgt_ind_get_band(ind), rxu_mgt_ind_get_center_freq(ind));
    }
...
uint32_t mac_ext_ie_find(uint32_t addr,
                         uint16_t buflen,
                         uint8_t ext_ie_id,
                         uint16_t *len);
    if (mac_ext_ie_find(var_part_addr, var_part_len, MAC_ELTID_EXT_HE_CAPA, &len))
    {
        scan_item.mode = (WIFI_MODE_802_11B | WIFI_MODE_802_11G | WIFI_MODE_802_11N_2_4 | WIFI_MODE_802_11AX_2_4);
    }
    else if (mac_ie_find(var_part_addr, var_part_len, MAC_ELTID_HT_CAPA, &len))
    {
    ...
    if (WLAN_CAPABILITY_PRIVACY & (le16_to_cpu(mgmt->u.beacon.capab_info))) {
        ...
        for (i = 0; i < parsed_wpa_ie_len; ++i) {
            ...
            if (ie->proto == WPA_PROTO_WPA) {
                ...
                } else if (ie->key_mgmt & WPA_KEY_MGMT_IEEE8021X) {
                    scan_item.auth = WIFI_EVENT_BEACON_IND_AUTH_WPA_ENT;
                }
                ...
                } else if (ie->key_mgmt & (WPA_KEY_MGMT_IEEE8021X | WPA_KEY_MGMT_IEEE8021X_SHA256)) {
                    scan_item.auth = WIFI_EVENT_BEACON_IND_AUTH_WPA_ENT;
                }
            }
        ...
        if (parsed_wpa_ie_len == sizeof(parsed_wpa_ie) / sizeof(parsed_wpa_ie[0])) {
            if (scan_item.auth != WIFI_EVENT_BEACON_IND_AUTH_WPA_ENT) {
     ...
     */
    _rx_handle_multiple_bssid_beacon_save(var_part_addr, var_part_len, &scan_item);
}
                                              ...
                                              ke_task_id_t const src_id)
{
    ...
    if (wireless_80211_is_beacon(mgmt->frame_control)) {
        _rx_handle_beacon(param, mgmt);
    } else if (wireless_80211_is_probe_resp(mgmt->frame_control)) {
        _rx_handle_probersp(param, mgmt);
    } else {
        fhost_printf("Bug Scan IND?\r\n");
    }

    // fhost_printf("    --> fhost cntrl rssi %d\r\n", param->rssi);
    ...
    if (-1 == find_ie_ds(mgmt->u.beacon.variable, rxu_mgt_ind_get_length(param), &channel)) {
        result.freq = rxu_mgt_ind_get_center_freq(param);
    } else {
        result.freq = phy_channel_to_freq(rxu_mgt_ind_get_band(param), channel);
    }
    ...
    if (!fhost_vif->is_cntrl_link) {
        if (fhost_cntrl_cfgrwnx_event_send(&result.hdr, fhost_vif->scan_sock, 1))
...
static int fhost_cntrl_ke_msg_scan_result_update(ke_msg_id_t const msgid,
                                              void *param,
                                              ke_task_id_t const dest_id,
                                              ke_task_id_t const src_id)
{
                                       ...
                                       ke_task_id_t const src_id)
{
    ...
    if (!fhost_vif->is_cntrl_link) {
        fhost_cntrl_cfgrwnx_event_send(&resp.hdr, fhost_vif->scan_sock, 0);
    }
    ...
    if (fhost_vif->is_cntrl_link) {
        platform_post_event(EV_WIFI, CODE_WIFI_ON_SCAN_DONE, 0);
    } else {
        platform_post_event(EV_WIFI, CODE_WIFI_ON_SCAN_DONE_CONNECTING, 0);
    }
...
static void fhost_cntrl_connect_ind_dump(enum fhost_status_space space,
                                         uint16_t status_code, uint16_t ieeetypes_code)
{
    ...
    fhost_printf("=================================================================\r\n");
    fhost_printf("[AT][RX] Connection Status\r\n");
    fhost_printf("[AT][RX]   status_code %u (%s)\r\n", status_code,
                 (space == FHOST_STATUS_MAC_ST) ? "802.11" : "WLAN_FW");
    fhost_printf("[AT][RX]   reason_code %u\r\n", ieeetypes_code);
    fhost_printf("[AT][RX]   status detail: %s\r\n",
                 _get_connect_status_str(space, status_code));
    if (status_code == 0) {
        fhost_printf("[AT][RX]   MAC %02X:%02X:%02X:%02X:%02X:%02X\r\n",
        ...
        fhost_printf("[AT][RX]   vif_idx %u\r\n", connection_info->vif_idx);
        fhost_printf("[AT][RX]   ap_idx %u\r\n", connection_info->ap_idx);
        fhost_printf("[AT][RX]   ch_idx %u\r\n", connection_info->ch_idx);
        fhost_printf("[AT][RX]   qos %u\r\n", connection_info->qos);
        fhost_printf("[AT][RX]   aid %u\r\n", connection_info->aid);
        fhost_printf("[AT][RX]   band %s\r\n", (connection_info->chan_band) ? "5G" : "2.4G");
        fhost_printf("[AT][RX]   channel %d\r\n", connection_info->channel);
    }
    fhost_printf("=================================================================\r\n");
                                          ...
                                          ke_task_id_t const src_id)
{
    ...
    {
        /* Bring up everything the 4-way handshake needs BEFORE anyone is told
         * the connection exists. The CONNECT event wakes the supplicant, the AP
         * fires EAPOL M1 right behind the assoc response, and the supplicant's
         * M2 goes out through net_l2_send() -- which refuses a down netif and a
         * missing TX STA entry. With this block after the event send, every
         * reconnect after a netif_set_down lost that race deterministically:
         * M2 failed in ~20 ms, wpa_sm deauthed (reason 1, then mislabelled
         * "pre-shared key may be incorrect"), and the loop repeated until a
         * reboot. A cold boot never saw it only because the netif starts up. */
        fhost_vif->ap_id = sm_connect_ind_get_ap_idx(param);
        fhost_vif->acm = sm_connect_ind_get_acm(param);

        if (fhost_vif->acm)
        {
            fhost_printf("admission control!!!\n");
        }
    ...
    if (sm_connect_ind_get_status_code(param)) {
        //connect fail
        // The FW fills this from sm_connect_ind(), which carries 802.11 status
        // codes only -- never a WLAN_FW_* one.
        fhost_cntrl_connect_ind_dump(FHOST_STATUS_MAC_ST,
                                     sm_connect_ind_get_status_code(param),
                                     sm_connect_ind_get_ieeetypes_code(param));
    } else {
        // connect success
        platform_post_event(EV_WIFI, CODE_WIFI_ON_CONNECTED, 0);
    }
    // Save connect info regardless of success/fail
    if (sm_connect_ind_get_status_code(param) == 0) {
        printf("[WIFI] ASSOC_OK vif=%d\r\n", sm_connect_ind_get_vif_idx(param));
    }
    ...
    // Use status_code directly — ieeetypes_code is a stub returning 0 in macsw,
    // which would make failures look like success to WPA supplicant
    event.status_code = sm_connect_ind_get_status_code(param);
    ...
    printf("CONN_EVT: sc=%d freq=%d sta=%d req_ie=%d rsp_ie=%d\r\n",
           event.status_code, event.freq, event.sta_idx,
           event.assoc_req_ie_len, event.assoc_resp_ie_len);
    ...
    if (fhost_cntrl_cfgrwnx_event_send(&event.hdr, fhost_vif->conn_sock, 0) &&
        event.req_resp_ies)
    {
        rtos_free(event.req_resp_ies);
        return 0;
    }
    /* Success-path bring-up (tx_sta_add + net_if_up_cb) happens BEFORE the
     * event sends above -- see the comment at the top of this handler. */
    return 0;
}
                                                ...
                                                ke_task_id_t const src_id)
{
    ...
    // A disconnection indication has no status code at all -- the field was
    // dropped in release v6.41.2 and sm_disconnect_ind_get_status_code() is a
    // stub returning 0, so printing it only ever said "success". The reason
    // code is the real payload: an 802.11 MAC_RS_*, or 0 when we tore the link
    // down ourselves.
    fhost_printf("=================================================================\r\n");
    fhost_printf("[AT][RX] Connection Status\r\n");
    fhost_printf("[AT][RX]   reason_code %u\r\n", sm_disconnect_ind_get_reason_code(param));
    fhost_printf("[AT][RX]   reason detail: %s\r\n",
                 _get_mac_reason_str(sm_disconnect_ind_get_reason_code(param)));
    ...
    fhost_cntrl_cfgrwnx_event_send(&event.hdr, fhost_vif->conn_sock, 0);
    ...
    if (!conn_flag)
    {
        ...
        platform_post_event(EV_WIFI, CODE_WIFI_ON_DISCONNECT, 0);
        /* Bench 2026-07-25: the link must come back by itself. Nothing else
         * re-armed it, so one AP deauth left the SDIO/USB bridge's iproeth0
         * carrier-down forever. Arms a backoff in a transient task - never
         * connect from this handler. */
                                              ...
                                              ke_task_id_t const src_id)
{
    ...
    fhost_cntrl_cfgrwnx_event_send(&event.hdr, fhost_vif->conn_sock, 1);
                                           ...
                                           ke_task_id_t const src_id)
{
    ...
    fhost_cntrl_cfgrwnx_event_send(&resp.hdr, fhost_vif->ftm_sock, 0);
...
static int fhost_cntrl_ke_msg_sm_connecting_status_ind(ke_msg_id_t const msgid,
                                           void *param,
                                           ke_task_id_t const dest_id,
                                           ke_task_id_t const src_id)
{
    ...
    max_i = sm_status_ind_get_array_size(param);
    for (i = 0; i < max_i; i++) {
        switch (sm_status_ind_get_array_id(param, i)) {
            case SM_STATUS_IND_ID_ASSOC_REFUSHED_TEMP:
            {
                fhost_printf("Status ind SM_STATUS_IND_ID_ASSOC_REFUSHED_TEMP: %u\r\n", sm_status_ind_get_array_val(param, i));
            }
            break;
            default:
            {
...
static int fhost_cntrl_ke_msg_csa_finish_ind(ke_msg_id_t const msgid,
                                            void *param,
                                            ke_task_id_t const dest_id,
                                            ke_task_id_t const src_id)
{
    struct fhost_vif_tag *fhost_vif = fhost_from_mac_vif(mm_csa_finish_ind_get_vif_index(param));
    uint8_t vif_idx = mm_csa_finish_ind_get_vif_index(param);
    uint8_t res = mm_csa_finish_ind_get_status(param);

    if (res) {
        fhost_printf("MM_CSA_FINISH_IND status failed\r\n");
        return 0;
    }
    switch (mac_vif_get_type(fhost_vif->mac_vif)) {
        case VIF_STA:
        {
            int sta_chan = 0;
            /* wifi_mgmr_sta_channel_get() writes a full int; the struct field is
             * uint8_t, so go through a local int to avoid a 4-byte write into a
             * 1-byte field (latent struct corruption, GCC 14 -Werror). */
            wifi_mgmr_sta_channel_get(&sta_chan);
            wifiMgmr.wifi_mgmr_stat_info.channel = (uint8_t)sta_chan;
            fhost_printf("MM_CSA_FINISH_IND, vif_idx:%u chan:%u\r\n", vif_idx, wifiMgmr.wifi_mgmr_stat_info.channel);
        }
        break;
        case VIF_AP:
        {
...
static const struct ke_msg_handler fhost_cntrl_ke_msg_handlers[] =
{
    ...
    {SCANU_RESULT_UPDATE, (ke_msg_func_t)fhost_cntrl_ke_msg_scan_result_update},
    ...
    {SM_STATUS_IND, (ke_msg_func_t)fhost_cntrl_ke_msg_sm_connecting_status_ind},
    {MM_CSA_FINISH_IND, (ke_msg_func_t)fhost_cntrl_ke_msg_csa_finish_ind},
    #if NX_BEACONING
...
static void fhost_cntrl_cfgrwnx_hw_feature(void *msg)
{
    ...
    feat.ap_11b_only = IS_11B_ONLY(wifiMgmr.ap_mode);
    fhost_cntrl_cfgrwnx_resp_send(&feat, feat.hdr.len, cmd->hdr.resp_queue);
}

/**
 ****************************************************************************************
 * @brief Process @ref CFGRWNX_ME_PARAM_CMD message
 *
 * @param[in] msg Command header
 ****************************************************************************************
 */
static void fhost_cntrl_cfgrwnx_me_param(void *msg)
{
    struct cfgrwnx_me_param *cmd = msg;
    struct cfgrwnx_me_param_resp feat;
    ...
    if (CFGRWNX_ME_PARAM_CMD_GET == cmd->cmd) {
        if (CFGRWNX_ME_PARAM_ID_TX_AMPDU_RETRY_CNT_LIMIT == cmd->id) {
            req.id = ME_PARAM_ID_TX_AMPDU_RETRY_CNT_LIMIT;
            req.cmd = ME_PARAM_CMD_GET;
            macif_kmsg_push(ME_PARAM_REQ, TASK_ME, &req, sizeof(req));
            fhost_cntrl_ke_msg_wait(ME_PARAM_CFM, &cfm, sizeof(cfm));
            memcpy(&feat.data, cfm.value, sizeof(uint32_t));                           //param_tx_ampdu_retry_cnt_limit
        } else if (CFGRWNX_ME_PARAM_ID_TX_AMPDU_PROTECT_ENABLE == cmd->id) {
            req.id = ME_PARAM_ID_TX_AMPDU_PROTECT_ENABLE;
            req.cmd = ME_PARAM_CMD_GET;
            macif_kmsg_push(ME_PARAM_REQ, TASK_ME, &req, sizeof(req));
            fhost_cntrl_ke_msg_wait(ME_PARAM_CFM, &cfm, sizeof(cfm));
            memcpy(&feat.data, cfm.value, sizeof(uint32_t));                            //param_tx_ampdu_protect_enable
        } else if (CFGRWNX_ME_PARAM_ID_TX_AMPDU_DROP_TO_SINGLETON_RETRYCNT_THRESHOLD == cmd->id) {
            req.id = ME_PARAM_ID_TX_AMPDU_DROP_TO_SINGLETON_RETRYCNT_THRESHOLD;
            req.cmd = ME_PARAM_CMD_GET;
            macif_kmsg_push(ME_PARAM_REQ, TASK_ME, &req, sizeof(req));
            fhost_cntrl_ke_msg_wait(ME_PARAM_CFM, &cfm, sizeof(cfm));
            memcpy(&feat.data, cfm.value, sizeof(uint32_t));                            //param_tx_ampdu_drop_to_singleton_retrycnt_threshold
        } else {
            goto fhost_cntrl_cfgrwnx_me_param_err;
        }
    } else if (CFGRWNX_ME_PARAM_CMD_SET == cmd->cmd) {
        req.id = (enum ME_PARAM_ID_E)cmd->id;
        req.cmd = ME_PARAM_CMD_SET;
        memcpy(req.value, cmd->data, sizeof(uint32_t));
        macif_kmsg_push(ME_PARAM_REQ, TASK_ME, &req, sizeof(req));
        fhost_cntrl_ke_msg_wait(ME_PARAM_CFM, &cfm, sizeof(cfm));
        memcpy(&feat.data, cmd->data, sizeof(uint32_t));
    } else {
        goto fhost_cntrl_cfgrwnx_me_param_err;
    }

    // resp app
    feat.hdr.id = CFGRWNX_ME_PARAM_RESP;
    feat.hdr.len = sizeof(feat);
    feat.id = cmd->id;
    feat.cmd = cmd->cmd;
...
fhost_cntrl_cfgrwnx_me_param_err:
    fhost_printf("fhost_cntrl_cfgrwnx_me_param arg error\r\n");
}
...
static void fhost_cntrl_cfgrwnx_set_key(void *msg)
{
    ...
    if (cmd->addr && !MAC_ADDR_GROUP(cmd->addr))
    {
        ...
        /* The accessor returns uint8_t, so the 0x3FF sentinel arrives as 0xFF;
         * widen it back before any comparison. `sta_idx` is an int seeded with
         * the full-width constant and tested again further down, so normalise
         * here rather than special-casing each test. */
        if (sta_idx == (uint8_t)INVALID_STA_IDX)
...
static void fhost_cntrl_cfgrwnx_scan(void *msg)
{
    ...
    int i, nb_ssid = 0, probe_interval;
    ...
    probe_interval = (cmd->duration?cmd->duration:215)/
                     (cmd->probe_cnt?cmd->probe_cnt:2);
    ...
    if (cmd->freqs && NULL == cmd->chans) {
        ...
        {
            resp.status = CFGRWNX_INVALID_PARAM;
            fhost_printf("Not support channel\r\n");
            goto send_resp;
        }
        scanu_start_req_set_chan_cnt(req, nb_scan);
    } else {
    ...
    scanu_start_req_set_probe_cnt(req, cmd->probe_cnt);
...
static void fhost_cntrl_cfgrwnx_connect(void *msg)
{
    ...
    sm_connect_req_set_mode(req, wifiMgmr.sta_mode);
...
static void fhost_cntrl_cfgrwnx_ctrl_port(void *msg)
{
    ...
    /* INVALID_STA_IDX is 0x3FF but both accessors above return uint8_t, so the
     * sentinel arrives truncated to 0xFF and a uint8_t can never compare equal
     * to the full-width constant -- the guard below used to be dead code, and
     * a disconnect then pushed sta_idx = 0xFF to TASK_ME, whose handler indexes
     * sta_info_tab[] unchecked and wrote one byte 255 entries past the table,
     * into another task's stack (bench-proven by JTAG watchpoint 2026-08-05).
     * fhost_tx.c:562 already translates this truncation back (PM46); do the
     * same here. Check BEFORE filling the request so an invalid index can
    ...
    if(mac_vif && mac_vif_get_type(mac_vif) == VIF_STA && me_set_control_port_cfm_get_control_port_open(cfm))
        fhost_cntrl_connect_ind_dump(FHOST_STATUS_WLAN_FW, WLAN_FW_SUCCESSFUL, 0);
    resp.status = CFGRWNX_SUCCESS;
...
static void fhost_cntrl_cfgrwnx_external_auth_status(void *msg)
{
    ...
    resp.hdr.id = CFGRWNX_EXTERNAL_AUTH_STATUS_RESP_RESP;
    resp.hdr.len = sizeof(resp);
    resp.status = CFGRWNX_ERROR;
    ...
    if (macif_kmsg_push(SM_EXTERNAL_AUTH_REQUIRED_RSP, TASK_SM, rsp, sm_external_auth_required_rsp_len()))
        goto send_resp;

    resp.status = CFGRWNX_SUCCESS;

  send_resp:
    fhost_cntrl_cfgrwnx_resp_send(&resp, resp.hdr.len, status->hdr.resp_queue);
    rtos_free(rsp);
}
...
static void fhost_cntrl_cfgrwnx_sta_add(void *msg)
{
    ...
    if (!mac_vif || ((mac_vif_get_type(mac_vif) != VIF_AP) && (mac_vif_get_type(mac_vif) != VIF_MESH_POINT)))
        goto send_resp;
    if (wifiMgmr.num_sta >= (wifiMgmr.max_sta_supported > CFG_STA_MAX ? CFG_STA_MAX : wifiMgmr.max_sta_supported)) {
        fhost_printf("Number of sta is %u now\r\n", wifiMgmr.num_sta);
        goto send_resp;
    }
    ...
    wifi_mgmr_ap_sta_info_add(sta_idx, cmd->aid, cmd->addr);
    platform_post_event(EV_WIFI, CODE_WIFI_ON_AP_STA_ADD, sta_idx);
...
static void fhost_cntrl_cfgrwnx_sta_remove(void *msg)
{
    ...
    /* Same truncated sentinel as in fhost_cntrl_cfgrwnx_ctrl_port(): the
     * accessor returns uint8_t, so INVALID_STA_IDX (0x3FF) reads back as
     * 0xFF and the full-width comparison never fires. */
    if (sta_idx == (uint8_t)INVALID_STA_IDX)
        goto send_resp;
    ...
    platform_post_event(EV_WIFI, CODE_WIFI_ON_AP_STA_DEL, sta_idx);
  send_resp:
    fhost_cntrl_cfgrwnx_resp_send(&resp, resp.hdr.len, cmd->hdr.resp_queue);
    rtos_free(req);
}
...
static void fhost_cntrl_cfgrwnx_set_state(int vif_idx, const char *new_state_str)
{
    struct fhost_vif_tag *vif;
    vif = &fhost_env.vif[vif_idx];

    // Get raw state from WPA
    if (!strncmp(new_state_str, "DISCONNECTED", sizeof("DISCONNECTED")-1)) {
        vif->raw_state = FHOST_VIF_DISCONNECTED;
    } else if (!strncmp(new_state_str, "INACTIVE", sizeof("INACTIVE")-1)) {
        vif->raw_state = FHOST_VIF_INACTIVE;
    } else if (!strncmp(new_state_str, "INTERFACE-DISABLED", sizeof("INTERFACE-DISABLED")-1)) {
        vif->raw_state = FHOST_VIF_INTERFACE_DISABLED;
    } else if (!strncmp(new_state_str, "SCANNING", sizeof("SCANNING")-1)) {
        vif->raw_state = FHOST_VIF_SCANNING;
    } else if (!strncmp(new_state_str, "AUTHENTICATING", sizeof("AUTHENTICATING")-1)) {
        vif->raw_state = FHOST_VIF_AUTHENTICATING;
    } else if (!strncmp(new_state_str, "ASSOCIATING", sizeof("ASSOCIATING")-1)) {
        vif->raw_state = FHOST_VIF_ASSOCIATING;
    } else if (!strncmp(new_state_str, "ASSOCIATED", sizeof("ASSOCIATED")-1)) {
        vif->raw_state = FHOST_VIF_ASSOCIATED;
    } else if (!strncmp(new_state_str, "4WAY_HANDSHAKE", sizeof("4WAY_HANDSHAKE")-1)) {
        vif->raw_state = FHOST_VIF_4WAY_HANDSHAKE;
    } else if (!strncmp(new_state_str, "GROUP_HANDSHAKE", sizeof("GROUP_HANDSHAKE")-1)) {
        vif->raw_state = FHOST_VIF_GROUP_HANDSHAKE;
    } else if (!strncmp(new_state_str, "COMPLETED", sizeof("COMPLETED")-1)) {
        vif->raw_state = FHOST_VIF_COMPLETED;
    }

    // Set state
    switch (vif->raw_state)
    {
    case FHOST_VIF_DISCONNECTED:
    case FHOST_VIF_INACTIVE:
    case FHOST_VIF_INTERFACE_DISABLED:
        if (MGMR_VIF_STA == vif_idx) {
            vif->state = FHOST_STA_DISCONNECTED;
        }else{
            vif->state = FHOST_AP_DISABLED;
        }
        break;
    case FHOST_VIF_SCANNING:
        if (MGMR_VIF_STA == vif_idx) {
            vif->state = FHOST_STA_SCANNING;
        }else{
            vif->state = FHOST_AP_SCANNING;
        }
        break;
    case FHOST_VIF_AUTHENTICATING:
    case FHOST_VIF_ASSOCIATING:
    case FHOST_VIF_ASSOCIATED:
        if (MGMR_VIF_STA == vif_idx) {
            vif->state = FHOST_STA_CONNECTING;
        }
        break;
    case FHOST_VIF_4WAY_HANDSHAKE:
        if (MGMR_VIF_STA == vif_idx) {
            vif->state = FHOST_STA_4WAY_HANDSHAKE;
        }
        break;
    case FHOST_VIF_GROUP_HANDSHAKE:
        if (MGMR_VIF_STA == vif_idx) {
            if (FHOST_STA_CONNECTED != vif->state) {
                vif->state = FHOST_STA_GROUP_HANDSHAKE;
            }
        }
        break;
    case FHOST_VIF_COMPLETED:
        if (MGMR_VIF_STA == vif_idx) {
            vif->state = FHOST_STA_CONNECTED;
        }else{
            vif->state = FHOST_AP_ENABLED;
        }
        break;
    default:
        break;
...
#endif
    if (!strncmp(cmd->msg, "State###", sizeof("State###")-1)) {
        new_state_str = strrchr(cmd->msg, ' ');
        if (!new_state_str)
            return;

        // Move to next_state and check
        new_state_str += 1;
        if (strlen(new_state_str) <= 0)
            return;

        fhost_cntrl_cfgrwnx_set_state(cmd->fhost_vif_idx, new_state_str);
    } else if (!strncmp(cmd->msg, "CTRL-EVENT-NETWORK-NOT-FOUND", sizeof("CTRL-EVENT-NETWORK-NOT-FOUND")-1)) {
        fhost_cntrl_connect_ind_dump(FHOST_STATUS_WLAN_FW, WLAN_FW_SCAN_NO_BSSID_AND_CHANNEL, 0);
    }
}
...
static const struct fhost_cntrl_cfgrwnx_handler fhost_cntrl_cfgrwnx_msg_handlers[] =
{
    ...
    #endif // NX_BEACONING
    {CFGRWNX_ME_PARAM_CMD, fhost_cntrl_cfgrwnx_me_param},
...
static int fhost_cntrl_start(void)
{
    ...
    #ifdef CFG_LPM
    me_config_req_set_chip_version(me_req, me_config.chip_version);
    #endif
    ...
    mm_start_req_set_coex_mode(mm_req, fhost_get_coex_enable());
...
int fhost_cntrl_init(void)
{
    ...
    if (rtos_mutex_create(&fhost_cntrl_link_mutex)) {
        ASSERT_ERR(0);
    }
...
struct fhost_cntrl_link *fhost_cntrl_cfgrwnx_link_open(void)
{
    ...
    cfgrwnx_link[i].sock_recv = ELOOP_EVT_WPA_DRV_CTRL;
    cfgrwnx_link[i].sock_send = ELOOP_EVT_WPA_DRV_CTRL;
    return &cfgrwnx_link[i];
}

struct fhost_cntrl_link *fhost_cntrl_cfgrwnx_link_open_without_sock(void)
{
    unsigned int i = 0;
                                 ...
                                 struct cfgrwnx_msg_hdr *resp)
{
    ...
    if (resp) {
         rtos_mutex_lock(fhost_cntrl_link_mutex);
    }
    ...
    if (err)
    {
        ...
        if (resp) {
            rtos_mutex_unlock(fhost_cntrl_link_mutex);
        }
        return err;
    }
...
int fhost_cntrl_cfgrwnx_event_send(struct cfgrwnx_msg_hdr *msg_hdr, int sock, int discardable)
{
    int err;
    ...
    err = eloop_event_commit(sock, (const char *)msg_hdr, msg_hdr->len, discardable);
    if (err) {
...
int fhost_cntrl_cfgrwnx_event_peek_header(struct fhost_cntrl_link *link,
                                          struct cfgrwnx_msg_hdr *msg_hdr)
{
    if (eloop_rtos_sock_recv(link->sock_recv, msg_hdr,
                             sizeof(*msg_hdr), MSG_PEEK) < 0) {
        fhost_printf("failed to peek event header\r\n");
        return -1;
    }
    return 0;
}
...
int fhost_cntrl_cfgrwnx_event_get(struct fhost_cntrl_link *link, void *event, int len)
{
    ...
    read = eloop_rtos_sock_recv(link->sock_recv, event, len, MSG_DONTWAIT);
    if (read < 0) {
        fhost_printf("failed to receive an event\r\n");
        return -1;
    }
    return read;
}
                                      ...
                                      struct cfgrwnx_msg_hdr *msg_hdr)
{
    ...
    if (msg_hdr->id == CFGRWNX_SCAN_RESULT_EVENT)
    {
        ...
        if (eloop_rtos_sock_recv(link->sock_recv, &event, sizeof(event),
                                 MSG_DONTWAIT) < 0)
            return -1;
        if (event.payload)
    ...
    else if (eloop_rtos_sock_recv(link->sock_recv, msg_hdr,
                             sizeof(*msg_hdr), MSG_DONTWAIT) < 0) {
        return -1;
    }
