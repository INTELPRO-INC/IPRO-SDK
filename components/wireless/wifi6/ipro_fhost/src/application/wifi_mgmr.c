...
static int phy_freq_to_channel(uint8_t band, uint16_t freq)
{
    ...
    else if ((band == 1) && (freq >= 5005) && (freq <= 5885))
    {
        return (freq - 5000 ) / 5;
    }
...
{
#define SCAN_UPDATE_LIMIT_TIME_MS (30000)
    ...
    if (i == sizeof(wifiMgmr.scan_items)/sizeof(wifiMgmr.scan_items[0])) {
        ...
        if(-1 != pos_same_ssid) {
            ...
            if(-1 != pos_empty) {
                ...
                if(strlcpy(wifiMgmr.scan_items[pos_empty].ssid, scan->ssid, sizeof(wifiMgmr.scan_items[0].ssid) + 1) >= \
                   sizeof(wifiMgmr.scan_items[0].ssid) + 1) {
                ...
                if(scan->rssi > lowest_rssi) {
                    ...
                    if(strlcpy(wifiMgmr.scan_items[pos_rssi].ssid, scan->ssid, sizeof(wifiMgmr.scan_items[0].ssid) + 1) >= \
                       sizeof(wifiMgmr.scan_items[0].ssid) + 1) {
                        printf("[OS]: strlcpy truncated \r\n");
                    }
...
#define SCAN_UPDATE_LIMIT_TIME_MS (3000)
    ...
    fhost_printf("channel %02u, bssid %02X:%02X:%02X:%02X:%02X:%02X, rssi %3d, ppm %d:%d, auth %s, cipher:%s \t, SSID %s\r\n",
            scan->channel,
    ...
    if (i == sizeof(wifiMgmr.scan_items)/sizeof(wifiMgmr.scan_items[0])) {
        ...
        if (-1 != i) {
            ...
            if(strlcpy(wifiMgmr.scan_items[i].ssid, scan->ssid, sizeof(wifiMgmr.scan_items[0].ssid) + 1) >= sizeof(wifiMgmr.scan_items[0].ssid) + 1) {
                printf("[OS]: strlcpy truncated \r\n");
            }
...
{
    uint32_t wpa_flags = 0;
    // fixme: stop current channel maybe double call chan_tmr_switch_cb, cause some ASSERT
    if (wifiMgmr.sta_connect_param.quick_connect)
        wpa_flags |= SCAN_STOP_ALL_CHANNEL_IF_TARGET_AP_FOUND; //|SCAN_STOP_CURRENT_CHANNEL_IF_TARGET_AP_FOUND;
    if (wifiMgmr.sta_connect_param.scan_mode)
        wpa_flags |= SCAN_STOP_ALL_CHANNEL_IF_TARGET_AP_FOUND;
    #if BL_RA_EN
    wpa_flags |= LOW_RATE_CONNECT;
    #endif
    return wpa_flags;
}
...
{
    wifiMgmr.sta_connect_param.scan_mode = 0;
...
void wifi_mgmr_ap_sta_info_add(uint8_t sta_idx, uint16_t aid, struct mac_addr *addr)
{
    ...
    wifiMgmr.ap_sta_info[sta_idx].aid = aid;
    memcpy(wifiMgmr.ap_sta_info[sta_idx].sta_mac, addr, sizeof(*addr));
    wifiMgmr.num_sta++;
    rtos_mutex_unlock(wifiMgmr.ap_sta_info_lock);
}
...
static RTOS_TASK_FCT(wifi_mgmr_task)
{
    ...
    extern void platform_post_event(int catalogue, int code1, int code2);
...
int wifi_mgmr_init(wifi_conf_t *conf)
{
    if (!wifiMgmr.ready) {
        ...
        rtos_mutex_create(&wifiMgmr.ap_sta_info_lock);
    }
    ...
    wifiMgmr.max_sta_supported = CFG_STA_MAX;
    wifiMgmr.num_sta = 0;
    wifiMgmr.ready = 1;
    wifi_mgmr_set_country_code(conf->country_code);
    wifi_mgmr_sta_connect_params_setdefault();
    wifiMgmr.ap_mode = WIFI_MODE_802_11B | WIFI_MODE_802_11G | WIFI_MODE_802_11N_2_4 | WIFI_MODE_802_11AX_2_4;
    wifiMgmr.sta_mode = WIFI_MODE_802_11B | WIFI_MODE_802_11G | WIFI_MODE_802_11N_2_4 | WIFI_MODE_802_11AX_2_4;

    if (rtos_task_create(wifi_mgmr_task, "wifi_mgmr", APPLICATION_TASK,
                         FHOST_IPC_STACK_SIZE, NULL, fhost_ipc_priority, NULL))
        return 0;
