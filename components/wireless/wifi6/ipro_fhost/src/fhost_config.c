...
/* Phase 11 2026-05-11: bump BAM TX retry lifetime 0x400 (1ms) -> 0x4000
 * (16ms) to give MAC/BAM full retry chain headroom under lossy WiFi.
 * 1ms was too short — measured cfm_lat avg 3.5ms means frames routinely
 * exceed 1ms during retries, BAM gives up early, TCP sees 8% retrans.
 * Matches `application/fhost_ipc.c:57` which already uses 0x4000.
 * Expected: TCP TX retrans drops, throughput approaches UDP wire. */
static uint16_t tx_agg_lifetime = 0x4000;
/* HT40 is gated twice. First by the modem's HDMCONFIG.CHBW capability field,
 * which phy_get_bw() reads and which is read-only (ipro6 silicon reports 40).
 * Second by this flag, which fhost_config_prepare() can only use to NARROW that
 * capability, never to widen it. It has shipped at 0 with no caller anywhere in
 * the tree — wifi_mgmr_set_ht40_enable() is unreferenced — so the stack has
 * always associated as a 20 MHz STA on hardware that can do 40. CFG_HT40 (from
 * CONFIG_WIFI_HT40_ENABLE) picks the boot default; `wifi_ht40` overrides it at
 * runtime, which must happen before stack_wifi. */
#ifdef CFG_HT40
static uint8_t ht40_enable = 1;
#else
static uint8_t ht40_enable = 0;
#endif
static uint8_t wifi_coex_en = 0; // default disable
                          ...
                          bool init)
{
    ...
    #ifdef CFG_LPM
    me_config->chip_version = platform_get_chip_version();
    #endif
    nss = phy_get_nss() + 1;
    ldpc_rx = phy_ldpc_rx_supported();
    #if defined(CFG_IPRO_WIFI_PS_ENABLE)
    bfmee = false;
    mu_rx = false;
    #else
    ...
    #ifdef CFG_LPM
    uint8_t ampdu_len = 0;
    // AMPDU LENGTH = 2^(13+ampdu_len) -1
    #if (NX_AMPDU_RX_BUF_SIZE > 8)
        ampdu_len = 2;
    #else
        ampdu_len = 1;
    #endif
    ...
    if (me_config->ht_supp)
    {
        ...
        #ifdef CFG_LPM
        me_config->ht_cap.a_mpdu_param = ((ampdu_len << MAC_AMPDU_LEN_EXP_OFT) |
        #else
    ...
    if (me_config->vht_supp)
    {
        ...
        #ifdef CFG_LPM
        me_config->vht_cap.vht_capa_info |= (ampdu_len << MAC_VHTCAPA_MAX_A_MPDU_LENGTH_EXP_OFT);
        #else
    ...
    if (me_config->he_supp)
    {
        ...
        #ifdef CFG_LPM
        HE_MAC_CAPA_VAL_SET(he_cap, TF_MAC_PAD_DUR, 16US);
        #endif
        ...
        // 2024-07-10: BL616 experiences PHY Errors when responding with Ng=16 in certain scenarios during TB Beamforming Report.
...
{
    /* Deliberately no phy_get_bw() probe. The flag has to be written before the
     * WiFi task starts, i.e. before the modem is powered, so reading a modem
     * register here is at best meaningless. The check was also redundant:
     * fhost_config_prepare() re-reads the capability at the moment of use and
     * this flag can only narrow it. */
    ht40_enable = !!value;
    return 0;
}
