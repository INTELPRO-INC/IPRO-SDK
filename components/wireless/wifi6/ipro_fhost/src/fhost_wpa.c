...
static void fhost_wpa_rx_cb(struct fhost_frame_info *info, void *arg)
{
    ...
    //filter by rx_filter or (Deauth and Disa)
    if ((wpa_vif->rx_filter & CO_BIT(MAC_FCTRL_SUBTYPE(hdr->fctl))) && !(
            MAC_FCTRL_DISASSOC_ST == (hdr->fctl & MAC_FCTRL_SUBT_MASK) || MAC_FCTRL_DEAUTHENT == (hdr->fctl & MAC_FCTRL_SUBT_MASK)
        )) {
        return;
    }
    ...
    if (MGMR_VIF_AP == info->fvif_idx) {
        event.hdr.id = CFGRWNX_RX_MGMT_EVENT;
    } else {
        //Deauth is Disa is always condisered as unprot disconnect
        if (MAC_FCTRL_DISASSOC_ST == (hdr->fctl & MAC_FCTRL_SUBT_MASK)) {
            event.hdr.id = CFGRWNX_UNPROT_DISASSOCIATE;
        } else if (MAC_FCTRL_DEAUTHENT == (hdr->fctl & MAC_FCTRL_SUBT_MASK)) {
            event.hdr.id = CFGRWNX_UNPROT_DEAUTHENTICATE;
        } else {
            event.hdr.id = CFGRWNX_RX_MGMT_EVENT;
        }
    }
    ...
    event.payload_in_heap = rtos_malloc(event.length);
    if (event.payload_in_heap == NULL)
        return;

    memcpy(event.payload_in_heap, info->payload, event.length);
    if (fhost_cntrl_cfgrwnx_event_send(&event.hdr, fhost_vif->conn_sock, 1)) {
...
int fhost_wpa_add_vif(int fhost_vif_idx)
{
    ...
    if (fhost_vif_idx == MGMR_VIF_STA &&
        ...
        fhost_printf("%s: set autoconnect failed\r\n", __func__);
    }
                          ...
                          int timeout_ms, const char *fmt, ...)
{
    ...
    req = rtos_malloc(ELOOP_MSG_INBUF_LEN);
    resp = rtos_malloc(ELOOP_MSG_OUTBUF_LEN);
    if (!req || !resp) {
        fhost_printf("alloc msg buf failed\r\n");
        ASSERT_ERR(0);
    }
    p = req;
    if (wpa_vif) {
        offset = snprintf(req, ELOOP_MSG_INBUF_LEN, "IFNAME=%s ",
                 wpa_vif->iface_name);
        p += offset;
    }
    ...
    // Format command
    va_start(args, fmt);
    req_len = dbg_vsnprintf(p, ELOOP_MSG_INBUF_LEN - offset, fmt, args);
    va_end(args);

    if ((req_len + offset) >= ELOOP_MSG_INBUF_LEN) {
        TRACE_FHOST(ERR, "WPA Command truncated. need %d bytes", req_len);
        fhost_printf("WPA Command truncated. need %d bytes\r\n", req_len);
        goto end;
    }

    // Send the cmd and wait for its response
    if (eloop_event_commit_sync(ELOOP_EVT_WPA_IFACE_CTRL,
                            req, req_len + offset + 1, resp, &len, -1) < 0) {
        fhost_printf("failed to commit eloop sync event\r\n");
        goto end;
    }
    if (strncmp(resp, "FAIL", 4) == 0) {
        fhost_printf("RESP: FAILED\r\n");
        goto end;
    }
    //check resp_buf, since resp_buf maybe NULL
    if (resp_buf) {
        if (*resp_buf_len >= len) {
            memcpy(resp_buf, resp, len);
            *resp_buf_len = len;
        } else {
            fhost_printf("No enough memory for wpa response %d vs%d\r\n", *resp_buf_len, len);//XXX should assert here???
            goto end;
        }
    }
    res = 0;
...
int fhost_wpa_create_network(int fhost_vif_idx, char *net_cfg, char *ssid_cfg, char *key_cfg, bool enable, int timeout_ms)
{
    ...
    while (tok)
    {
        ...
        {
            fhost_printf("SET_NETWORK (%s) failed\r\n", tok);
    ...
    fhost_printf("WPA network %d: created and configured", wpa_vif->network_id);
...
int fhost_wpa_get_supplicant_state(int fhost_vif_idx)
{
    ...
    switch (state) {
        case 0:
            fhost_printf("WPA_DISCONNECTED\r\n");
            break;
        case 1:
            fhost_printf("WPA_INTERFACE_DISABLED\r\n");
            break;
        case 2:
            fhost_printf("WPA_INACTIVE\r\n");
            break;
        case 3:
            fhost_printf("WPA_SCANNING\r\n");
            break;
        case 4:
            fhost_printf("WPA_AUTHENTICATING\r\n");
            break;
        case 5:
            fhost_printf("WPA_ASSOCIATING\r\n");
            break;
        case 6:
            fhost_printf("WPA_ASSOCIATED\r\n");
            break;
        case 7:
            fhost_printf("WPA_4WAY_HANDSHAKE\r\n");
            break;
        case 8:
            fhost_printf("WPA_GROUP_HANDSHAKE\r\n");
            break;
        case 9:
            fhost_printf("WPA_COMPLETED\r\n");
            break;
    }
#endif
