...
enum wpa_rwnx_driver_status {
	...
	RWNX_SCANNING = BIT(5),
	RWNX_EXT_AUTH = BIT(6),
	/* A CONNECT_CMD is in flight: the firmware is authenticating/associating
	 * but has not reported the outcome yet. RWNX_ASSOCIATED and drv->bssid are
	 * only filled once the connect event lands, so neither can be used to
	 * recognise this window. */
	RWNX_CONNECTING = BIT(7),
	/* The supplicant gave up on that in-flight connect. The firmware cannot be
	 * stopped mid-association, so its outcome is discarded when it arrives. */
	RWNX_CONNECT_ABORTED = BIT(8),
};
...
static int wpa_rwnx_driver_send_disconnect(struct wpa_rwnx_driver_itf_data *drv,
					   u16 reason_code);
...
static void wpa_rwnx_driver_process_connect_event(struct wpa_rwnx_driver_data *gdrv)
{
	...
	/* The connect is no longer in flight: the firmware has reported an outcome. */
	drv->status &= ~RWNX_CONNECTING;

	/* The supplicant abandoned this connect while it was in flight, so it is
	 * no longer expecting the outcome and wpa_supplicant_event() would drop it
	 * anyway ("unexpected EVENT_ASSOC in disconnected state"). Discard it here
	 * instead of handing up an event for a connection nobody owns.
	 *
	 * NOTE: if the attempt succeeded the MAC is now associated to an AP the
	 * supplicant has forgotten. It is deliberately NOT torn down here -- see
	 * wpa_rwnx_driver_send_disconnect(): TASK_SM has not returned to SM_IDLE
	 * at this point, so the disconnect would be deferred and block this
	 * thread. Reclaiming that association needs an abortable connect in the
	 * firmware or an asynchronous disconnect path; neither exists today. */
	if (drv->status & RWNX_CONNECT_ABORTED) {
		drv->status &= ~RWNX_CONNECT_ABORTED;
...
static void wpa_rwnx_driver_process_disconnect_event(struct wpa_rwnx_driver_data *gdrv)
{
	...
	drv->status &= ~(RWNX_ASSOCIATED | RWNX_DISASSOC_PENDING | RWNX_CONNECTING |
			 RWNX_CONNECT_ABORTED);
	if (drv->ssid) {
		os_free(drv->ssid);
		drv->ssid = NULL;
		drv->ssid_len = 0;
...
static void wpa_rwnx_driver_process_unprot_disconnect(struct wpa_rwnx_driver_data *gdrv, enum wpa_event_type wpa_event)
{
	struct wpa_rwnx_driver_itf_data *drv;
	union wpa_event_data data;
	struct cfgrwnx_rx_mgmt_event event;
	...
	if ((fhost_cntrl_cfgrwnx_event_get(gdrv->link, &event, sizeof(event)) < 0) || !event.payload_in_heap) {
		return;
	}
	drv = &gdrv->itfs[event.fhost_vif_idx];
	if (!(drv->status & RWNX_INITIALIZED)) {
...
static void wpa_rwnx_driver_tx_status(uint32_t frame_id, bool acknowledged, void *arg)
{
	...
	if (!(drv->status & RWNX_INITIALIZED) ||
	    fhost_cntrl_cfgrwnx_event_send(&event.hdr, gdrv->link->sock_send, 0))
		wpa_rwnx_driver_release_tx_frame(tx_frame);
}
...
static void wpa_rwnx_driver_event(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	if (fhost_cntrl_cfgrwnx_event_peek_header(gdrv->link, &msg_hdr) < 0) {
        TRACE_FHOST(ERR, "wpa_rwnx_driver_data ERR-<<<<<<\r\n");
		return;
    }

    TRACE_FHOST(WPA_INFO, "wpa_rwnx_driver_data %u\r\n", msg_hdr.id);
	switch (msg_hdr.id) {
	...
	case CFGRWNX_RX_MGMT_EVENT:
		...
		break;
	case CFGRWNX_UNPROT_DISASSOCIATE:
		wpa_rwnx_driver_process_unprot_disconnect(gdrv, EVENT_UNPROT_DISASSOC);
		break;
	case CFGRWNX_UNPROT_DEAUTHENTICATE:
		wpa_rwnx_driver_process_unprot_disconnect(gdrv, EVENT_UNPROT_DEAUTH);
		break;
				     ...
				     struct wpa_driver_associate_params *params)
{
	...
	/* A fresh attempt must not inherit an abort aimed at a previous one. */
	drv->status &= ~RWNX_CONNECT_ABORTED;
	drv->status |= RWNX_CONNECTING;
...
/**
 * Push a DISCONNECT_CMD down to the firmware.
 *
 * Only safe to call while TASK_SM is idle: sm_disconnect_req_handler() returns
 * KE_MSG_SAVED whenever the state machine is mid-association, and the fhost
 * handler waits synchronously for SM_DISCONNECT_CFM -- so issuing this during
 * an in-flight connect blocks the supplicant thread until that connect ends.
 */
static int wpa_rwnx_driver_send_disconnect(struct wpa_rwnx_driver_itf_data *drv,
					   u16 reason_code)
{
...
static int wpa_rwnx_driver_deauthenticate(void *priv, const u8 *addr, u16 reason_code)
{
	...
	/* Abort requested while the connect is still in flight (typically the
	 * supplicant's authentication timeout firing before the firmware has
	 * finished retrying). The firmware cannot be told to stop here -- the
	 * state machine defers SM_DISCONNECT_REQ until it leaves SM_ASSOCIATING
	 * and the caller would block until then. Record the intent instead and
	 * reconcile in wpa_rwnx_driver_process_connect_event(), which runs once
	 * the firmware reports an outcome and TASK_SM is idle again.
	 *
	 * Without this the association completes behind the supplicant's back:
	 * the late CONNECT_EVENT is discarded by the "unexpected EVENT_ASSOC in
	 * disconnected state" guard, so the MAC stays associated while the
	 * supplicant believes it is not -- the 4-way handshake never runs and
	 * nothing ever tears the link down again. */
	if ((drv->status & RWNX_CONNECTING) && !(drv->status & RWNX_ASSOCIATED)) {
		drv->status |= RWNX_CONNECT_ABORTED;
		return 0;
	}
	...
	if (wpa_rwnx_driver_send_disconnect(drv, reason_code))
		return -1;
...
{
	struct wpa_rwnx_driver_itf_data *drv = priv;
	struct cfgrwnx_external_auth_status status;
	struct cfgrwnx_resp resp;
	...
	wpa_rwnx_msg_hdr_init(drv, &status.hdr, CFGRWNX_EXTERNAL_AUTH_STATUS_RESP, sizeof(status));
	wpa_rwnx_msg_hdr_init(drv, &resp.hdr, CFGRWNX_EXTERNAL_AUTH_STATUS_RESP_RESP, sizeof(resp));
	status.fhost_vif_idx = drv->fhost_vif_idx;
	status.status = params->status;

	if (fhost_cntrl_cfgrwnx_cmd_send(&status.hdr, &resp.hdr) || (resp.status != CFGRWNX_SUCCESS))
		return -1;
					   ...
					   const u8 *own_addr, u32 flags)
{
	...
	if (fhost_cntrl_cfgrwnx_event_send(&event.hdr, drv->gdrv->link->sock_send, 0))
		wpa_rwnx_driver_release_tx_frame(tx_frame);
