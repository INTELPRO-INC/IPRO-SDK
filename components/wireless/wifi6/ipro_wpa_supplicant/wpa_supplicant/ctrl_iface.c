...
#elif !defined(__CYGWIN__) && !defined(CONFIG_NATIVE_WINDOWS) && !defined(CONFIG_RWNX)
...
#if defined(CONFIG_TESTING_OPTIONS) || defined(CONFIG_TWT)
...
#endif /* CONFIG_TESTING_OPTIONS || CONFIG_TWT */
					 ...
					 char *buf, size_t *resp_len)
{
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		printf("[ctrl] TWT_SETUP matched (2nd block)\r\n");
		if (wpas_ctrl_iface_send_twt_setup(wpa_s, buf + 10))
			reply_len = -1;
	} else if (os_strcmp(buf, "TWT_SETUP") == 0) {
		printf("[ctrl] TWT_SETUP (no args) matched\r\n");
		if (wpas_ctrl_iface_send_twt_setup(wpa_s, ""))
			reply_len = -1;
	} else if (os_strncmp(buf, "TWT_TEARDOWN ", 13) == 0) {
		if (wpas_ctrl_iface_send_twt_teardown(wpa_s, buf + 13))
			reply_len = -1;
	} else if (os_strcmp(buf, "TWT_TEARDOWN") == 0) {
		if (wpas_ctrl_iface_send_twt_teardown(wpa_s, ""))
			reply_len = -1;
