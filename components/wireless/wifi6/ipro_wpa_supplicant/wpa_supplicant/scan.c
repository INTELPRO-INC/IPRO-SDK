...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (ssid->duration) {
		...
		params.duration = (u16)ssid->duration;
	}
	if (ssid->probe_cnt) {
		params.probe_cnt = (u16)ssid->probe_cnt;
	}
				...
				struct scan_info *info, int new_scan)
{
	...
	if (scan_res->res) {
#ifdef CONFIG_EXTRA_SIZE_OPT
		wpa_selection_sort(scan_res->res, scan_res->num, sizeof(struct wpa_scan_res *), compar);
#else
