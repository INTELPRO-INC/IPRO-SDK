...
{
	unsigned int i;
	struct wpa_ssid *best_signal_ssid;
	struct wpa_bss  *best_signal_bss = NULL;
	...
	for (i = 0; i < wpa_s->last_scan_res_used; i++) {
		...
		*selected_ssid = wpa_scan_res_match(wpa_s, i, bss, group,
		...
		if ((!best_signal_bss) || (bss->level > best_signal_bss->level)) {
			best_signal_ssid = *selected_ssid;
			best_signal_bss = bss;
		}
	}

	if (wpa_s->select_best_signal && best_signal_bss) {
		*selected_ssid = best_signal_ssid;
		wpa_dbg(wpa_s, MSG_DEBUG, "   selected %sBSS " MACSTR
			" ssid='%s'",
			best_signal_bss == wpa_s->current_bss ? "current ": "",
			MAC2STR(best_signal_bss->bssid),
			wpa_ssid_txt(best_signal_bss->ssid, best_signal_bss->ssid_len));
		return best_signal_bss;
	}
					      ...
					      int own_request, int update_only)
{
	...
	if (ap) {
		...
		if (wpa_s->ap_iface->scan_cb)
			wpa_s->ap_iface->scan_cb(wpa_s->ap_iface, scan_res);
#endif /* CONFIG_AP */
		goto scan_work_done;
	}
					      ...
					      int new_scan, int own_request)
{
	...
	/* No need any more, just reset */
	wpa_s->select_best_signal = 0;
				       ...
				       union wpa_event_data *data)
{
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_NONE ||
		...
		/* Timeout for the entire 4-Way Handshake (not just first EAPOL) */
		int this_to = (wpa_s->current_ssid) ? (wpa_s->current_ssid->eapol_1_timeout) : 10;
		wpa_supplicant_req_auth_timeout(wpa_s, this_to ? this_to : 10, 0);
	}
...
u16 wpa_supplicant_get_status_code(void *ctx)
{
    ...
    switch (wpa_s->wpa_state) {
        ...
        case WPA_GROUP_HANDSHAKE:
            break;
        case WPA_COMPLETED:
            status_code = WLAN_FW_DISCONNECT_BY_USER_WITH_DEAUTH;
            break;
    }
    return status_code;
}
