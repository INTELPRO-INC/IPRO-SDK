...
/* 2026-07-16: 5s was too tight for WPA3-SAE. This single timer covers the
 * WHOLE external-auth exchange (commit + confirm + accept, several RTTs) and is
 * not re-armed per SAE message. With software elliptic-curve crypto on both
 * ends, a slow AP that takes >3s to answer the commit leaves no room for the
 * confirm, so the STA times out in ASSOCIATING even though SAE is progressing.
 * Match the macsw-side SM_EXTERNAL_AUTH_TIMEOUT (10s, NX_FULLY_HOSTED) so both
 * layers give SAE the same budget. Open-auth just waits longer before failing. */
#define SME_AUTH_TIMEOUT 10
					     ...
					     struct wpa_ssid *ssid)
{
    ...
    wpa_bl_printf("[WPA]Send Auth,sn:%d, status:%d\r\n", wpa_s->sme.seq_num, status);
					       ...
					       const u8 *da)
{
	...
	sme_external_auth_build_buf(buf, resp, wpa_s->own_addr,
				    ...
				    WLAN_STATUS_SUCCESS);
	wpa_drv_send_mlme(wpa_s, wpabuf_head(buf), wpabuf_len(buf), 1, 0, 0);
    wpa_bl_printf("[WPA]Send Auth sae_confirm,sn:%d\r\n", wpa_s->sme.seq_num);
	wpabuf_free(resp);
	wpabuf_free(buf);
}
				 ...
				 const u8 *da, u16 reason_code)
{
	...
	if (os_memcmp(sa, wpa_s->bssid, ETH_ALEN) != 0)
		return;
#if 0
    //For Xiaomi AX300 which send unprot disassoc and deauth
