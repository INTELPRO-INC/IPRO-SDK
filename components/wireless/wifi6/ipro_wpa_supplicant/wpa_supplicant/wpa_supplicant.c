...
#ifdef CONFIG_BL_EXTRA_WPA_SSID_FLAG
#include <fhost_wpa.h>
#endif /* CONFIG_BL_EXTRA_WPA_SSID_FLAG */

#ifdef CONFIG_BL_PMK_CACHE_IN_MGMR
#include <wifi_mgmr_pmk.h>
#endif /* CONFIG_BL_PMK_CACHE_IN_MGMR */
...
static void wpa_supplicant_timeout(void *eloop_ctx, void *timeout_ctx)
{
    ...
    wpa_bl_printf("[WPA] %s: send deauth, Line:%d\r\n", __func__, __LINE__);
			      ...
			      enum wpa_states state)
{
	...
	if (state == WPA_COMPLETED) {
		...
		wpa_s->normal_scans = 0;
		/* Set link up + DHCP after WPA completes */
		{
			extern void net_al_ext_dhcp_connect(void);
			extern void net_if_up_link(void);
			printf("[WPA] COMPLETED — link up + DHCP\r\n");
			net_if_up_link();
	...
	if (state == WPA_COMPLETED && wpa_s->new_connection) {
		...
		wpa_drv_set_operstate(wpa_s, 1, old_state, state);
		...
		wpa_drv_set_operstate(wpa_s, 0, old_state, state);
#ifndef IEEE8021X_EAPOL
...
#ifdef CONFIG_BL_EXTRA_WPA_SSID_FLAG
        if ((ssid->wpa_flags & FHOST_WPA_SSID_PREFER_WPA2_TO_WPA3) &&
...
#else
			/* 2026-07-16: was 5s. This single timer covers the whole
			 * ASSOCIATING phase, which for WPA3-SAE includes the
			 * external-auth commit+confirm exchange. With software EC
			 * crypto and a slow AP (>3s to answer the commit), 5s
			 * expires mid-SAE and the STA deauths before the confirm.
			 *
			 * 2026-07-26: 10s -> 30s. With the rwnx driver the firmware
			 * owns the whole connect: wpa_rwnx_driver_associate() hands
			 * it auth+assoc in one CONNECT_CMD and macsw always reports a
			 * terminal outcome (sm_rsp_timeout_ind_handler never retries
			 * silently - it either fails immediately or defers to the TX
			 * confirm, both of which end in sm_connect_ind()). So this
			 * timer is a backstop for a driver that says nothing at all,
			 * not a second deadline racing the firmware's own.
			 *
			 * Racing it is actively harmful: a connect that the firmware
			 * is still completing gets abandoned here, and because
			 * wpa_rwnx_driver_deauthenticate() cannot cancel an in-flight
			 * connect, the association then completes behind our back and
			 * the late EVENT_ASSOC is discarded as "unexpected ... in
			 * disconnected state" - MAC associated, supplicant not, 4-way
			 * never runs, and nothing recovers it.
			 *
			 * Firmware-side connects here take up to ~14 s when AC-timer
			 * recoveries are involved (each stalled TX descriptor costs a
			 * couple of seconds), measured on-bench. 30 s clears that with
			 * margin. Real failures are not slowed by this: they arrive as
			 * a CONNECT_EVENT carrying a failure status long before the
			 * timer, which only fires when the firmware is still working. */
			timeout = 30;
#endif
		}
		int this_to = (wpa_s->current_ssid) ? (wpa_s->current_ssid->auth_timeout) : timeout;
		wpa_supplicant_req_auth_timeout(wpa_s, this_to ? this_to : timeout, 0);
	}
				   ...
				   u16 reason_code)
{
    ...
    wpa_bl_printf( "Request to deauthenticate - bssid=" MACSTR
        " pending_bssid=" MACSTR " reason=%d (%s) state=%s\r\n",
        MAC2STR(wpa_s->bssid), MAC2STR(wpa_s->pending_bssid),
        reason_code, reason2str(reason_code),
        wpa_supplicant_state_txt(wpa_s->wpa_state));
			     ...
			     const u8 *buf, size_t len)
{
	...
	if (wpa_s->eapol_received == 0 &&
		...
		int this_to = (wpa_s->current_ssid) ? (wpa_s->current_ssid->eapol_rem_timeout) : timeout;
		wpa_supplicant_req_auth_timeout(wpa_s, this_to ? this_to : timeout, 0);
	}
...
void wpas_connection_failed(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
    ...
    if (!wpa_s->auto_reconnect_disabled) {
        printf("Auto connect enabled: try to reconnect\r\n");
          wpa_supplicant_req_scan(wpa_s, timeout / 1000,
                                  1000 * (timeout % 1000));
    } else {
        printf("Auto connect disabled: do not try to re-connect\r\n");
    }
}
