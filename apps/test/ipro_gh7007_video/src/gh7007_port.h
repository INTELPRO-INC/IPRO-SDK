...
#define GH_SPI_CLK_HZ    48000000u  /* 48 MHz = BCLK(96M)/2, the divider's exact
                                     * top step. NOTE the IPRO7 divider rounds
                                     * DOWN: a 40M request actually ran 24M (all
                                     * earlier "40 MHz clean" benching was 24M).
                                     * 48M is the first true >24M exposure — if
                                     * artifacts appear, `lcd clk 40000000` drops
                                     * back to 24M at runtime. Full frame at 48M:
                                     * ~26 ms SPI floor => ~30 fps ceiling. */
