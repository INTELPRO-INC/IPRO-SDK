...
static int cmd_lcd(int argc, char *argv[])
{
    ...
    if (strcmp(sub, "init") == 0) {
        ...
        printf("atomic = %d (scheduler suspended across each row transfer)\n",
               gh7007_get_atomic());
    } else if (strcmp(sub, "dmaprobe") == 0) {
        gh7007_dma_probe();
    } else if (strcmp(sub, "dmaprobe2") == 0) {
        gh7007_dma_probe2();
    } else if (strcmp(sub, "fastrow") == 0) {
        if (argc > 2) {
            gh7007_set_fastrow(atoi(argv[2]));
        }
        printf("fastrow = %d (video rows skip the dcsetup delays)\n",
               gh7007_get_fastrow());
