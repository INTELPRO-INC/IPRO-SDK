...
static volatile int gJDECKickDone = 0;
int gJDECKickDoneCnt = 0;
...
#if (MJDEC_TEST_KICK_MODE_EN==1)
uint32_t g_udCpyDestStartAddr_yy=0;
uint32_t g_udCpyDestStartAddr_uv=0;
uint32_t g_udCpyDestOffset_yy=0;
uint32_t g_udCpyDestOffset_uv=0;

int mjdec_test_kick_done(void *usrdata, mjdec_frame_desc_t *frame_desc)
{
    if(frame_desc->ucIntType == MJDEC_HAL_INT_NORMAL)
    {
        ...
        printf("mjdec single block kick int cnt %d, type %d, y buf: 0x%x 0x%x y buf size: %d, uv buf: 0x%x 0x%x uv buf size: %d\r\n", gJDECKickDoneCnt, (unsigned int)frame_desc->ucIntType, 
        ...
        #if 0   /*1D copy*/
            memcpy((void*)g_udCpyDestStartAddr_yy+g_udCpyDestOffset_yy, (void*)frame_desc->frame_start_addr_yy, frame_desc->frame_size_yy);
            memcpy((void*)g_udCpyDestStartAddr_uv+g_udCpyDestOffset_uv, (void*)frame_desc->frame_start_addr_uv, frame_desc->frame_size_uv);
        
            printf("memcpy yy 1D from 0x%x to 0x%x\r\n", (unsigned int)frame_desc->frame_start_addr_yy, (unsigned int)(g_udCpyDestStartAddr_yy+g_udCpyDestOffset_yy)); 
            printf("memcpy uv 1D from 0x%x to 0x%x\r\n", (unsigned int)frame_desc->frame_start_addr_uv, (unsigned int)(g_udCpyDestStartAddr_uv+g_udCpyDestOffset_uv));

        #else
            /*2D copy*/
            uint32_t udCurrBlockNum, udTotalBlockPerLine, udCurrBlockLineNum, udBlockWidth, udBlockHeight, udRemainBlockWidth, udLastBlockWidth;
            uint8_t  *pSrc, *pDst, x, y;
        
            udBlockWidth  = 128;
            udBlockHeight = 8;
            udRemainBlockWidth  = g_tMjdecCfg.width & (udBlockWidth-1);
            udLastBlockWidth    = (udRemainBlockWidth!=0)?udRemainBlockWidth:128;
            udTotalBlockPerLine = g_tMjdecCfg.width/udBlockWidth + (udRemainBlockWidth!=0);
            udCurrBlockNum      = gJDECKickDoneCnt % udTotalBlockPerLine;
            udCurrBlockLineNum  = gJDECKickDoneCnt / udTotalBlockPerLine;
            
            /*copy yy*/
            if (g_tMjdecCfg.yuv_mode == MJDEC_HAL_FORMAT_YUV420_SP) 
                udBlockHeight = 16;
            pSrc = (uint8_t *)frame_desc->frame_start_addr_yy;
            pDst = (uint8_t *)(g_udCpyDestStartAddr_yy+udCurrBlockLineNum*g_tMjdecCfg.width*udBlockHeight + udCurrBlockNum*udBlockWidth);
            printf("memcpy yy 2D from 0x%x to 0x%x\r\n", (unsigned int)pSrc, (unsigned int)pDst); 
            if(udCurrBlockNum == (udTotalBlockPerLine-1))
                udBlockWidth = udLastBlockWidth;    /*last block*/
            for(y=0;y<udBlockHeight;y++)
                for(x=0;x<udBlockWidth;x++)
                    pDst[x+y*g_tMjdecCfg.width]=pSrc[x+udBlockWidth*y];

            /*copy yy*/
            if (g_tMjdecCfg.yuv_mode != MJDEC_HAL_FORMAT_YUV400) 
            {
                 /*uv width == yy width due to semi-planar placement.*/
                udBlockWidth  = 128;
                udBlockHeight = 8;
                pSrc = (uint8_t *)frame_desc->frame_start_addr_uv;
                pDst = (uint8_t *)(g_udCpyDestStartAddr_uv+udCurrBlockLineNum*g_tMjdecCfg.width*udBlockHeight + udCurrBlockNum*udBlockWidth); 
                printf("memcpy uv 2D from 0x%x to 0x%x\r\n", (unsigned int)pSrc, (unsigned int)pDst); 
                if(udCurrBlockNum == (udTotalBlockPerLine-1))
                    udBlockWidth = udLastBlockWidth;    /*last block*/
                for(y=0;y<udBlockHeight;y++)
                    for(x=0;x<udBlockWidth;x++)
        ...
        g_udCpyDestOffset_yy += frame_desc->frame_size_yy;
        g_udCpyDestOffset_uv += frame_desc->frame_size_uv;
        ...
        #if (MJDEC_TEST_BLOCK_LINE_MODE_OUT_DIRECT == 0)   /*memcpy to final output buffer*/
            memcpy((void*)g_udCpyDestStartAddr_yy+g_udCpyDestOffset_yy, (void*)frame_desc->frame_start_addr_yy, frame_desc->frame_size_yy);
            memcpy((void*)g_udCpyDestStartAddr_uv+g_udCpyDestOffset_uv, (void*)frame_desc->frame_start_addr_uv, frame_desc->frame_size_uv);
            printf("memcpy yy from 0x%x to 0x%x\r\n", (unsigned int)frame_desc->frame_start_addr_yy, (unsigned int)(g_udCpyDestStartAddr_yy+g_udCpyDestOffset_yy)); 
            printf("memcpy uv from 0x%x to 0x%x\r\n", (unsigned int)frame_desc->frame_start_addr_uv, (unsigned int)(g_udCpyDestStartAddr_uv+g_udCpyDestOffset_uv));
            g_udCpyDestOffset_yy += frame_desc->frame_size_yy;
            g_udCpyDestOffset_uv += frame_desc->frame_size_uv;

        #else      /*HW direct to final output buffer*/
            g_udCpyDestOffset_yy += frame_desc->frame_size_yy;
            g_udCpyDestOffset_uv += frame_desc->frame_size_uv;


            /*set next block line start address*/
            printf("Set Next HW direct Out yy to 0x%x\r\n", (unsigned int)(g_udCpyDestStartAddr_yy+g_udCpyDestOffset_yy)); 
            printf("Set Next HW direct Out uv to 0x%x\r\n", (unsigned int)(g_udCpyDestStartAddr_uv+g_udCpyDestOffset_uv));

            mjdec_set_out_addr(g_udCpyDestStartAddr_yy+g_udCpyDestOffset_yy, g_udCpyDestStartAddr_uv+g_udCpyDestOffset_uv);
        #endif
...
int sample_jpg_dec(void)
{
    ...
    config.out_kick_mode_en = MJDEC_TEST_KICK_MODE_EN; 
    config.out_kick_mode_sel= MJDEC_TEST_KICK_MODE_SEL;  /*0: single block mode. 1: block line mode*/
    config.input_buf_addr  = (uint8_t*)MJDEC_TEST_JPG_BIT_STREAM_BUF_ADDR;

#if defined(CONFIG_FPGA)    /*cpoy bit stream to tcm range because JPEG HW cannot access flash & wram address range.*/
    printf("Copy JPEG bit stream with size %d bytes from 0x%08x to 0x%08x!\r\n", sizeof(MJDEC_TEST_JPG_BIT_STREAM_BUF_ADDR), 
                                                                                 (unsigned int)config.input_buf_addr,
                                                                                 (unsigned int)REALCHIP_MJDEC_TEST_RSVD_MEM_START_ADDR);
    memcpy((void*)REALCHIP_MJDEC_TEST_RSVD_MEM_START_ADDR, (void*)config.input_buf_addr, sizeof(MJDEC_TEST_JPG_BIT_STREAM_BUF_ADDR));
#endif
...
#else
    //Kick Mode ==>  HW out and final out buffer (for output checksum) are different

    final_out_buf_addr    = alloc_buf_addr;             /*use CPU to move from HW output buffer to final_out_buf_addr for calculating checksum.*/
    final_out_buf_size    = frm_size;

    g_udCpyDestStartAddr_yy = (uint32_t)final_out_buf_addr;
    g_udCpyDestStartAddr_uv = (uint32_t)final_out_buf_addr + YUV400_FRAME_SIZE(img_width, img_height);
    g_udCpyDestOffset_yy    = 0;
    g_udCpyDestOffset_uv    = 0;
...
#if (MJDEC_TEST_KICK_MODE_EN==1) && defined(MJDEC_TEST_BLOCK_LINE_MODE_OUT_DIRECT)
    #if (MJDEC_TEST_BLOCK_LINE_MODE_OUT_DIRECT == 1)    /*HW direct to final output buffer*/
        /*set next block line start address*/
        mjdec_set_out_addr(g_udCpyDestStartAddr_yy, g_udCpyDestStartAddr_uv);
    #endif
#endif
...
#else
    mjdec_set_callback(mjdec_test_kick_done, NULL);
#endif
