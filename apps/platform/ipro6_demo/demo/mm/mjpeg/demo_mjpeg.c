...
static uint32_t g_udJencStreamBufSecHalfSize;
static uint8_t *g_pucJencStreamBufSecHalfAddr;

static uint32_t g_udJencStreamBufWriteOffset;

volatile int32_t g_udJencMenualPopFrameCnt=10;
...
int sample_jpg_enc(void)
{
    ...
    L1C_DCACHE_CLEAN_INVALID_RANGE((unsigned long)g_pucJencOutBufAddr, (unsigned long)g_udJencOutBufSize);/*invalidate before HW start to output. ==> fix case22 kick mode checksum error issue.*/
    ...
    if ( (mjpeg_cfg.format==MJPEG_HAL_FORMAT_YUV420SP_NV12) ||
        ...
        //TBD. //mjpeg_cfg.rows           = COMMON_AlignCeil(TSRC_OUT_RING_LINE, temp);  /*input memory buffer height must be 8 alignment for YCC400/YCC422, 16 alignement for YCC420*/
        mjpeg_cfg.hw_mode_swen_en = 1;  /*0: wait first frame done*/
    #else
    ...
    //TBD. //mjpeg_cfg.input_bufaddr1 = MJENC_TEST_YUV_BUF_ADDR + MJENC_TEST_IMG_W*TSRC_OUT_RING_LINE;
    #else
...
#else
    //TBD. //L1C_DCACHE_CLEAN_INVALID_RANGE((unsigned long)TSRC_IN_ADDR, (unsigned long)(TSRC_WIDTH*TSRC_HEIGHT*2));
#endif
    
    //while (1)     /*run multi frames*/
    if(1)
    {
        ...
        #endif
        {
            ...
            printf("Frame done count %u\r\n", (unsigned int)loop_cnt);
                ...
                ret = mjpeg_get_one_frame((uint32_t *)&g_pucJencStreamBufAddr, (uint32_t *)&g_udJencStreamBufWriteOffset, 
                                          (uint32_t *)&g_pucJencStreamBufSecHalfAddr, (uint32_t *)&g_udJencStreamBufSecHalfSize );
                if (ret == 0) {
                    printf("Frame ready 0x%lx 0x%lx, sec_half  0x%lx 0x%lx\r\n", (uint32_t)g_pucJencStreamBufAddr, (uint32_t)g_pucJencStreamBufAddr + g_udJencStreamBufWriteOffset,
                                                                                 (uint32_t)g_pucJencStreamBufSecHalfAddr, (uint32_t)g_pucJencStreamBufSecHalfAddr+g_udJencStreamBufSecHalfSize);
                }
            #endif
            ...
            /*check result*/
                ...
                unsigned int udCheckSumValueTemp=0;
                unsigned int udCheckXORValueTemp=0;
                ...
                if(g_udJencStreamBufSecHalfSize > 0)
                {
                    L1C_DCACHE_CLEAN_INVALID_RANGE((unsigned long)g_pucJencStreamBufSecHalfAddr, (unsigned long)g_udJencStreamBufSecHalfSize);
                    GetCheckSumXOR((int) g_pucJencStreamBufSecHalfAddr, g_udJencStreamBufSecHalfSize, &udCheckSumValueTemp, &udCheckXORValueTemp);

                    udCheckSumValue += udCheckSumValueTemp;
                    udCheckXORValue ^= udCheckXORValueTemp;
                }
                printf("======================================\r\n");
