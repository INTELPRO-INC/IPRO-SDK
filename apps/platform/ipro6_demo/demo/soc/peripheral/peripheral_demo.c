...
static void tick_proc(void *arg)
{
    ...
    uint32_t soc_tick_diff;
    uint32_t soc_tick_us;

#ifdef CONFIG_FPGA
    /* FPGA uses 32MHz clock for soc timer */
    const uint32_t soc_timer_clk_hz = 32000000;
#else
    /* Normal chip uses 1MHz clock for soc timer */
    const uint32_t soc_timer_clk_hz = 1000000;
#endif
    ...
    while(1) {
        ...
        soc_tick_diff = curr_soctick - last_soctick;
        soc_tick_us = (uint32_t)((uint64_t)soc_tick_diff * 1000000 / soc_timer_clk_hz);
        printf("soc timer: %lu us (tick: %lu, clk: %lu Hz), mtime tick: %llu, rtos: %lu, rtc: %llu (cur: %llu)\r\n",
               soc_tick_us, soc_tick_diff, soc_timer_clk_hz,
               curr_cputick - last_cputick, (uint32_t)xTaskGetTickCount(),
               curr_rtctick - last_rtctick, curr_rtctick);
        arch_delay_ms(1000);
...
#endif /* CONFIG_FATFS_SDH_SDCARD */

#include <hal_boot2.h>
#include <hal_flash.h>
#include <hal_sys.h>

/* OTA apply trigger. The compressed .ota.xz package is staged in FW-B (slot 1,
 * the mfg-overlapping region @ 0x130000) beforehand; here we only flip the FW
 * entry's activeIndex to slot 1 and reboot. boot2 then streams FW-B, verifies
 * SHA256, decompresses into FW-A (slot 0), and resets activeIndex back to 0.
 *
 * Self-contained: loads the partition table straight from flash rather than via
 * hal_boot2_init() (which is FPGA-gated and runs flash_init(), breaking XIP). */
int cmd_ota_test(int argc, char **argv)
{
    static PtTable_Stuff_Config pt[2];
    PtTable_Entry_Config fwEntry;
    PtTable_ID_Type activeId;
    int valid0, valid1;

    flash_read(PT_TABLE0_ADDRESS, (uint8_t *)&pt[0], sizeof(pt[0]));
    flash_read(PT_TABLE1_ADDRESS, (uint8_t *)&pt[1], sizeof(pt[1]));

    valid0 = (pt[0].ptTable.magicCode == PT_MAGIC_CODE);
    valid1 = (pt[1].ptTable.magicCode == PT_MAGIC_CODE);
    if (!valid0 && !valid1) {
        printf("ota_test: no valid partition table\r\n");
        return -1;
    }
    /* Active table = the valid one with the higher age */
    if (valid0 && valid1)
        activeId = (pt[1].ptTable.age > pt[0].ptTable.age) ? PT_TABLE_ID_1
                                                           : PT_TABLE_ID_0;
    else
        activeId = valid0 ? PT_TABLE_ID_0 : PT_TABLE_ID_1;

    if (PtTable_Get_Active_Entries(&pt[activeId], PT_ENTRY_FW_CPU0, &fwEntry)
            != PT_ERROR_SUCCESS) {
        printf("ota_test: FW entry not found\r\n");
        return -1;
    }

    printf("ota_test: FW activeIndex %d -> 1, slot1 @ 0x%lx len 0x%lx\r\n",
           fwEntry.activeIndex, (unsigned long)fwEntry.Address[1],
           (unsigned long)fwEntry.maxLen[1]);

    /* Point the FW entry at slot 1 (FW-B) and commit to the inactive table;
     * PtTable_Update_Entry bumps age so boot2 picks it on next boot. */
    fwEntry.activeIndex = 1;
    if (PtTable_Update_Entry(NULL, !activeId, &pt[activeId], &fwEntry)
            != PT_ERROR_SUCCESS) {
        printf("ota_test: partition update failed\r\n");
        return -1;
    }

    printf("ota_test: rebooting to apply OTA...\r\n");
    hal_reboot();
    return 0;
}
SHELL_CMD_EXPORT_ALIAS(cmd_ota_test, ota_test, apply staged OTA);
