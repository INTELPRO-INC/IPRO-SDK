...
static int ili9488_spi_peripheral_init(void)
{
    ...
    //dma_reload(g_dma_dev.id, g_dma_dev.ch, (uint32_t)send_data, (uint32_t)DMA_ADDR_SPI_TDR, SPI_TEST_DATA_LEN, NULL);
...
void ili9488_spi_draw_area(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, ili9488_spi_color_t color)
{
    ...
    dma_reload(g_dma_dev.id, g_dma_dev.ch, (uint32_t)&color_src, (uint32_t)DMA_ADDR_SPI_TDR, pixelDataSize, NULL);
    dma_channel_start(g_dma_dev.id, g_dma_dev.ch);
...
void ili9488_spi_draw_picture_nonblocking(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, ili9488_spi_color_t *picture)
{
    ...
    dma_reload(g_dma_dev.id, g_dma_dev.ch, (uint32_t)picture, (uint32_t)DMA_ADDR_SPI_TDR, (picture_size << 2), NULL);
    dma_channel_start(g_dma_dev.id, g_dma_dev.ch);
