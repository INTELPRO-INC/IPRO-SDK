...
/*
 * Per-app lwIP configuration for ipro6_demo.
 *
 ...
 * This file only takes effect in the non-WiFi (lwip-port) build: with the
 * wifi6 stack enabled, lwipopts.h resolves to wifi6_lwip_adapter's copy and
 * per-app WiFi overrides go in lwipopts_wifi_app.h instead. (An earlier
 * 44*MSS "WiFi branch" here was dead code — the WiFi TX in-flight window is
 * bounded by LWIP_HEAP_SIZE, see wifi6/CMakeLists.txt.) */
