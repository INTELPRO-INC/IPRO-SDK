...
/**
 * @brief HBN fastboot reboot: enter HBN_0 for N RTC ticks, then cold boot.
 *
 * Unlike hbn_demo (pm_hbn_mode_enter), this path uses hal_lp_enter_fastboot()
 * which snapshots HBN RAM context so post-wake code can detect it via
 * hal_lp_is_fastboot_wakeup(). Intended for stability / reboot-loop testing.
 *
 * Usage: hbn_fastboot [ms]   default 30 ms (~1000 RTC ticks @ 32.768 kHz)
 */
static int cmd_hbn_fastboot(int argc, char **argv)
{
    ...
    hal_lp_cfg_t cfg = {
        .rtc_wakeup_en = 1,
        .rtc_sleep_us  = (uint64_t)sleep_ms * 1000ULL,
        .gpio_wakeup_en = 0,
        .gpio_cfg = NULL,
    };
    printf("hbn_fastboot: entering HBN for %lu ms...\r\n", (unsigned long)sleep_ms);
    arch_delay_ms(5);
    int ret = hal_lp_enter_fastboot(&cfg);
    /* Only returns on error — success path is a cold boot */
    printf("hbn_fastboot: hal_lp_enter_fastboot failed: %d\r\n", ret);
    return ret;
}
SHELL_CMD_EXPORT_ALIAS(cmd_hbn_fastboot, hbn_fastboot,
                       HBN fastboot reboot [ms]);
