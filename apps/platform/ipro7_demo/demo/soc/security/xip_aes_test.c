...
static uint8_t user_data_buf[4096] ATTR_EALIGN(32) ATTR_PSRAM_BSS_SECTION;
...
{
    uint8_t *pdata = (uint8_t *)(DRV_FLASH_XIP_BASE + USER_DATA_START_ADDR - SF_Ctrl_Get_Flash_Image_Offset(0, 0));
    ...
    /* flush cache before DMA read (ensure DMA sees CPU's write) */
    L1C_DCACHE_CLEAN_RANGE(user_data_buf, sizeof(user_data_buf));
    ...
    /* invalidate cache after DMA write (ensure CPU sees DMA's result) */
    L1C_DCACHE_INVALID_RANGE(user_data_buf, sizeof(user_data_buf));
