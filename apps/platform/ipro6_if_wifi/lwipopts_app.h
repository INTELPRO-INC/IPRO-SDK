...
/*
 * ****  INERT FOR THIS APP. EDITING THIS FILE CHANGES NOTHING.  ****
 * ****  The file the WiFi build actually reads is lwipopts_wifi_app.h. ****
 *
 * Why (established 2026-07-28): this app's include path resolves "lwipopts.h"
 * to components/wireless/wifi6/wifi6_lwip_adapter/include/lwipopts.h, which
 * wins over components/network/lwip/lwip/lwip-port/config/lwipopts.h. Only the
 * latter carries the __has_include("lwipopts_app.h") hook, so this file has
 * never been compiled into ipro6_if_wifi. Two claims below were therefore never
 * true in a built image: the "deepened tcpip input queues" (TCPIP_MBOX_SIZE
 * 128 / MEMP_NUM_TCPIP_MSG_INPKT 160) never took effect — the app has always
 * run the adapter's values — and "tune lwIP for this app HERE" pointed at a
 * file with no effect.
 *
 * Kept rather than deleted only because it documents the lwip-port shim's
 * whole-file-replacement format, which non-WiFi builds do use. If this app
 * never gains a non-WiFi variant, deleting it is the cleaner end state.
 *
 * ---- original header, retained for provenance ----
 * Per-app lwIP configuration for ipro6_if_wifi (SMID SDIO-WiFi bridge).
 ...
 * Snapshot of the SDK default config the app was developed and tested
 * against (2026-07, deepened tcpip input queues: TCPIP_MBOX_SIZE 128,
 * MEMP_NUM_TCPIP_MSG_INPKT 160).
 */
